//! Public library interface for signapps-forms.
//!
//! Exposes [`router`] so the single-binary runtime can mount the
//! forms routes without owning its own pool.
...
#[derive(Clone)]
/// Application state for the Forms service.
pub struct AppState {
...
/// Build the forms router using the shared runtime state.
///
/// # Errors
///
/// Returns an error when the forms schema SQL cannot be applied.
...
async fn build_state(shared: &SharedState) -> anyhow::Result<AppState> {
    // Apply fallback SQL creation for forms schema (bypasses sqlx caching).
    let fallback_sql = include_str!("../../../migrations/051_create_forms.sql");
    use sqlx::Executor;
    if let Err(e) = shared.pool.inner().execute(fallback_sql).await {
        tracing::warn!(?e, "forms fallback SQL execution failed (non-fatal)");
    }
