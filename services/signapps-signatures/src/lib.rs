//! Public library interface for signapps-signatures.
//!
//! Exposes [`router`] so the single-binary runtime (`signapps-platform`)
//! can mount the electronic signature workflow routes (envelopes, steps,
//! transitions, user signature/stamp management) without owning its own
//! pool or JWT config.
...
/// Build the signatures router using the shared runtime state.
///
/// # Errors
///
