//! Public library interface for signapps-collaboration.
//!
//! Exposes [`router`] so the single-binary runtime can mount the
//! collaboration routes (boards, mind maps, kanban) without owning its
//! own pool.
...
/// Build the collaboration router using the shared runtime state.
///
/// # Errors
///
