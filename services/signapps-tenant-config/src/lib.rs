//! Public library interface for signapps-tenant-config.
//!
//! Exposes [`router`] so the single-binary runtime (`signapps-platform`)
//! can mount the per-tenant white-label configuration routes (branding,
//! CSS overrides, feature flags) without owning its own pool or JWT config.
...
/// Build the tenant-config router using the shared runtime state.
///
/// # Errors
///
