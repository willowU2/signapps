...
mod tests {
    ...
    #[test]
    fn resize_decodes_real_png_bytes() {
        // Build a small in-memory PNG (8x8 grey) using the image crate
        // itself, then run the resize round-trip on it.
        let img = image::DynamicImage::new_rgb8(8, 8);
        let mut png = Vec::new();
        img.write_to(&mut std::io::Cursor::new(&mut png), image::ImageFormat::Png)
            .expect("encode PNG");
        let (bytes, mime, size) = resize_to_512(&png, "image/png").expect("resize");
        assert_eq!(mime, "image/png");
        assert!(size > 0);
        assert_eq!(bytes.len(), size);
        // The output should be 512x512.
        let resized = image::load_from_memory(&bytes).expect("decode resized");
