...
mod tests {
    ...
    fn test_csv_roundtrip() {
        ...
        let spreadsheet = csv_to_spreadsheet(csv_data, Some(','), true)
            .expect("csv_to_spreadsheet should succeed");

        assert_eq!(spreadsheet.sheets.len(), 1);
        assert_eq!(spreadsheet.sheets[0].rows.len(), 3);

        let exported = spreadsheet_to_csv(&spreadsheet, Some(','), None)
            .expect("spreadsheet_to_csv should succeed");
