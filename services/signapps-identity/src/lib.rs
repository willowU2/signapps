...
//! keystore, or JWT config.
//!
//! The legacy binary at `src/main.rs` still wires this library into
//! a standalone Axum server for `just start-legacy` and targeted
//! debugging workflows.
...
pub struct AppState {
    ...
    /// Legacy HS256 JWT secret string (retained for handlers that still read
    /// the raw secret; new code should prefer [`AppState::jwt_config`]).
    pub jwt_secret: String,
    /// Full JWT configuration (algorithm, keys, issuer, audience, TTLs).
    pub jwt_config: JwtConfig,
    /// In-process cache used for token blacklisting and rate-limit counters.
...
/// # Errors
///
/// Returns an error when the OAuth catalog cannot be loaded or when any
/// shared-state field is misconfigured.
///
/// # Examples
///
/// ```no_run
...
/// let _router = signapps_identity::router(shared).await?;
/// # Ok(())
/// # }
/// ```
...
async fn build_state(shared: &SharedState) -> anyhow::Result<AppState> {
    ...
    // Pull the HS256 secret out of env so handlers that still read the raw
    // string continue to work. The canonical config lives on shared.jwt.
