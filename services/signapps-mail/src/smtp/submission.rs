...
//!
//! Accepts outgoing email from authenticated local users. This is a stub
//! implementation that will be extended with full [`signapps_smtp`] session
//! handling (including AUTH and STARTTLS enforcement) in a later sprint.
...
///
/// Unlike the inbound listener (port 25), submission requires authentication
/// before accepting mail. This will be enforced once the full session handler
/// is wired up.
///
/// # Errors
///
...
/// Handle a single SMTP submission connection.
///
/// Stub implementation: sends greeting, responds to EHLO/HELO with capabilities
/// (including AUTH), and closes on QUIT.
async fn handle_connection(
    ...
    // Send greeting
    writer
        .write_all(b"220 signapps.local ESMTP submission ready\r\n")
        .await?;
