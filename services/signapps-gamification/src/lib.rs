//! Public library interface for signapps-gamification.
//!
//! Exposes [`router`] so the single-binary runtime (`signapps-platform`)
//! can mount the XP / badges / streaks / leaderboard routes without
...
/// Build the gamification router using the shared runtime state.
///
/// # Errors
///
