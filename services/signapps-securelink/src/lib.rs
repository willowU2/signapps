//! Public library interface for signapps-securelink.
//!
//! Exposes [`router`] so the single-binary runtime can mount the
//! securelink routes (web tunnels, relays, DNS) without owning its own
//! pool. Spawns the rolling traffic-history sampler and (optionally)
//! the DHCP server inside [`router`] so they die with the service
//! factory.
...
/// Build the securelink router using the shared runtime state.
/// Spawns the rolling traffic-history sampler and (optionally) the DHCP
/// listener as detached tasks tied to the factory.
///
/// # Errors
///
    ...
    #[cfg(feature = "dhcp-server")]
    {
        ...
        if dhcp_enabled {
            ...
            let dhcp_port: u16 = std::env::var("DHCP_PORT")
                ...
                .and_then(|p| p.parse().ok())
                .unwrap_or(6767);
            let dhcp_domain = std::env::var("DHCP_DOMAIN").unwrap_or_else(|_| "local".to_string());
            ...
            // Inside the platform runtime we don't need a Ctrl+C bridge —
            // the supervisor cancels the spawned task on factory drop.
            let (_shutdown_tx, shutdown_rx) = tokio::sync::watch::channel(false);

            tracing::info!(port = dhcp_port, "DHCP server enabled — spawning listener");
            tokio::spawn(async move {
                if let Err(e) = dhcp::run_dhcp_listener(dhcp_config, pool, shutdown_rx).await {
