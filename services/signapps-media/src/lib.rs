//! Public library interface for signapps-media.
//!
//! Exposes [`router`] so the single-binary runtime can mount the media
//! routes (OCR / TTS / STT / voice WebSocket) without owning its own
//! pool. All backend selection (HTTP vs native vs stub) happens inside
//! [`router`] so it dies with the service factory.
...
#[derive(Clone)]
/// Application state for media service.
pub struct AppState {
    pub pool: PgPool,
...
/// Build the media router using the shared runtime state.
///
/// # Errors
///
/// Returns an error if a native STT/TTS/OCR backend fails to initialise
/// (only when the corresponding feature is enabled and HTTP fallback is
/// not configured).
pub async fn router(shared: SharedState) -> anyhow::Result<Router> {
    let state = build_state(&shared).await?;
    Ok(create_router(Arc::new(state)))
}
