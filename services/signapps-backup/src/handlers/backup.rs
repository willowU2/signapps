...
//!
//! Routes (admin-only):
//! - `POST /api/v1/backup`    — run pg_dump, return file metadata
//! - `GET  /api/v1/backups`   — list existing backup files
...
/// # Errors
///
/// Returns `Error::Internal` if `pg_dump` is not available or exits with a
/// non-zero status.
///
/// # Panics
///
...
/// # Errors
///
/// Returns `Error::Internal` if the backup directory cannot be read.
///
/// # Panics
///
/// No panics possible — all errors are propagated via `Result`.
