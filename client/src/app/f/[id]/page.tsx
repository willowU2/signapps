...
function isFieldVisible(field: ExtendedFormField, answers: Record<string, any>): boolean {
...
export default function PublicFormPage() {
  ...
  const handleFieldChange = (fieldId: string, value: any) => {
    ...
    if (touched[fieldId]) {
      ...
      if (field) {
        setFieldErrors(e => ({ ...e, [fieldId]: validateField(field, value) }))
      }
    }
  }
  ...
  const handleCheckboxToggle = (fieldId: string, option: string, checked: boolean) => {
    setAnswers(prev => {
      ...
      if (checked && !current.includes(option)) {
        ...
        if (touched[fieldId]) {
          const field = form?.fields?.find(f => f.id === fieldId)
          if (field) setFieldErrors(e => ({ ...e, [fieldId]: validateField(field, newVal) }))
        }
  ...
  const validatePageFields = (pageFields: ExtendedFormField[]): boolean => {
    ...
    for (const field of pageFields) {
      ...
      if (err) { errors[field.id] = err; valid = false }
    }
  ...
  const scoreResult = useMemo(() => {
    ...
    const hasScores = fields.some(f => f.scores && Object.keys(f.scores).length > 0)
      ...
      <Card className="max-w-md w-full shadow-xl border-t-4 border-t-emerald-500 overflow-hidden">
        ...
        <CardContent className="pt-10 pb-8 flex flex-col items-center text-center">
          ...
          <p className="text-muted-foreground">
            Votre réponse au formulaire "{form?.title}" a bien été enregistrée.
          </p>
          {scoreResult && scoreResult.maxScore > 0 && (
            <div className="mt-4 px-6 py-4 bg-primary/10 rounded-xl border border-primary/20">
              ...
              <p className="text-3xl font-bold text-primary mt-1">
                {scoreResult.score} <span className="text-lg text-muted-foreground">/ {scoreResult.maxScore}</span>
              </p>
  ...
  const renderField = (field: ExtendedFormField) => {
    ...
    return (
      <Card key={field.id} className="shadow-sm border-border/60 hover:border-border transition-colors">
        <CardContent className="pt-6">
          ...
          {(field.field_type === 'Text' || field.field_type === 'Email' || field.field_type === 'Number' || field.field_type === 'Date') && (
            <Input
              type={field.field_type === 'Text' ? 'text' : field.field_type.toLowerCase()}
                ...
                if (activeLayout === 'advanced-2') {
                  return (
                    ...
                    >
                      ...
                      <Label htmlFor={`${field.id}-${i}`} className="cursor-pointer font-semibold text-lg flex-1">{opt}</Label>
                    </div>
                  )
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="grid flex-1 gap-1 leading-none">
                        <Label htmlFor={`${field.id}-${i}`} className="cursor-pointer font-medium">{opt}</Label>
                      </div>
                    </div>
                  )
                } else {
              ...
              {field.options?.map((opt, i) => {
                ...
                return (
                  <div key={i} className="flex items-center space-x-3 cursor-pointer">
                    ...
                    />
                    <Label htmlFor={`cb-${field.id}-${i}`} className="font-normal cursor-pointer flex-1 py-1">{opt}</Label>
                  </div>
                )
              })}
            </div>
