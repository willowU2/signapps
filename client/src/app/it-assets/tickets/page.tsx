...
import {
  Ticket as TicketIcon, Plus, Clock, AlertTriangle,
  CheckCircle2, MessageSquare, Timer, Filter,
} from "lucide-react"
...
function SlaCountdown({ due }: { due?: string }) {
  ...
  if (diffMs <= 0) return <span className="text-red-600 text-xs font-semibold">OVERDUE</span>
  const diffH = Math.floor(diffMs / 3600000)
  const diffM = Math.floor((diffMs % 3600000) / 60000)
  const color = diffH < 2 ? "text-red-600" : diffH < 8 ? "text-yellow-600" : "text-emerald-600"
  return <span className={`${color} text-xs font-semibold`}>{diffH}h {diffM}m</span>
}
...
function KanbanView({ tickets, onSelect }: { tickets: Ticket[]; onSelect: (t: Ticket) => void }) {
  return (
...
function TicketDetailPanel({
  ...
  const commentMut = useMutation({
    mutationFn: () => itAssetsApi.addTicketComment(ticket.id, {
      content: comment,
  ...
  const timeMut = useMutation({
    mutationFn: () => itAssetsApi.logTicketTime(ticket.id, {
  ...
  const updateMut = useMutation({
    mutationFn: (status: string) => itAssetsApi.updateTicket(ticket.id, { status }),
    onSuccess: () => {
    ...
    <div className="fixed inset-y-0 right-0 w-[480px] bg-background border-l shadow-xl z-50 flex flex-col">
      ...
      <div className="flex items-start justify-between p-4 border-b">
        <div>
          ...
          <h2 className="font-semibold text-base line-clamp-2">{ticket.title}</h2>
        </div>
      ...
      <div className="px-4 py-2 border-b flex gap-2 flex-wrap">
        {STATUSES.filter(s => s !== ticket.status).map(s => (
          <Button key={s} size="sm" variant="outline" onClick={() => updateMut.mutate(s)}>
            → {STATUS_LABELS[s]}
          </Button>
        ))}
      </div>
      ...
      <Tabs defaultValue="comments" className="flex-1 overflow-hidden flex flex-col">
        ...
        {/* Comments tab */}
        <TabsContent value="comments" className="flex-1 overflow-auto flex flex-col px-4 gap-3 pb-2">
          <div className="flex-1 overflow-auto space-y-3 pt-2">
            {detail?.comments.map(c => (
              <div key={c.id} className={`rounded-lg p-3 text-sm ${c.is_internal ? "bg-yellow-50 border border-yellow-200 dark:bg-yellow-900/20" : "bg-muted/40"}`}>
                <div className="flex items-center gap-2 mb-1">
                  <span className="font-medium">{c.author_name ?? "Anonyme"}</span>
                  {c.is_internal && <Badge className="text-[10px] px-1 py-0 bg-yellow-200 text-yellow-800">Internal</Badge>}
          ...
          <div className="border-t pt-3 space-y-2">
            ...
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" disabled={!comment.trim() || commentMut.isPending} onClick={() => commentMut.mutate()}>
                Send
              </Button>
            </div>
          </div>
        ...
        {/* Time entries */}
        <TabsContent value="time" className="flex-1 overflow-auto flex flex-col px-4 gap-3 pb-2">
          ...
          <div className="border-t pt-3 space-y-2">
            ...
            <Button size="sm" disabled={!timeMinutes || timeMut.isPending} onClick={() => timeMut.mutate()}>
              Log Time
            </Button>
          </div>
        </TabsContent>
        ...
        {/* Details tab */}
        <TabsContent value="details" className="flex-1 overflow-auto px-4 py-2 text-sm space-y-3">
          {ticket.description && (
            <div>
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div><span className="text-muted-foreground">Requester:</span> <span>{ticket.requester_name ?? "—"}</span></div>
            <div><span className="text-muted-foreground">Email:</span> <span>{ticket.requester_email ?? "—"}</span></div>
            <div><span className="text-muted-foreground">Created:</span> <span>{new Date(ticket.created_at).toLocaleString()}</span></div>
            {ticket.resolved_at && <div><span className="text-muted-foreground">Resolved:</span> <span>{new Date(ticket.resolved_at).toLocaleString()}</span></div>}
          </div>
          {ticket.tags.length > 0 && (
            <div className="flex gap-1 flex-wrap">
              {ticket.tags.map(t => <Badge key={t} variant="outline" className="text-xs">{t}</Badge>)}
            </div>
          )}
        </TabsContent>
      </Tabs>
...
export default function TicketsPage() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowCreate(false)
      setForm({ title: "", description: "", priority: "medium", category: "", requester_name: "", requester_email: "" })
    ...
    <AppLayout>
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <Card>
            <CardContent className="pt-4">
              <div className={`text-2xl font-bold ${(stats?.sla_compliance_pct ?? 100) < 80 ? "text-red-600" : "text-emerald-600"}`}>
        ...
        {/* Kanban or List view */}
        {isLoading ? (
          <div className="text-center text-muted-foreground py-12">Loading tickets...</div>
        ) : view === "kanban" ? (
              ...
              <TableBody>
                {filtered.map(t => (
                  <TableRow key={t.id} className="cursor-pointer hover:bg-muted/50" onClick={() => setSelected(t)}>
                    ...
                    <TableCell><Badge className={`text-xs ${STATUS_COLORS[t.status]}`}>{STATUS_LABELS[t.status]}</Badge></TableCell>
                    <TableCell><Badge className={`text-xs ${PRIORITY_COLORS[t.priority]}`}>{t.priority}</Badge></TableCell>
                    <TableCell>{t.category ?? "—"}</TableCell>
                    <TableCell>{t.requester_name ?? "—"}</TableCell>
                    <TableCell><SlaCountdown due={t.sla_resolution_due} /></TableCell>
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Select value={form.hardware_id ?? ""} onValueChange={v => setForm(f => ({ ...f, hardware_id: v || undefined }))}>
                <SelectTrigger><SelectValue placeholder="None" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="">None</SelectItem>
                  {hardware.map(h => <SelectItem key={h.id} value={h.id}>{h.name}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </div>
