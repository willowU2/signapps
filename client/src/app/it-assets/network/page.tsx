...
import {
    Network, Search, Plus, RefreshCw, Radar, Monitor,
    Globe, Cpu, Server, Clock, Wifi,
} from "lucide-react"
...
function getOsIcon(osGuess?: string) {
    if (!osGuess) return <Monitor className="h-3.5 w-3.5 text-muted-foreground" />
    if (osGuess.toLowerCase().includes("windows")) return <Monitor className="h-3.5 w-3.5 text-blue-500" />
    if (osGuess.toLowerCase().includes("linux")) return <Server className="h-3.5 w-3.5 text-orange-500" />
...
export default function NetworkDiscoveryPage() {
    ...
    const [addDialog, setAddDialog] = useState<{ open: boolean; discoveryId: string; ip: string }>({
        open: false, discoveryId: "", ip: "",
    })
    ...
    const handleScan = async () => {
        ...
        try {
            ...
            toast.success(`Scan termine: ${res.data.hosts_found} hotes trouves sur ${res.data.hosts_scanned} analyses`)
            // Refresh discoveries
            loadDiscoveries()
        } catch (err: unknown) {
    ...
    const openAddDialog = (discovery: NetworkDiscovery) => {
        ...
        setAddDialog({ open: true, discoveryId: discovery.id, ip: discovery.ip_address })
    }
    ...
    const handleAddToInventory = async () => {
        ...
        setAdding(true)
        try {
            await itAssetsApi.addDiscoveryToInventory(addDialog.discoveryId, newAssetName.trim())
    ...
    return (
        <AppLayout>
            <div className="space-y-6">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button
                            ...
                            size="sm"
                            onClick={() => { setView("topology"); if (discoveries.length === 0) loadDiscoveries() }}
                        >
                ...
                <Card>
                    <CardHeader className="pb-3">
                        ...
                        <CardDescription>
                            Entrez un bloc CIDR (ex: 192.168.1.0/24). Le scan utilise des connexions TCP sur les ports courants.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        ...
                        {scanResult && (
                            <div className="mt-4 flex gap-4 text-sm">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="font-semibold">{scanResult.hosts_scanned}</span>
                                </div>
                                <div className="flex items-center gap-1.5">
                                    <Globe className="h-4 w-4 text-emerald-500" />
                                    <span className="text-muted-foreground">Trouves:</span>
                                    <span className="font-semibold text-emerald-600">{scanResult.hosts_found}</span>
                                </div>
                                <div className="flex items-center gap-1.5 text-muted-foreground text-xs">
                                    Sous-reseau: <code className="font-mono">{scanResult.subnet}</code>
                                </div>
                            </div>
                        )}
                    </CardContent>
                    ...
                    <Card>
                        <CardHeader className="pb-2">
                            <div className="flex items-center justify-between">
                                ...
                                >
                                    <RefreshCw className={`h-3.5 w-3.5 ${loadingDiscoveries ? "animate-spin" : ""}`} />
                                    Actualiser
                                </Button>
                            </div>
                        </CardHeader>
                                        ...
                                        {discoveries.length === 0 ? (
                                            <TableRow>
                                                <TableCell colSpan={8} className="text-center py-10 text-muted-foreground">
                                                    {loadingDiscoveries
                                                        ? "Chargement..."
                                                        : "Aucun hote decouvert. Lancez un scan ou cliquez sur Actualiser."}
                                                </TableCell>
                                            </TableRow>
                                        ) : (
                                            discoveries.map((d) => (
                                                <TableRow key={d.id}>
                                                    ...
                                                    <TableCell>
                                                        {d.hardware_id ? (
                                                            ...
                                                            <span className="text-muted-foreground text-xs">Non enregistre</span>
                                                        )}
                                                    </TableCell>
            ...
            {/* Add to inventory dialog */}
            <Dialog open={addDialog.open} onOpenChange={(o) => !o && setAddDialog(s => ({ ...s, open: false }))}>
                <DialogContent className="sm:max-w-[400px]">
                    ...
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setAddDialog(s => ({ ...s, open: false }))}>
                            Annuler
                        </Button>
                        <Button onClick={handleAddToInventory} disabled={adding || !newAssetName.trim()}>
