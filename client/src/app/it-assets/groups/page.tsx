...
import { Layers, Tag, Plus, Trash2, Edit, FolderTree, ChevronRight } from "lucide-react"
...
function useGroups() {
  return useQuery({
    queryKey: ["it-groups"],
    queryFn: () => client.get<DeviceGroup[]>("/it-assets/groups").then(r => r.data),
  })
}
...
function useTags() {
  return useQuery({
    queryKey: ["it-tags"],
    queryFn: () => client.get<DeviceTag[]>("/it-assets/tags").then(r => r.data),
  })
}

// ─── Group tree helpers ───────────────────────────────────────────────────────

function buildTree(groups: DeviceGroup[]): (DeviceGroup & { children: DeviceGroup[] })[] {
  ...
  return (
    <>
      <TableRow>
        ...
        <TableCell>
          <div className="flex gap-1">
            ...
            <Button size="sm" variant="ghost" className="text-red-500" onClick={() => onDelete(group.id)}>
      ...
      {expanded && group.children.map(child => (
        <GroupNode key={child.id} group={child as DeviceGroup & { children: DeviceGroup[] }} depth={depth + 1} onEdit={onEdit} onDelete={onDelete} />
      ))}
    </>
  )
}
...
export default function GroupsPage() {
  ...
  const createGroup = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["it-groups"] }); setGroupDialog(false); toast.success("Group created") },
  ...
  const updateGroup = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["it-groups"] }); setGroupDialog(false); toast.success("Group updated") },
    onError: () => toast.error("Failed to update group"),
  })

  const deleteGroup = useMutation({
    mutationFn: (id: string) => client.delete(`/it-assets/groups/${id}`),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["it-groups"] }); toast.success("Group deleted") },
  ...
  const createTag = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["it-tags"] }); setTagDialog(false); toast.success("Tag created") },
    onError: () => toast.error("Failed to create tag"),
  })

  const deleteTag = useMutation({
    mutationFn: (id: string) => client.delete(`/it-assets/tags/${id}`),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["it-tags"] }); toast.success("Tag deleted") },
    ...
    <AppLayout>
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Organize devices into hierarchical groups and label them with color tags
            </p>
          </div>
        </div>
        ...
        <Tabs defaultValue="groups">
          <TabsList>
            <TabsTrigger value="groups"><FolderTree className="h-4 w-4 mr-1" />Groups ({groups.length})</TabsTrigger>
            <TabsTrigger value="tags"><Tag className="h-4 w-4 mr-1" />Tags ({tags.length})</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="tags">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between py-3">
                ...
                <Button size="sm" onClick={() => { setTagName(""); setTagColor("#3b82f6"); setTagDialog(true) }}>
              ...
              <CardContent>
                {loadingTags ? (
                  ...
                  <div className="flex flex-wrap gap-3">
                    {tags.map(tag => (
                      <div key={tag.id} className="flex items-center gap-2 border rounded-full px-3 py-1.5">
                        <span
          ...
          <DialogContent>
            ...
            <div className="space-y-4 py-2">
              <div className="space-y-1">
                <Label>Name *</Label>
                <Input value={groupName} onChange={e => setGroupName(e.target.value)} placeholder="e.g. Engineering" />
              </div>
              <div className="space-y-1">
                <Label>Description</Label>
                <Textarea value={groupDesc} onChange={e => setGroupDesc(e.target.value)} rows={2} placeholder="Optional description" />
              </div>
              <div className="space-y-1">
          ...
          <DialogContent className="max-w-sm">
            ...
            <div className="space-y-4 py-2">
              <div className="space-y-1">
                ...
                <Input value={tagName} onChange={e => setTagName(e.target.value)} placeholder="e.g. Production" />
              </div>
              <div className="space-y-1">
                <Label>Color</Label>
            ...
            <DialogFooter>
              ...
              <Button
                onClick={() => createTag.mutate({ name: tagName, color: tagColor })}
