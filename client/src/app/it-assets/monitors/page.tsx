...
const monitorsApi = {
  ...
  create: (data: CreateMonitorRequest) =>
    client.post<CustomMonitor>("/it-assets/monitors", data),
  delete: (id: string) => client.delete(`/it-assets/monitors/${id}`),
  toggle: (id: string, enabled: boolean) =>
    client.patch<CustomMonitor>(`/it-assets/monitors/${id}`, { enabled }),
};
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>New Custom Monitor</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
            ...
            <div className="space-y-3 rounded-md border p-3 bg-muted/30">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    set("expected_status_code", Number(e.target.value))
                  }
                />
              </div>
              <div>
                <Label className="text-xs">
                  Response must contain (optional)
                </Label>
                <Input
                  className="mt-1"
                  value={form.response_contains ?? ""}
                  onChange={(e) => set("response_contains", e.target.value)}
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(v) =>
                set("interval_minutes", Number(v) as Interval)
              }
            >
              <SelectTrigger className="mt-1">
                <SelectValue />
          ...
          <div className="flex items-center gap-3">
            <Switch
              ...
              onCheckedChange={(v) => set("alert_on_failure", v)}
            />
...
export default function CustomMonitorsPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["custom-monitors"] });
      setShowForm(false);
    },
  });
  ...
  const stats = {
    ...
    ok: monitors.filter((m) => m.last_status === "ok").length,
    fail: monitors.filter((m) => m.last_status === "fail").length,
  };
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Define and schedule custom health checks for any endpoint or
              service
            </p>
          </div>
          ...
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <h3 className="text-lg font-semibold">Aucun moniteur</h3>
                <p className="text-sm text-muted-foreground mt-1 max-w-sm">
                  Creez un moniteur pour surveiller la disponibilite de vos
                  services.
                </p>
              </div>
            ) : (
