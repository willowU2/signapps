...
export default function MonitoringPage() {
  ...
  const { data: metrics = [], isLoading: metricsLoading } = useQuery<AgentMetric[]>({
    queryKey: ["metrics", selectedHw, range],
    queryFn: () => itAssetsApi.getMetrics(selectedHw, range).then(r => r.data),
    enabled: !!selectedHw,
  })
  ...
  const chartData = metrics.map(m => ({
    ...
    memory: m.memory_usage != null ? Math.round(m.memory_usage * 10) / 10 : null,
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Real-time CPU, RAM, and disk metrics per machine</p>
          </div>
              ...
              <SelectContent>
                {hardware.map(hw => (
                  <SelectItem key={hw.id} value={hw.id}>
                    {hw.name} {hw.status && <span className="text-muted-foreground">— {hw.status}</span>}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
        ...
        {selectedMachine && (
          <div className="flex gap-2 flex-wrap">
            ...
            {selectedMachine.manufacturer && <Badge variant="outline">{selectedMachine.manufacturer}</Badge>}
            {selectedMachine.location && <Badge variant="outline">{selectedMachine.location}</Badge>}
          </div>
        )}
        ...
        {!selectedHw ? (
          ...
          <Card>
            <CardContent className="py-12 text-center text-muted-foreground">Loading metrics...</CardContent>
          </Card>
        ) : chartData.length === 0 ? (
          <Card>
            <CardContent className="py-12 text-center text-muted-foreground">
          ...
          <div className="grid gap-4">
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium flex items-center gap-2">
                  <MemoryStick className="h-4 w-4 text-emerald-500" /> Memory Usage (%)
                </CardTitle>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={200}>
                ...
                {activeAlerts.map(alert => (
                  <div key={alert.id} className="flex items-center justify-between p-3 rounded-lg bg-destructive/5 border border-destructive/20">
                    <div className="text-sm">
                      <span className="font-medium">Machine {alert.hardware_id.slice(0, 8)}</span>
                      {alert.value != null && <span className="text-muted-foreground ml-2">value: {alert.value.toFixed(1)}</span>}
                    </div>
