...
import { itAssetsApi, ConfigurationItem, CIRelationship } from "@/lib/api/it-assets"
import { usePageTitle } from "@/hooks/use-page-title"

const CI_TYPES = ["server", "workstation", "network", "storage", "application", "service", "database", "other"]
const REL_TYPES = ["depends_on", "hosted_on", "connected_to", "backup_of", "part_of", "monitors"]
...
function ForceGraph({ cis, relationships, selectedId, onSelect }: {
  ...
  return (
    <svg ref={svgRef} viewBox={`0 0 ${W} ${H}`} className="w-full border rounded-lg bg-muted/30">
      <defs>
      ...
      {edges.map(e => {
        ...
        return (
          <g key={e.id}>
            <line x1={src.x} y1={src.y} x2={ex} y2={ey}
              stroke="#6b7280" strokeWidth={1.5} markerEnd="url(#arrow)" strokeOpacity={0.6} />
            <text x={(src.x + ex) / 2} y={(src.y + ey) / 2 - 4}
      ...
      {nodes.map(n => (
        <g key={n.id} onClick={() => onSelect(n.id)} className="cursor-pointer">
          <circle cx={n.x} cy={n.y} r={R}
            ...
            fillOpacity={0.85}
          />
          <text x={n.x} y={n.y + 4} textAnchor="middle" fontSize={10} fill="white" fontWeight="600">
            {n.name.slice(0, 6)}
          </text>
        </g>
      ))}
...
export default function CmdbPage() {
  ...
  const { data: rels = [] } = useQuery<CIRelationship[]>({
    ...
    queryFn: () => itAssetsApi.listCIRelationships(selectedCI!).then(r => r.data),
    enabled: !!selectedCI,
  })
  ...
  const filtered = typeFilter === "all" ? cis : cis.filter(c => c.ci_type === typeFilter)
  ...
  const { data: allRels = [] } = useQuery<CIRelationship[]>({
    ...
    queryFn: async () => {
      ...
      for (const ci of cis.slice(0, 20)) {
        try {
          const r = await itAssetsApi.listCIRelationships(ci.id).then(res => res.data)
          all.push(...r)
        } catch { /* skip */ }
      }
      // deduplicate
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Configuration items and relationships</p>
          </div>
          ...
          <Card>
            ...
            <CardContent>
              <ForceGraph
                ...
                onSelect={(id) => setSelectedCI(prev => prev === id ? null : id)}
              />
              <p className="text-xs text-muted-foreground mt-2 text-center">Click a node to see impact analysis</p>
            </CardContent>
          </Card>
        )}
          ...
          <Card className="border-primary/30">
            <CardHeader>
              <CardTitle className="text-sm font-medium flex items-center gap-2">
                ...
                Impact Analysis: <span className="text-primary">{selectedCIObj?.name}</span>
              </CardTitle>
            </CardHeader>
            <CardContent>
              {impact.length === 0 ? (
                <p className="text-sm text-muted-foreground">No dependent CIs found</p>
              ) : (
                <div className="flex flex-wrap gap-2">
                  {impact.map(ci => (
                    <Badge key={ci.id} variant="outline">{ci.name} <span className="ml-1 text-muted-foreground">({ci.ci_type})</span></Badge>
                  ))}
                </div>
              )}
            </CardContent>
          ...
          <CardContent className="p-0">
            <Table>
              ...
              <TableBody>
                {filtered.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={5} className="text-center text-muted-foreground py-8">No CIs found</TableCell>
                  </TableRow>
                )}
                {filtered.map(ci => (
                  <TableRow
                    ...
                    onClick={() => setSelectedCI(prev => prev === ci.id ? null : ci.id)}
                  >
                    <TableCell className="font-medium">{ci.name}</TableCell>
                    <TableCell><Badge variant="secondary">{ci.ci_type}</Badge></TableCell>
                    <TableCell>
                      <Badge variant={ci.status === "active" ? "default" : "outline"}>{ci.status ?? "unknown"}</Badge>
                    </TableCell>
                    ...
                    <TableCell>
                      <Button
                        ...
                        onClick={(e) => { e.stopPropagation(); deleteMut.mutate(ci.id) }}
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label>Type</Label>
              <Select value={createForm.ci_type} onValueChange={v => setCreateForm(p => ({ ...p, ci_type: v }))}>
