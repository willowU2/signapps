...
import {
    MapPin, Search, Server, MonitorSmartphone, Network, Cpu, Printer,
...
function getTypeIcon(type: string, size = "h-4 w-4") {
    switch (type) {
        case "server":      return <Server className={`${size} text-blue-500`} />
        case "switch":      return <Network className={`${size} text-purple-500`} />
        case "workstation": return <Cpu className={`${size} text-indigo-500`} />
        case "printer":     return <Printer className={`${size} text-amber-500`} />
        default:            return <MonitorSmartphone className={`${size} text-emerald-500`} />
    }
}
...
function statusColor(status?: string): string {
    switch (status) {
        ...
        case "retired":     return "#94a3b8"   // gray
...
function healthLabel(score: number): { label: string; badge: string } {
    if (score >= 80) return { label: "Sain", badge: "bg-emerald-500/10 text-emerald-600" }
    if (score >= 50) return { label: "Attention", badge: "bg-yellow-500/10 text-yellow-700" }
    return { label: "Critique", badge: "bg-red-500/10 text-red-600" }
}

const STATUS_LABEL: Record<string, string> = {
    active: "Actif", maintenance: "Maintenance", retired: "Retire", stock: "En stock",
}
    ...
    return (
        ...
        >
            ...
            {group.healthScore < 50 && (
                <circle r={24} fill="none" stroke="#ef4444" strokeWidth={2} opacity={0.3}
                    className="animate-ping" style={{ animationDuration: "2s" }} />
            )}
            <circle
                r={isSelected ? 22 : 18}
                fill={group.healthScore >= 80 ? "#10b981" : group.healthScore >= 50 ? "#f59e0b" : "#ef4444"}
                stroke="white"
    ...
    const siteGroups = useMemo<SiteGroup[]>(() => {
        ...
        return Array.from(map.entries()).map(([site, assets]) => {
            ...
            const healthScore   = assets.length > 0
                ...
                : 100
            return { site, assets, healthScore: Math.max(0, Math.min(100, healthScore)) }
        })
    ...
    const filteredGroups = useMemo(() =>
        search.trim()
            ? siteGroups.filter(g =>
                ...
                g.assets.some(a => a.name.toLowerCase().includes(search.toLowerCase()))
            )
            : siteGroups,
        [siteGroups, search]
    )

    const selectedGroup = selectedSite ? filteredGroups.find(g => g.site === selectedSite) : null
    ...
    return (
        <AppLayout>
            <div className="container mx-auto max-w-7xl space-y-6 p-6">
                ...
                <div className="grid grid-cols-4 gap-3">
                    ...
                    <Card>
                        <CardContent className="p-4">
                            ...
                            <p className={`text-2xl font-bold ${activeAlerts.length > 0 ? "text-red-600" : "text-emerald-600"}`}>
                                {activeAlerts.length}
                            </p>
                        </CardContent>
                    </Card>
                            ...
                            <div className="flex gap-3 px-2 pb-2 text-xs text-muted-foreground">
                                {[
                                    ...
                                    <span key={label} className="flex items-center gap-1">
                                        <span className={`h-2.5 w-2.5 rounded-full inline-block ${color}`} />
                                        {label}
                                    </span>
                                ))}
                                <span className="text-muted-foreground ml-2">Chiffre = nb d&apos;equipements</span>
                            </div>
                                ...
                                <div className="overflow-hidden rounded-lg bg-muted/30 border border-dashed">
                                    ...
                                    >
                                        ...
                                        {PIN_POSITIONS.map(pos => {
                                            const group = filteredGroups.find(g => g.site === pos.site)
                        ...
                        <CardContent>
                            {!selectedGroup ? (
                                <div className="space-y-2">
                                    <p className="text-xs text-muted-foreground">Cliquez sur un pin pour voir les details</p>
                                    <div className="space-y-1.5 border-t pt-2">
                                        {filteredGroups.slice(0, 8).map(g => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    <span className={`h-2 w-2 rounded-full shrink-0 ${sitePinColor(g.healthScore)}`} />
                                                    <span className="flex-1 truncate">{g.site}</span>
                                                    <Badge className={`text-xs ${badge}`}>{g.assets.length}</Badge>
                                                </button>
                                            )
                                        })}
                                ...
                                <div className="space-y-3">
                                    {/* Site health */}
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs text-muted-foreground">Sante du site</span>
                                        <Badge className={`text-xs ${healthLabel(selectedGroup.healthScore).badge}`}>
                                            {healthLabel(selectedGroup.healthScore).label} — {selectedGroup.healthScore}%
                                        </Badge>
                                    </div>
                                    ...
                                    <div className="space-y-1 max-h-64 overflow-y-auto">
                                        {selectedGroup.assets.map(asset => {
                                            const alertCount = activeAlerts.filter(a => a.hardware_id === asset.id).length
                                    ...
                                    {selectedAsset && (
                                        <div className="rounded-lg border bg-muted/30 p-2.5 text-xs space-y-1">
                                            ...
                                            <div className="grid grid-cols-2 gap-x-2 gap-y-0.5 text-muted-foreground">
                                                ...
                                                <span>Statut</span>     <span>{STATUS_LABEL[selectedAsset.status ?? ""] ?? selectedAsset.status}</span>
                                                {selectedAsset.manufacturer && <><span>Fabricant</span><span>{selectedAsset.manufacturer}</span></>}
                                                {selectedAsset.model && <><span>Modele</span><span>{selectedAsset.model}</span></>}
                                                {selectedAsset.serial_number && <><span>S/N</span><span className="font-mono">{selectedAsset.serial_number}</span></>}
                                            </div>
                                            {activeAlerts.filter(a => a.hardware_id === selectedAsset.id).length > 0 && (
                                                <div className="flex items-center gap-1 text-red-600 mt-1">
                                                    <AlertTriangle className="h-3 w-3 shrink-0" />
                                                    {activeAlerts.filter(a => a.hardware_id === selectedAsset.id).length} alerte(s) active(s)
                                                </div>
                                            )}
                                        </div>
                                    )}
                                    ...
                                    <Button
                                        ...
                                        onClick={() => { setSelectedSite(null); setSelectedAsset(null) }}
                                    >
                ...
                {/* All sites grid */}
                <div>
                    <h2 className="text-sm font-semibold mb-3">Tous les sites ({filteredGroups.length})</h2>
                    {filteredGroups.length === 0 ? (
                        <div className="rounded-xl border border-dashed p-12 text-center text-muted-foreground text-sm">
                            {isLoading ? "Chargement des equipements..." : "Aucun equipement avec localisation"}
                        </div>
                    ) : (
                            ...
                            {filteredGroups.map(group => {
                                ...
                                return (
                                    <button
                                        ...
                                        onClick={() => setSelectedSite(group.site === selectedSite ? null : group.site)}
                                    >
                                        <div className="flex items-start justify-between mb-2">
                                            <MapPin className={`h-4 w-4 shrink-0 ${
                                                group.healthScore >= 80 ? "text-emerald-500"
                                                : group.healthScore >= 50 ? "text-yellow-500"
                                                : "text-red-500"
                                            }`} />
                                            <Badge className={`text-xs ${hBadge}`}>{hLabel}</Badge>
                                        </div>
                                        <p className="text-sm font-semibold leading-tight mb-1 truncate">{group.site}</p>
                                        <p className="text-xs text-muted-foreground">
                                            {group.assets.length} equipement{group.assets.length > 1 ? "s" : ""}
                                        </p>
                                        {alertedAssets.length > 0 && (
                                            <p className="flex items-center gap-1 text-xs text-red-600 mt-1">
                                                <AlertTriangle className="h-3 w-3" />
                                                {alertedAssets.length} alerte{alertedAssets.length > 1 ? "s" : ""}
                                            </p>
                                        )}
                                    </button>
                                )
