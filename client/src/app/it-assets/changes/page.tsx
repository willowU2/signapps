...
import { itAssetsApi, ChangeRequest, CreateChangeRequestInput } from "@/lib/api/it-assets"
import { usePageTitle } from "@/hooks/use-page-title"

const STATUSES = ["submitted", "reviewed", "approved", "implemented", "verified"]
...
export default function ChangesPage() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowCreate(false)
      setForm({ title: "", description: "", impact_analysis: "", risk_level: "low" })
    },
  })
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">RFC workflow: submit → review → approve → implement → verify</p>
          </div>
          ...
          {STATUSES.map(s => {
            ...
            return (
              <div key={s} className="flex-1 min-w-28 text-center">
                <div className={`text-xs font-semibold py-1 px-2 rounded-lg ${STATUS_COLORS[s] ?? "bg-muted text-muted-foreground"}`}>
                  {s} ({count})
                </div>
              </div>
            )
              ...
              <TableBody>
                ...
                {changes.map(c => (
                  <TableRow key={c.id} className="cursor-pointer" onClick={() => setSelected(c)}>
                    ...
                    <TableCell>
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${RISK_COLORS[c.risk_level ?? "low"] ?? "bg-muted text-muted-foreground"}`}>
                        {c.risk_level ?? "low"}
                      </span>
                    </TableCell>
                    <TableCell>
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_COLORS[c.status ?? "submitted"] ?? "bg-muted text-muted-foreground"}`}>
                        {c.status ?? "submitted"}
                      </span>
                    </TableCell>
          ...
          <DialogContent className="max-w-lg">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex gap-2">
                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_COLORS[selected.status ?? "submitted"]}`}>
                  {selected.status}
                </span>
                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${RISK_COLORS[selected.risk_level ?? "low"]}`}>
