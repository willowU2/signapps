...
import {
  Package, Plus, Trash2, Edit, Download, Search, Upload,
  MonitorSmartphone, CheckCircle2, Clock, AlertCircle, Store,
} from "lucide-react"
...
const APP_STORE_CATALOG = [
  { name: "Google Chrome", publisher: "Google", category: "Navigateur", icon: "🌐" },
  { name: "Mozilla Firefox", publisher: "Mozilla", category: "Navigateur", icon: "🦊" },
  { name: "Visual Studio Code", publisher: "Microsoft", category: "Dev", icon: "💻" },
  { name: "7-Zip", publisher: "7-Zip", category: "Utilitaires", icon: "📦" },
  { name: "VLC Media Player", publisher: "VideoLAN", category: "Multimédia", icon: "🎬" },
  { name: "Notepad++", publisher: "Notepad++", category: "Éditeur", icon: "📝" },
  { name: "WinSCP", publisher: "WinSCP", category: "Réseau", icon: "🔌" },
  { name: "PuTTY", publisher: "Simon Tatham", category: "Réseau", icon: "🔒" },
  { name: "LibreOffice", publisher: "LibreOffice", category: "Bureautique", icon: "📄" },
  { name: "Zoom", publisher: "Zoom Video", category: "Communication", icon: "📹" },
...
export default function PackagesPage() {
  ...
  const [form, setForm] = useState<CreatePackageForm>({
    name: "", version: "", publisher: "", platform: "windows",
    installer_type: "msi", silent_args: "", file_path: "",
  })
  ...
  const deployMutation = useMutation({
    mutationFn: ({ id, hardwareIds, scheduledAt }: { id: string; hardwareIds: string[]; scheduledAt?: string }) =>
  ...
  const filteredStore = useMemo(() => {
    return APP_STORE_CATALOG.filter(app => {
      const matchSearch = !storeSearch || app.name.toLowerCase().includes(storeSearch.toLowerCase())
      const matchCat = storeCategory === "all" || app.category === storeCategory
  ...
  const openCreate = () => {
    ...
    setForm({ name: "", version: "", publisher: "", platform: "windows", installer_type: "msi", silent_args: "", file_path: "" })
    setDialogOpen(true)
  }
  ...
  const installerLabel = (t: string) => INSTALLER_TYPES.find(it => it.value === t)?.label ?? t
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-sm">Catalogue, déploiement et libre-service</p>
          </div>
              ...
              <CardContent>
                {packagesLoading ? (
                  ...
                  <div className="text-center py-12 text-muted-foreground">
                    ...
                    <p>Aucun paquet. Ajoutez votre premier logiciel.</p>
                  </div>
                ) : (
                  <Table>
                    <TableHeader>
                    ...
                    <TableBody>
                      {filteredPackages.map(pkg => (
                        <TableRow key={pkg.id}>
                          ...
                          <TableCell className="text-muted-foreground text-sm">{pkg.publisher ?? "—"}</TableCell>
                          <TableCell>
                            <Badge variant="secondary" className="text-xs">{platformLabel(pkg.platform)}</Badge>
                          </TableCell>
                          <TableCell className="text-muted-foreground text-xs">{installerLabel(pkg.installer_type)}</TableCell>
                          <TableCell className="text-muted-foreground text-xs">
                            {pkg.file_size ? `${(pkg.file_size / 1024 / 1024).toFixed(1)} MB` : "—"}
                          </TableCell>
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-1">
                              <Button size="icon" variant="ghost" className="h-7 w-7" title="Déployer"
          ...
          <TabsContent value="store">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Applications disponibles pour les utilisateurs finaux. Cliquez sur &quot;Installer&quot; pour déclencher un déploiement.
                </CardDescription>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {filteredStore.map(app => {
                    ...
                    const catalogPkg = (packages ?? []).find(
                      p => p.name.toLowerCase().includes(app.name.split(" ")[0].toLowerCase())
                    )
                    return (
                      <div key={app.name} className="p-4 border rounded-md flex items-start gap-3 hover:bg-muted/20 transition-colors">
                        <div className="text-2xl shrink-0">{app.icon}</div>
                        <div className="flex-1 min-w-0">
                          <div className="font-medium text-sm">{app.name}</div>
                          <div className="text-xs text-muted-foreground">{app.publisher}</div>
                        ...
                        {catalogPkg ? (
                          ...
                          <Button size="sm" variant="ghost" disabled className="shrink-0 text-xs text-muted-foreground">
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle>{editingPackage ? "Modifier le paquet" : "Ajouter un paquet"}</DialogTitle>
            </DialogHeader>
            <div className="space-y-4 py-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <Select value={form.installer_type} onValueChange={v => setForm(f => ({ ...f, installer_type: v }))}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      {INSTALLER_TYPES.map(t => <SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-1">
                  <Label>Arguments silencieux</Label>
                  <Input value={form.silent_args} onChange={e => setForm(f => ({ ...f, silent_args: e.target.value }))} placeholder="/quiet /norestart" />
                </div>
              </div>
              <div className="space-y-1">
            ...
            <DialogFooter>
              ...
              <Button
                onClick={handleSave}
                disabled={!form.name || !form.version || createMutation.isPending || updateMutation.isPending}
              >
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <div className="max-h-48 overflow-y-auto border rounded-md divide-y">
                  {(hardware ?? []).length === 0 ? (
                    <div className="p-3 text-sm text-muted-foreground">Aucune machine disponible</div>
                  ) : (
                    (hardware ?? []).map(hw => (
                      <label key={hw.id} className="flex items-center gap-3 p-3 hover:bg-muted/30 cursor-pointer">
                        <input
                          type="checkbox"
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-muted-foreground">{hw.type} · {hw.location ?? "—"}</div>
                        </div>
                        {hw.status && (
                          <Badge variant={hw.status === "active" ? "default" : "outline"} className="text-xs">
                            {hw.status}
                          </Badge>
                        )}
                      </label>
              ...
              <div className="space-y-1">
                ...
                />
                <div className="text-xs text-muted-foreground">
                  Laissez vide pour déployer immédiatement lors du prochain heartbeat agent
                </div>
              </div>
            </div>
            <DialogFooter>
              ...
              <Button
                ...
                disabled={selectedMachines.length === 0 || deployMutation.isPending}
              >
