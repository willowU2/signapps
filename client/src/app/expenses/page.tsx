...
type ExpenseCategory = 'Transport' | 'Repas' | 'Hôtel' | 'Fournitures' | 'Autre'
...
const STATUS_CONFIG: Record<ExpenseStatus, { label: string; color: string; icon: React.ReactNode }> = {
  draft: { label: 'Brouillon', color: 'bg-muted text-muted-foreground', icon: <Clock className="w-3 h-3" /> },
  submitted: { label: 'En attente', color: 'bg-blue-100 text-blue-700', icon: <Send className="w-3 h-3" /> },
  approved: { label: 'Approuvé', color: 'bg-green-100 text-green-700', icon: <CheckCircle2 className="w-3 h-3" /> },
  rejected: { label: 'Rejeté', color: 'bg-red-100 text-red-700', icon: <XCircle className="w-3 h-3" /> },
  paid: { label: 'Payé', color: 'bg-purple-100 text-purple-700', icon: <DollarSign className="w-3 h-3" /> },
}

const CATEGORIES: ExpenseCategory[] = ['Transport', 'Repas', 'Hôtel', 'Fournitures', 'Autre']
const LS_KEY = 'signapps_expenses_v1'
...
function ExpenseFormDialog({ open, onClose, onSave, initial }: ExpenseFormDialogProps) {
          ...
          <div>
            ...
            >
              {scanning ? (
                ...
                <><Camera className="w-4 h-4 mr-2" /><Sparkles className="w-4 h-4 mr-1 text-primary" /> Scanner un reçu (OCR IA)</>
              )}
            </Button>
            {form.receiptDataUrl && form.receiptDataUrl.startsWith('data:image') && (
      ...
      <DialogContent>
        ...
        <div className="space-y-3">
          <div className="rounded-lg border p-3 bg-muted/30 space-y-1 text-sm">
            <p><span className="font-medium">Fournisseur:</span> {expense.vendor || '—'}</p>
            <p><span className="font-medium">Montant:</span> {expense.amount.toFixed(2)} €</p>
            <p><span className="font-medium">Date:</span> {expense.date}</p>
            <p><span className="font-medium">Catégorie:</span> {expense.category}</p>
        ...
        <DialogFooter>
          ...
          <Button variant="destructive" onClick={() => onDecision(expense.id, 'rejected', comment)}>
...
export default function ExpensesPage() {
  ...
  const handleSave = (data: Omit<Expense, 'id' | 'status'>) => {
    const newExpense: Expense = { ...data, id: genId(), status: 'draft' }
  ...
  const handleSubmit = (id: string) => {
    ...
    if (expense.amount < AUTO_APPROVE_THRESHOLD) {
      persist(
        expenses.map((e) =>
          e.id === id
            ? { ...e, status: 'approved', approvalComment: 'Auto-approuvé (montant < seuil)', approvedAt: new Date().toISOString() }
            : e
        )
      )
      toast.success(`Auto-approuvé (montant < ${AUTO_APPROVE_THRESHOLD}€)`)
    } else {
      persist(
        expenses.map((e) =>
          e.id === id ? { ...e, status: 'submitted', submittedAt: new Date().toISOString() } : e
        )
      )
  ...
  const handleDecision = (id: string, decision: 'approved' | 'rejected', comment: string) => {
    persist(
      expenses.map((e) =>
        e.id === id
          ? {
    ...
    toast.success(decision === 'approved' ? 'Note approuvée' : 'Note rejetée')
  }
  ...
  const totals = {
    ...
    pending: expenses.filter((e) => e.status === 'submitted').reduce((s, e) => s + e.amount, 0),
  }
    ...
    <div className="container max-w-5xl py-6 space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: 'Approuvé', value: totals.approved, color: 'text-green-600' },
          { label: 'En attente', value: totals.pending, color: 'text-blue-600' },
        ].map((s) => (
          <Card key={s.label}>
            <CardContent className="p-4">
              <p className="text-sm text-muted-foreground">{s.label}</p>
              <p className={`text-2xl font-bold ${s.color}`}>{s.value.toFixed(2)} €</p>
            </CardContent>
          </Card>
        ))}
      </div>
              ...
              filtered.map((e) => {
                ...
                return (
                  <TableRow key={e.id}>
                    ...
                    <TableCell className="max-w-[120px] truncate">{e.vendor || '—'}</TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      ...
                      {e.approvalComment && (
                        <p className="text-xs text-muted-foreground mt-0.5 truncate max-w-[120px]" title={e.approvalComment}>
                          {e.approvalComment}
                        </p>
                      )}
                    </TableCell>
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-1">
                        {e.status === 'draft' && (
                          <Button size="sm" variant="ghost" onClick={() => handleSubmit(e.id)}>
                            <Send className="w-4 h-4" />
                          </Button>
                        )}
                        {e.status === 'submitted' && (
                          <Button size="sm" variant="ghost" onClick={() => setApprovalTarget(e)}>
