...
import { APP_CATEGORIES, AppEntry } from "@/lib/app-registry";
import { useAppRegistry } from "@/hooks/use-app-registry";
import { cn } from "@/lib/utils";
// AQ-PERF: lazy-load each lucide icon individually instead of the full
// barrel.  See sidebar.tsx for the same pattern.
    ...
    >
      <div className="flex flex-col h-full justify-between">
        <div className="flex items-start gap-4">
          <div
            className={cn(
              "flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-muted/60",
              app.color,
            )}
          >
...
export default function AllAppsPage() {
  ...
  const filteredApps = useMemo(() => {
    ...
    return result;
  }, [search, activeCategory, appRegistry]);
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Lancez vos applications intelligentes depuis votre portail
              centralisé.
            </p>
          </div>
        </div>
