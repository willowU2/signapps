...
import {
  GuacamoleDisplay,
  ConnectionStatus,
} from "@/components/remote/guacamole-display";
...
export default function RemoteAccessDashboard() {
  ...
  const handleUpdateConnection = async () => {
    ...
    try {
      setEditing(true);
      const updated = (
        await remoteApi.updateConnection(activeConnection.id, editConnection)
      ).data;
      setConnections((prev) =>
  ...
  return (
    <AppLayout>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm">
              Accs scuris via navigateur votre infrastructure (RDP, VNC, SSH).
            </p>
          </div>
                ...
                <div className="grid gap-4 py-4">
                  ...
                  <div className="grid gap-2">
                    ...
                    <Select
                      ...
                      onValueChange={(v: "rdp" | "vnc" | "ssh" | "telnet") =>
                        setNewConnection((prev) => ({
                          ...prev,
                          protocol: v,
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <Card className="border-border/50 bg-card overflow-hidden relative group">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Guacamole translation layer actif
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="border-border/50 bg-card overflow-hidden relative group">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                vnements enregistrs aujourd'hui
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        {loading ? (
          ...
          <Card className="p-12 text-center">
            ...
            <h3 className="text-lg font-semibold">
              Aucune connexion configure
            </h3>
            <p className="text-muted-foreground mt-2">
              Crez votre premire connexion pour accder vos serveurs.
            </p>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {connections.map((conn) => (
              <Card
                key={conn.id}
                className="group hover:border-blue-500/50 transition-colors overflow-hidden relative"
              >
                <div
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <CardDescription className="flex items-center gap-1 mt-0.5">
                          {conn.username && (
                            <>
                              <Lock className="h-3 w-3" /> {conn.username}
                            </>
                          )}
                          {!conn.username && (
            ...
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Enregistre toutes les instructions Guacamole pour rejouer
                    ulterieurement
                  </p>
                </div>
              </div>
              <button
                ...
                onClick={() =>
                  setEditConnection((prev) => ({
                    ...prev,
                    recording_enabled: !prev.recording_enabled,
                  }))
                }
              >
                <span
                  className={`inline-block h-3 w-3 transform rounded-full bg-white dark:bg-zinc-200 transition-transform ${editConnection.recording_enabled ? "translate-x-5" : "translate-x-1"}`}
                />
              </button>
            </div>
          </div>
      ...
      <Dialog
        open={viewerOpen}
        onOpenChange={(open) => !open && handleDisconnect()}
      >
      ...
      >
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleDeleteConnectionConfirm}>
              Supprimer
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
