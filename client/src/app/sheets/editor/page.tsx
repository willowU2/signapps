...
function SheetsContent() {
  ...
  useEffect(() => {
    if (id && id !== "new-spreadsheet") {
      trackDocVisit({
        id,
        name: name || "Classeur sans titre",
        kind: "sheet",
        href: `/sheets/editor?id=${id}&name=${encodeURIComponent(name)}`,
      });
    }
  }, [id, name]);

  const [initialData, setInitialData] = useState<any>(undefined);
  const [initialColWidths, setInitialColWidths] = useState<
    Record<number, number> | undefined
  >(undefined);
  const [initialRowHeights, setInitialRowHeights] = useState<
    Record<number, number> | undefined
  >(undefined);
  ...
  useEffect(() => {
    ...
    const templateContent =
      typeof window !== "undefined" ? localStorage.getItem(templateKey) : null;
    if (templateContent) {
      try {
      ...
      <div className="flex flex-col h-full relative">
        ...
        <div className="flex-1 overflow-hidden relative">
          {!loading && (
            <Spreadsheet
              documentId={id}
              documentName={name || "document.xlsx"}
