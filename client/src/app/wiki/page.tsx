...
import {
  BookOpen, Plus, Trash2, Edit2, Save, X, ChevronRight, ChevronDown,
...
export default function WikiPage() {
  ...
  const saveEdit = useCallback(() => {
    ...
    setPages(prev => prev.map(p =>
      p.id === selected.id
        ? { ...p, title: draftTitle.trim() || p.title, content: draft, updatedAt: new Date().toISOString() }
        : p
    ));
    setEditing(false);
  ...
  const deletePage = useCallback((id: string) => {
    if (id === 'home') { toast.error('La page d\'accueil ne peut pas être supprimée'); return; }
      ...
      <div className="flex h-[calc(100vh-8rem)] gap-0 border rounded-lg overflow-hidden">
        ...
        <aside className="w-56 shrink-0 border-r flex flex-col bg-muted/20">
              ...
              filteredPages.length === 0
                ...
                : filteredPages.map(p => (
                    <div
                      ...
                      onClick={() => { setSelectedId(p.id); setSearchQuery(''); setEditing(false); }}
                    >
                      <FileText className="h-3.5 w-3.5 shrink-0" />
          ...
          <div className="p-2 border-t">
            <Button size="sm" variant="outline" onClick={addPage} className="w-full gap-1.5 text-xs">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <Tabs defaultValue="page" className="flex flex-col flex-1">
            <div className="flex items-center justify-between px-4 pt-3 pb-0 border-b">
              ...
              {!editing ? (
                <Button size="sm" variant="ghost" onClick={startEdit} className="gap-1.5 text-xs h-7">
                  <Edit2 className="h-3.5 w-3.5" />Modifier
                </Button>
              ) : (
                <div className="flex gap-1">
                  <Button size="sm" variant="ghost" onClick={() => setEditing(false)} className="gap-1 text-xs h-7">
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Mis à jour le {new Date(selected.updatedAt).toLocaleDateString('fr-FR')}
                      </p>
                    </div>
                    <div className="flex gap-1">
                      {selected.tags.map(t => (
