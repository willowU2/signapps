...
export default function NewsletterPage() {
  ...
  const {
    ...
    queryFn: () => commsApi.listNewsletters(),
  });
  ...
  const saveDraft = async () => {
    ...
    try {
      if (editing) {
        await commsApi.updateNewsletter(editing.id, { subject, sections });
      } else {
        await commsApi.createNewsletter({ subject, sections });
      }
  ...
  const sendNow = async () => {
    if (!subject.trim() || sections.every((s) => !s.content.trim())) {
      toast.error("Ajoutez du contenu avant d'envoyer");
      return;
    }
    try {
      let id = editing?.id;
      if (!id) {
        const created = await commsApi.createNewsletter({ subject, sections });
        id = created.id;
      }
      await commsApi.sendNewsletter(id);
      setEditing(null);
      setSubject("");
      setSections([{ id: "1", type: "heading", content: "" }]);
      toast.success("Newsletter envoyee !");
      refetch();
    } catch {
            ...
            {subject !== "" || sections.length > 0 ? (
                  ...
                  {!previewMode ? (
                          ...
                          >
                            <div className="flex items-center gap-2">
                              <select
                                value={s.type}
                                onChange={(e) =>
                                  updateSection(s.id, "type", e.target.value)
                                }
                    ...
                    <div className="border rounded-lg p-6 bg-background space-y-4">
                      ...
                      {sections.map((s) => (
                        <div
                          key={s.id}
                          className={`${sectionBg(s.type)} ${s.type === "highlight" ? "p-3 rounded" : ""}`}
                        >
                          {s.type === "heading" && (
                            <h2 className="text-xl font-bold">{s.content}</h2>
                          )}
                          {s.type === "text" && (
              ...
              <Card className="border-dashed">
                <CardContent className="flex flex-col items-center py-16 text-muted-foreground">
                  ...
                  <p className="font-medium">No newsletter in progress</p>
                  <p className="text-sm">
                    Click "New Newsletter" to get started
                  </p>
                </CardContent>
              </Card>
            )}
            ...
            {newsletters.length === 0 && (
              <Card className="border-dashed">
                <CardContent className="flex flex-col items-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">Aucune newsletter</p>
                </CardContent>
              </Card>
            )}
            {newsletters.map((n) => (
                ...
                <CardContent className="p-4 space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground flex items-center gap-1">
                    ...
                    {n.sentAt
                      ...
                      : `Draft · ${formatDistanceToNow(new Date(n.createdAt), { addSuffix: true })}`}
                  </p>
                </CardContent>
              </Card>
            ))}
