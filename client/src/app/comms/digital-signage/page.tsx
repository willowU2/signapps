...
import { commsApi, type Slide, type SlideType } from "@/lib/api/comms";
...
export default function DigitalSignagePage() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      await commsApi.createSlide(form);
      setForm({
        type: "text",
        title: "",
        content: "",
  ...
  const move = (id: string, dir: "up" | "down") => {
    ...
    if (
      (dir === "up" && idx === 0) ||
      (dir === "down" && idx === slides.length - 1)
    )
      return;
    const arr = [...slides];
    const swapIdx = dir === "up" ? idx - 1 : idx + 1;
    [arr[idx], arr[swapIdx]] = [arr[swapIdx], arr[idx]];
    commsApi.reorderSlides(arr.map((s) => s.id)).catch(() => {
  ...
  const toggleActive = (id: string) => {
    // Optimistic toggle — backend will handle persistence
    refetch();
  };

  const remove = async (id: string) => {
    try {
      await commsApi.deleteSlide(id);
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Manage display screen content and slides
                </p>
              </div>
            </div>
          </div>
                      ...
                      <div className="flex gap-1 mt-1">
                        {TEXT_COLORS.map((c) => (
                          <button
                            key={c}
                            type="button"
                            onClick={() => setForm({ ...form, textColor: c })}
                            className={cn(
                              "h-7 w-7 rounded-full border-2 transition-transform hover:scale-110",
                              form.textColor === c
                                ? "border-foreground scale-110"
          ...
          <div className="lg:col-span-1">
            <Card>
              ...
              <CardContent>
                {preview ? (
                  <div
                    className="rounded-lg aspect-video flex flex-col items-center justify-center p-4 text-center"
                    style={{
                  ...
                  >
                    ...
                    <p className="text-sm mt-2 whitespace-pre-line opacity-90">
                      {preview.content}
                    </p>
                ...
                <div className="mt-3 text-xs text-muted-foreground text-center">
                  {slides.filter((s) => s.active).length} active slides · Total{" "}
                  {slides
                    .filter((s) => s.active)
              ...
              return (
                <Card
                  ...
                  className={cn(
                    ...
                    preview?.id === slide.id && "ring-2 ring-primary",
                  )}
