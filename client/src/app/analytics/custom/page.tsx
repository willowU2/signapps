...
import {
  LayoutGrid, Plus, X, GripVertical, DollarSign,
  Mail, CheckSquare, TrendingUp, Users, BarChart3, RefreshCw,
} from 'lucide-react';
...
type WidgetType = 'revenue' | 'emails_day' | 'tasks_completed' | 'deals_won' | 'active_users' | 'tasks_pending' | 'tasks_blocked';
...
const WIDGET_CATALOG: Array<{ type: WidgetType; label: string; icon: React.ElementType; color: string }> = [
  { type: 'revenue',         label: 'Revenue',            icon: DollarSign,  color: 'text-green-500' },
  { type: 'emails_day',      label: 'Emails / jour',      icon: Mail,        color: 'text-blue-500' },
  { type: 'tasks_completed', label: 'Tâches terminées',   icon: CheckSquare, color: 'text-emerald-500' },
  { type: 'tasks_pending',   label: 'Tâches en attente',  icon: BarChart3,   color: 'text-amber-500' },
  { type: 'tasks_blocked',   label: 'Tâches bloquées',    icon: BarChart3,   color: 'text-red-500' },
  { type: 'deals_won',       label: 'Deals gagnés',       icon: TrendingUp,  color: 'text-violet-500' },
  { type: 'active_users',    label: 'Utilisateurs actifs',icon: Users,       color: 'text-sky-500' },
];
...
function WidgetCard({
  ...
  const getValue = (): string | number => {
    switch (config.type) {
      case 'tasks_completed': return workload?.completed ?? '—';
      ...
      <CardHeader className="pb-2 pt-3 px-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            ...
            <CardTitle className="text-xs font-medium text-muted-foreground">{def.label}</CardTitle>
          </div>
          <button
...
export default function CustomAnalyticsDashboard() {
  ...
  const addWidget = useCallback((type: WidgetType) => {
    ...
    setShowPicker(false);
    toast.success(`Widget "${WIDGET_CATALOG.find(w => w.type === type)?.label}" ajouté`);
      ...
      <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">Glissez-déposez les widgets, ajoutez ou supprimez des indicateurs.</p>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={handleReset} className="gap-1.5">
              <RefreshCw className="h-3.5 w-3.5" />Réinitialiser
            </Button>
            <Button size="sm" onClick={() => setShowPicker(v => !v)} className="gap-1.5">
        ...
        {showPicker && (
          <Card className="p-4">
            <h3 className="text-sm font-semibold mb-3 flex items-center gap-2">
              <LayoutGrid className="h-4 w-4 text-primary" />Choisir un widget
            </h3>
              ...
              {WIDGET_CATALOG.map(w => {
                ...
                return (
                  ...
                  >
                    ...
                    {w.label}
                    {alreadyAdded && <Badge variant="secondary" className="text-xs ml-1">Ajouté</Badge>}
                  </Button>
                );
              })}
            </div>
        ...
        {layout.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-muted-foreground">Aucun widget — ajoutez des indicateurs pour commencer</p>
          </div>
        ) : (
