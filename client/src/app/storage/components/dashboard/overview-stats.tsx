            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">
                  {formatBytes(stats.used_bytes)} utilises sur{" "}
              ...
              <div className="flex items-center justify-between text-xs text-muted-foreground">
                ...
                <span>
                  {stats.buckets_count} bucket
                  {stats.buckets_count !== 1 ? "s" : ""} -{" "}
                  {stats.files_count?.toLocaleString() ?? 0} fichier
                  {(stats.files_count ?? 0) !== 1 ? "s" : ""}
                </span>
              </div>
            </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 text-center">
                <div>
                  <p className="text-2xl font-bold">
                    {raidHealth.arrays_total || 0}
                  </p>
                  <p className="text-sm text-muted-foreground">Arrays</p>
                </div>
                <div>
                  <p className="text-2xl font-bold text-green-600">
                    {raidHealth.arrays_healthy || 0}
                  </p>
                  <p className="text-sm text-muted-foreground">OK</p>
                </div>
                <div>
                  <p className="text-2xl font-bold text-orange-600">
                    {(raidHealth.arrays_degraded || 0) +
                      (raidHealth.arrays_failed || 0)}
                  </p>
