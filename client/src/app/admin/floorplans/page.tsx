        ...
        <AppLayout>
            <div className="space-y-6 w-full">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight">FloorPlan Builder</h1>
                        <p className="text-muted-foreground mt-1">Gérer les cartes interactives des locaux de l'entreprise.</p>
                    </div>
                    <Button onClick={() => router.push('/admin/floorplans/new')} className="gap-2">
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center p-12 text-center rounded-xl border border-dashed bg-muted/30">
                        ...
                        <p className="text-muted-foreground text-sm max-w-[400px] mb-6">
                            Vous n'avez pas encore d'espace cartographié. Ajoutez un plan SVG pour commencer à rendre vos réservations interactives.
                        </p>
                        <Button onClick={() => router.push('/admin/floorplans/new')} variant="outline" className="gap-2">
                            <Plus className="w-4 h-4" />
                    ...
                    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                        {floorPlans.map((plan) => (
                            <Card key={plan.id} className="overflow-hidden flex flex-col hover:border-blue-200 transition-colors group">
                                <div className="h-32 bg-muted/50 border-b flex items-center justify-center relative overflow-hidden">
                                    {plan.svgContent ? (
                                        <img src={plan.svgContent.startsWith('<svg') ? `data:image/svg+xml;utf8,${encodeURIComponent(plan.svgContent)}` : plan.svgContent} alt={plan.name} className="w-full h-full object-cover opacity-50 group-hover:opacity-100 transition-opacity" />
                                    ) : (
                                ...
                                <CardHeader className="pb-2 flex-grow">
                                    ...
                                    <p className="text-sm text-muted-foreground truncate">{plan.description || "Aucune description"}</p>
                                </CardHeader>
                                <CardContent className="pb-4">
                                    <div className="flex items-center gap-4 text-xs font-medium text-muted-foreground">
                                        <span className="bg-blue-50 text-blue-700 px-2 py-0.5 rounded-sm">
                                            {plan.resources.length} Hitbox{plan.resources.length > 1 ? 's' : ''}
                                        </span>
                                    </div>
                                </CardContent>
