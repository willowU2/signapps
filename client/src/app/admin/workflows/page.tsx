...
import {
    ...
    Mail, FileText, UserPlus, FolderOpen, Bell, CheckSquare,
...
interface WorkflowCondition {
    field: string
    operator: 'contains' | 'equals' | 'starts_with' | 'ends_with' | 'is_type' | 'greater_than'
    value: string
}
...
const TRIGGERS: WorkflowTrigger[] = [
    ...
    { type: 'document_created', label: 'Quand un document est cr\u00e9\u00e9', icon: 'FileText' },
    { type: 'contact_added', label: 'Quand un contact est ajout\u00e9', icon: 'UserPlus' },
    { type: 'file_uploaded', label: 'Quand un fichier est upload\u00e9', icon: 'FolderOpen' },
    { type: 'task_completed', label: 'Quand une t\u00e2che est termin\u00e9e', icon: 'CheckSquare' },
...
const ACTION_TYPES = [
    { type: 'move_to_folder', label: 'D\u00e9placer vers un dossier', icon: 'FolderOpen' },
    { type: 'notify_user', label: "Notifier l'utilisateur", icon: 'Bell' },
    { type: 'create_task', label: 'Cr\u00e9er une t\u00e2che', icon: 'CheckSquare' },
    ...
    return [
        {
            ...
            trigger: { type: 'email_received', label: 'Quand un email arrive', icon: 'Mail' },
            conditions: [{ field: 'subject', operator: 'contains', value: 'urgent' }],
            actions: [
                { type: 'move_to_folder', label: 'D\u00e9placer vers un dossier', config: { folder: 'Urgent' } },
                { type: 'notify_user', label: "Notifier l'utilisateur", config: { message: 'Email urgent re\u00e7u !' } },
            ],
            enabled: true,
        ...
        },
        {
            ...
            trigger: { type: 'file_uploaded', label: 'Quand un fichier est upload\u00e9', icon: 'FolderOpen' },
            conditions: [{ field: 'type', operator: 'is_type', value: 'PDF' }],
            actions: [
                { type: 'move_to_folder', label: 'D\u00e9placer vers un dossier', config: { folder: 'Documents PDF' } },
                { type: 'add_label', label: 'Ajouter un libell\u00e9', config: { label: 'PDF' } },
            ],
            enabled: true,
            createdAt: new Date().toISOString(),
            triggerCount: 45,
        },
        {
            ...
            description: 'Cr\u00e9er une t\u00e2che de suivi pour les nouveaux contacts',
            trigger: { type: 'contact_added', label: 'Quand un contact est ajout\u00e9', icon: 'UserPlus' },
            conditions: [],
            actions: [
                { type: 'create_task', label: 'Cr\u00e9er une t\u00e2che', config: { title: 'Suivre le nouveau contact', priority: 'haute' } },
                { type: 'notify_user', label: "Notifier l'utilisateur", config: { message: 'Nouveau contact ajout\u00e9 au CRM' } },
            ],
            enabled: false,
            createdAt: new Date().toISOString(),
            triggerCount: 0,
...
function TriggerIcon({ name, className }: { name: string; className?: string }) {
  ...
  usePageTitle('Workflows');
    ...
    const handleSave = () => {
        ...
        const updatedRules = editingRule
            ? rules.map(r => r.id === editingRule.id ? {
                ...r,
                name: formName,
    ...
    const addCondition = () => {
        setFormConditions([...formConditions, { field: 'subject', operator: 'contains', value: '' }])
    }

    const updateCondition = (index: number, updates: Partial<WorkflowCondition>) => {
        const updated = formConditions.map((c, i) => i === index ? { ...c, ...updates } : c)
    ...
    const addAction = (actionType: typeof ACTION_TYPES[0]) => {
        setFormActions([...formActions, { type: actionType.type, label: actionType.label, config: {} }])
    }

    const updateActionConfig = (index: number, key: string, value: string) => {
        const updated = formActions.map((a, i) => i === index ? { ...a, config: { ...a.config, [key]: value } } : a)
    ...
    return (
        <AppLayout>
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-muted-foreground mt-1">
                            D\u00e9finissez des r\u00e8gles pour automatiser les actions r\u00e9p\u00e9titives
                        </p>
                    </div>
                ...
                <div className="grid gap-4 md:grid-cols-3">
                    <Card>
                        ...
                        <CardContent>
                            ...
                            <p className="text-xs text-muted-foreground">sur {rules.length} au total</p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    ...
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-sm font-medium">D\u00e9clencheurs</CardTitle>
                ...
                <div className="space-y-3">
                    {rules.length === 0 ? (
                        <Card className="p-12 text-center">
                            ...
                            <h3 className="text-lg font-medium text-muted-foreground">Aucune r\u00e8gle d'automatisation</h3>
                            <p className="text-sm text-muted-foreground/70 mt-1">Cr\u00e9ez votre premi\u00e8re r\u00e8gle pour commencer</p>
                            ...
                            <Card key={rule.id} className={`transition-all ${!rule.enabled ? 'opacity-60' : ''}`}>
                                <div className="p-4 flex items-center gap-4">
                                    <div className="flex items-center justify-center h-10 w-10 rounded-lg bg-amber-500/10 shrink-0">
                                        <TriggerIcon name={rule.trigger.icon} className="h-5 w-5 text-amber-600" />
                                    </div>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            {rule.enabled ? (
                                                <Badge variant="secondary" className="bg-green-500/10 text-green-600 text-[10px]">Actif</Badge>
                                            ) : (
                                                <Badge variant="secondary" className="text-[10px]">Inactif</Badge>
                                            )}
                                        </div>
                                        <p className="text-sm text-muted-foreground truncate">{rule.description || rule.trigger.label}</p>
                                        <div className="flex items-center gap-4 mt-1.5">
                                            <span className="text-xs text-muted-foreground flex items-center gap-1">
                                                <TriggerIcon name={rule.trigger.icon} className="h-3 w-3" />
                                                {rule.trigger.label}
                                            </span>
                                            ...
                                            <span className="text-xs text-muted-foreground">
                                                {rule.triggerCount} ex\u00e9cution{rule.triggerCount > 1 ? 's' : ''}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                        ...
                                        <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => setDeleteTarget(rule.id)}>
            ...
            {/* Create / Edit Dialog */}
            <Dialog open={isDialogOpen} onOpenChange={(v) => { if (!v) { setIsDialogOpen(false); resetForm(); } }}>
                <DialogContent className="sm:max-w-[600px] max-h-[85vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>{editingRule ? 'Modifier la r\u00e8gle' : 'Nouvelle r\u00e8gle d\'automatisation'}</DialogTitle>
                        <DialogDescription>
                    ...
                    <div className="space-y-6 py-4">
                        ...
                        <div className="space-y-3">
                            <div className="space-y-2">
                                ...
                                <Input value={formName} onChange={e => setFormName(e.target.value)} placeholder="Ex: Emails urgents" />
                            </div>
                            <div className="space-y-2">
                                <Label>Description (optionnel)</Label>
                                <Input value={formDescription} onChange={e => setFormDescription(e.target.value)} placeholder="Ex: D\u00e9placer et notifier pour les emails urgents" />
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <Select value={formTrigger.type} onValueChange={v => {
                        ...
                        <div className="space-y-3">
                            <Label className="flex items-center gap-2">
                                <Filter className="h-4 w-4 text-blue-500" /> Conditions (optionnel)
                            </Label>
                            {formConditions.map((cond, i) => (
                                <div key={i} className="flex items-center gap-2 p-3 rounded-lg bg-muted/30 border">
                                    ...
                                    <Select value={cond.operator} onValueChange={v => updateCondition(i, { operator: v as WorkflowCondition['operator'] })}>
                                        <SelectTrigger className="w-[140px]">
                                            <SelectValue />
                                        </SelectTrigger>
                                        <SelectContent>
                                ...
                                <div key={i} className="p-3 rounded-lg bg-muted/30 border space-y-2">
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm font-medium flex items-center gap-2">
                                            <TriggerIcon name={ACTION_TYPES.find(a => a.type === action.type)?.icon || 'Zap'} className="h-4 w-4 text-green-600" />
                                            {action.label}
                                        </span>
                                    ...
                                    {action.type === 'move_to_folder' && (
                                        <Input
                                            ...
                                            onChange={e => updateActionConfig(i, 'folder', e.target.value)}
                                    ...
                                    {action.type === 'add_label' && (
                                        <Input
                                            ...
                                            onChange={e => updateActionConfig(i, 'label', e.target.value)}
                    ...
                    <DialogFooter>
                        <Button variant="ghost" onClick={() => { setIsDialogOpen(false); resetForm(); }}>Annuler</Button>
                        <Button onClick={handleSave}>{editingRule ? 'Mettre \u00e0 jour' : 'Cr\u00e9er la r\u00e8gle'}</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            <AlertDialog open={!!deleteTarget} onOpenChange={v => !v && setDeleteTarget(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Cette action est irr\u00e9versible. La r\u00e8gle d'automatisation sera supprim\u00e9e.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
