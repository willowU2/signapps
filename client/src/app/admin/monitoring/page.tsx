                ...
                >
                  ...
                  <td
                    className={cn(
                      ...
                      q.duration_seconds > 10
                        ? "text-red-500"
                        : q.duration_seconds > 3
                          ? "text-amber-500"
                          : "text-foreground",
                    )}
                  >
                  ...
                  <td className="px-4 py-2 max-w-xs">
                    <code
                      className="text-[11px] text-muted-foreground truncate block max-w-xs"
                      title={q.query ?? ""}
                    >
                      {q.query ?? "—"}
                    </code>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="sm:col-span-2 lg:col-span-4">
            ...
            <p className="text-xs text-muted-foreground mt-2">
              Ajustez DB_MAX_CONNECTIONS dans .env si le pool est frequemment
              sature.
            </p>
          </div>
        </div>
      ...
      getSystemMetrics()
        .then((newMetrics) => {
          setPrevMetrics((prev) => {
            if (prev) {
              const rxDiff =
                (newMetrics.network_rx_bytes || 0) -
                (prev.network_rx_bytes || 0);
              const txDiff =
                (newMetrics.network_tx_bytes || 0) -
                (prev.network_tx_bytes || 0);
                ...
                <CardContent>
                  ...
                  <div className="mt-4 h-2 w-full bg-muted rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, metrics?.cpu_usage || 0)}%`,
                      }}
                    />
                  </div>
                </CardContent>
                ...
                <CardContent>
                  ...
                  <div className="mt-4 h-2 w-full bg-muted rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, metrics?.memory_usage || 0)}%`,
                      }}
                    />
                  </div>
                </CardContent>
                ...
                <CardContent>
                  ...
                  <div className="mt-4 h-2 w-full bg-muted rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, metrics?.disk_usage || 0)}%`,
                      }}
                    />
                  </div>
                </CardContent>
                  ...
                  <div className="flex justify-between items-end">
                    <div>
                      <div className="text-2xl font-bold">
                        {formatSpeed(netSpeed.rx)}
                      </div>
                      <p className="text-xs text-muted-foreground">Inbound</p>
                    </div>
                    <div className="text-right">
                      <div className="text-2xl font-bold">
                        {formatSpeed(netSpeed.tx)}
                      </div>
