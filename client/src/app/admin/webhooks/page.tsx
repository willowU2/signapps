...
function generateMockDeliveryLog(): DeliveryLogEntry[] {
  const webhooks = ['CI/CD Pipeline', 'Slack Notifier', 'Analytics Tracker', 'Audit Logger'];
  const events = ['user.created', 'user.updated', 'document.created', 'document.updated', 'auth.login', 'auth.login_failed', 'backup.completed', 'container.started'];
  const now = Date.now();
  ...
  return Array.from({ length: 25 }, (_, i) => {
    ...
    return {
      ...
      status_code: pending ? null : success ? 200 : [400, 500, 502, 503, 0][Math.floor(Math.random() * 5)],
      duration_ms: pending ? null : Math.floor(Math.random() * 2000) + 50,
      timestamp: new Date(now - i * 3600000 * (0.5 + Math.random())).toISOString(),
      request_body: JSON.stringify({ event: events[0], data: { id: `usr-${i}` } }),
      response_body: success ? '{"ok":true}' : pending ? null : '{"error":"timeout"}',
      error: pending ? null : success ? null : 'Connection timeout after 5000ms',
    };
  });
}
...
function DeliveryLog() {
  const [entries, setEntries] = useState<DeliveryLogEntry[]>(() => generateMockDeliveryLog());
      ...
      <CardContent>
        <div className="space-y-1">
          {filteredEntries.length === 0 && (
            <p className="text-sm text-muted-foreground text-center py-8">Aucune entree trouvee.</p>
          )}
          {filteredEntries.map((entry) => (
            <div key={entry.id}>
              ...
              >
                ...
                {entry.status === 'failed' && <XCircle className="h-4 w-4 text-red-500 shrink-0" />}
                {entry.status === 'pending' && <Clock className="h-4 w-4 text-yellow-500 shrink-0" />}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium truncate">{entry.webhook_name}</span>
                    <Badge variant="outline" className="text-[10px]">{entry.event}</Badge>
                  </div>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  {entry.status_code !== null && entry.status_code > 0 && (
                    <Badge
                      variant={entry.status_code >= 200 && entry.status_code < 300 ? 'default' : 'destructive'}
                      className="text-[10px]"
                    >
              ...
              {expandedId === entry.id && (
                <div className="ml-7 mb-2 p-3 bg-muted/30 rounded-lg text-xs space-y-2">
                  <div>
                    ...
                    <pre className="mt-1 bg-background rounded p-2 overflow-auto max-h-32 text-[11px]">{entry.request_body}</pre>
                  </div>
                  {entry.response_body && (
                    <div>
                      <span className="font-medium">Reponse :</span>
                      <pre className="mt-1 bg-background rounded p-2 overflow-auto max-h-32 text-[11px]">{entry.response_body}</pre>
                    </div>
                  )}
                  {entry.error && (
                    <div>
