...
const ALL_SERVICES: ServiceEntry[] = [
  ...
  {
    name: ServiceName.CONTACTS,
    label: "Contacts",
    port: 3021,
    healthPath: "/health",
  },
  {
    name: ServiceName.IT_ASSETS,
    label: "IT Assets",
    port: 3022,
    healthPath: "/health",
  },
...
function ServiceCard({ result }: { result: ServiceHealthResult }) {
  ...
  return (
    <Card
      className={`transition-colors ${isHealthy ? "border-green-500/30" : isPending ? "" : "border-red-500/30"}`}
    >
      ...
      <CardContent className="px-4 pb-4 space-y-1">
        ...
        <Badge
          variant={
            isPending ? "outline" : isHealthy ? "default" : "destructive"
          }
...
function useSseHealth(onSnapshot: (snap: SseSnapshot) => void) {
  useEffect(() => {
    ...
    const es = new EventSource(
      "http://localhost:3007/api/v1/scheduler/health-stream",
    );
...
export default function HealthPage() {
  ...
  const handleSseSnapshot = useCallback((snap: SseSnapshot) => {
    ...
    setResults((prev) =>
      prev.map((r) => {
        const match = snap.services.find(
          (s) => s.name === r.name || s.port === r.port,
        );
        if (!match) return r;
        return {
          ...r,
    ...
    <AppLayout>
      <div className="space-y-6">
        <PageHeader
          ...
          description="Live status for all SignApps backend services — auto-refreshes every 30 seconds"
            ...
            <div>
              <p className="text-sm font-semibold">
                {allHealthy
                  ...
                  : `${totalCount - healthyCount} service(s) degraded`}
              </p>
        ...
        {/* Grid */}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 stagger-in">
          {results.map((result) => (
