  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span
            className={`text-sm font-semibold tabular-nums ${scoreToColor(dim.score)}`}
          >
            {dim.score}/100
          </span>
          <Badge
            variant={scoreToBadge(dim.score).variant}
  ...
  const loadStats = async () => {
    ...
    try {
      ...
      const volumeScore = Math.min(
        100,
        Math.round((aiStats.documents_count / 500) * 100),
      );
      const diversityScore = Math.min(
        100,
        Math.round((aiStats.chunks_count / aiStats.documents_count) * 10),
      );
  ...
  const qualityDimensions: QualityDimension[] = stats
    ? [
        ...
        },
        {
          ...
          description:
            "Ratio chunks/documents. Un ratio élevé indique une bonne granularité du contenu.",
        ...
        },
        {
          ...
          description:
            "Score composite prenant en compte le volume et la diversité des données.",
    ...
    <AppLayout>
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        <PageHeader
          ...
          description="Statistiques des données d'entraînement indexées et qualité du corpus."
        ...
        <div className="flex items-start gap-3 p-3 rounded-lg bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 text-sm">
          ...
          <div>
            ...
            <p className="text-blue-700 dark:text-blue-300 text-xs mt-0.5">
              L'export supprime tous les identifiants personnels (noms, emails,
              IDs). Seule la structure sémantique est conservée.
            </p>
          </div>
        </div>
        ...
        <Card className="border-dashed">
          ...
          <CardContent>
            <CardDescription className="text-xs leading-relaxed">
              Cette page est un outil de <strong>monitoring et d'export</strong>
              . Le fine-tuning réel (LoRA, QLoRA, SFT) requiert une
              infrastructure GPU dédiée (ex. RunPod, Lambda Labs) et des outils
              spécialisés (Axolotl, LLaMA Factory). Exportez les données ici,
              puis importez-les dans votre pipeline d'entraînement.
            </CardDescription>
          </CardContent>
        </Card>
        ...
        {stats?.lastIndexed && (
          <p className="text-xs text-muted-foreground text-right flex items-center justify-end gap-1">
            ...
            Dernière indexation :{" "}
            {new Date(stats.lastIndexed).toLocaleString("fr-FR")}
          </p>
        )}
      </div>
    </AppLayout>
