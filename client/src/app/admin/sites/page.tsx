...
const SITE_TYPE_CONFIG: Record<SiteType, { label: string; icon: React.ReactNode; color: string }> = {
  campus: { label: 'Campus', icon: <Building2 className="h-4 w-4" />, color: 'text-purple-600' },
  building: { label: 'Bâtiment', icon: <Building2 className="h-4 w-4" />, color: 'text-blue-600' },
  floor: { label: 'Étage', icon: <MapPin className="h-4 w-4" />, color: 'text-teal-600' },
  room: { label: 'Salle', icon: <MapPin className="h-4 w-4" />, color: 'text-green-600' },
};
...
function SiteTreeItem({
  ...
  const cfg = SITE_TYPE_CONFIG[site.site_type as SiteType] ?? { label: site.site_type, icon: <MapPin className="h-4 w-4" />, color: 'text-muted-foreground' };
    ...
    <div>
      ...
      >
        <button
          ...
          onClick={(e) => { e.stopPropagation(); onToggleExpand(site.id); }}
        >
...
export default function SitesPage() {
  ...
  useEffect(() => {
    if (selectedSite) {
      orgApi.sites.persons(selectedSite.id)
    ...
    <AppLayout>
      <div className="px-6 py-6 space-y-6">
        <PageHeader
          ...
          actions={
            <Button size="sm" onClick={() => { setEditSite(null); resetForm(); setCreateOpen(true); }}>
          ...
          <div className="flex-1 border rounded-lg bg-card overflow-hidden">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center h-48 text-muted-foreground gap-2">
                ...
                <p className="text-sm">Aucun site — cliquez sur «Nouveau site»</p>
              </div>
            ) : (
              ...
              <div className="border rounded-lg bg-card p-4 space-y-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {SITE_TYPE_CONFIG[selectedSite.site_type as SiteType]?.label ?? selectedSite.site_type}
                  </p>
                </div>

                {selectedSite.address && (
                  <div className="text-sm space-y-1">
                    <p className="text-muted-foreground text-xs uppercase tracking-wide font-medium">Adresse</p>
                    <p>{selectedSite.address}</p>
                    {selectedSite.city && <p>{selectedSite.city}{selectedSite.country ? `, ${selectedSite.country}` : ''}</p>}
                  </div>
                )}

                {selectedSite.capacity && (
                  <div className="text-sm">
                    <p className="text-muted-foreground text-xs uppercase tracking-wide font-medium mb-1">Capacité</p>
                    <p>{selectedSite.capacity} personnes</p>
                  </div>
                )}

                <div className="text-sm">
                  <p className="text-muted-foreground text-xs uppercase tracking-wide font-medium mb-1">Fuseau horaire</p>
                ...
                <div>
                  ...
                  {sitePersons.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Aucune personne</p>
                  ) : (
      ...
      {/* ── Create/Edit dialog ── */}
      <Dialog open={createOpen} onOpenChange={(open) => { setCreateOpen(open); if (!open) setEditSite(null); }}>
          ...
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2 col-span-2">
                ...
                <Input id="site-name" value={formName} onChange={(e) => setFormName(e.target.value)} placeholder="Ex: Tour A, Salle Conférence 1" autoFocus />
              </div>
              <div className="space-y-2">
                <Label>Type *</Label>
                <Select value={formType} onValueChange={(v) => setFormType(v as SiteType)}>
              ...
              <div className="space-y-2">
                ...
                <Input id="capacity" type="number" value={formCapacity} onChange={(e) => setFormCapacity(e.target.value)} placeholder="50" />
              </div>
            </div>
            <div className="space-y-2">
              <Label htmlFor="address">Adresse</Label>
              <Input id="address" value={formAddress} onChange={(e) => setFormAddress(e.target.value)} placeholder="12 rue de la Paix" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <Label htmlFor="city">Ville</Label>
                <Input id="city" value={formCity} onChange={(e) => setFormCity(e.target.value)} placeholder="Paris" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="country">Pays</Label>
                <Input id="country" value={formCountry} onChange={(e) => setFormCountry(e.target.value)} placeholder="France" />
              </div>
            </div>
            <div className="space-y-2">
              <Label htmlFor="timezone">Fuseau horaire</Label>
              <Input id="timezone" value={formTimezone} onChange={(e) => setFormTimezone(e.target.value)} placeholder="Europe/Paris" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="parent-site">Site parent (ID)</Label>
              <Input id="parent-site" value={formParentId} onChange={(e) => setFormParentId(e.target.value)} placeholder="UUID du site parent" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => { setCreateOpen(false); setEditSite(null); }}>Annuler</Button>
      ...
      <Dialog open={!!deleteSite} onOpenChange={(open) => !open && setDeleteSite(null)}>
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground py-2">
            Ce site sera désactivé. Les sous-sites et personnes associées ne seront pas supprimés.
          </p>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeleteSite(null)}>Annuler</Button>
