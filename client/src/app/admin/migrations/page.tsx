...
const STATUS_CONFIG = {
  applied: { icon: CheckCircle2, color: 'text-green-500', bg: 'bg-green-500/10', label: 'Appliquée' },
  pending: { icon: Clock, color: 'text-yellow-500', bg: 'bg-yellow-500/10', label: 'En attente' },
  failed: { icon: XCircle, color: 'text-red-500', bg: 'bg-red-500/10', label: 'Échouée' },
};
            ...
            <CardHeader className="py-3">
              <div className="flex items-center justify-between">
                ...
                {svc.error ? (
                  ...
                  <Badge variant="secondary">{svc.migrations.length} migrations</Badge>
                )}
              </div>
            </CardHeader>
              ...
              <CardContent className="pt-0">
                <div className="space-y-1">
                  {svc.migrations.map((m) => {
                    ...
                    const Icon = cfg.icon;
                    return (
                      <div key={m.version} className="flex items-center gap-3 py-1.5 text-sm">
                        <Icon className={`h-4 w-4 ${cfg.color} shrink-0`} />
                        <span className="font-mono text-xs text-muted-foreground w-32 shrink-0">{m.version}</span>
                        ...
                        {m.applied_at && (
                          <span className="text-xs text-muted-foreground">{new Date(m.applied_at).toLocaleDateString()}</span>
                        )}
                      </div>
                    );
                  })}
