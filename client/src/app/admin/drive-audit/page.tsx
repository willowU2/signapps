    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <div>
            <p className={`text-sm font-semibold ${config.colorCls}`}>
              {config.label}
            </p>
...
export default function DriveAuditPage() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (res.data?.valid) {
        toast.success(
          "Chaîne d'intégrité vérifiée — aucune altération détectée",
        );
      } else {
  ...
  const toggleAlert = (key: string) => {
    ...
    toast.success(
      `Alerte "${key}" ${alertStates[key] ? "désactivée" : "activée"}`,
    );
  };
  ...
  const getActionBadge = (action: string) => {
    ...
    return (
      <Badge
        variant="outline"
        className={`text-[10px] px-1.5 py-0 h-5 font-medium ${cls}`}
      >
        {label}
      </Badge>
    );
  ...
  return (
    <AppLayout>
      <div className="flex flex-col gap-6 pb-24 w-full max-w-7xl mx-auto">
        {/* Header */}
        <PageHeader
          title="Audit du Drive"
          description="Journal forensique des accès et modifications"
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/40 text-muted-foreground text-xs uppercase tracking-wider">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left font-medium w-28 hidden lg:table-cell">
                    IP
                  </th>
                  <th className="px-4 py-3 text-left font-medium w-16 hidden lg:table-cell">
                    Géoloc
                  </th>
                  <th className="px-4 py-3 text-left font-medium w-28 hidden xl:table-cell">
                    Hash
                  </th>
                </tr>
              </thead>
