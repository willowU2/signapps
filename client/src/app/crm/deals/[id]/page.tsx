      ...
      <AppLayout>
        <div className="p-8 text-muted-foreground flex flex-col items-center gap-4">
          ...
          <Button variant="outline" asChild>
            <Link href="/crm">
              <ArrowLeft className="h-4 w-4 mr-2" /> Retour CRM
            </Link>
          </Button>
        </div>
      </AppLayout>
  ...
  return (
    <AppLayout>
      <div
        ...
        data-testid="crm-deal-detail-root"
        data-deal-id={id}
      >
        {/* Header */}
        ...
        {(() => {
          ...
          if (deal.contactEmail)
            cl.push(
              crossLinkHelpers.toMail(
                deal.contactEmail,
                `Emails ${deal.company}`,
              ),
            );
          cl.push(
            crossLinkHelpers.toCalendarEvent(id, `Rendez-vous ${deal.title}`),
          );
          return cl.length > 0 ? (
            <div>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Cette action supprimera définitivement "{deal.title}". Les
              activités et tâches associées seront également supprimées.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
