...
export const metadata = { title: 'MCP Integration — SignApps Social' };
...
const MCP_TOOLS = [
  {
    ...
    params: [
      ...
      { name: 'hashtags', type: 'string[]', required: false, description: 'Hashtag list (without #)' },
      { name: 'media_urls', type: 'string[]', required: false, description: 'Image/video URLs to attach' },
    ],
    example: `{
  ...
  },
  {
    ...
    params: [
      { name: 'post_id', type: 'string', required: true, description: 'ID of an existing draft post' },
      { name: 'scheduled_at', type: 'ISO 8601', required: true, description: 'When to publish the post' },
    ],
    example: `{
  "post_id": "post_abc123",
  ...
  {
    name: 'get_analytics',
    description: 'Retrieve analytics overview: followers, engagement rate, reach, top posts',
    params: [
      { name: 'period', type: 'string', required: false, description: '7d | 30d | 90d (default: 30d)' },
  ...
  },
  {
    ...
    params: [
      { name: 'platform', type: 'string', required: false, description: 'Filter by platform' },
      { name: 'active_only', type: 'boolean', required: false, description: 'Only return active accounts' },
    ],
    example: `{
  "active_only": true
}`,
  },
  {
    ...
    params: [
      ...
      { name: 'unread_only', type: 'boolean', required: false, description: 'Only unread items' },
  ...
  },
  {
    ...
    params: [
      { name: 'inbox_item_id', type: 'string', required: true, description: 'ID of the inbox item to reply to' },
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-8">
      <div>
        ...
        <p className="text-muted-foreground text-lg">
          Connect AI agents (like Claude) to SignApps Social using the Model Context Protocol.
          Let your AI assistant create, schedule and manage social posts on your behalf.
        </p>
        ...
        <CardContent className="space-y-4 text-sm">
          <ol className="list-decimal list-inside space-y-3">
            <li>
              ...
              <a href="/social/settings/api-keys" className="text-primary underline">
                Settings → API Keys
              </a>{' '}
              and generate a new key.
            </li>
            <li>
              <strong>Add to your MCP client config</strong> (e.g. <code className="bg-muted px-1.5 py-0.5 rounded text-xs">claude_desktop_config.json</code>):
            </li>
          </ol>
          <pre className="bg-muted rounded-lg p-4 text-xs overflow-x-auto font-mono">{CONFIG_EXAMPLE}</pre>
          <ol start={3} className="list-decimal list-inside space-y-2">
            <li>Restart your MCP client (Claude Desktop, Continue, etc.)</li>
            <li>The <strong>signapps-social</strong> tools will appear automatically</li>
          </ol>

          <div className="border rounded-lg p-3 bg-yellow-50 dark:bg-yellow-950/30 text-yellow-800 dark:text-yellow-300 text-xs">
            <strong>Note:</strong> The MCP server package <code>@signapps/mcp-server</code> is currently in preview.
          ...
          <Card key={tool.name}>
            <CardContent className="pt-4 space-y-3">
              <div className="flex items-center gap-2">
                <code className="font-mono text-sm font-bold bg-muted px-2 py-1 rounded">{tool.name}</code>
              </div>
              ...
              <div>
                ...
                <pre className="bg-muted rounded-lg p-3 text-xs overflow-x-auto font-mono">{tool.example}</pre>
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <Card>
        ...
        <CardContent>
          <pre className="bg-muted rounded-lg p-4 text-xs overflow-x-auto font-mono whitespace-pre-wrap">{CLAUDE_EXAMPLE}</pre>
          <p className="text-xs text-muted-foreground mt-3">
            With MCP, Claude can autonomously create posts, check analytics, reply to mentions and schedule content —
            all based on your natural language instructions.
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <ul className="space-y-2 text-sm text-muted-foreground list-disc list-inside">
            <li>Ask Claude to write and schedule a week of LinkedIn posts in one prompt</li>
            <li>Automatically reply to unread mentions with context-aware responses</li>
            <li>Generate monthly analytics reports and suggest content improvements</li>
            <li>Recycle top performing posts from last quarter</li>
            <li>Post breaking news from RSS feeds with AI-generated commentary</li>
          </ul>
        </CardContent>
      </Card>
    </div>
