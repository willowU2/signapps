...
import {
  DocAutoSaveDrive,
  DocExportDrive,
} from "@/components/interop/DocAutoSaveDrive";
...
function EditorContent() {
  ...
  useEffect(() => {
    if (id && id !== "new") {
      trackDocVisit({
        id,
        name: name || "Document sans titre",
        kind: "text",
        href: `/docs/editor?id=${id}&name=${encodeURIComponent(name)}`,
      });
    }
  }, [id, name]);
