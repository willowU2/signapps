    ...
    <div className="w-full space-y-6 p-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Aliases, unified inbox, recurring emails, delegation, tracking, AI
          categorization, CRM variables, and undo send.
        </p>
      </div>
        ...
        <div className="mt-4">
          ...
          <TabsContent value="crm-vars">
            <div className="space-y-2">
              <p className="text-sm text-muted-foreground">
                Compose with CRM merge fields. Variables resolve when a contact
                email is set.
              </p>
              <CrmTemplateVariables body={crmBody} onBodyChange={setCrmBody} />
            </div>
          </TabsContent>
          ...
          <TabsContent value="undo">
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                Configure a grace period after clicking Send to allow
                cancellation.
              </p>
              <UndoSendSettings
                enabled={undoEnabled}
                ...
                onToggle={(v) => {
                  setUndoEnabled(v);
                  saveUndoSettings(v, undoDelay);
                }}
                onDelayChange={(v) => {
                  setUndoDelay(v);
                  saveUndoSettings(undoEnabled, v);
                }}
              />
            </div>
          </TabsContent>
