...
const ITEMS: StockItem[] = [
  {
    ...
    unit: "pcs",
    lastMovement: new Date(Date.now() - 2 * 86400000),
    cost: 249,
  },
  {
    id: "2",
  ...
  },
  {
    ...
    unit: "pcs",
    lastMovement: new Date(Date.now() - 5 * 86400000),
    cost: 49,
  },
  {
    id: "4",
    ...
    location: "Warehouse B",
    unit: "pcs",
    lastMovement: new Date(Date.now() - 3 * 86400000),
    cost: 79,
  },
  {
    id: "5",
  ...
  },
  {
    ...
    location: "Storeroom 1",
    unit: "pcs",
    lastMovement: new Date(Date.now() - 10 * 86400000),
    cost: 35,
  },
];
...
export default function InventoryPage() {
  ...
  const handleMove = (e: React.FormEvent) => {
    ...
    if (!item) return;
    if (moveForm.type === "out" && moveForm.quantity > item.quantity) {
      toast.error("Insufficient stock");
      return;
    }
    setItems((prev) =>
      prev.map((i) =>
        i.id === moveForm.itemId
          ? {
              ...i,
              quantity:
                moveForm.type === "in"
                  ? i.quantity + moveForm.quantity
                  : i.quantity - moveForm.quantity,
              lastMovement: new Date(),
            }
          : i,
    ...
    setMovements((prev) => [
      {
        ...
        [moveForm.type === "in" ? "to" : "from"]: item.location,
    ...
    setMoveOpen(false);
    toast.success(
      `Stock movement recorded: ${moveForm.type === "in" ? "+" : "-"}${moveForm.quantity} ${item.name}`,
    );
  };
    ...
    <AppLayout>
      <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Stock levels, locations, and movements
              </p>
            </div>
          </div>
              ...
              <form onSubmit={handleMove} className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setMoveForm({ ...moveForm, itemId: e.target.value })
                    }
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="e.g. PO-001, Deployment, Disposal..."
                    className="mt-1"
                  />
                </div>
          ...
          <Card className="border-yellow-200 bg-yellow-50 dark:border-yellow-900 dark:bg-yellow-950/30">
            <CardContent className="p-4">
              ...
              <div className="flex flex-wrap gap-2">
                {alerts.map((a) => {
                  const sc = statusConfig[getStatus(a)];
                  return (
                    <Badge key={a.id} className={sc.color}>
                      {a.name}:{" "}
                      {getStatus(a) === "out"
                        ? "Rupture de stock"
                        : getStatus(a) === "low"
                          ? `Low (${a.quantity})`
                          : `Overstock (${a.quantity})`}
                    </Badge>
                  );
                })}
              </div>
        ...
        <Tabs defaultValue="stock">
          <TabsList>
            <TabsTrigger value="stock">Stock ({items.length})</TabsTrigger>
                        ...
                        return (
                          <TableRow key={item.id}>
                            ...
                            <TableCell className="text-xs text-muted-foreground">
                              {format(item.lastMovement, "MMM d")}
                            </TableCell>
                          </TableRow>
                        );
                      })}
