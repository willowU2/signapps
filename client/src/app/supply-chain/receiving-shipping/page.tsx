...
const LOGS: GoodsLog[] = [
  {
    id: "1",
    type: "receiving",
    referenceNumber: "RCV-2026-041",
    poNumber: "PO-2026-041",
    carrier: "DHL Express",
    items: "USB-C Hub 7-in-1 x20, HDMI Cable x30",
    quantity: 50,
    unit: "pcs",
    location: "Zone B",
    status: "in-progress",
  ...
  },
  {
    id: "2",
    type: "receiving",
    referenceNumber: "RCV-2026-040",
    poNumber: "PO-2026-040",
    carrier: "La Poste",
    items: "A4 Paper 500-sheet reams x100",
    quantity: 100,
    unit: "reams",
  ...
  },
  {
    id: "3",
    type: "shipping",
    referenceNumber: "SHP-2026-015",
    carrier: "Chronopost",
    items: "Surplus office chairs x3",
    quantity: 3,
    unit: "pcs",
    location: "Zone A",
    status: "pending",
  ...
  },
  {
    id: "4",
    type: "shipping",
    referenceNumber: "SHP-2026-014",
    carrier: "DHL",
    items: "Defective monitors x2",
    quantity: 2,
    unit: "pcs",
    location: "Shipping Dock",
    status: "completed",
    operator: "Dave L.",
    notes: "RMA return — warranty claim",
  ...
  },
  {
    id: "5",
    type: "receiving",
    referenceNumber: "RCV-2026-039",
    poNumber: "PO-2026-039",
    carrier: "Geodis",
    items: "Ergonomic chairs x5",
    quantity: 5,
    unit: "pcs",
    location: "Zone A",
    status: "issue",
    operator: "Alice M.",
    notes: "Missing 2 units — contacting supplier",
...
export default function ReceivingShippingPage() {
  ...
  const filtered =
    activeTab === "all" ? logs : logs.filter((l) => l.type === activeTab);
  ...
  const handleCreate = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.referenceNumber.trim() || !form.items.trim()) {
      toast.error("Reference and items required");
      return;
    }
    const log: GoodsLog = {
  ...
  const stats = [
    {
      ...
      value: receiving.filter((l) => l.status === "pending").length,
      icon: ArrowDownToLine,
      color: "text-green-600",
    },
    {
      label: "Pending Shipping",
      value: shipping.filter((l) => l.status === "pending").length,
      icon: ArrowUpFromLine,
      color: "text-blue-600",
    },
    {
      label: "In Progress",
      value: logs.filter((l) => l.status === "in-progress").length,
      icon: Truck,
      color: "text-yellow-600",
    },
    {
      label: "Issues",
      value: logs.filter((l) => l.status === "issue").length,
      icon: Package,
      color: "text-red-600",
    },
  ];
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Log and track all inbound and outbound goods
              </p>
            </div>
          </div>
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="max-w-lg">
              <DialogHeader>
                <DialogTitle>New Goods Log</DialogTitle>
              </DialogHeader>
                        ...
                        return (
                          <TableRow key={log.id}>
                            ...
                            <TableCell className="max-w-[200px]">
                              <p className="text-sm truncate" title={log.items}>
                                {log.items}
                              </p>
                            ...
                            <TableCell className="text-xs text-muted-foreground">
                              {format(log.scheduledDate, "MMM d")}
                            </TableCell>
                            <TableCell>
                              ...
                              <div className="flex gap-1">
                                {log.status === "pending" && (
                                  <Button
                                    ...
                                    onClick={() =>
                                      updateStatus(log.id, "in-progress")
                                    }
                                  >
                                    Start
                                  </Button>
