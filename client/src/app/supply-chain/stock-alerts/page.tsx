...
const ACTIVE_ALERTS: StockAlert[] = RULES.filter(
  (r) => r.active && getAlertType(r),
).map((r) => ({
  id: r.id,
  ruleId: r.id,
  itemName: r.itemName,
  ...
  triggeredAt: new Date(Date.now() - Math.random() * 86400000 * 3),
  resolved: false,
}));
...
const alertTypeConfig = {
  out: {
    ...
    icon: AlertTriangle,
    bgColor: "border-red-200 bg-red-50 dark:border-red-900 dark:bg-red-950/20",
  },
  low: {
    label: "Low Stock",
    color:
...
export default function StockAlertsPage() {
  ...
  const toggleRule = (
    id: string,
    field: "active" | "alertEmail" | "alertDashboard",
    val: boolean,
  ) => {
    setRules((prev) =>
      ...
      <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Low stock and overstock thresholds with notifications
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            {
              type: "out",
              count: alerts.filter((a) => a.type === "out").length,
            },
            {
              type: "low",
              count: alerts.filter((a) => a.type === "low").length,
            },
            {
              type: "overstock",
              count: alerts.filter((a) => a.type === "overstock").length,
            },
          ].map(({ type, count }) => {
              ...
              <Card key={type} className={cn(count > 0 && tc.bgColor)}>
                <CardContent className="p-4 flex items-center gap-3">
                  <Icon
                    className={cn(
                      "h-6 w-6",
                      count > 0 ? "text-current" : "text-muted-foreground",
                    )}
                  />
                  <div>
        ...
        <Tabs defaultValue="active">
          <TabsList>
            <TabsTrigger value="active">
              Active Alerts{" "}
              {alerts.length > 0 && (
                <Badge className="ml-1 h-4 px-1 text-xs">{alerts.length}</Badge>
              )}
            </TabsTrigger>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <Badge className={cn("text-xs", tc.color)}>
                              {tc.label}
                            </Badge>
                          </div>
                          <p className="text-sm text-muted-foreground">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {format(alert.triggeredAt, "MMM d, HH:mm")}
                          </p>
                        </div>
                      </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b bg-muted/50">
                        ...
                        <th className="p-4 text-right font-medium">Min</th>
                        <th className="p-4 text-right font-medium">Max</th>
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="p-4 text-center">
                              <Switch
                                checked={rule.alertEmail}
                                onCheckedChange={(v) =>
                                  toggleRule(rule.id, "alertEmail", v)
                                }
                              />
                            </td>
                            <td className="p-4 text-center">
                              <Switch
                                checked={rule.alertDashboard}
                                onCheckedChange={(v) =>
                                  toggleRule(rule.id, "alertDashboard", v)
                                }
                              />
                            </td>
                            <td className="p-4 text-center">
