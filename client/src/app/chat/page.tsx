...
import { Menu, MessageSquare, Settings, Search, ChevronDown } from "lucide-react"
...
export default function ChatPage() {
    ...
    useEffect(() => {
        if (!selectedChannel || isDm) { setIsPrivate(false); return }
        if (!FEATURES.CHAT_PRIVATE_CHANNELS) return
        chatApi.getChannel(selectedChannel)
            .then(res => setIsPrivate(res.data?.is_private ?? false))
            .catch(() => setIsPrivate(false))
    }, [selectedChannel, isDm])

    const handleSelectChannel = (id: string, name?: string, dmFlag?: boolean, priv?: boolean) => {
