...
import {
  LiveAudienceView,
  type ReactionType,
} from "@/components/slides/live-presentation";
    ...
    <div className="w-full h-full bg-background rounded-lg shadow-lg flex items-center justify-center p-8">
      <div className="text-center space-y-4">
        ...
        <p className="text-muted-foreground">
          Contenu synchronise avec le presentateur
        </p>
      </div>
    </div>
  );
...
function LivePageContent() {
      ...
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="text-center space-y-4 p-8">
          ...
          <h1 className="text-2xl font-bold text-white">
            Presentation en direct
          </h1>
          <p className="text-white/60 max-w-md">
            Utilisez le lien fourni par le presentateur pour rejoindre la
            presentation. Le lien doit contenir un identifiant de presentation
            (parametre &quot;id&quot;).
          </p>
        </div>
      </div>
  ...
  const transitions: SlideTransition[] = Array.from(
    { length: slideCount },
    () => ({
      type: "fade" as const,
      duration: 500,
