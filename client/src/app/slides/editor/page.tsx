...
function SlidesEditorContent() {
  ...
  useEffect(() => {
    if (id && id !== "new-presentation") {
      trackDocVisit({
        id,
        name: name || "Présentation sans titre",
        kind: "slide",
        href: `/slides/editor?id=${id}&name=${encodeURIComponent(name)}`,
      });
    }
  }, [id, name]);
  ...
  return (
    <EditorLayout
      ...
      icon={<Presentation className="w-5 h-5 text-yellow-600" />}
    >
      <div
        data-testid="slides-editor-root"
      ...
      >
        ...
        <div className="flex-1 overflow-hidden relative">
          {!loading && (
            <SlidesContent
              documentId={id}
              documentName={name || "presentation.signslides"}
