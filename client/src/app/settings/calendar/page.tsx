...
} from "@/stores/external-sync-store";
      ...
      {syncLogs.map((log) => (
        ...
        >
          ...
          <div className="text-right text-sm">
            <p>
              +{log.events_imported} / -{log.events_deleted} / ~
              {log.events_updated}
            </p>
      ...
      {unresolvedConflicts.map((conflict) => (
        <div key={conflict.id} className="p-4 border rounded-lg space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="font-medium">Conflit: {conflict.conflict_type}</p>
              <p className="text-sm text-muted-foreground">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="p-3 bg-purple-50 dark:bg-purple-950/30 rounded-lg">
              ...
              <p className="text-xs text-muted-foreground">
                Modifié:{" "}
                {new Date(conflict.external_updated_at).toLocaleString("fr-FR")}
              </p>
              <Button
                variant="outline"
                size="sm"
...
export default function CalendarSettingsPage() {
  ...
  const hasUnresolvedConflicts = useExternalSyncStore(
    selectHasUnresolvedConflicts,
  );
      ...
      <div className="w-full py-10">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground mt-2">
            Gérez vos connexions aux calendriers externes et configurez la
            synchronisation
          </p>
        </div>
