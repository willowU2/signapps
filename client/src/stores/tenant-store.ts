...
import { Tenant, Workspace, WorkspaceMember, tenantApi, workspacesApi } from "@/lib/api/tenant";
...
interface TenantState {
  ...
  updateWorkspace: (id: string, data: { name?: string; description?: string; color?: string }) => Promise<void>;
  ...
  persist(
    (set, get) => ({
      ...
      fetchWorkspaces: async () => {
        ...
        try {
          ...
          if (!currentWorkspace && response.data.length > 0) {
            const defaultWs = response.data.find(w => w.is_default) || response.data[0];
      ...
      createWorkspace: async (name, description, color) => {
        const response = await workspacesApi.create({ name, description, color });
      ...
      addMember: async (workspaceId, userId, role) => {
        await workspacesApi.addMember(workspaceId, { user_id: userId, role: role as 'owner' | 'admin' | 'member' | 'viewer' });
        // Refresh members
      ...
      updateMemberRole: async (workspaceId, userId, role) => {
        await workspacesApi.updateMemberRole(workspaceId, userId, { role: role as 'owner' | 'admin' | 'member' | 'viewer' });
        set(state => ({
          members: state.members.map(m =>
            m.user_id === userId ? { ...m, role: role as 'owner' | 'admin' | 'member' | 'viewer' } : m
          ),
        }));
      },
...
// Selectors
export const useCurrentWorkspace = () => useTenantStore(state => state.currentWorkspace);
