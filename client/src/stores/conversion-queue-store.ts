    ...
    deleteJob: async (jobId: string) => {
      try {
        ...
        set((state) => ({
          ...
          totalJobs: state.totalJobs - 1,
          selectedJob:
            state.selectedJob?.id === jobId ? null : state.selectedJob,
        }));
    ...
    updateJobInState: (job: ConversionJob) => {
      set((state) => {
        ...
        return {
          ...
          failedJobs: newJobs.filter((j) => j.status === "failed"),
          selectedJob:
            state.selectedJob?.id === job.id ? job : state.selectedJob,
        };
      });
    },
  }),
...
export const selectActiveJobs = (state: ConversionQueueState) =>
  state.activeJobs;
export const selectCompletedJobs = (state: ConversionQueueState) =>
  state.completedJobs;
export const selectFailedJobs = (state: ConversionQueueState) =>
  state.failedJobs;
