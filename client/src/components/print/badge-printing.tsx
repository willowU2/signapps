...
function BadgePreview({ badge }: { badge: BadgeData }) {
  return (
    <div className="rounded-xl overflow-hidden shadow-md inline-flex flex-col items-center text-white"
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 text-center">
        ...
        {badge.department && <p className="text-[10px] opacity-60 mt-0.5">{badge.department}</p>}
      </div>
...
export function BadgePrinting() {
  const [badges, setBadges] = useState<BadgeData[]>([
    { id: '1', name: 'Alice Dupont', role: 'Developer', department: 'Engineering', eventOrId: 'SignApps Summit', color: '#6366f1' },
  ]);
  const printRef = useRef<HTMLDivElement>(null);

  const add = () => setBadges(p => [...p, { id: Date.now().toString(), name: '', role: '', department: '', eventOrId: 'SignApps Event', color: BADGE_COLORS[p.length % BADGE_COLORS.length] }]);
  ...
  const print = () => {
    ...
    win?.document.write(`<html><head><style>body{margin:20px;background:#f0f0f0;}@media print{body{background:white;}}div{display:inline-block;margin:8px;}</style></head><body>${printRef.current.innerHTML}</body></html>`);
    win?.document.close();
    win?.print();
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        {/* Preview */}
        <div ref={printRef} className="flex gap-4 flex-wrap justify-center py-4 bg-muted/20 rounded-lg">
          {badges.map(b => <BadgePreview key={b.id} badge={b} />)}
        </div>
            ...
            <div key={badge.id} className="p-3 border rounded-lg">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  {BADGE_COLORS.map(c => (
                    <button key={c} onClick={() => update(badge.id, 'color', c)}
                      className={`w-5 h-5 rounded-full ${badge.color === c ? 'ring-2 ring-offset-1 ring-gray-400' : ''}`}
                      style={{ backgroundColor: c }} />
                  ))}
                </div>
                <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive" onClick={() => remove(badge.id)}>
                  <Trash2 className="h-3 w-3" />
                </Button>
              </div>
              <div className="grid grid-cols-2 gap-2">
                {([['name', 'Name'], ['role', 'Role'], ['department', 'Department'], ['eventOrId', 'Event / ID']] as [keyof BadgeData, string][]).map(([f, l]) => (
                  <div key={f} className="space-y-0.5">
                    <Label className="text-[10px]">{l}</Label>
                    <Input value={badge[f] as string} onChange={e => update(badge.id, f, e.target.value)} className="h-7 text-xs" />
                  </div>
                ))}
              </div>
            </div>
