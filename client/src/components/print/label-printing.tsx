...
export function LabelPrinting() {
  ...
  const addLabel = () =>
    setLabels((p) => [...p, { line1: "", line2: "", line3: "" }]);
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            ...
            <Select
              ...
              onValueChange={(id) =>
                setFormat(
                  AVERY_FORMATS.find((f) => f.id === id) || AVERY_FORMATS[0],
                )
              }
            >
        ...
        <div
          ref={printRef}
          className="border rounded-lg p-3 bg-card overflow-auto max-h-40"
        >
          <div className="flex flex-wrap gap-1">
            {labels.slice(0, 6).map((l, i) => (
              <div
                key={i}
                className="text-xs border border-dashed border-border rounded p-2"
                style={{ width: Math.min(format.width / 2, 160) }}
              >
                <p className="font-semibold truncate">{l.line1 || "—"}</p>
                {l.line2 && (
                  <p className="truncate text-muted-foreground">{l.line2}</p>
                )}
                {l.line3 && (
                  <p className="truncate text-muted-foreground">{l.line3}</p>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="space-y-2">
          {labels.map((l, i) => (
            <div key={i} className="p-3 border rounded-lg space-y-2">
              ...
              <Input
                value={l.line3}
                onChange={(e) => update(i, "line3", e.target.value)}
                placeholder="Line 3 (City, Country)"
                className="h-7 text-xs"
              />
            </div>
          ))}
