    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        >
          {/* Header */}
          <div
            className="px-8 py-5 border-b-4"
          ...
          <div className="px-8 py-6">
            <p className="text-xs text-right text-muted-foreground mb-6">
              Paris, le{" "}
          ...
          {/* Footer */}
          <div
            className="px-8 py-3 mt-auto border-t"
            style={{ borderColor: cfg.accentColor }}
          >
            <div className="flex justify-between text-[10px] text-gray-400">
              <span>
                {cfg.companyName} — {cfg.address}
              </span>
        ...
        <div className="flex gap-2">
          ...
          {COLORS.map((c) => (
            <button
              key={c}
              onClick={() => setCfg((p) => ({ ...p, accentColor: c }))}
        ...
        <div className="grid grid-cols-2 gap-3">
          {(
            [
              ...
              ["email", "Email"],
              ["website", "Website"],
            ] as [keyof LetterheadConfig, string][]
          ).map(([f, l]) => (
            <div key={f} className="space-y-1">
              <Label className="text-xs">{l}</Label>
              <Input
                value={cfg[f] as string}
                onChange={(e) => setCfg((p) => ({ ...p, [f]: e.target.value }))}
        ...
        <div className="space-y-1">
          ...
          <Textarea
            value={cfg.bodyText}
            onChange={(e) =>
              setCfg((p) => ({ ...p, bodyText: e.target.value }))
            }
            rows={6}
