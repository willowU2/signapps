          ...
          <div key={zone.id} className="border rounded-lg overflow-hidden">
            ...
            >
              <div className="flex items-center gap-3">
                <span className="font-semibold text-foreground">{zone.domain}</span>
                <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded">
                  {zone.recordCount} records
                </span>
              </div>
              <span className="text-sm text-muted-foreground">Updated: {new Date(zone.lastUpdated).toLocaleDateString()}</span>
            </button>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-muted border-b sticky top-0 z-10">
                    <tr>
                      ...
                      <th className="px-4 py-2 text-left font-semibold text-foreground">TTL</th>
                  ...
                  <tbody className="divide-y">
                    {zone.records.map((record) => (
                      <tr key={record.id} className="hover:bg-muted">
                        ...
                        <td className="px-4 py-2 font-mono text-muted-foreground">{record.name}</td>
                        <td className="px-4 py-2 text-muted-foreground">{record.value}</td>
                        <td className="px-4 py-2 text-muted-foreground">{record.ttl}s</td>
