...
export function DataRetentionPolicies() {
  ...
  function openEdit(p: RetentionPolicy) {
    ...
    setForm({
      ...
      retention_unit: p.retention_unit,
      purge_action: p.purge_action,
  ...
  async function handleSave() {
    if (!form.name || !form.data_type) {
      toast.error("Le nom et le type de données sont requis");
      return;
    }
    setSaving(true);
    try {
      if (editPolicy) {
        const res = await fetch(
          `/api/compliance/retention-policies/${editPolicy.id}`,
          {
            method: "PATCH",
        ...
        setPolicies((prev) =>
          prev.map((p) => (p.id === editPolicy.id ? updated : p)),
        );
  ...
  async function runPolicy(id: string) {
    setRunningId(id);
    try {
      const res = await fetch(`/api/compliance/retention-policies/${id}/run`, {
        method: "POST",
      });
  ...
  async function deletePolicy(id: string) {
    try {
      await fetch(`/api/compliance/retention-policies/${id}`, {
        method: "DELETE",
      });
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1">
                  <Badge
                    variant={ACTION_VARIANTS[p.purge_action]}
                    className="text-xs"
                  >
                    {ACTION_LABELS[p.purge_action]}
                  </Badge>
                  {p.auto_purge && (
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3 py-2">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5 col-span-2">
                ...
                <Input
                  ...
                  placeholder="emails / contacts / logs…"
                />
              </div>
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    onChange={(e) =>
                      setForm((p) => ({
                        ...p,
                        retention_value: Number(e.target.value),
                      }))
                    }
                    className="w-20"
                  />
                  <Select
                    ...
                    onValueChange={(v) =>
                      setForm((p) => ({
                        ...p,
                        retention_unit: v as RetentionUnit,
                      }))
                    }
                  >
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={form.purge_action}
                  onValueChange={(v) =>
                    setForm((p) => ({ ...p, purge_action: v as PurgeAction }))
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
            ...
            <div className="flex items-center justify-between">
              ...
              <Switch
                checked={form.auto_purge}
                onCheckedChange={(v) =>
                  setForm((p) => ({ ...p, auto_purge: v }))
                }
              />
            </div>
            <div className="flex items-center justify-between">
