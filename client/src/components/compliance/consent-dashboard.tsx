...
import { UserCheck, RefreshCw, Download, Search, Filter, CheckCircle2, XCircle, Clock } from "lucide-react"
...
export function ConsentDashboard() {
  ...
  const givenPct = stats && stats.total > 0 ? Math.round((stats.given / stats.total) * 100) : 0
    ...
    <div className="space-y-4">
      ...
      {stats && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            { label: "Total", value: stats.total, icon: <UserCheck className="h-4 w-4" /> },
            { label: "Given", value: stats.given, icon: <CheckCircle2 className="h-4 w-4 text-green-500" /> },
            { label: "Withdrawn", value: stats.withdrawn, icon: <XCircle className="h-4 w-4 text-red-500" /> },
            { label: "Expired", value: stats.expired, icon: <Clock className="h-4 w-4 text-orange-500" /> },
      ...
      <div className="flex items-center gap-2">
        ...
        <Select value={filter} onValueChange={v => setFilter(v as ConsentFilter)}>
      ...
      <Card>
        <CardContent className="p-0">
          <ScrollArea className="h-72">
            ...
            {records.map(r => (
              <div key={r.id} className="flex items-start justify-between px-4 py-3 border-b last:border-0">
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium">{r.subject_name ?? r.subject_email}</p>
                  <p className="text-xs text-muted-foreground">{r.subject_email}</p>
                  <div className="flex items-center gap-2 mt-1">
                    <Badge variant="outline" className="text-xs">{r.purpose}</Badge>
                  ...
                  <p className="text-xs text-muted-foreground">
                    ...
                    {r.expiry_date && ` · Expires ${format(new Date(r.expiry_date), "MMM d, yyyy")}`}
                  </p>
                </div>
                <Badge
