...
import { Cookie, Eye, Palette, Save, Globe, ToggleLeft, ToggleRight } from "lucide-react"
...
const DEFAULT_CONFIG: BannerConfig = {
  ...
  description: "We use cookies to improve your experience and analyze site usage. By clicking 'Accept All', you consent to our use of cookies.",
  ...
  auto_accept_days: 0,
  categories: [
    { id: "necessary", name: "Necessary", description: "Essential for the website to function.", required: true, enabled: true },
    { id: "analytics", name: "Analytics", description: "Help us understand how visitors use the site.", required: false, enabled: false },
    { id: "marketing", name: "Marketing", description: "Used to deliver relevant advertisements.", required: false, enabled: false },
    { id: "preferences", name: "Preferences", description: "Remember your settings and preferences.", required: false, enabled: false },
  ],
}
...
export function CookieBannerConfig() {
  ...
  function updateCategory(id: string, patch: Partial<CookieCategory>) {
    setConfig(p => ({
      ...p,
      categories: p.categories.map(c => c.id === id ? { ...c, ...patch } : c),
    }))
  }
      ...
      <Tabs defaultValue="content">
        ...
        <TabsContent value="content" className="space-y-3 mt-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Input value={config.accept_all_text} onChange={e => update("accept_all_text", e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">Reject all button</Label>
              <Input value={config.reject_all_text} onChange={e => update("reject_all_text", e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">Manage preferences</Label>
              <Input value={config.manage_text} onChange={e => update("manage_text", e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">Privacy policy URL</Label>
              <Input value={config.privacy_policy_url} onChange={e => update("privacy_policy_url", e.target.value)} />
            </div>
          </div>
        </TabsContent>
        ...
        <TabsContent value="appearance" className="space-y-3 mt-3">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Select value={config.position} onValueChange={v => update("position", v as BannerConfig["position"])}>
            ...
            <div className="space-y-1.5">
              ...
              <Select value={config.layout} onValueChange={v => update("layout", v as BannerConfig["layout"])}>
                <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <Input value={config.primary_color} onChange={e => update("primary_color", e.target.value)} className="h-8 text-xs font-mono" />
              </div>
            </div>
          </div>
          <div className="flex items-center justify-between">
          ...
          {config.show_logo && (
            <div className="space-y-1.5">
              ...
              <Input value={config.logo_url ?? ""} onChange={e => update("logo_url", e.target.value)} placeholder="/logo.png" />
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="categories" className="space-y-2 mt-3">
          {config.categories.map(cat => (
            <div key={cat.id} className="flex items-start justify-between rounded-md border px-3 py-2">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  />
                  {cat.required && <Badge variant="secondary" className="text-xs">Required</Badge>}
                </div>
                <Input
              ...
              <Switch
                checked={cat.enabled}
                onCheckedChange={v => !cat.required && updateCategory(cat.id, { enabled: v })}
                disabled={cat.required}
