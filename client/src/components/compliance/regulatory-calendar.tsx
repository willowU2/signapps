...
const GDPR_TEMPLATES: Omit<ComplianceDeadline, "id" | "completed">[] = [
  {
    ...
    description: "Review and update privacy policies, DPAs, and ROPAs.",
    due_date: "",
    type: "gdpr",
    recurrence: "annual",
    reminder_days: 30,
  ...
  {
    title: "Data breach notification window",
    description: "72h window to notify supervisory authority of breaches.",
    due_date: "",
    type: "gdpr",
    recurrence: "none",
    reminder_days: 1,
  ...
  },
  {
    ...
    description: "Review and re-obtain consent for long-standing processing.",
    due_date: "",
    type: "gdpr",
    recurrence: "annual",
    reminder_days: 60,
...
export function RegulatoryCalendar() {
  ...
  async function handleCreate() {
    if (!form.title || !form.due_date) {
      toast.error("Le titre et la date d'échéance sont requis");
      return;
    }
    setSaving(true);
    try {
  ...
  const filtered = deadlines
    .filter((d) => {
      if (filter === "upcoming")
        return !d.completed && !isPast(new Date(d.due_date));
      if (filter === "overdue")
        return !d.completed && isPast(new Date(d.due_date));
  ...
  const deadlinesByDay = (day: Date) =>
    deadlines.filter((d) => isSameDay(new Date(d.due_date), day));
          ...
          <div className="space-y-3 py-2">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="Annual GDPR review"
              />
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={form.type}
                  onValueChange={(v) =>
                    setForm((p) => ({ ...p, type: v as DeadlineType }))
                  }
                >
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setForm((p) => ({ ...p, recurrence: v as RecurrenceType }))
                  }
                >
                  <SelectTrigger className="h-8 text-xs">
                    <SelectValue />
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((p) => ({ ...p, regulation: e.target.value }))
                }
                placeholder="GDPR Art. 33, DORA Art. 17…"
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((p) => ({ ...p, reminder_email: e.target.value }))
                }
