...
import { ClipboardList, Plus, Eye, Download, Send, CheckCircle2, Clock, AlertTriangle, User } from "lucide-react"
...
type DsarType = "access" | "erasure" | "portability" | "rectification" | "restriction" | "object"
type DsarStatus = "received" | "verifying" | "processing" | "fulfilled" | "rejected" | "extended"
...
export function DsarWorkflow() {
  ...
  function getDaysLeft(req: DsarRequest) {
    ...
    return LEGAL_DEADLINE_DAYS - differenceInDays(new Date(), new Date(req.received_at))
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
        {(["received", "processing", "fulfilled", "rejected"] as DsarStatus[]).map(s => (
            ...
            >
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  <Badge variant="outline" className="text-xs">{DSAR_TYPES[req.type]}</Badge>
                </div>
                <p className="text-sm font-medium">{req.subject_name}</p>
                <p className="text-xs text-muted-foreground">{req.subject_email}</p>
                <p className="text-xs text-muted-foreground">
                  Received {format(new Date(req.received_at), "MMM d, yyyy")}
                  {daysLeft !== null && (
                    <span className={cn("ml-2", urgent ? "text-orange-600 font-medium" : "")}>
                      · {daysLeft}d left
                    </span>
                  )}
                </p>
              ...
              <div className="ml-3 flex-shrink-0">
                <div className={cn("h-2 w-2 rounded-full mt-2", STATUS_CONFIG[req.status].color)} />
              </div>
            </div>
          )
        })}
        {!loading && requests.length === 0 && (
          <p className="text-center text-sm text-muted-foreground py-8">No DSAR requests</p>
        )}
      </ScrollArea>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3 py-2">
            <div className="space-y-1.5">
              ...
              <Select value={form.type} onValueChange={v => setForm(p => ({ ...p, type: v as DsarType }))}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {(Object.keys(DSAR_TYPES) as DsarType[]).map(t => (
                    <SelectItem key={t} value={t}>{DSAR_TYPES[t]}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <Input value={form.subject_email} onChange={e => setForm(p => ({ ...p, subject_email: e.target.value }))} />
              </div>
            </div>
            <div className="space-y-1.5">
          ...
          <DialogContent className="max-w-md">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <div className={cn("h-2.5 w-2.5 rounded-full", STATUS_CONFIG[selectedReq.status].color)} />
                <span className="text-sm font-medium">{STATUS_CONFIG[selectedReq.status].label}</span>
              </div>
              <p className="text-sm"><span className="font-medium">{selectedReq.subject_name}</span> · {selectedReq.subject_email}</p>
              <p className="text-xs text-muted-foreground">{DSAR_TYPES[selectedReq.type]}</p>
              {selectedReq.description && <p className="text-sm text-muted-foreground">{selectedReq.description}</p>}
              <p className="text-xs text-muted-foreground">
                Deadline: {format(new Date(selectedReq.deadline_at), "MMM d, yyyy")}
              </p>
              ...
              {NEXT_STATUSES[selectedReq.status].length > 0 && (
                <div className="flex flex-wrap gap-2 pt-2 border-t">
                  {NEXT_STATUSES[selectedReq.status].map(s => (
                    <Button key={s} size="sm" variant="outline" onClick={() => updateStatus(selectedReq.id, s)}>
              ...
              {selectedReq.type === "access" || selectedReq.type === "portability" ? (
