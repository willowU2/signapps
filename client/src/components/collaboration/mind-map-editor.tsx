...
import { collaborationApi, type Board } from "@/lib/api/collaboration";
...
export function MindMapEditor() {
  ...
  const saveBoard = useCallback(async () => {
    try {
      if (boardId) {
        await collaborationApi.updateBoard(boardId, { data: { nodes } });
      } else {
        const board = await collaborationApi.createBoard({
          title: "Mind Map",
          board_type: "mindmap",
          data: { nodes },
        });
  ...
  const loadBoard = useCallback(async (id: string) => {
    try {
      const board = await collaborationApi.getBoard(id);
      const data = board.data as { nodes?: Record<string, MindNode> };
      if (data.nodes) {
        setNodes(data.nodes);
        setBoardId(board.id);
      }
  ...
  const saveEdit = useCallback(() => {
    ...
    setNodes((prev) => ({
      ...prev,
      [editing]: { ...prev[editing], text: editText },
    }));
    setEditing(null);
  }, [editing, editText]);
    ...
    <Card>
      ...
      <CardContent>
        ...
        <p className="text-xs text-muted-foreground mt-2">
          Click a node to select, then add children or edit. Click the edit
          circle to rename.
        </p>
      </CardContent>
    </Card>
