...
export function JournalEntry() {
  ...
  const totalDebit =
    draft?.lines.reduce((s, l) => s + (Number(l.debit) || 0), 0) || 0;
  const totalCredit =
    draft?.lines.reduce((s, l) => s + (Number(l.credit) || 0), 0) || 0;
  const isBalanced =
  ...
  const addLine = () =>
    setDraft((d) => (d ? { ...d, lines: [...d.lines, newLine()] } : null));
  const removeLine = (id: string) =>
    setDraft((d) =>
      d ? { ...d, lines: d.lines.filter((l) => l.id !== id) } : null,
    );
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Débit/crédit avec vérification d'équilibre
          </p>
        </div>
        <button
      ...
      {showForm && draft && (
        <div className="rounded-lg border bg-background p-5 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                value={draft.date}
                onChange={(e) =>
                  setDraft((d) => (d ? { ...d, date: e.target.value } : null))
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setDraft((d) =>
                    d ? { ...d, reference: e.target.value } : null,
                  )
                }
                placeholder="VTE-001"
                className="w-full border rounded px-3 py-2 text-sm"
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted border-b sticky top-0 z-10">
                <tr>
                  ...
                  <th className="px-3 py-2 text-right font-semibold text-blue-700 w-28">
                    Débit (€)
                  </th>
                  <th className="px-3 py-2 text-right font-semibold text-red-700 w-28">
                    Crédit (€)
                  </th>
              ...
              <tbody className="divide-y">
                  ...
                  <tr key={line.id}>
                    ...
                    <td className="px-2 py-1.5">
                      <input
                        ...
                        className="w-full border rounded px-2 py-1 text-sm text-right focus:ring-2 focus:ring-blue-300"
                        min="0"
                        step="0.01"
                        placeholder="0.00"
                      />
                    ...
                    <td className="px-2 py-1.5">
                      <input
                        ...
                        className="w-full border rounded px-2 py-1 text-sm text-right focus:ring-2 focus:ring-red-300"
                        min="0"
                        step="0.01"
                        placeholder="0.00"
                      />
                ...
                <tr
                  className={`border-t-2 font-bold ${isBalanced ? "bg-green-50" : totalDebit > 0 ? "bg-red-50" : "bg-muted"}`}
                >
          ...
          <div
            className={`flex items-center gap-3 p-3 rounded-lg ${isBalanced ? "bg-green-50 text-green-700" : totalDebit > 0 ? "bg-red-50 text-red-700" : "bg-muted text-muted-foreground"}`}
          >
            {isBalanced ? (
            ...
            <span className="text-sm font-medium">
              {isBalanced
                ? "Écriture équilibrée"
                : totalDebit > 0
                  ? `Déséquilibre : ${(totalDebit - totalCredit).toFixed(2)} €`
                  : "Saisissez les montants débit/crédit"}
            </span>
            <button
              onClick={handlePost}
              disabled={!isBalanced}
              className="ml-auto flex items-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors"
            >
              ...
              <tbody>
                {entry.lines.map((l) => (
                  <tr key={l.id} className="border-t">
                    <td className="px-8 py-1.5 text-muted-foreground w-52">
                      {l.account}
                    </td>
                    <td className="px-3 py-1.5 text-muted-foreground">
                      {l.label}
                    </td>
                    <td className="px-3 py-1.5 text-right text-blue-700 w-24">
                      {l.debit ? `${Number(l.debit).toFixed(2)} €` : ""}
                    </td>
                    <td className="px-3 py-1.5 text-right text-red-700 w-24">
