...
import { CheckCircle, AlertCircle, FileText, Download, Info } from "lucide-react";
...
export function VatDeclaration() {
  ...
  const [status, setStatus] = useState<"draft" | "validated" | "submitted">("draft");
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Déclaration TVA assistée</h2>
          <p className="text-gray-600">Calculée automatiquement depuis les écritures comptables</p>
        </div>
      ...
      <div className="flex gap-3">
        {PERIODS.map(p => (
          <button key={p.period} onClick={() => setSelectedPeriod(p.period)} className={`flex-1 rounded-lg border p-3 text-left transition-all ${selectedPeriod === p.period ? "ring-2 ring-blue-500 bg-blue-50" : "bg-white hover:border-blue-200"}`}>
            <p className="text-sm font-semibold text-gray-900">{p.label}</p>
            <span className={`text-xs px-2 py-0.5 rounded-full font-medium mt-1 inline-block ${p.status === "submitted" ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}>
      ...
      <div className="rounded-lg border bg-blue-50 p-4 flex items-start gap-3">
        ...
        <p className="text-sm text-blue-800">Les montants ci-dessous sont calculés automatiquement depuis les écritures validées du journal. Vérifiez les données avant de valider la déclaration.</p>
      </div>

      <div className="grid md:grid-cols-2 gap-6">
        <div className="rounded-lg border bg-white overflow-hidden">
          <div className="bg-green-50 border-b px-4 py-3">
            <h3 className="font-semibold text-green-900">TVA collectée (Ventes)</h3>
          </div>
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr><th className="px-4 py-2 text-left text-gray-600">Ligne</th><th className="px-4 py-2 text-right text-gray-600">Base HT</th><th className="px-4 py-2 text-right text-gray-600">Taux</th><th className="px-4 py-2 text-right text-gray-600">TVA</th></tr>
            </thead>
            <tbody className="divide-y">
              ...
              <tr className="bg-green-50 font-bold border-t-2">
                ...
                <td className="px-4 py-2.5 text-right">{fmtBase(totalSalesBase)}</td>
                <td />
                <td className="px-4 py-2.5 text-right text-green-700">{fmtBase(totalSalesVat)}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div className="rounded-lg border bg-white overflow-hidden">
          <div className="bg-orange-50 border-b px-4 py-3">
            <h3 className="font-semibold text-orange-900">TVA déductible (Achats)</h3>
          </div>
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr><th className="px-4 py-2 text-left text-gray-600">Ligne</th><th className="px-4 py-2 text-right text-gray-600">Base HT</th><th className="px-4 py-2 text-right text-gray-600">Taux</th><th className="px-4 py-2 text-right text-gray-600">TVA</th></tr>
            ...
            <tbody className="divide-y">
              ...
              <tr className="bg-orange-50 font-bold border-t-2">
                <td className="px-4 py-2.5">Total</td>
                <td className="px-4 py-2.5 text-right">{fmtBase(totalPurchaseBase)}</td>
                <td />
                <td className="px-4 py-2.5 text-right text-orange-700">{fmtBase(totalPurchaseVat)}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div className={`rounded-xl border-2 p-6 ${isCredit ? "bg-blue-50 border-blue-300" : "bg-red-50 border-red-300"}`}>
        <div className="flex items-center justify-between">
          ...
          <p className={`text-4xl font-bold ${isCredit ? "text-blue-700" : "text-red-700"}`}>{fmt(Math.abs(vatDue))}</p>
        </div>
        {status === "draft" && (
          <div className="mt-4 flex gap-3">
            <button onClick={() => setStatus("validated")} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium">
        ...
        {status === "validated" && (
          <div className="mt-4 flex items-center gap-3">
            ...
            <button onClick={() => setStatus("submitted")} className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-medium">
