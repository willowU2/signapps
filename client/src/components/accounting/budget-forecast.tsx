...
const pct = (actual: number, budget: number) =>
  budget > 0 ? ((actual / budget - 1) * 100).toFixed(1) : "0";
...
export function BudgetForecast() {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-foreground">
              Budget prévisionnel par centre de coûts
            </h2>
          </div>
        </div>
  ...
  const handleSaveBudget = (id: string) => {
    ...
    if (!isNaN(val) && val > 0) {
      setCenters((prev) =>
        prev.map((c) => (c.id === id ? { ...c, budget: val } : c)),
      );
    }
    setEditBudget(null);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Budget vs réalisé avec graphique comparatif
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div
          className={`rounded-lg border p-4 ${isOverBudget ? "bg-red-50" : "bg-green-50"}`}
        >
          <div className="flex items-center gap-2 mb-1">
            {isOverBudget ? (
            ...
            <span
              className={`text-xs font-medium ${isOverBudget ? "text-red-700" : "text-green-700"}`}
            >
              Écart
            </span>
          </div>
          <p
            className={`text-2xl font-bold ${isOverBudget ? "text-red-900" : "text-green-900"}`}
          >
            {variance >= 0 ? "+" : ""}
            {fmt(variance)}
          </p>
          ...
          >
            {isOverBudget
              ? `Dépassement de ${pct(totalActual, totalBudget)}%`
              : `Sous-consommation de ${Math.abs(Number(pct(totalActual, totalBudget)))}%`}
          </p>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
          ...
          {centers.map((c) => {
            ...
            return (
              <div
                ...
                className={`rounded-lg border p-3 cursor-pointer transition-all ${selectedCenter === c.id ? "ring-2 ring-blue-500 bg-blue-50 dark:bg-blue-950" : "bg-background hover:border-blue-200"}`}
              >
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          setEditBudget(c.id);
                          setBudgetInput(String(c.budget));
                        }}
                        className="p-0.5 hover:text-blue-600 text-gray-300"
                      >
        ...
        <div className="md:col-span-2 rounded-lg border bg-background p-4">
          <h3 className="font-semibold text-foreground mb-4">
            {center.name} — Budget vs Réalisé (mensuel)
          </h3>
            ...
            >
              ...
              <Bar dataKey="Réalisé" radius={[4, 4, 0, 0]}>
                {chartData.map((entry, i) => (
                  <Cell
                    key={i}
                    fill={entry.Réalisé > entry.Budget ? "#ef4444" : "#22c55e"}
                  />
                ))}
              </Bar>
            </BarChart>
