...
function parseFEC(raw: string): { entries: ParsedEntry[]; stats: ImportStats } {
  ...
  const entries: ParsedEntry[] = lines.map((line, i) => {
    ...
    if (cols.length < 7)
      return {
        ...
        credit: 0,
        valid: false,
        error: "Format invalide (colonnes insuffisantes)",
      };
  ...
  return {
    ...
    stats: {
      ...
      valid: valid.length,
      errors: entries.length - valid.length,
...
export function CsvFecImport() {
  ...
  const isBalanced = stats
    ? Math.abs(stats.totalDebit - stats.totalCredit) < 0.01
    : false;
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-foreground">
            Import écritures CSV / FEC
          </h2>
          <p className="text-muted-foreground">
            Parsez et créez des écritures depuis un fichier CSV ou FEC
          </p>
        </div>
        <button
          onClick={handleSample}
          className="flex items-center gap-2 border hover:bg-muted px-4 py-2 rounded-lg text-sm font-medium text-blue-600 border-blue-200"
        >
      ...
      {step === "upload" && (
        ...
        >
          ...
          <p className="text-xs text-gray-400 mt-3">
            Formats supportés : CSV (séparateur ; ou ,), FEC (format DGFiP)
          </p>
          <input
            ref={fileRef}
            type="file"
      ...
      {(step === "preview" || step === "imported") && stats && (
        <>
          ...
          <div className="grid grid-cols-4 gap-4">
            ...
            <div
              className={`rounded-lg border p-4 ${stats.errors > 0 ? "bg-red-50" : "bg-muted"}`}
            >
              <p
                className={`text-xs font-medium ${stats.errors > 0 ? "text-red-700" : "text-muted-foreground"}`}
              >
                Erreurs
              </p>
              <p
                className={`text-2xl font-bold ${stats.errors > 0 ? "text-red-900" : "text-gray-400"}`}
              >
                {stats.errors}
              </p>
            </div>
            <div
              className={`rounded-lg border p-4 ${isBalanced ? "bg-green-50" : "bg-amber-50"}`}
            >
              <p
                className={`text-xs font-medium ${isBalanced ? "text-green-700" : "text-amber-700"}`}
              >
                Équilibre D/C
              </p>
              <div className="flex items-center gap-1 mt-1">
                ...
                <p
                  className={`text-sm font-bold ${isBalanced ? "text-green-900" : "text-amber-900"}`}
                >
                  {isBalanced
                    ? "OK"
                    : `Écart: ${fmt(Math.abs(stats.totalDebit - stats.totalCredit))}`}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-lg border bg-background overflow-hidden">
            <div className="bg-muted border-b px-4 py-3 flex items-center justify-between">
              <h3 className="font-semibold text-foreground text-sm">
                Aperçu des écritures ({entries.length})
              </h3>
              {step === "preview" && (
                <button
                  onClick={handleImport}
                  disabled={stats.valid === 0}
                  className="bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 text-white px-4 py-1.5 rounded-lg text-sm font-medium flex items-center gap-2"
                >
                  <CheckCircle className="w-4 h-4" /> Importer {stats.valid}{" "}
                  écriture{stats.valid > 1 ? "s" : ""}
                </button>
              )}
              {step === "imported" && (
              ...
              <table className="w-full text-xs">
                ...
                <tbody className="divide-y">
                  {entries.map((e, i) => (
                    <tr
                      key={i}
                      className={`hover:bg-muted ${!e.valid ? "bg-red-50" : ""}`}
                    >
                      ...
                      <td className="px-3 py-2 text-center">
                        {e.valid ? (
                          ...
                          <span
                            ...
                            title={e.error}
                          >
                            <AlertCircle className="w-3.5 h-3.5 mx-auto" />
                          </span>
                        )}
                      </td>
                    </tr>
