...
function calcTotals(lines: InvoiceLine[]) {
  ...
  const vat = lines.reduce(
    (s, l) => s + (l.qty * l.unitPrice * l.vatRate) / 100,
    0,
  );
...
export function ClientInvoiceManagement() {
  ...
  const handleCreate = () => {
    ...
    setInvoices((prev) => [
      ...
      {
        id: String(Date.now()),
        number: `FA-2026-${String(n).padStart(3, "0")}`,
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewInvoice((f) => ({ ...f, client: e.target.value }))
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewInvoice((f) => ({ ...f, clientEmail: e.target.value }))
                }
                className="w-full border rounded px-3 py-2 text-sm"
              />
            </div>
            <div>
              <label className="block text-xs font-medium mb-1">Échéance</label>
              <input
                type="date"
                value={newInvoice.dueDate}
                onChange={(e) =>
                  setNewInvoice((f) => ({ ...f, dueDate: e.target.value }))
                }
                className="w-full border rounded px-3 py-2 text-sm"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewInvoice((f) => ({ ...f, notes: e.target.value }))
                }
                className="w-full border rounded px-3 py-2 text-sm"
              />
            </div>
      ...
      <div className="flex gap-1">
        {(["all", "draft", "sent", "paid", "overdue"] as const).map((s) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${statusFilter === s ? "bg-foreground text-background" : "bg-muted hover:bg-muted/80 text-foreground"}`}
          >
          ...
          <tbody className="divide-y">
            {filtered.map((inv) => {
              ...
              return (
                <tr
                  key={inv.id}
                  className={`hover:bg-muted ${inv.status === "overdue" ? "bg-red-50/30" : ""}`}
                >
                  <td className="px-4 py-3 font-mono text-xs font-semibold text-muted-foreground">
                    {inv.number}
                  </td>
                  <td className="px-4 py-3">
                    <p className="font-medium text-foreground">{inv.client}</p>
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full w-fit ${cfg.color}`}
                    >
                      {cfg.icon}
                      {cfg.label}
                    </span>
      ...
      {viewInvoice && (
        ...
        >
          <div
            className="bg-background rounded-2xl p-6 max-w-lg w-full shadow-xl space-y-4"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-xl font-bold">{viewInvoice.number}</h3>
                <p className="text-muted-foreground">{viewInvoice.client}</p>
              </div>
              <button onClick={() => setViewInvoice(null)}>
                <X className="w-5 h-5 text-gray-400 hover:text-muted-foreground" />
              </button>
            </div>
            <table className="w-full text-sm">
              <thead className="bg-muted border-b sticky top-0 z-10">
            ...
            {(() => {
              const { ht, vat, ttc } = calcTotals(viewInvoice.lines);
              return (
