...
export function ExpenseManagement() {
  ...
  const [form, setForm] = useState({ date: "", category: "Transport", description: "", amount: "", vatRate: "20", hasReceipt: false });
  ...
  const handleSubmit = () => {
    ...
    setExpenses(prev => [...prev, {
      id: String(Date.now()), submittedBy: "Moi", date: form.date, category: form.category,
      description: form.description, amount: Number(form.amount), vatRate: Number(form.vatRate),
      hasReceipt: form.hasReceipt, status: "pending",
    }]);
    setShowForm(false);
    setForm({ date: "", category: "Transport", description: "", amount: "", vatRate: "20", hasReceipt: false });
  };

  const handleApprove = (id: string) => setExpenses(prev => prev.map(e => e.id === id ? { ...e, status: "approved" } : e));

  const handleReject = () => {
    if (!rejectId) return;
    setExpenses(prev => prev.map(e => e.id === rejectId ? { ...e, status: "rejected", notes: rejectNote } : e));
    setRejectId(null);
    setRejectNote("");
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-lg border bg-green-50 p-4">
          ...
          <p className="text-xs text-green-600">{expenses.filter(e => e.status === "approved").length} notes</p>
        </div>
        <div className="rounded-lg border bg-amber-50 p-4">
          <p className="text-xs text-amber-700 font-medium">En attente</p>
          <p className="text-2xl font-bold text-amber-900">{fmt(totals.pending)}</p>
          <p className="text-xs text-amber-600">{expenses.filter(e => e.status === "pending").length} notes</p>
        </div>
        <div className="rounded-lg border bg-red-50 p-4">
          <p className="text-xs text-red-700 font-medium">Sans justificatif</p>
          <p className="text-2xl font-bold text-red-900">{expenses.filter(e => !e.hasReceipt && e.status !== "rejected").length}</p>
      ...
      {showForm && (
        <div className="rounded-lg border bg-blue-50 p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2"><label className="block text-xs font-medium mb-1">Description</label><input value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} className="w-full border rounded px-3 py-2 text-sm" /></div>
            <div><label className="block text-xs font-medium mb-1">Montant TTC (€)</label><input type="number" value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))} className="w-full border rounded px-3 py-2 text-sm" min="0" step="0.01" /></div>
            <div><label className="block text-xs font-medium mb-1">Taux TVA</label><select value={form.vatRate} onChange={e => setForm(f => ({ ...f, vatRate: e.target.value }))} className="w-full border rounded px-3 py-2 text-sm"><option value="0">0%</option><option value="10">10%</option><option value="20">20%</option></select></div>
          </div>
                ...
                <td className="px-4 py-3">
                  {exp.status === "pending" && (
                    <div className="flex items-center gap-1 justify-end">
                      <button onClick={() => handleApprove(exp.id)} className="p-1.5 hover:bg-green-100 rounded text-green-600"><Check className="w-4 h-4" /></button>
                      <button onClick={() => setRejectId(exp.id)} className="p-1.5 hover:bg-red-100 rounded text-red-600"><X className="w-4 h-4" /></button>
                    </div>
                  )}
                </td>
              </tr>
