  ...
  const importAll = () => {
    const toImport = rows
      .map((row) => {
        ...
        (Object.entries(mapping) as [DealFieldKey, string][]).forEach(
          ([field, col]) => {
            if (!obj[col]) return;
            if (field === "value" || field === "probability")
              d[field] = Number(obj[col]) || 0;
            else if (field === "stage" && STAGE_LABELS[obj[col] as DealStage])
              d[field] = obj[col];
    ...
    if (toImport.length === 0) {
      toast.error(
        "Aucun deal valide trouvé (colonne 'title' ou 'company' requise).",
      );
      return;
    }
    ...
    dealsApi.importMany(
      toImport as Omit<
        import("@/lib/api/crm").Deal,
        "id" | "createdAt" | "updatedAt"
      >[],
    );
    toast.success(
      `${toImport.length} deal${toImport.length > 1 ? "s" : ""} importé${toImport.length > 1 ? "s" : ""} avec succès.`,
    );
    setDone(true);
    onImport();
  };
