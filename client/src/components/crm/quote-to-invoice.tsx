...
export function QuoteToInvoice({
  ...
  const formattedDate = format(parseISO(quoteDate), 'd MMM yyyy', { locale: fr });
    ...
    <Card className="p-5 space-y-4">
            ...
            return (
              <div key={step.id} className="flex items-center flex-1">
                ...
                {index < STEPS.length - 1 && (
                  <div className={`flex-1 h-1 mx-1 ${isCompleted ? 'bg-emerald-600' : 'bg-gray-200'}`} />
                )}
              </div>
            );
          })}
      ...
      {currentStep === 'paid' && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3 text-center">
          <p className="text-sm font-semibold text-emerald-800">✓ Devis complètement traité</p>
        </div>
      )}
    </Card>
  );
