  ...
  const columns: ColumnDef<Followup>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        header: "J+N",
        cell: ({ row }) => (
          <span className="rounded-full bg-muted px-2 py-1 text-sm">{`J+${row.original.daysSinceCreation}`}</span>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-sm text-muted-foreground">
            {format(parseISO(row.original.nextActionDate), "d MMM yyyy", {
              locale: fr,
            })}
          </span>
        ),
            ...
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button
                  size="sm"
                  variant="outline"
                  disabled={markingDone === row.original.id}
                >
                  {markingDone === row.original.id
                    ? "En cours..."
                    : "Marquer fait"}
                </Button>
              </AlertDialogTrigger>
              <AlertDialogContent>
                ...
                <AlertDialogDescription>
                  Êtes-vous sûr de vouloir marquer le suivi de{" "}
                  <span className="font-medium">{row.original.clientName}</span>{" "}
                  ({row.original.quoteRef}) comme complété ?
                </AlertDialogDescription>
                <div className="flex justify-end gap-3">
                  <AlertDialogCancel>Annuler</AlertDialogCancel>
                  <AlertDialogAction
                    onClick={() => handleMarkDone(row.original.id)}
