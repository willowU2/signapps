      ...
      {sortedInteractions.length === 0 ? (
        ...
        <div className="relative space-y-6 pl-8">
          ...
          {sortedInteractions.map((interaction, index) => {
            ...
            const date = format(parseISO(interaction.date), 'd MMM yyyy à HH:mm', { locale: fr });
            const contentPreview = interaction.content.slice(0, 80) + (interaction.content.length > 80 ? '...' : '');

            return (
              <div key={interaction.id} className="relative">
                {/* Timeline dot */}
                <div className={`absolute -left-6 top-2 h-5 w-5 rounded-full border-4 border-white ${config.dotColor}`} />
