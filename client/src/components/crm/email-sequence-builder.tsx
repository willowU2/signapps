          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="space-y-1">
              ...
              <Input
                ...
                onChange={e =>
                  onChange({ ...step, delayDays: parseInt(e.target.value) || 0 })
                }
...
function ActiveRow({
  ...
  const progress = seq.steps.length > 0 ? (active.currentStep / seq.steps.length) * 100 : 0
  ...
  return (
    <div className="flex items-center gap-3 border rounded-lg p-3">
      ...
      {active.status === "active" && (
        <Button size="sm" variant="ghost" className="h-7 text-xs shrink-0" onClick={() => onPause(active.sequenceId + active.contactEmail)}>
          Pause
        </Button>
      )}
      {active.status === "paused" && (
        <Button size="sm" variant="ghost" className="h-7 text-xs shrink-0" onClick={() => onResume(active.sequenceId + active.contactEmail)}>
          Reprendre
        </Button>
      )}
    </div>
...
export function EmailSequenceBuilder() {
  ...
  const saveSequence = () => {
    ...
    setSteps([])
    setCreating(false)
    toast.success(`Séquence "${seq.name}" créée avec ${seq.steps.length} étapes.`)
  }
    ...
    <div className="space-y-6">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Button size="sm" variant="outline" onClick={addStep} className="gap-1 h-7">
              ...
              <CardContent className="pt-4 space-y-3">
                ...
                {launchTarget === seq.id ? (
                  <div className="flex gap-2">
                    ...
                    <Button size="sm" variant="outline" onClick={() => setLaunchTarget("")}>
                      ×
                    </Button>
                  </div>
                ) : (
      ...
      {sequences.length === 0 && !creating && (
        <div className="text-center py-10 text-muted-foreground">
          ...
          <p className="text-xs mt-1">Créez votre première campagne drip pour automatiser vos relances.</p>
        </div>
      )}
      ...
      {activeSeqs.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h4 className="font-medium text-sm">Séquences actives ({activeSeqs.length})</h4>
          </div>
          {activeSeqs.map(a => (
            <ActiveRow
