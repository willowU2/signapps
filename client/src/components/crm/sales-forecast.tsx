...
export function SalesForecast({ deals }: Props) {
  const data = useMemo(() => {
    const byMonth: Record<
      string,
      { month: string; weighted: number; bestCase: number; count: number }
    > = {};
    ...
    deals
      ...
      .forEach((d) => {
        ...
        if (!byMonth[key])
          byMonth[key] = { month: key, weighted: 0, bestCase: 0, count: 0 };
          ...
          {data.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={240}>
              ...
              >
                ...
                <Legend
                  formatter={(name) =>
                    name === "weighted" ? "Pondéré (prob.)" : "Best case"
                  }
                />
