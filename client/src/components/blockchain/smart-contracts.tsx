...
export function SmartContracts({ contracts = [], onExecute }: SmartContractsProps) {
      ...
      {contracts.length === 0 ? (
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <p className="text-sm text-slate-500">No smart contracts available</p>
          </CardContent>
        </Card>
      ) : (
              ...
              <CardContent className="p-0">
                ...
                >
                  ...
                  <div className="flex items-center gap-2 ml-4">
                    <Badge className={cn('capitalize', getStatusColor(contract.status))}>
                      {contract.status}
                    </Badge>
                    <ChevronDown
                      className={cn(
                  ...
                  <div className="border-t bg-slate-50 p-4 space-y-3">
                          ...
                          <div key={idx} className="flex items-start gap-2 text-sm">
                            <div
                              className={cn(
                                ...
                                trigger.satisfied ? 'bg-green-500' : 'bg-slate-300'
                              )}
                            />
                            <div className="flex-1">
                              <p className="font-medium text-slate-700">{trigger.name}</p>
                    ...
                    <div className="flex gap-2 pt-2">
                      <Button
                        ...
                        disabled={
                          executingId === contract.id || contract.status !== 'pending'
                        }
                      >
                        <Play className="w-4 h-4 mr-1" />
                        {executingId === contract.id ? 'Executing...' : 'Execute'}
                      </Button>
                    </div>
                  </div>
                )}
