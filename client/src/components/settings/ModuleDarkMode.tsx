...
export function useModuleTheme(moduleId: string) {
  ...
  const applyModuleTheme = useCallback((t: ModuleTheme) => {
    setTheme(t);
    const prefs = loadPrefs();
    prefs[moduleId] = t;
    savePrefs(prefs);
...
const THEME_OPTIONS: { value: ModuleTheme; label: string; icon: React.ReactNode }[] = [
  { value: 'system', label: 'Système', icon: <Monitor className="w-3.5 h-3.5" /> },
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          {customized > 0 && (
            <Badge variant="secondary" className="text-xs">{customized} personnalisé(s)</Badge>
          )}
        </CardTitle>
        <CardDescription>
          Définissez le mode clair/sombre indépendamment pour chaque section de l'application.
        </CardDescription>
      </CardHeader>
      <CardContent>
          ...
          {MODULES.map(mod => {
            ...
            return (
              <div key={mod.id} className="flex items-center justify-between py-1">
                ...
                <Select value={current} onValueChange={(v) => update(mod.id, v as ModuleTheme)}>
        ...
        <p className="text-xs text-muted-foreground mt-3">
          Note: Les préférences par module s'appliquent lors de la navigation dans la section correspondante.
        </p>
      </CardContent>
    </Card>
  );
