...
function hexToOklch(hex: string): OklchColor {
  ...
  const chroma =
    Math.sqrt(Math.pow(r - g, 2) + Math.pow(g - b, 2) + Math.pow(b - r, 2)) *
    0.3;
  const h = Math.atan2(g - b, r - g) * (180 / Math.PI);
  return {
    l: Math.min(0.95, Math.max(0.1, l)),
    c: Math.min(0.3, chroma * 0.5),
    h: (h + 360) % 360,
  };
}
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Définissez une couleur principale. La palette complète (accent, ring,
          sidebar) sera dérivée automatiquement.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center gap-2 p-3 rounded-lg bg-muted/50 text-sm">
          ...
          <span className="text-muted-foreground">
            Aperçu de la couleur primaire
          </span>
          <Button
            size="sm"
            variant="default"
            className="ml-auto h-7 text-xs pointer-events-none"
