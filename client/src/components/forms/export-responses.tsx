...
function getAnswerValue(r: FormResponse, fieldId: string): string {
  const entry = r.answers.find((a) => a.field_id === fieldId)?.value;
  return Array.isArray(entry)
    ? (entry as unknown[]).join(", ")
    : String(entry ?? "");
}
...
export function ExportResponses({ fields, responses }: Props) {
  ...
  const exportCSV = () => {
    ...
    const rows = responses.map((r) => [
      ...
      `"${(r.respondent ?? r.respondent_email ?? "").replace(/"/g, '""')}"`,
  ...
  const exportXLSX = async () => {
    ...
    try {
      ...
      ws.getRow(1).fill = {
        type: "pattern",
        pattern: "solid",
        fgColor: { argb: "FFE8E8F8" },
      };
      responses.forEach((r) => {
        ws.addRow([
          new Date(r.submitted_at).toLocaleString("fr-FR"),
          r.respondent ?? r.respondent_email ?? "",
