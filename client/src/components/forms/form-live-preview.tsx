...
export function FormLivePreview({ form }: FormLivePreviewProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => setBranding(loadBranding(form.id)), 2000);
        ...
        <div className={cn("w-full transition-all duration-300", widthClass)}>
          {branding?.coverImageUrl && (
            ...
            <img
              ...
              style={{ borderRadius: `${branding?.borderRadius ?? 8}px ${branding?.borderRadius ?? 8}px 0 0` }}
            />
          )}
          <Card
            ...
            <CardContent className="space-y-6">
              {(form.fields || []).length === 0 ? (
                <p className="text-sm text-muted-foreground italic text-center py-8">
                  Aucun champ. Retourne à l&apos;onglet Éditeur pour ajouter
                  des questions.
                </p>
              ) : (
