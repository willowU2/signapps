...
import type { Conflict, AvailableSlot } from '@/lib/scheduling/utils/conflict-detection';
  ...
  return (
    <div
      className={cn(
        ...
        isError ? 'border-destructive/50 bg-destructive/5' : 'border-yellow-500/50 bg-yellow-500/5'
      )}
    >
      <Icon
        className={cn(
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          <span className="font-medium text-sm truncate">{conflict.event.title}</span>
          <Badge
    ...
    >
      <Alert variant={hasErrors ? 'destructive' : 'default'} className="relative">
        ...
        <div className="flex items-start gap-2">
          <AlertTriangle className={cn('h-5 w-5', hasErrors ? 'text-destructive' : 'text-yellow-600')} />
          <div className="flex-1">
            ...
            <AlertDescription>
              {errorCount > 0 && (
                <span className="text-destructive mr-2">
                  {errorCount} conflit{errorCount > 1 ? 's' : ''} bloquant{errorCount > 1 ? 's' : ''}
                </span>
              )}
              {warningCount > 0 && (
        ...
        {suggestions.length > 0 && (
          <div className="mt-4 pt-4 border-t">
            ...
            >
              ...
              <span>
                {showSuggestions ? 'Masquer' : 'Afficher'} {suggestions.length} créneau{suggestions.length > 1 ? 'x' : ''} suggéré{suggestions.length > 1 ? 's' : ''}
              </span>
            </button>
