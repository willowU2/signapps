...
export function PrintableAgendaButton({
  events,
  currentDate,
...
export function WeeklyAgendaPrintPreview({
  events,
  currentDate,
  calendarName = "My Calendar",
    ...
    <div className="space-y-0 print:block">
      ...
      {weekDays.map((day) => {
        ...
        return (
          <div
            key={day.toISOString()}
            className={`flex border-t border-border/50 py-3 gap-4 ${isToday ? "bg-blue-50/30" : ""}`}
          >
            <div className="w-20 shrink-0">
              <p
                className={`text-xs font-semibold uppercase tracking-wide ${isToday ? "text-blue-600" : "text-muted-foreground"}`}
              >
                {format(day, "EEE")}
              </p>
              <p
                className={`text-2xl font-light leading-tight ${isToday ? "text-blue-600" : "text-foreground"}`}
              >
                {format(day, "d")}
              </p>
            </div>
            <div className="flex-1 space-y-1.5 pt-1">
              {dayEvents.length === 0 ? (
                ...
                dayEvents.map((e) => (
                  <div
                    key={e.id}
                    className="flex gap-2 text-sm border-l-2 border-blue-500 pl-2"
                  >
