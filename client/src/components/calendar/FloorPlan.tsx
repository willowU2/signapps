...
export function FloorPlan({
  ...
  const [hoveredResourceId, setHoveredResourceId] = React.useState<
    string | null
  >(null);
      ...
      >
          ...
          {filteredResources.map((fpResource) => {
            ...
            if (!status.available && status.nextAvailable) {
              const minutesUntilAvailable =
                (status.nextAvailable.getTime() - currentTime.getTime()) /
                60000;
        ...
        <div className="absolute bottom-4 left-4 flex items-center gap-4 p-2 rounded-lg bg-background/80 backdrop-blur-sm border text-xs">
          <div className="flex items-center gap-1.5">
            <div
              className={cn("w-3 h-3 rounded-full", STATUS_COLORS.available.bg)}
            />
            <span>Libre</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div
              className={cn("w-3 h-3 rounded-full", STATUS_COLORS.soon.bg)}
            />
            <span>Bient\u00f4t libre</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div
              className={cn("w-3 h-3 rounded-full", STATUS_COLORS.occupied.bg)}
            />
            <span>Occup\u00e9</span>
          </div>
        </div>
...
function ResourceDetailPanel({
  ...
  const fpResource = floorPlanResources.find(
    (fp) => fp.resourceId === resourceId,
  );
      ...
      {!status.available && status.nextAvailable && (
        <p className="mt-3 text-sm text-muted-foreground">
          ...
          <span className="font-medium text-foreground">
            {formatTime(status.nextAvailable)}
          </span>
        </p>
      )}
