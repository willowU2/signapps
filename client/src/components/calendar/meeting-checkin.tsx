  ...
  const loadUpcoming = useCallback(async () => {
    try {
      ...
      const eventArrays = await Promise.all(
        allCals.map((cal) =>
          calendarApi
            .listEvents(cal.id, windowStart, windowEnd)
            .then((r) => r.data ?? []),
        ),
      );
      ...
      const enriched: CheckinEvent[] = await Promise.all(
        events.map(async (event) => {
          ...
          const meta: Record<string, AttendanceStatus> =
            (event.metadata?.checkin as
              | Record<string, AttendanceStatus>
              | undefined) ?? {};

          const attendees: AttendeeStatus[] = attendeesRaw.map(
  ...
  const handleMyStatus = async (eventId: string, status: AttendanceStatus) => {
    ...
    try {
      ...
      const currentCheckin =
        (currentMeta.checkin as Record<string, unknown> | undefined) ?? {};
      ...
      setCheckins((prev) =>
        prev.map((c) =>
          c.event.id === eventId ? { ...c, updating: false } : c,
        ),
      );
    }
  };
