...
function getDefaultCategories(): EventCategory[] {
    return [
        ...
        { id: "personal", name: "Personal", color: "#33b679", textColor: "#ffffff" },
        { id: "focus", name: "Focus", color: "#8e24aa", textColor: "#ffffff" },
        { id: "external", name: "External", color: "#e67c73", textColor: "#ffffff" },
    ]
}
...
function ColorSwatch({ color, label, selected, onClick }: { color: string; label: string; selected: boolean; onClick: () => void }) {
    return (
        <button
            type="button"
            className={cn("w-7 h-7 rounded-full border-2 transition-all", selected ? "border-foreground scale-110" : "border-transparent hover:scale-105")}
...
// Category badge
export function CategoryBadge({ category, size = "sm" }: { category: EventCategory; size?: "xs" | "sm" }) {
        ...
        <Popover open={open} onOpenChange={setOpen}>
            ...
            <PopoverContent align="start" className="w-48 p-1">
                ...
                {categories.map((cat) => (
                    <button
                        key={cat.id}
                        className={cn("flex items-center gap-2 w-full px-2 py-1.5 text-sm rounded hover:bg-muted", value === cat.id && "bg-muted")}
...
export function CategoryManager() {
    ...
    const openCreate = () => {
        setEditing({ name: "", color: PRESET_COLORS[0].bg, textColor: PRESET_COLORS[0].text })
    ...
    const openEdit = (cat: EventCategory) => {
        setEditing({ ...cat })
        const preset = PRESET_COLORS.find((p) => p.bg === cat.color) || PRESET_COLORS[0]
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="space-y-1.5">
                {categories.map((cat) => (
                    <div key={cat.id} className="flex items-center gap-3 p-2 rounded-lg border border-border/50 hover:bg-muted/30 transition-colors group">
                        <span className="w-5 h-5 rounded-full shrink-0 border border-white/30" style={{ background: cat.color }} />
                        ...
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <Button variant="ghost" size="icon" className="h-6 w-6 hover:text-red-500" onClick={() => handleDelete(cat.id)}>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {PRESET_COLORS.map((preset) => (
                                        <ColorSwatch
                                            ...
                                            onClick={() => {
                                                setSelectedPreset(preset)
                                                setEditing({ ...editing, color: preset.bg, textColor: preset.text })
                                            }}
                                        />
                                    ))}
                                </div>
                                <div className="flex items-center gap-2 mt-2">
                                    ...
                                    {editing.color && editing.name && (
                                        <CategoryBadge
                                            category={{ id: "preview", name: editing.name, color: editing.color, textColor: editing.textColor || "#fff" }}
                                            size="sm"
                                        />
                                    )}
                                </div>
