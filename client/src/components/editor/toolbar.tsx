...
export function Toolbar({ className, children, ...props }: ToolbarProps) {
  return (
    <div
      className={cn(
        "flex flex-wrap items-center gap-0.5 px-4 py-1.5 w-full bg-[#edf2fa] dark:bg-[#3c4043] shrink-0 border-b border-transparent dark:border-[#5f6368]",
        className,
      )}
      {...props}
    >
...
export function ToolbarDivider({ className }: { className?: string }) {
  return (
    <div
      className={cn(
        "w-px h-5 bg-[#e3e3e3] dark:bg-[#5f6368] mx-1 self-center shrink-0",
        className,
      )}
    />
  );
...
export function ToolbarGroup({
  ...
  return (
    <div
      className={cn("flex items-center mx-1 relative", className)}
      {...props}
    >
      {children}
    </div>
