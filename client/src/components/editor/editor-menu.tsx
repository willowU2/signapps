...
export function EditorMenu({
  ...
  const renderMenuItem = (item: MenuItem, idx: number, level: number = 0) => {
    if (item.sep) {
      return (
        <div
          key={`sep-${idx}`}
          className="h-px bg-[#dadce0] dark:bg-[#5f6368] my-1 mx-0"
        />
      );
    }
      ...
      <div key={item.label || idx} className="relative group/sub">
        ...
        >
          <div className="flex items-center gap-3">
            {item.icon ? (
              <div className="text-[#5f6368] dark:text-[#9aa0a6] w-4">
                {item.icon}
              </div>
            ) : (
              <div className="w-4" />
          ...
          <div className="flex items-center">
            {item.shortcut && (
              <span className="text-[11px] text-[#5f6368] dark:text-[#9aa0a6] font-sans tracking-wide ml-4">
                {item.shortcut}
              </span>
            )}
            {hasSubMenu && (
              <span className="ml-3 text-[10px] text-[#5f6368] dark:text-[#9aa0a6]">
                ▶
              </span>
            )}
          </div>
        ...
        {hasSubMenu && (
          <div className="absolute top-0 left-full -mt-1 hidden group-hover/sub:block bg-background dark:bg-[#2d2e30] border border-[#dadce0] dark:border-[#5f6368] rounded shadow-lg z-50 py-1 min-w-[220px]">
            {item.subItems!.map((subItem, sIdx) =>
              renderMenuItem(subItem, sIdx, level + 1),
            )}
          </div>
        )}
      </div>
  ...
  return (
    <div
      ref={menuRef}
      className={`flex gap-1 text-[13px] text-[#444746] dark:text-[#9aa0a6] mt-0.5 relative ${className}`}
    >
      {menus.map((menu) => (
