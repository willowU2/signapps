...
export function AiAgent() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    // Add user message
    const userMsg: AgentMessage = { role: 'user', content: trimmed, timestamp: new Date() };
...
Only include steps actually needed. Use ISO dates for dates.`,
      ...
      let parsedPlan: { steps: Array<{ id: string; action: string; label: string; params: Record<string, string> }>; intent: string; summary: string } | null = null;
      try {
        ...
        setMessages((prev) =>
          prev.map((m, i) =>
            i === prev.length - 1 ? { ...m, content: planResponse.data.answer, steps: [] } : m
          )
        );
        scrollToBottom();
        return;
      ...
      const updateStep = (stepId: string, status: StepStatus, result?: string) => {
        setMessages((prev) =>
          prev.map((m, i) =>
            i === prev.length - 1
              ? {
          ...
          switch (step.action) {
            case 'search_contact': {
              ...
              const found = contacts.data.filter(
                (c) =>
                  `${c.first_name} ${c.last_name}`.toLowerCase().includes(name) ||
                  (c.email ?? '').toLowerCase().includes(name)
              );
              results[step.id] = found;
              updateStep(step.id, 'done', found.length ? `${found.length} contact(s) trouvé(s)` : 'Aucun contact trouvé');
              break;
            }
            ...
            case 'check_calendar': {
              ...
              const events = await calendarApi.listEvents(DEFAULT_CALENDAR_ID, now, nextWeek);
              results[step.id] = events.data;
              updateStep(step.id, 'done', `${Array.isArray(events.data) ? events.data.length : 0} événement(s) récupéré(s)`);
              break;
            }
            ...
            case 'create_event': {
              ...
              updateStep(step.id, 'done', `Événement créé : ${ev.data?.title ?? step.params.title}`);
              break;
            }

            case 'send_email': {
              const prevContacts = results['step_1'] as Array<{ email?: string }> | undefined;
              ...
              await mailApi.sendEmail({
                ...
                body_text: step.params.body ?? `Bonjour,\n\n${parsedPlan!.summary}\n\nCordialement`,
              });
            ...
            case 'find_document': {
              ...
              const found = nodes?.filter((n) => n.name.toLowerCase().includes(query));
              results[step.id] = found;
              updateStep(step.id, 'done', found?.length ? `Document trouvé : ${found[0].name}` : 'Document non trouvé');
              break;
            }
            ...
            case 'list_team_members': {
              ...
              updateStep(step.id, 'done', `${contacts.data.length} membre(s) récupéré(s)`);
              break;
            }
      ...
      const summaryResp = await aiApi.chat(
        `Résume en 1-2 phrases ce qui a été accompli pour la tâche : "${trimmed}". Résultats: ${JSON.stringify(
          Object.fromEntries(
            Object.entries(results).map(([k, v]) => [k, Array.isArray(v) ? `${(v as unknown[]).length} items` : String(v)])
          )
        )}`,
        {
          enableTools: false,
          systemPrompt: 'Tu es un assistant qui résume des actions. Réponds en français, de façon concise et positive.',
        }
      );

      setMessages((prev) =>
        prev.map((m, i) =>
          i === prev.length - 1 ? { ...m, content: summaryResp.data.answer } : m
        )
      );
    } catch (err) {
      ...
      setMessages((prev) =>
        prev.map((m, i) =>
          i === prev.length - 1
            ? { ...m, content: 'Une erreur s\'est produite lors de l\'exécution.', steps: [] }
            : m
        )
      );
    } finally {
    ...
    <Card className="w-full flex flex-col h-[600px]">
      <CardHeader className="pb-3 border-b">
        <CardTitle className="flex items-center gap-2 text-lg">
          ...
          Agent IA autonome
          <Badge variant="secondary" className="text-xs font-normal">multi-étapes</Badge>
        </CardTitle>
        <p className="text-xs text-muted-foreground">
          Décrivez une action complexe en langage naturel. L'agent la décompose et l'exécute automatiquement.
        </p>
      </CardHeader>
          ...
          {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center text-muted-foreground gap-3">
              ...
              <div>
                ...
                <p className="text-xs mt-1 opacity-75">"Organise une reunion avec Jean mardi"</p>
                <p className="text-xs opacity-75">"Envoie le rapport Q1 à l'équipe"</p>
                <p className="text-xs opacity-75">"Crée un événement de formation vendredi 14h"</p>
              </div>
            </div>
          )}
                  ...
                  <>
                    ...
                    {msg.steps && msg.steps.length > 0 && (
                      <div className="bg-muted/50 border rounded-xl p-3 space-y-2 w-full">
                        <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Exécution</p>
                        {msg.steps.map((step) => (
                          <div key={step.id} className="flex items-start gap-2">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className={`text-xs font-medium ${step.status === 'error' ? 'text-red-600' : ''}`}>
                                {step.label}
                              </p>
                              {step.result && (
                                <p className="text-xs text-muted-foreground truncate">{step.result}</p>
                              )}
                            </div>
                          </div>
                        ))}
                    ...
                    {msg.content && (
                      <div className="bg-background border rounded-2xl rounded-tl-sm px-3 py-2 text-sm">
                        {msg.steps && msg.steps.some((s) => s.status === 'running') ? (
                    ...
                    {!msg.content && msg.steps && msg.steps.some((s) => s.status === 'running') && (
                      <div className="bg-background border rounded-2xl rounded-tl-sm px-3 py-2 text-sm">
                        <span className="flex items-center gap-1.5 text-muted-foreground">
                          <Loader2 className="h-3 w-3 animate-spin" />
                          Analyse en cours...
