...
import {
  ...
  Deletion,
  TrackChanges,
} from "../docs/extensions/track-changes";
  ...
  const editor = useEditor({
    ...
    extensions: [
      ...
      Comment.configure({
        HTMLAttributes: {
          class:
            "comment-highlight bg-yellow-100 dark:bg-yellow-900/30 border-b-2 border-yellow-400",
        },
      }),
      Mention.configure({
        HTMLAttributes: {
          class:
            "mention bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded px-1 font-medium",
        },
  ...
  const handleAiGenerate = useCallback(
    async (e?: React.FormEvent) => {
      ...
      await stream(
        ...
        {
          systemPrompt:
            "You are an expert writing assistant. Provide direct, formatting-friendly plain text without filler phrases.",
          language: "fr",
        },
      );
    },
    ...
    <div className="flex flex-col h-screen w-full bg-[#f8f9fa] dark:bg-background overflow-hidden font-sans">
      ...
      <div className="flex flex-col shrink-0 drop-shadow-sm z-20">
        <DocumentHeader
          ...
          awarenessStates={
            awareness
              ? (Array.from(awareness.getStates().values()) as unknown[])
              : []
          }
          ...
          <div className="w-[816px] shrink-0 min-h-[1056px] bg-background dark:bg-[#1f1f1f] shadow-[0_1px_3px_auto_rgba(0,0,0,0.1)] ring-1 ring-[#e2e2e2] dark:ring-[#ffffff1a] rounded-sm relative mt-2 mb-10 block">
            ...
            {editor.isEmpty && (
              <div className="absolute top-[300px] left-0 right-0 flex items-center justify-center gap-2 z-10 select-none pointer-events-none">
                ...
                >
                  <Sparkles
                    className="h-[18px] w-[18px] mb-0.5 mr-2 text-[#0b57d0]"
                    fill="#0b57d0"
                  />
                  Générer un document
                </Button>
