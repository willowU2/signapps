...
export function MailClassifier({
  emails = [],
  onCategoryChange,
}: MailClassifierProps) {
  ...
  const updateCategory = (
    ...
    setItems((prev) =>
      prev.map((item) => (item.id === emailId ? { ...item, category } : item)),
    );
  ...
  return (
    <Card className="w-full">
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        >
          <RefreshCw
            className={`h-4 w-4 mr-1 ${isClassifying ? "animate-spin" : ""}`}
          />
          Auto-Classify
        </Button>
      </CardHeader>
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="ml-2 flex-shrink-0">
                    {email.category && (
                      <span
                        className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${categoryColors[email.category]}`}
                      >
                        {email.category}
                      </span>
                    )}
