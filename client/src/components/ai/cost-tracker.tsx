...
export function AiCostTracker() {
  const [data, setData] = useState<CostSummary>({ models: [], total_cost: 0, total_requests: 0 });
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <Card>
          <CardContent className="pt-3 text-center">
            ...
            <p className="text-xl font-bold">{data.total_cost === 0 ? 'Gratuit' : `$${data.total_cost.toFixed(2)}`}</p>
      ...
      {data.models.length > 0 && (
        <div className="space-y-2">
          {data.models.map((m) => (
            <div key={m.model} className="flex items-center gap-3 p-2 rounded-lg border text-sm">
              <Badge variant="secondary" className="font-mono text-xs">{m.model}</Badge>
              <span className="text-muted-foreground">{m.total_requests} req</span>
              <span className="text-muted-foreground">{(m.total_tokens / 1000).toFixed(0)}K tokens</span>
