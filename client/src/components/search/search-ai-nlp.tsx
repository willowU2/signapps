...
export function AiNaturalLanguageSearch() {
  ...
  const search = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const { data } = await client.post<NlpSearchResponse>('/search/nlp', { query });
      setResponse(data);
    } catch {
      setResponse({ results: [], interpretation: 'Erreur de recherche IA', suggestedQuery: '' });
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="text-sm font-semibold">Recherche IA en langage naturel</h3>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-between items-center">
          <div className="flex gap-1 flex-wrap">
            {examples.map((ex, i) => (
              <Button key={i} variant="outline" size="sm" className="h-6 text-xs" onClick={() => setQuery(ex)}>
                {ex.slice(0, 30)}…
              </Button>
            ))}
          </div>
          <Button onClick={search} disabled={loading || !query.trim()} size="sm" className="gap-1.5 shrink-0">
      ...
      {response && (
        <div className="space-y-3">
          ...
          {response.results.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-4">Aucun résultat correspondant</p>
          ) : (
            <ScrollArea className="max-h-72">
              <div className="space-y-2">
                {response.results.map(r => (
                  <a key={r.id} href={r.url} className="block border rounded-lg p-3 hover:bg-muted/50 transition-colors group">
                    <div className="flex items-start gap-2">
                      <div className="flex-1 min-w-0">
                        ...
                        {r.explanation && (
                          <p className="text-xs text-purple-600 dark:text-purple-400 mt-1">{r.explanation}</p>
                        )}
                      </div>
