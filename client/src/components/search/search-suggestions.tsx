...
const DICTIONARY = [
  'contrat', 'contact', 'document', 'facture', 'devis', 'projet', 'tache',
  'email', 'calendrier', 'reunion', 'rapport', 'workflow', 'archive',
  'client', 'fournisseur', 'commande', 'livraison', 'paiement', 'budget',
];
        ...
        {suggestions.map((s, i) => (
          <li key={`${s.type}-${i}`}>
            <button
              ...
              onClick={() => { onSelect(s.text); addRecentSearch(s.text); }}
            >
              {s.type === 'recent' && <Clock className="h-4 w-4 text-muted-foreground shrink-0" />}
              {s.type === 'autocomplete' && <Search className="h-4 w-4 text-primary shrink-0" />}
              {s.type === 'correction' && <AlertCircle className="h-4 w-4 text-amber-500 shrink-0" />}

              <span className="flex-1 min-w-0">
                {s.type === 'correction' ? (
                  <span>
                    Vouliez-vous dire &ldquo;<strong className="text-primary">{s.text}</strong>&rdquo; ?
                  </span>
                ) : (
                  <span>{s.text}</span>
                )}
