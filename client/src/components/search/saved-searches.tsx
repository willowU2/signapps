...
export function SavedSearches({
    ...
    const interval = setInterval(() => {
      setSearches((prev) => {
        const updated = prev.map((s) => {
          ...
          const newCount =
            Math.random() > 0.85 ? Math.floor(1 + Math.random() * 3) : 0;
          return newCount > 0
            ? { ...s, newResultCount: (s.newResultCount || 0) + newCount }
            : s;
        });
        save(updated);
        return updated;
  ...
  const handleToggleAlert = (id: string) => {
    const updated = searches.map((s) =>
      s.id === id
        ? { ...s, alertEnabled: !s.alertEnabled, newResultCount: 0 }
        : s,
    );
    save(updated);
    setSearches(updated);
    ...
    toast.success(
      search?.alertEnabled ? "Alerte activée" : "Alerte désactivée",
    );
  };
  ...
  const handleRun = (search: SavedSearch) => {
    ...
    const updated = searches.map((s) =>
      s.id === search.id ? { ...s, newResultCount: 0 } : s,
    );
    save(updated);
    setSearches(updated);
    onRun?.(search);
    ...
    <div className="space-y-3">
      ...
      {searches.length === 0 ? (
        <p className="text-xs text-muted-foreground py-4 text-center">
          ...
          <br />
          Lancez une recherche et cliquez sur &ldquo;Sauvegarder&rdquo;.
        </p>
      ) : (
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3 py-2">
            ...
            <p className="text-xs text-muted-foreground">
              Requête : &ldquo;
              <span className="font-medium">{currentQuery}</span>&rdquo;
            </p>
          </div>
          <DialogFooter>
