...
const MODULE_COLORS: Record<string, string> = {
  ...
  calendar_event: "text-green-500",
  mail_message: "text-orange-500",
  contact: "text-purple-500",
  task: "text-yellow-500",
  drive_node: "text-cyan-500",
};
  ...
  useEffect(() => {
    ...
    const search = async () => {
      ...
      try {
        ...
        const { data } = await client().get<SearchResult[]>("/search/unified", {
          params,
        });
        setResults(data);
      } catch {
        ...
        {results.length > 0 && (
          <div className="flex gap-1 px-3 py-2 border-b overflow-x-auto">
            ...
            {types.map((t) => (
              ...
              >
                {TYPE_LABELS[t] || t} (
                {results.filter((r) => r.entity_type === t).length})
              </Button>
            ))}
          </div>
        )}
          ...
          <div className="p-2">
            {Object.entries(grouped).map(([type, items]) => (
              <div key={type} className="mb-3">
                <div
                  className={`flex items-center gap-1.5 px-2 py-1 text-xs font-medium ${MODULE_COLORS[type] || "text-muted-foreground"}`}
                >
                  {TYPE_ICONS[type]}
