...
export function VaultList({
  ...
  const { items, folders, sharedItems, loading, deleteItem, startBrowse } =
    useVaultStore();
    ...
    if (search.trim()) {
      ...
      pool = pool.filter(
        (i) =>
          ...
          (i.data as Record<string, string>)?.username
            ?.toLowerCase()
            .includes(q) ||
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 z-10 bg-background border-b border-border">
                  <tr className="text-left text-muted-foreground">
                    ...
                    <th className="px-2 py-3 font-medium hidden md:table-cell">
                      Identifiant
                    </th>
                    <th className="px-2 py-3 font-medium hidden lg:table-cell">
                      Site
                    </th>
