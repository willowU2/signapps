...
function extractConfidence(content: string): number {
  ...
  if (content.includes('je ne sais pas') || content.includes('incertain') || content.includes('peut-etre')) return 0.55;
  if (content.includes('malheureusement') || content.includes('pas sur')) return 0.60;
...
const WELCOME_MSG: ChatMessage = {
  id: mkId(),
  role: 'assistant',
  content: 'Bonjour ! Je suis l\'assistant IA de SignApps. Je peux vous aider avec vos questions sur la plateforme. Comment puis-je vous aider ?',
  confidence: 1.0,
  timestamp: new Date(),
};

// ── Escalation Form ──
function EscalationForm({ initialDesc, onSubmit, onCancel }: {
  initialDesc: string;
  ...
  const [priority, setPriority] = useState<EscalationFormData['priority']>('medium');
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 p-3 bg-orange-50 rounded-lg border border-orange-200">
      <p className="text-xs font-semibold text-orange-800">Transfert vers un agent humain</p>
      <div className="space-y-1">
        <label htmlFor="escalation-title" className="text-xs font-medium">Titre du ticket *</label>
        <Input
          id="escalation-title"
          value={title}
      ...
      <div className="space-y-1">
        <label htmlFor="escalation-desc" className="text-xs font-medium">Description</label>
        <Textarea
          id="escalation-desc"
          value={description}
      ...
      <div className="space-y-1">
        <label htmlFor="escalation-priority" className="text-xs font-medium">Priorite</label>
        <select
          id="escalation-priority"
          value={priority}
          onChange={(e) => setPriority(e.target.value as EscalationFormData['priority'])}
...
function MessageBubble({ message }: { message: ChatMessage }) {
  ...
  const isLowConfidence = typeof message.confidence === 'number' && message.confidence < 0.7;
    ...
    <div className={cn('flex gap-2', isUser ? 'flex-row-reverse' : 'flex-row')}>
      ...
      <div className={cn('max-w-[80%] space-y-1', isUser ? 'items-end' : 'items-start', 'flex flex-col')}>
        <div className={cn(
          ...
          message.isEscalation && 'bg-orange-50 border border-orange-200 text-orange-900',
        )}>
          {message.content}
        </div>
        {!isUser && typeof message.confidence === 'number' && message.confidence < 1.0 && (
          <div className="flex items-center gap-1">
            <div className="h-1 w-16 bg-muted rounded-full overflow-hidden">
              <div
                className={cn(
                  'h-full rounded-full transition-all',
                  message.confidence >= 0.7 ? 'bg-green-500' : message.confidence >= 0.5 ? 'bg-orange-400' : 'bg-red-400'
                )}
            ...
            {isLowConfidence && <AlertTriangle className="w-3 h-3 text-orange-400" />}
          </div>
        )}
      </div>
    </div>
...
Si tu n'es pas sur de la reponse, dis-le clairement.`;
      ...
      const answerContent = (response as { data?: { answer?: string; response?: string }; answer?: string; response?: string })?.data?.answer
        ?? (response as { data?: { answer?: string; response?: string }; answer?: string; response?: string })?.data?.response
      ...
      if (confidence < 0.70) {
        const escalationHint: ChatMessage = {
          id: mkId(),
          role: 'assistant',
          content: 'Ma reponse n\'est pas certaine a 100%. Souhaitez-vous parler a un agent humain ?',
          confidence: 1.0,
          isEscalation: true,
          timestamp: new Date(),
        };
      ...
      const errMsg: ChatMessage = {
        id: mkId(),
        role: 'assistant',
        content: 'Desolee, le service IA est temporairement indisponible. Puis-je vous rediriger vers notre equipe de support ?',
        confidence: 1.0,
        isEscalation: true,
        timestamp: new Date(),
      };
  ...
  const handleTicketSubmit = (data: EscalationFormData) => {
    ...
    try {
      const existing = JSON.parse(localStorage.getItem('signapps-helpdesk-tickets') ?? '[]');
      localStorage.setItem('signapps-helpdesk-tickets', JSON.stringify([ticket, ...existing]));
    } catch { /* ignore */ }
            ...
            <CardHeader className="pb-2 bg-primary/5 rounded-t-lg">
              <div className="flex items-center justify-between">
                <CardTitle className="text-sm flex items-center gap-2">
                  ...
                  Assistant SignApps
                  <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" aria-label="En ligne" />
                </CardTitle>
                <button
            ...
            <CardContent className="p-0">
              ...
              {messages.length === 1 && (
                <div className="px-3 pb-2">
                  <p className="text-xs text-muted-foreground mb-1.5">Questions frequentes :</p>
