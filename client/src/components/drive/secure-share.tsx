    ...
    try {
      ...
      const message =
        (err as { response?: { data?: { message?: string } } })?.response?.data
          ?.message ?? "Impossible de générer le lien de partage";
      toast.error(message);
    } finally {
      setGenerating(false);
    }
          ...
          <div className="space-y-4 py-2">
            ...
            {node.name.toLowerCase().endsWith(".pdf") && (
              <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-3 text-xs text-amber-700 dark:text-amber-300">
                <strong>Note :</strong> Un filigrane &ldquo;CONFIDENTIEL&rdquo;
                sera ajouté à l&apos;aperçu PDF lors du partage externe.
              </div>
            )}
          </div>
        ) : (
              ...
              <Badge variant="secondary" className="gap-1">
                <Clock className="h-3 w-3" />
                {
                  EXPIRATION_OPTIONS.find(
                    (o) => o.value === generated.expiration,
                  )?.label
                }
              </Badge>
