...
const INITIAL_REPORT: HealthReport = {
  ...
  summary:
    "Le projet avance avec quelques risques sur le planning. Le budget est sous contrôle.",
  metrics: [
    {
      id: "1",
      name: "Planning",
    ...
    },
    {
      ...
      status: "red",
      trend: "up",
      comment: "1 risque élevé identifié",
    },
    {
      id: "5",
      name: "Satisfaction",
    ...
    <div className="space-y-4 p-4 border rounded-lg bg-background">
      ...
      <div className="text-sm text-muted-foreground bg-muted/30 rounded-lg p-3">
        <Input
          ...
          onChange={(e) =>
            setReport((p) => ({ ...p, summary: e.target.value }))
          }
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {report.metrics.map((m) => (
          <div
            key={m.id}
            className={cn(
              "border rounded-lg p-3 space-y-2 cursor-pointer hover:shadow-sm transition-shadow",
              ...
              >
                ...
                <div className="flex gap-2">
                  <select
                    value={m.status}
                    onChange={(e) =>
                      updateMetric(m.id, "status", e.target.value as RAGStatus)
                    }
                    className="flex-1 h-7 rounded border text-xs px-1"
                  >
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateMetric(
                        m.id,
                        "trend",
                        e.target.value as "up" | "down" | "stable",
                      )
                    }
                    className="flex-1 h-7 rounded border text-xs px-1"
                  >
                ...
                <Input
                  ...
                  value={m.comment}
                  onChange={(e) =>
                    updateMetric(m.id, "comment", e.target.value)
                  }
                  className="h-7 text-xs"
                />
                <Button
