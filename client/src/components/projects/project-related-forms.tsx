...
const DEMO_FORMS: RelatedForm[] = [
  {
    id: "f1",
    title: "Satisfaction sprint 3",
    type: "feedback",
    responses: 5,
    targetAudience: "Équipe projet",
    createdAt: "2026-03-20",
    status: "active",
  },
  {
    id: "f2",
    title: "Rétrospective Q1",
    type: "survey",
    responses: 8,
    targetAudience: "Toute l'équipe",
    createdAt: "2026-03-15",
    status: "active",
  },
  {
    id: "f3",
    title: "Checklist déploiement v2",
    type: "checklist",
    responses: 3,
...
export function ProjectRelatedForms({
  projectName = "Refonte Backend Auth",
  forms = DEMO_FORMS,
}: ProjectRelatedFormsProps) {
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        <Badge variant="secondary">
          {forms.filter((f) => f.status === "active").length} actifs
        </Badge>
      </CardHeader>
      <CardContent className="space-y-2">
        {displayed.map((form) => (
          ...
          >
            <span className={TYPE_CONFIG[form.type].class}>
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5">
                ...
                <span
                  className={`shrink-0 rounded-full px-1.5 py-0.5 text-[10px] ${STATUS_CONFIG[form.status].class}`}
                >
