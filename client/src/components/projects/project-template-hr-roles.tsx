...
export function ProjectTemplateHRRoles() {
  const [templates, setTemplates] =
    useState<TemplateWithRoles[]>(DEMO_TEMPLATES);
      ...
      prev.map((t) =>
        t.id === activeTemplate
          ? {
              ...
              roleAssignments: [
                ...t.roleAssignments,
                { id: `ra-${Date.now()}`, ...newRole, requiredSkills: [] },
              ],
            }
          : t,
      ),
  ...
  function applyTemplate() {
    ...
    toast.success(`Template appliqué`, {
      description: `${total} rôle(s) RH pré-assignés au nouveau projet.`,
    });
  }
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="secondary">
            {current.roleAssignments.length} rôles
          </Badge>
        </div>
      </CardHeader>
      <CardContent className="space-y-3">
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[10px] text-muted-foreground">
                  {role.department} · {role.allocation}% alloué
                </p>
              </div>
              <Button
                variant="ghost"
        ...
        <div className="flex gap-2">
          <Input
            ...
            onChange={(e) =>
              setNewRole((p) => ({ ...p, roleName: e.target.value }))
            }
          ...
          <Select
            ...
            onValueChange={(v) => setNewRole((p) => ({ ...p, department: v }))}
          >
          ...
          <Button size="sm" className="h-7 px-2" onClick={addRole}>
