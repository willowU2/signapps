  ...
  const startRecording = useCallback(async () => {
    try {
      ...
      if (SpeechRec) {
        ...
        rec.lang = 'fr-FR';
        rec.onresult = (e: any) => {
          currentTranscriptRef.current += ' ' + e.results[e.results.length - 1][0].transcript;
        };
        rec.start();
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center gap-3">
          ...
          {recording && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-mono text-red-500">{fmt(duration)}</span>
            </div>
          )}
        </div>
        ...
        <ScrollArea className="h-72">
          {notes.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-8">No voice notes yet. Record one to get started.</p>
          ) : (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Button
                        ...
                        disabled={!note.blob}
                      >
                        {note.playing ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}
                      </Button>
                    ...
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => remove(note.id)}>
