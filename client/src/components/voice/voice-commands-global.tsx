...
const STATIC_COMMANDS: Array<{
  ...
  {
    patterns: ["créer document", "create doc", "nouveau document"],
    action: "Create document",
    fn: (r) => r.push("/docs/editor"),
  },
  {
    patterns: ["planifier réunion", "schedule meeting", "nouveau événement"],
    action: "Schedule meeting",
    fn: (r) => r.push("/calendar?new=true"),
  },
  {
    patterns: [
      "tableau de bord",
    ...
    action: "Dashboard",
    fn: (r) => r.push("/dashboard"),
  },
  {
    patterns: ["paramètres", "settings", "open settings"],
    action: "Settings",
    fn: (r) => r.push("/settings"),
  },
  {
    patterns: ["retour", "go back", "previous page"],
    action: "Go back",
    fn: (r) => r.back(),
  },
];
...
const EMAIL_PATTERN =
  /(?:envoyer?\s+mail\s+[àa]|compose\s+email\s+to|send\s+email\s+to)\s+(.+)/i;
// "create task: X" / "nouvelle tâche X"
const TASK_PATTERN =
  /(?:cr[ée]+r?\s+(?:une?\s+)?t[aâ]che|nouvelle\s+t[aâ]che|create\s+task)[:\s]+(.+)/i;
...
export function VoiceCommandsGlobal({
  compact = false,
}: VoiceCommandsGlobalProps) {
  ...
  const processCommand = useCallback(
    (text: string) => {
      ...
      const staticMatch = STATIC_COMMANDS.find((cmd) =>
        cmd.patterns.some((p) => lower.includes(p)),
      );
      if (staticMatch) {
