...
function parseTasksFromSpeech(text: string): string[] {
  ...
  return sentences
    ...
    .map((s) => {
      const cleaned = s
        .replace(
          /^(je dois|il faut|please|doit|need to|we need to|faut)\s+/i,
          "",
        )
        .trim();
  ...
  const addManual = () => {
    ...
    setTasks((prev) => [
      ...prev,
      {
        ...
        title: newTitle,
        priority: detectPriority(newTitle),
        done: false,
        source: "manual",
      },
    ]);
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <p className="text-sm text-muted-foreground">
          Speak your tasks or instructions — they'll be automatically extracted
          and added to your task list.
        </p>
        ...
        <div className="flex gap-2">
          ...
          {recording && (
            <Badge className="bg-red-500 animate-pulse self-center">
              Listening...
            </Badge>
          )}
        </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <p className="text-xs text-muted-foreground">
                {tasks.filter((t) => !t.done).length} pending / {tasks.length}{" "}
                total
              </p>
              <Button
                variant="ghost"
            ...
            {tasks.map((task) => (
              <div
                key={task.id}
                className={`flex items-center gap-3 p-2.5 rounded-lg border ${task.done ? "opacity-50" : ""}`}
              >
                <button
