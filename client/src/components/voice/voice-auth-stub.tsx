...
export function VoiceAuthStub() {
  ...
  const handleEnroll = async () => {
    ...
    for (let i = 0; i < ENROLL_REPEATS; i++) {
      setMessage(
        `Dites la phrase (${i + 1}/${ENROLL_REPEATS}) : « ${ENROLL_PHRASE} »`,
      );
      try {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (avgSimilarity >= 0.6) {
        setPhase("success");
        setMessage(
          `Authentification réussie (score : ${Math.round(avgSimilarity * 100)}%)`,
        );
      } else {
        setPhase("failed");
        setMessage(
          `Authentification échouée (score : ${Math.round(avgSimilarity * 100)}%). Réessayez.`,
        );
      }
    } catch (err) {
      setPhase("failed");
