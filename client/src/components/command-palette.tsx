...
export function CommandPalette() {
  ...
  const getBlockIcon = (type: string) => {
    switch (type) {
      ...
      case "folder":
        return <HardDrive className="mr-2 h-4 w-4 text-slate-500" />;
      case "user":
        return <User className="mr-2 h-4 w-4 text-green-500" />;
      case "task":
        return <ListTodo className="mr-2 h-4 w-4 text-orange-500" />;
      case "event":
  ...
  const getBlockRoute = (block: any) => {
    switch (block.type) {
      case "file":
        return `/storage?preview=${block.data.id || block.data.key}`;
      case "folder":
        return `/storage?folder=${block.data.id}`;
      case "user":
        return `/workforce?user=${block.data.id}`;
      case "task":
        return `/tasks`;
      case "event":
        return `/scheduler`;
    ...
    <CommandDialog open={open} onOpenChange={setOpen}>
      <CommandInput
        ...
        value={
          search +
          (interimSearch
            ? (search && !search.endsWith(" ") ? " " : "") + interimSearch
            : "")
        }
        onValueChange={(v) => {
          setSearch(v);
      ...
      <CommandList className="custom-scrollbar">
          ...
          <CommandGroup heading="Contexte actuel (Actions IA)">
            {pathname.includes("/docs") && (
              <>
                <CommandItem
                  onSelect={() =>
                    dispatchAppEvent("app:ai-action", { action: "summarize" })
                  }
                >
                ...
                <CommandItem
                  onSelect={() =>
                    dispatchAppEvent("app:ai-action", { action: "draft" })
                  }
                >
                ...
                <CommandItem
                  onSelect={() =>
                    dispatchAppEvent("app:editor-action", {
                      action: "format-fix",
                    })
                  }
                >
            ...
            {pathname.includes("/mail") && (
              <>
                <CommandItem
                  onSelect={() =>
                    dispatchAppEvent("app:ai-action", { action: "draft-reply" })
                  }
                >
                ...
                <CommandItem
                  onSelect={() =>
                    dispatchAppEvent("app:ai-action", {
        ...
        {(pathname.includes("/docs") ||
          pathname.includes("/mail") ||
          pathname.includes("/slides")) && <CommandSeparator />}
            ...
            <CommandGroup heading="Résultats de recherche (Fichiers, Utilisateurs, Tâches, Événements)">
              {blocks.map((block: any, idx: number) => (
                <CommandItem
                  ...
                  onSelect={() =>
                    runCommand(() => router.push(getBlockRoute(block)))
                  }
                >
                  {getBlockIcon(block.type)}
                  <span>{getBlockTitle(block)}</span>
                  {block.data.department && (
                    <span className="ml-2 pl-2 border-l text-xs text-muted-foreground">
                      {block.data.department}
                    </span>
                  )}
                  {block.data.email && (
        ...
        <CommandGroup heading="Système">
          ...
          <CommandItem
            onSelect={() => runCommand(() => router.push("/containers"))}
          >
        ...
        <CommandGroup heading="Fonctionnalités">
          ...
          <CommandItem
            onSelect={() => runCommand(() => router.push("/storage/trash"))}
          >
            <Trash2 className="mr-2 h-4 w-4" />
            <span>Corbeille</span>
          </CommandItem>
          <CommandItem
            onSelect={() => runCommand(() => router.push("/storage/shares"))}
          >
          ...
          <CommandItem
            onSelect={() =>
              runCommand(() => router.push("/search?mode=semantic"))
            }
          >
        ...
        <CommandGroup heading="Réseau et infrastructure">
          <CommandItem
            onSelect={() => runCommand(() => router.push("/routes"))}
          >
