...
export function FocusOrderValidator() {
  ...
  const runValidation = useCallback(() => {
    ...
    const items: FocusItem[] = elements.slice(0, 50).map((el, i) => {
      ...
      if (tabIdx > 0) issues.push(`tabindex=${tabIdx} (positive value disrupts order)`);
  ...
  return (
    <>
      <Button variant="outline" size="sm" onClick={runValidation} className="gap-1.5">
        ...
        <div className="fixed inset-0 z-[9998] bg-black/40 flex items-center justify-center p-4">
          <Card className="w-full max-w-2xl max-h-[80vh] flex flex-col">
            <CardHeader className="flex flex-row items-center justify-between pb-3">
              <div>
                <CardTitle className="text-base">Focus Order Validation</CardTitle>
                <p className="text-sm text-muted-foreground mt-0.5">
                  {results.length} focusable elements — {' '}
                  {totalIssues === 0 ? (
                    <span className="text-green-600">No issues found</span>
                  ) : (
                    <span className="text-amber-600">{totalIssues} issue{totalIssues !== 1 ? 's' : ''}</span>
                  )}
                </p>
              </div>
                  ...
                  {results.map((item) => (
                    <div
                      ...
                      className={`flex items-start gap-3 p-2.5 rounded-lg border text-sm ${
                        item.issues.length > 0 ? 'border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950/30' : 'border-border bg-muted/20'
                      }`}
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <Badge variant="outline" className="text-xs font-mono px-1.5 py-0">{item.tag}</Badge>
                          <span className="truncate">{item.text}</span>
                        </div>
                        {item.issues.map((issue, i) => (
                          <p key={i} className="text-xs text-amber-600 dark:text-amber-400 mt-0.5 flex items-center gap-1">
