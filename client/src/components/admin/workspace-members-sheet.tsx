...
export function WorkspaceMembersSheet({
  ...
  const handleUpdateRole = async (userId: string, newRole: WorkspaceRole) => {
    if (!workspace) return;
    try {
      await workspacesApi.updateMemberRole(workspace.id, userId, {
        role: newRole,
      });
            ...
            <div className="flex items-start gap-4 mb-1">
              ...
              <div className="space-y-1">
                ...
                <DialogDescription className="text-[14.5px] font-medium text-muted-foreground leading-snug text-left">
                  {workspace?.name} - Add or remove members and manage their
                  roles.
                </DialogDescription>
              </div>
            </div>
                          ...
                          <TableCell className="py-3 px-4 w-[140px]">
                            {isOwner ? (
                              <Badge
                                variant="outline"
                                className={cn(
                                  "px-2.5 py-1 whitespace-nowrap bg-background shadow-sm border-border/80",
                                  roleConfig.color,
                                )}
                              >
