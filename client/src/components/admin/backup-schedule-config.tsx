...
function parseCron(cron: string): { min: string; hour: string; dom: string; month: string; dow: string } {
...
export function BackupScheduleConfig() {
  const [config, setConfig] = useState<BackupConfig>(() => {
    ...
    try {
      return JSON.parse(localStorage.getItem(STORAGE_KEY) || '') as BackupConfig;
  ...
  const description = useMemo(() => {
    try { return describeCron(customCron); }
    ...
    <Card>
      ...
      <CardContent className="space-y-5">
        ...
        <div className="space-y-3">
          <Label className="text-xs text-muted-foreground uppercase tracking-wide block">Expression builder</Label>
          <div className="grid grid-cols-5 gap-2 text-center text-xs">
            {(['min', 'hour', 'dom', 'month', 'dow'] as const).map((field, fi) => {
              ...
              return (
                <div key={field} className="space-y-1">
                  ...
                  <Input
                    value={val}
                    onChange={(e) => {
                      const parts = [cronParts.min, cronParts.hour, cronParts.dom, cronParts.month, cronParts.dow];
          ...
          <div className="flex items-center gap-2 p-2 bg-muted/30 rounded text-xs">
            ...
            <span className="font-mono text-muted-foreground">{customCron}</span>
