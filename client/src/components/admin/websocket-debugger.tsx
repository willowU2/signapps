...
import { Wifi, WifiOff, Send, Trash2, ArrowDownLeft, ArrowUpRight } from 'lucide-react';
    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex gap-2">
          ...
          >
            {connected ? <WifiOff className="h-4 w-4" /> : <Wifi className="h-4 w-4" />}
          ...
          {PRESET_URLS.map(u => (
            <button
              ...
              className={`text-xs px-2 py-1 rounded border transition-colors ${
                url === u ? 'bg-primary text-primary-foreground' : 'bg-muted hover:bg-accent'
              ...
              messages.map(msg => (
                <div
                  ...
                  className={`flex items-start gap-2 p-2 rounded text-xs font-mono ${
                    msg.direction === 'in' ? 'bg-blue-50 dark:bg-blue-950/30' : 'bg-green-50 dark:bg-green-950/30'
                  }`}
                >
                  {msg.direction === 'in'
                  ...
                  <span className="text-muted-foreground shrink-0">{msg.timestamp.toLocaleTimeString()}</span>
                </div>
              ))
            )}
          </div>
        ...
        <div className="flex gap-2">
          <Textarea
            ...
            disabled={!connected}
          />
          <Button onClick={sendMessage} disabled={!connected} className="gap-1.5 shrink-0 self-end">
