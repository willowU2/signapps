...
export function PermissionsEditor({ value, onChange, disabled }: PermissionsEditorProps) {
    ...
    const toggleAllActions = (resource: ResourceKey, actions: readonly string[]) => {
        if (disabled) return
        ...
        <div className="space-y-2">
            ...
            <div className="border rounded-lg divide-y">
                {AVAILABLE_RESOURCES.map((resource) => {
                    const enabledCount = getEnabledCount(resource.key, resource.actions.length)
                                ...
                                <div className="px-4 pb-3 pt-1">
                                    <div className="flex items-center gap-2 mb-3">
                                        <Checkbox
                                            id={`${resource.key}-all`}
                                            checked={allEnabled}
                                            onCheckedChange={() => toggleAllActions(resource.key, resource.actions)}
                                            disabled={disabled}
                                        />
                                        <label
                                        ...
                                        {resource.actions.map((action) => (
                                            ...
                                            >
                                                <Checkbox
                                                    ...
                                                    onCheckedChange={() => toggleAction(resource.key, action)}
                                                    disabled={disabled}
                                                />
                                                <label
