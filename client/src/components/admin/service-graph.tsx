...
function ServiceNode({ data }: { data: ServiceNodeData }) {
  return (
    <div
      className={`rounded-lg border-2 px-3 py-2 shadow-sm min-w-[120px] ${STATUS_COLORS[data.type]}`}
    >
      ...
      <div className="mt-1 flex items-center justify-between gap-2">
        ...
        <span
          className={`rounded-full px-1.5 py-0.5 text-xs font-medium ${STATUS_BADGE[data.type]}`}
        >
          {data.type}
        </span>
      </div>
...
const INITIAL_NODES: Node<ServiceNodeData>[] = [
  // Frontend
  {
    ...
    type: "service",
    position: { x: 360, y: 0 },
    data: { label: "Next.js Client", port: 3000, type: "frontend" },
  },
  // Gateway/Proxy
  {
    id: "proxy",
    ...
    data: { label: "Proxy", port: 3003, type: "infra" },
  },
  // Core services row 1
  {
    id: "identity",
    type: "service",
    position: { x: 0, y: 260 },
    data: { label: "Identity", port: 3001, type: "backend" },
  },
  {
    id: "storage",
    type: "service",
    position: { x: 160, y: 260 },
    data: { label: "Storage", port: 3004, type: "backend" },
  },
  {
    id: "ai",
    type: "service",
    position: { x: 320, y: 260 },
    data: { label: "AI", port: 3005, type: "backend" },
  },
  {
    id: "docs",
    type: "service",
    position: { x: 480, y: 260 },
    data: { label: "Docs", port: 3010, type: "backend" },
  },
  {
    id: "calendar",
    type: "service",
    position: { x: 640, y: 260 },
    data: { label: "Calendar", port: 3011, type: "backend" },
  },
  // Core services row 2
  {
    id: "mail",
    type: "service",
    position: { x: 0, y: 400 },
    data: { label: "Mail", port: 3012, type: "backend" },
  },
  {
    id: "scheduler",
    type: "service",
    position: { x: 160, y: 400 },
    data: { label: "Scheduler", port: 3007, type: "backend" },
  },
  {
    id: "office",
    type: "service",
    position: { x: 320, y: 400 },
    data: { label: "Office", port: 3018, type: "backend" },
  },
  {
    id: "metrics",
    type: "service",
    position: { x: 480, y: 400 },
    data: { label: "Metrics", port: 3008, type: "backend" },
  },
  {
    id: "collab",
    type: "service",
    position: { x: 640, y: 400 },
    data: { label: "Collab", port: 3013, type: "backend" },
  },
  // Infrastructure
  {
    id: "postgres",
    ...
    data: { label: "PostgreSQL", port: 5432, type: "infra" },
  },
  {
    id: "redis",
    type: "service",
    position: { x: 360, y: 540 },
    data: { label: "Redis", port: 6379, type: "infra" },
  },
];
...
const INITIAL_EDGES: Edge[] = [
  ...
  // Services → DB
  {
    ...
    source: "identity",
    target: "postgres",
    style: { stroke: "#9333ea", strokeDasharray: "4" },
  },
  {
    id: "st-pg",
    source: "storage",
  ...
  // Services → Redis
  {
    ...
    source: "identity",
    target: "redis",
    style: { stroke: "#ef4444", strokeDasharray: "4" },
  },
  {
    id: "sc-rd",
    source: "scheduler",
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-gray-400">
        Nodes are draggable. Solid lines = HTTP calls. Dashed lines = DB/cache
        connections.
      </p>
    </div>
  );
