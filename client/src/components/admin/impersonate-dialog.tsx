...
export function ImpersonateDialog({
  ...
  const handleConfirm = () => {
    // SECURITY NOTE: Client-side impersonation only (UI preview).
    // Backend does NOT validate impersonation. For production use,
    // server-side impersonation should issue a scoped JWT and log
    // all impersonated actions to the audit trail.
    // Store the impersonation context in sessionStorage (clears on tab close)
    sessionStorage.setItem(IMPERSONATE_KEY, user.id);
    sessionStorage.setItem(
      IMPERSONATE_BANNER_KEY,
      JSON.stringify({
        userId: user.id,
        ...
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="space-y-3">
            ...
            <p className="text-amber-600 text-sm font-medium">
              Your actions may affect real data. A yellow banner will remind you
              that you are impersonating. Close the tab or click "Exit" in the
              banner to stop.
            </p>
          </AlertDialogDescription>
        </AlertDialogHeader>
