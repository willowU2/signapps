...
import { storageSettingsApi, IndexingRule, UpsertIndexingRule } from "@/lib/api/storageSettingsApi";
...
export function AiIndexingSettings() {
  ...
  const handleSave = async () => {
    try {
      ...
      if (typeof parsedTypes === 'string') {
        parsedTypes = (parsedTypes as string).split(',').map(s => s.trim()).filter(s => s.length > 0);
  ...
  const handleFileTypesChange = (value: string) => {
    ...
    setEditForm({...editForm, file_types_allowed: list.length > 0 ? list : null});
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between rounded-md border p-4 shadow-sm">
        <div className="space-y-0.5">
          <h3 className="text-base font-medium leading-none">Index all files by default</h3>
          <p className="text-sm text-muted-foreground">
            Automatically sends all new file uploads to the AI vector database unless excluded by a specific rule below.
          </p>
        </div>
        <Switch 
            ...
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={6} className="text-center h-24 text-muted-foreground">
                  No indexing rules configured. Your AI will lack context of stored files.
                </TableCell>
              </TableRow>
            ) : null}
              ...
              <TableRow className="bg-muted/30">
                ...
                <TableCell>
                  <Input 
                    ...
                    onChange={(e) => setEditForm({...editForm, folder_path: e.target.value})}
                    className="h-8 font-mono text-sm"
                  />
                </TableCell>
                 <TableCell>
                  <Switch 
                    ...
                    onCheckedChange={(c) => setEditForm({...editForm, include_subfolders: c})} 
                  />
                </TableCell>
                <TableCell>
                  <Input 
            ...
            {rules.map((rule) => 
              editingId === rule.id ? (
                ...
                <TableRow key={rule.id} className={!rule.is_active ? "opacity-60" : ""}>
                  ...
                  <TableCell className="font-mono text-sm">{rule.bucket}</TableCell>
                  <TableCell>
                  ...
                  <TableCell>{rule.include_subfolders ? "Yes" : "No"}</TableCell>
                  <TableCell className="font-mono text-sm">{rule.file_types_allowed?.join(", ") || "All"}</TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
