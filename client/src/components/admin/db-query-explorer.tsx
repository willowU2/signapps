...
export function DbQueryExplorer() {
  ...
  const run = async () => {
    ...
    if (!trimmed.startsWith('SELECT') && !trimmed.startsWith('SHOW') && !trimmed.startsWith('EXPLAIN')) {
      toast.error('Only SELECT, SHOW, and EXPLAIN queries are allowed in this explorer');
      return;
    }
    setLoading(true);
    setError('');
    ...
    try {
      ...
      const res = await client.post<QueryResult>('/admin/db/query', { sql: query });
      ...
      <CardContent className="space-y-3">
        <div className="flex items-start gap-2 p-2.5 rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800">
          ...
          <p className="text-xs text-amber-700 dark:text-amber-300">
            Only SELECT, SHOW, and EXPLAIN queries are permitted. All writes are blocked.
          </p>
        </div>
        ...
        <div className="space-y-2">
          ...
          />
          <Button onClick={run} disabled={loading || !query.trim()} className="gap-1.5">
        ...
        {result && (
          <div className="space-y-2">
            <div className="flex items-center gap-3 text-xs text-muted-foreground">
              ...
              <span className="flex items-center gap-1"><Clock className="h-3 w-3" />{result.duration}ms</span>
            </div>
                  ...
                  <tbody>
                    {result.rows.map((row, i) => (
                      <tr key={i} className="border-b hover:bg-muted/50">
                        {result.columns.map(col => (
                          <td key={col} className="px-3 py-2 font-mono whitespace-nowrap max-w-[200px] truncate">
