...
const TRIGGER_ACTIONS: { value: TriggerAction; label: string; placeholder: string; icon: React.ReactNode }[] = [
  { value: 'send_reminder', label: 'Envoyer un rappel', placeholder: 'Message du rappel', icon: <Bell className="h-3.5 w-3.5" /> },
  { value: 'create_task', label: 'Créer une tâche suivi', placeholder: 'Titre de la tâche', icon: <CheckSquare className="h-3.5 w-3.5" /> },
  { value: 'send_email', label: 'Envoyer un email', placeholder: 'destinataire@exemple.com', icon: <Bell className="h-3.5 w-3.5" /> },
];
...
function TriggerEditor({ trigger, onChange, onDelete }: TriggerEditorProps) {
  const update = (patch: Partial<CalendarTrigger>) => onChange({ ...trigger, ...patch });
    ...
    <Card>
      ...
      <CardContent className="space-y-3 pt-0">
        ...
        <div className="space-y-1">
          <Label className="text-xs text-muted-foreground">Filtre événement (titre contient)</Label>
          <Input
        ...
        <div className="flex items-end gap-2">
          <div className="flex-1 space-y-1">
            <Label className="text-xs text-muted-foreground">Déclenchement</Label>
            <Select
          ...
          {showOffset && (
            <div className="w-24 space-y-1">
              <Label className="text-xs text-muted-foreground">Minutes avant</Label>
              <Input
                type="number"
                min={0}
                max={120}
                value={trigger.triggerOffsetMin}
                onChange={(e) => update({ triggerOffsetMin: Number(e.target.value) })}
                className="h-8 text-xs"
              />
            </div>
          )}
        ...
        <div className="flex items-end gap-2">
          <div className="flex-1 space-y-1">
            ...
            <Select
              ...
              onValueChange={(v) => update({ action: v as TriggerAction, actionPayload: '' })}
            >
              <SelectTrigger className="h-8 text-xs">
                <SelectValue />
              </SelectTrigger>
