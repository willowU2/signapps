...
import { Zap, Play, Pause, Trash2, Plus, Clock, CheckCircle } from 'lucide-react';
...
export function WorkflowAutomationsList() {
  const { automations, templates, toggle, remove, addFromTemplate } = useWorkflowAutomations();
  ...
  return (
    <div className="space-y-4">
      ...
      {automations.length === 0 && (
        <div>
          <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2">Modèles rapides</p>
          <div className="grid grid-cols-2 gap-2">
            {templates.slice(0, 6).map((t, i) => (
              <Card key={i} className="cursor-pointer hover:border-primary/50 transition-colors" onClick={() => handleTemplate(i)}>
                <CardContent className="p-3">
                  <div className="flex items-start gap-2">
                    <Zap className="w-3.5 h-3.5 text-yellow-500 mt-0.5 shrink-0" />
                    <div>
                      <p className="text-xs font-medium">{t.name}</p>
                      <p className="text-xs text-muted-foreground">{TRIGGER_LABELS[t.trigger.type]}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
      ...
      {automations.length > 0 && (
        <ScrollArea className="max-h-[500px]">
          <div className="space-y-2 pr-1">
            {automations.map(a => (
              <Card key={a.id} className={`transition-all ${a.enabled ? 'border-primary/30' : ''}`}>
                <CardContent className="p-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {a.enabled && <Badge className="h-4 text-xs px-1 bg-green-100 text-green-700">Actif</Badge>}
                      </div>
                      <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                        <Badge variant="outline" className="text-xs h-4 px-1">{TRIGGER_LABELS[a.trigger.type] || a.trigger.type}</Badge>
                        ...
                        {a.actions.length > 2 && (
                          <span className="text-xs text-muted-foreground">+{a.actions.length - 2}</span>
                        )}
                      </div>
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          {a.lastRunAt && (
                            <span className="text-xs text-muted-foreground flex items-center gap-0.5">
                              ...
                              {formatDistanceToNow(new Date(a.lastRunAt), { addSuffix: true, locale: fr })}
                            </span>
                          )}
                          {a.runCount > 0 && (
                            <span className="text-xs text-muted-foreground flex items-center gap-0.5">
                          ...
                          {a.schedule && (
                            <Badge variant="outline" className="text-xs h-4 px-1 font-mono">{a.schedule}</Badge>
                          )}
                        </div>
                      )}
                    </div>
                    <Button
                      ...
                      onClick={() => { remove(a.id); toast.success('Automation supprimée'); }}
                    >
