...
export function ApprovalChainPanel({
  ...
  const { requests, pending, loading, submit, decide, refresh } =
    useApprovalChain(entityId || undefined);
  ...
  const handleSubmit = async () => {
    ...
    const approverIds = newApprovers
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
    if (approverIds.length === 0) {
      toast.error("Ajoutez au moins un approbateur");
      return;
    }
    try {
        ...
        <CardContent className="space-y-3">
          <div>
            ...
            <Input
              ...
              placeholder="Ex: Approbation contrat client ABC"
          ...
          <div>
            <Label className="text-xs">
              Approbateurs (IDs séparés par virgules) *
            </Label>
            <Input
              value={newApprovers}
        ...
        <Card className="border-yellow-200">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm flex items-center gap-1.5">
              ...
              <Badge className="bg-yellow-100 text-yellow-700 text-xs h-4 px-1">
                {pending.length}
              </Badge>
            </CardTitle>
          </CardHeader>
            ...
            {requests.length === 0 ? (
              ...
              requests.map((r) => {
                ...
                return (
                  ...
                  >
                    <Icon
                      className={`w-3.5 h-3.5 mt-0.5 shrink-0 ${r.status === "approved" || r.status === "executed" ? "text-green-600" : r.status === "rejected" ? "text-red-500" : "text-yellow-600"}`}
                    />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium truncate">{r.title}</p>
                      <div className="flex items-center gap-2 mt-0.5">
                        <Badge className={`text-xs h-4 px-1 ${cfg.class}`}>
                          {cfg.label}
                        </Badge>
