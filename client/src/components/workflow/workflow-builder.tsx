...
} from "@/hooks/use-workflow-automations";
...
export function WorkflowBuilder({ onSave }: { onSave?: (id: string) => void }) {
  ...
  const [actions, setActions] = useState<
    Array<{ type: ActionType; params: Record<string, string> }>
  >([]);
  ...
  const handleSave = () => {
    if (!name.trim()) {
      toast.error("Donnez un nom au workflow");
      return;
    }
    if (actions.length === 0) {
        ...
        ? [
            {
              ...
              operator:
                branchCondition.operator as WorkflowCondition["operator"],
              value: branchCondition.value,
            },
          ]
        : conditions.map((c) => ({
            field: c.field,
            operator: c.operator as WorkflowCondition["operator"],
            value: c.value,
          })),
    ...
    <div className="space-y-4 max-w-2xl">
      ...
      <div className="flex gap-1 border-b pb-3">
        {steps.map((s, i) => (
          ...
          >
            <span className="w-4 h-4 rounded-full bg-muted text-muted-foreground flex items-center justify-center text-xs font-bold">
              {i + 1}
            </span>
            {s.label}
          </Button>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center gap-2">
              <Switch
                checked={hasBranch}
                onCheckedChange={setHasBranch}
                id="branch-toggle"
              />
              <Label htmlFor="branch-toggle" className="text-sm cursor-pointer">
                Branchement conditionnel (si montant &gt; X → approuver sinon
                rejeter)
              </Label>
            </div>
            ...
            {hasBranch ? (
              <div className="space-y-2 p-3 bg-muted/30 rounded-lg">
                ...
                <div className="flex gap-2">
                  <Input
                    placeholder="Champ (ex: amount)"
                    value={branchCondition.field}
                    onChange={(e) =>
                      setBranchCondition((b) => ({
                        ...b,
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setBranchCondition((b) => ({ ...b, operator: v }))
                    }
                  >
            ...
            <div className="space-y-2">
              {actions.map((a, i) => (
                ...
                >
                  ...
                  <Select
                    value={a.type}
                    onValueChange={(v) => updateAction(i, v as ActionType)}
                  >
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder="Ex: Email client → Tâche de suivi"
                className="mt-1"
              />
            </div>
            <div className="bg-muted/30 rounded-lg p-3 space-y-1.5">
              ...
              <div className="flex items-center gap-2">
                <Badge variant="outline" className="text-xs">
                  {TRIGGERS.find((t) => t.value === triggerType)?.label}
                </Badge>
                <span className="text-xs text-muted-foreground">→</span>
                <Badge variant="outline" className="text-xs">
                  {conditions.length + (hasBranch ? 1 : 0)} condition(s)
                </Badge>
                <span className="text-xs text-muted-foreground">→</span>
                <Badge variant="outline" className="text-xs">
