...
function getStatusIcon(status: JobStatus, className?: string) {
  switch (status) {
    ...
    default:
      return (
        <FileText className={cn("h-4 w-4 text-muted-foreground", className)} />
      );
  }
}
...
function JobItem({
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3 mt-1">
          ...
          {job.status !== "processing" && (
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <Badge
                variant="secondary"
                className={cn("text-xs", JOB_STATUS_COLORS[job.status])}
              >
      ...
      <Badge
        variant="outline"
        className={cn("text-xs", JOB_PRIORITY_COLORS[job.priority])}
      >
...
export function ConversionJobList({
  className,
  onJobSelect,
}: ConversionJobListProps) {
  const {
    jobs,
    selectedJob,
    isLoading,
    ...
    <div className={cn("flex flex-col h-full", className)}>
      ...
      <div className="flex items-center gap-2 p-3 border-b">
        ...
        <Select
          ...
          onValueChange={(value) =>
            setStatusFilter(value === "all" ? null : (value as JobStatus))
          }
        >
      ...
      {stats && (
        <div className="flex items-center justify-between px-4 py-2 border-t bg-muted/30 text-xs text-muted-foreground">
          ...
          <span>Temps moyen: {Math.round(stats.averageProcessingTime)}s</span>
        </div>
      )}
    </div>
  );
