...
export function AiChatWidget() {
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: "assistant",
      content:
        "Hello! I am your AI assistant for this document. How can I help you today?",
    },
  ]);
  ...
  const config =
    routes.docs?.providerId && routes.docs?.modelId
      ? routes.docs
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    setTimeout(async () => {
      const systemPrompt =
        "You are a helpful AI assistant integrated into a document editor. Be concise and professional.";
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar">
        {messages.map((msg) => (
          ...
          >
            ...
            <div
              className={`p-3 rounded-2xl text-sm leading-relaxed overflow-hidden break-words ${
                msg.role === "user"
