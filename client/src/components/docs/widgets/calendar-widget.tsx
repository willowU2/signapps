      ...
      try {
        ...
        setEvents(
          data.map((e: any) => ({
            ...
            time: new Date(e.start_time || e.start).toLocaleTimeString(
              "fr-FR",
        ...
        {loading ? (
          ...
          <div className="relative border-l border-border dark:border-gray-700/50 pl-4 py-2 space-y-6">
            ...
            {events.map((event) => (
              <div
                key={event.id}
                className="relative group p-3 bg-background dark:bg-gray-800/80 rounded-xl shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] border border-gray-100 dark:border-gray-700/50 hover:border-indigo-200 dark:hover:border-indigo-500/30 transition-all hover:shadow-md cursor-pointer"
              >
