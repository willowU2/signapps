...
function computeDiff(oldLines: string[], newLines: string[]): DiffLine[] {
  ...
  while (i < oldLines.length || j < newLines.length) {
    if (i < oldLines.length && j < newLines.length && oldLines[i] === newLines[j]) {
      ...
      result.push({ kind: 'add', text: newLines[j], oldLineNo: null, newLineNo });
      newLineNo++;
      j++;
    } else {
      result.push({ kind: 'remove', text: oldLines[i], oldLineNo, newLineNo: null });
      oldLineNo++;
      i++;
    }
  }
    ...
    <div className={cn('flex flex-col h-full bg-background border rounded-lg overflow-hidden', className)}>
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b bg-muted/30 shrink-0">
        <div className="flex items-center gap-3 text-sm">
          <span className="text-muted-foreground font-medium truncate max-w-[180px]" title={oldLabel}>
            {oldLabel}
          </span>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {addCount > 0 && (
            <Badge variant="secondary" className="bg-green-950 text-green-400 gap-1 text-xs">
              <Plus className="h-3 w-3" />
              {addCount}
            </Badge>
          )}
          {removeCount > 0 && (
            <Badge variant="secondary" className="bg-red-950 text-red-400 gap-1 text-xs">
