  ...
  const getState = useCallback(() => {
    if (!isOnline) {
      return {
        ...
        label: "Hors ligne",
        description:
          "Les modifications sont sauvegardees localement et seront synchronisees automatiquement.",
        icon: WifiOff,
        pulse: true,
      };
    }
    ...
    if (connectionStatus === "disconnected") {
      ...
      return {
        ...
        label: "Mode local",
        description:
          "Collaboration desactivee. Les modifications sont sauvegardees localement.",
        icon: CloudOff,
        pulse: false,
      };
    }
      ...
      <PopoverTrigger asChild>
        ...
        >
          <div className="relative">
            ...
            {state.pulse && (
              <div
                className={`absolute inset-0 w-2 h-2 rounded-full ${state.pulseColor} animate-ping`}
              />
            )}
          </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1.5 pt-1 border-t">
            <div className="flex items-center justify-between text-xs">
              ...
              <span
                className={`font-medium ${isOnline ? "text-emerald-600" : "text-red-500"}`}
              >
          ...
          {!isOnline && (
            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-md px-3 py-2 text-xs text-amber-700 dark:text-amber-300">
              Vos modifications sont sauvegardees automatiquement en local.
              Elles seront synchronisees des que la connexion sera retablie.
            </div>
          )}
        </div>
      </PopoverContent>
