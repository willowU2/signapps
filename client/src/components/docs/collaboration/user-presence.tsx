...
export function UserPresenceList({ maxVisible = 5 }: UserPresenceListProps) {
  ...
  const clearInactiveUsers = usePresenceStore(
    (state) => state.clearInactiveUsers,
  );
...
export function ConnectionStatus({ className }: ConnectionStatusProps) {
  ...
  const statusConfig = {
    ...
    disconnected: {
      color: "bg-red-500",
      text:
        pendingChanges > 0 ? `Offline (${pendingChanges} pending)` : "Offline",
      pulse: false,
    },
    reconnecting: {
      color: "bg-yellow-500",
...
export function RemoteCursors() {
  ...
  const usersWithCursors = Array.from(users.values()).filter(
    (u) => u.isOnline && u.userId !== currentUserId && u.cursor,
  );
