    ...
    return (
        <div className="flex flex-col h-full bg-slate-50/30 dark:bg-black/10">
            <div className="px-4 py-3 flex items-center gap-2 border-b border-border/50 dark:border-gray-800/50">
                ...
                <h3 className="text-xs font-semibold text-muted-foreground dark:text-gray-300 uppercase tracking-wider">Outline</h3>
            </div>
            <div className="flex-1 overflow-y-auto p-2 no-scrollbar space-y-0.5">
                {items.map((item) => (
                    <button
                        ...
                        className={`w-full flex items-center gap-2 px-2 py-1.5 rounded-lg text-left transition-colors hover:bg-muted dark:hover:bg-gray-800 ${item.level === 1 ? 'ml-0 font-medium text-gray-800 dark:text-gray-200' :
                            item.level === 2 ? 'ml-3 text-muted-foreground dark:text-gray-400 text-sm' :
