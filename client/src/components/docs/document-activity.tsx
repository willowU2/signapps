...
export function logDocActivity(docId: string, action: ActivityEntry['action'], user: string = 'admin', details?: string) {
  try {
    const all = JSON.parse(localStorage.getItem(STORAGE_KEY) || '{}');
    const entries: ActivityEntry[] = all[docId] || [];
    entries.unshift({ id: Date.now().toString(), action, user, timestamp: new Date().toISOString(), details });
