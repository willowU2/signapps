    ...
    <div className="bg-background dark:bg-[#202124] rounded-lg shadow-xl border border-border dark:border-gray-800 overflow-hidden flex flex-col w-72">
      {props.items.length > 0 ? (
        props.items.map((item, index) => (
          <button
            className={`flex items-center gap-3 px-4 py-2 text-left w-full transition-colors ${
              index === selectedIndex
                ? "bg-muted dark:bg-[#303134]"
                : "hover:bg-muted dark:hover:bg-[#303134]/50"
            }`}
            key={index}
        ...
        <div className="px-4 py-2 text-sm text-muted-foreground dark:text-gray-400">
