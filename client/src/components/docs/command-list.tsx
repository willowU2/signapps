        ...
        <div className="bg-background dark:bg-[#202124] rounded-lg shadow-xl border border-gray-200 dark:border-gray-800 overflow-hidden flex flex-col w-72">
            {props.items.length > 0 ? (
                props.items.map((item, index) => (
                    <button
                        className={`flex items-center gap-3 px-4 py-2 text-left w-full transition-colors ${index === selectedIndex ? 'bg-gray-100 dark:bg-[#303134]' : 'hover:bg-gray-50 dark:hover:bg-[#303134]/50'
                            }`}
                        key={index}
                        onClick={() => selectItem(index)}
                    >
                        <div className="flex bg-gray-100 dark:bg-gray-800 p-2 rounded text-gray-700 dark:text-gray-300">
                            {item.icon}
                        </div>
                        <div>
