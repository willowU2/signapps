...
export interface FileHandlerOptions {
   ...
   */
  onDrop: (editor: TiptapEditor, files: File[], pos?: number) => void;
  /**
   * Handler for pasted files
   */
  onPaste: (editor: TiptapEditor, files: File[]) => void;
  /**
   * Error handler
   */
...
export const FileHandler = Extension.create<FileHandlerOptions>({
  ...
  addProseMirrorPlugins() {
    const { allowedMimeTypes, maxFileSize, onDrop, onPaste, onError } =
      this.options;
    ...
    const handleFiles = (
      files: File[],
      handler: (editor: TiptapEditor, files: File[], pos?: number) => void,
      pos?: number,
    ): boolean => {
