...
const renderMermaid = async (code: string, dom: HTMLElement): Promise<void> => {
  try {
    ...
    const mermaid: any = await new Function("pkg", "return import(pkg)")(
      "mermaid",
    ).catch(() => null);
    if (!mermaid) {
...
export const MermaidDiagram = Node.create({
  ...
  addNodeView() {
    return ({ node }) => {
      ...
      wrapper.className =
        "mermaid-diagram my-4 flex justify-center overflow-auto p-4 bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700";
