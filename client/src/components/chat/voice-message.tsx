            ...
            <div className="flex items-center gap-2">
                ...
                {state === "recording" && (
                    <div className="flex items-center gap-2 px-2 py-1 rounded-full bg-destructive/10 border border-destructive/20">
                        ...
                        <span className="text-xs font-mono text-destructive">{fmtTime(durationSec)}</span>
                        <Button
                            type="button"
                            variant="ghost"
                            size="icon"
                ...
                {state === "preview" && (
                    <div className="flex items-center gap-2 px-2 py-1 rounded-full bg-muted/50 border">
                        ...
                        <div className="w-24 h-1.5 rounded-full bg-muted">
                            <div className={cn("h-full rounded-full bg-primary transition-all", isPlaying ? "w-full" : "w-0")} style={{ transition: isPlaying ? `width ${durationSec}s linear` : "none" }} />
                        </div>
                        <span className="text-xs font-mono text-muted-foreground">{fmtTime(durationSec)}</span>
                        <Button
                            type="button"
                            variant="ghost"
                            size="icon"
...
export function VoiceMessagePlayer({ src, durationSec = 0 }: VoiceMessagePlayerProps) {
    ...
    return (
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-muted/50 border w-52 mt-1">
            <Button type="button" variant="ghost" size="icon" className="h-7 w-7 text-primary shrink-0" onClick={toggle}>
                {playing ? <Pause className="h-3.5 w-3.5" /> : <Play className="h-3.5 w-3.5" />}
            </Button>
            <div className="flex-1 h-1.5 rounded-full bg-muted">
                <div className={cn("h-full rounded-full bg-primary", playing ? "w-full" : "w-0")}
                    style={{ transition: playing ? `width ${durationSec}s linear` : "none" }} />
            </div>
            <span className="text-xs font-mono text-muted-foreground shrink-0">{fmtTime(durationSec)}</span>
        </div>
    );
}
