...
export function ChatSidebar({
    ...
    try {
        ...
        const dms = (dmsRes.data || []).map((dm: DirectMessage) => {
          ...
          if (others.length > 0) {
            const names = others.map((op) => {
              ...
              if (u && (u.display_name || u.username || u.email))
      ...
      if (FEATURES.CHAT_UNREAD_COUNT) {
        try {
          ...
          for (const s of unreadRes.data || [])
            map[s.channel_id] = s.unread_count;
          setUnreadMap(map);
        } catch {}
      }
    } catch (err) {
  ...
  const handleCreateChannel = async () => {
    ...
    try {
      ...
      toast.error("Impossible de créer channel");
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const handleCreateDm = async (userId: string) => {
    ...
    try {
      const res = await chatApi.createDirectMessage({
      ...
      if (!directMessages.find((d) => d.id === dm.id))
        setDirectMessages([newDm, ...directMessages]);
  ...
  const handleRenameChannel = async () => {
    ...
    setIsRenaming(true);
    try {
      const res = await chatApi.updateChannel(renameTarget.id, {
        name: renameValue.trim(),
      });
      setChannels((prev) =>
        prev.map((c) =>
          c.id === renameTarget.id
            ? { ...c, name: res.data?.name ?? renameValue.trim() }
            : c,
        ),
      );
  ...
  const handleDeleteChannel = async () => {
    ...
    try {
      ...
      setChannels((prev) =>
        prev.filter((c) => c.id !== deleteChannelTarget.id),
      );
  ...
  const getPresenceStatus = (
    ...
    return (
      (presenceMap[recipientId]?.status as
        | "online"
        | "away"
        | "busy"
        | "offline") || "offline"
      ...
      <ScrollArea className="flex-1 px-3 mt-2">
        <div className="space-y-4 pb-4">
          ...
          <div className="space-y-0.5">
            ...
            <Button
              variant="ghost"
              className="w-full justify-start gap-3 h-8 px-3 text-sm font-medium rounded-full text-muted-foreground hover:bg-muted"
            >
              <span className="font-bold text-lg leading-none mt-[-4px]">
                @
              </span>
          ...
          <div>
            <div className="flex items-center justify-between px-3 h-8 group hover:bg-muted rounded-full cursor-pointer transition-colors mb-1">
              ...
              <Button
                variant="ghost"
                size="icon"
                className="h-5 w-5 hover:bg-muted rounded overflow-hidden"
                onClick={(e) => {
                  e.stopPropagation();
            ...
            <div className="space-y-0.5">
              ...
              {filteredDMs.map((dm) => {
                const targetUser = dm.recipientId
                  ? usersMap[dm.recipientId]
                  : undefined;
                ...
                if (targetUser) {
                  ...
                  if (dn) dynamicName = dn;
                    ...
                    dynamicName = targetUser.email.split("@")[0];
                }
                const avatarUrl =
                  targetUser?.avatar_url ||
                  `https://api.dicebear.com/7.x/avataaars/svg?seed=${dynamicName}`;
                  ...
                  >
                      ...
                      <div className="h-6 w-6 rounded-full overflow-hidden">
                        <Avatar className="h-full w-full">
                          ...
                          <AvatarFallback className="bg-primary/10 text-primary text-[10px]">
                            {dynamicName.charAt(0).toUpperCase()}
                          </AvatarFallback>
                        </Avatar>
                      </div>
                    ...
                    <span
                      className={cn(
                        ...
                        dmUnread > 0 && selectedChannel !== dm.id
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={cn(
                          "truncate flex-1 text-left",
                          chUnread > 0 && selectedChannel !== channel.id
                            ? "font-bold text-foreground"
                            : "",
                        )}
                      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-background rounded-xl shadow-xl w-full max-w-sm p-6 mx-4">
            <h2 className="text-base font-semibold mb-2">
              Supprimer &laquo;{deleteChannelTarget.name}&raquo; ?
            </h2>
