    ...
    return (
        ...
        >
            ...
            <div className="flex-1 min-w-0">
                <p className="text-xs font-medium truncate group-hover:underline">{filename}</p>
...
export function PendingAttachment({ attachment, onRemove }: PendingAttachmentProps) {
    return (
        <div className="flex items-center gap-1.5 rounded-full border bg-muted/40 pl-2 pr-1 py-0.5 max-w-[180px]">
            <div className="text-primary shrink-0">{getFileIcon(attachment.content_type ?? "application/octet-stream")}</div>
            <span className="text-xs truncate">{attachment.filename ?? "fichier"}</span>
            <Button
                type="button"
                variant="ghost"
                size="icon"
