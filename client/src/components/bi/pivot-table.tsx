...
export default function PivotTable() {
  ...
  const rows = Array.from(
    new Set(data.map((d) => d[rowDimension as keyof PivotData])),
  );
  const cols = Array.from(
    new Set(data.map((d) => d[colDimension as keyof PivotData])),
  );
  ...
  const getValue = (rowVal: any, colVal: any): number => {
    return data
      .filter(
        (d) =>
          d[rowDimension as keyof PivotData] === rowVal &&
          d[colDimension as keyof PivotData] === colVal,
      )
              ...
              <thead>
                <tr className="bg-muted border-b">
                  ...
                  {cols.map((col) => (
                    <th
                      key={col}
                      className="border px-3 py-2 text-right font-semibold text-muted-foreground"
                    >
                      {String(col)}
                    </th>
                  ))}
                  <th className="border px-3 py-2 text-right font-semibold text-muted-foreground">
                    Total
                  </th>
                </tr>
              </thead>
              <tbody>
                {rows.map((row) => (
                  <tr key={row} className="hover:bg-muted border-b">
                    <td className="border px-3 py-2 font-medium text-foreground">
                      {String(row)}
                    </td>
                    {cols.map((col) => (
                      <td
                        key={`${row}-${col}`}
                        className="border px-3 py-2 text-right text-muted-foreground"
                      >
                    ...
                    <td className="border px-3 py-2 text-right font-medium text-foreground">
                      ...
                      {cols
                        .reduce(
                          (sum: number, col) => sum + getValue(row, col),
                          0,
                        )
                        .toLocaleString()}
                    </td>
