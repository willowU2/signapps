...
export function RestApiConnector() {
  ...
  const [form, setForm] = useState<Partial<ApiCall>>({
    method: "POST",
    headers: [],
    body: "",
  ...
  const save = () => {
    ...
    if (editing === "new") {
      ...
      setCalls((cs) => [
        ...cs,
        { id: Date.now().toString(), ...rest, enabled: true },
      ]);
    } else {
      setCalls((cs) =>
        cs.map((c) => (c.id === editing ? { ...c, ...form } : c)),
      );
    }
    setEditing(null);
    toast.success("API call saved");
  };
  ...
  const removeHeader = (i: number) =>
    setForm((f) => ({
      ...f,
      headers: (f.headers || []).filter((_, idx) => idx !== i),
    }));
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">
                <Code2 className="h-5 w-5" /> Generic REST API Connector
              </CardTitle>
              <CardDescription>
                Configure HTTP calls triggered by SignApps events
              </CardDescription>
            </div>
            <Button onClick={openNew}>
              <Plus className="mr-2 h-4 w-4" /> New API Call
            </Button>
          </div>
        </CardHeader>
        ...
        <Card className="border-primary/30">
          <CardHeader>
            <CardTitle className="text-base">
              {editing === "new" ? "New API Call" : "Edit API Call"}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="grid gap-2 md:grid-cols-2">
              ...
              <div className="space-y-1">
                ...
                <Select
                  value={form.trigger_event || "manual"}
                  onValueChange={(v) =>
