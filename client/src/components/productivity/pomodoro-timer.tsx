...
export default function PomodoroTimer() {
  ...
  const progress =
    (SESSION_DURATIONS[state.sessionType] - state.timeRemaining) /
    SESSION_DURATIONS[state.sessionType];
  const circumference = 2 * Math.PI * 45;
  const strokeDashoffset = circumference * (1 - progress);

  const bgColor =
    state.sessionType === "work" ? "text-red-500" : "text-blue-500";
  const circleColor =
    state.sessionType === "work" ? "stroke-red-500" : "stroke-blue-500";
