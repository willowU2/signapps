    ...
    <Card>
      ...
      <CardContent className="space-y-6">
            ...
            {topTasks.map((task) => (
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <span
                    className={`px-2 py-1 rounded-full text-xs font-semibold flex-shrink-0 ${getPriorityColor(task.priority)}`}
                  >
                    {task.priority}
                  </span>
                </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {timeBlocks.map((block) => (
              <div
                key={block.id}
                className="flex items-center gap-3 p-2 bg-muted rounded border border-border"
              >
                <div className="text-xs font-semibold text-muted-foreground w-12">
                  {block.time}
                </div>
        ...
        <div className="pt-3 border-t text-xs text-muted-foreground">
          <p>
            Progress: {topTasks.filter((t) => t.status === "done").length}/
            {topTasks.length} tasks completed
          </p>
        </div>
      </CardContent>
    </Card>
