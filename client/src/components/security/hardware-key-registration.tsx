          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">
                <ShieldCheck className="h-5 w-5" /> WebAuthn / FIDO2 Keys
              </CardTitle>
              <CardDescription>
                Register hardware security keys (YubiKey, Google Titan, etc.)
                for phishing-resistant MFA
              </CardDescription>
            </div>
            <Button onClick={() => setShowForm((v) => !v)}>
              <Plus className="mr-2 h-4 w-4" /> Register Key
            </Button>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          <Alert>
            ...
            <AlertDescription className="text-sm">
              WebAuthn/FIDO2 requires a compatible authenticator (USB, NFC, or
              platform authenticator). Keys are stored per-user and cannot be
              transferred.
            </AlertDescription>
          </Alert>
          ...
          {showForm && (
            <div className="border rounded-lg p-4 space-y-3 bg-muted/30">
              <div className="space-y-2">
                ...
                <Input
                  placeholder="e.g., YubiKey Blue, Work Key..."
                  value={keyName}
              ...
              <p className="text-xs text-muted-foreground">
                When prompted, touch or tap your security key.
              </p>
            </div>
          )}
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <span className="text-xs text-muted-foreground">
                          Registered{" "}
                          {new Date(key.registered_at).toLocaleDateString()}
                        </span>
                        {key.last_used && (
                          <span className="text-xs text-muted-foreground">
                            · Used{" "}
