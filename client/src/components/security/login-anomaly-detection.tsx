...
const SAMPLE_EVENTS: AnomalyEvent[] = [
  {
    ...
    type: "new_country",
    detail: "Login from Romania (RO) — never seen before",
    ip: "89.32.44.1",
  ...
  },
  {
    ...
    user: "bob@corp.local",
    type: "unusual_time",
    detail: "Login at 3:42 AM — outside normal hours (9-18h)",
    ip: "10.0.0.55",
  ...
  },
  {
    ...
    detail: "8 failed attempts before success",
    ip: "192.168.1.12",
  ...
  {
    id: "4",
    user: "carol@corp.local",
    type: "new_device",
    detail: "New browser fingerprint detected (Firefox/Linux)",
    ip: "10.0.1.33",
...
export function LoginAnomalyDetection() {
  ...
  const resolve = (id: string) => {
    setEvents((es) =>
      es.map((e) => (e.id === id ? { ...e, resolved: true } : e)),
    );
  ...
  return (
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-3 md:grid-cols-2">
            {[
              {
                label: "New country login",
                desc: "Alert when user logs in from a new country",
                val: detectCountry,
                set: setDetectCountry,
              },
              {
                label: "New device/browser",
                desc: "Alert on unrecognized browser fingerprint",
                val: detectDevice,
                set: setDetectDevice,
              },
              {
                label: "Unusual time",
                desc: "Alert on logins outside working hours",
                val: detectTime,
                set: setDetectTime,
              },
              {
                label: "Multiple failures",
                desc: "Alert after repeated failed attempts",
                val: detectFailures,
                set: setDetectFailures,
              },
          ...
          <CardContent className="space-y-3">
            {unresolved.map((ev) => {
              ...
              return (
                <div
                  key={ev.id}
                  className="flex items-start justify-between border rounded-lg p-3 bg-orange-500/5"
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        IP: {ev.ip} · {new Date(ev.timestamp).toLocaleString()}
                      </p>
                    </div>
                  </div>
                  <Button
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            <Eye className="h-4 w-4" /> Event History ({resolved.length}{" "}
            resolved)
          </CardTitle>
        </CardHeader>
