...
export function GeoFencingConfig() {
  ...
  const [defaultAction, setDefaultAction] = useState<"allow" | "block">(
    "allow",
  );
  ...
  const addRule = () => {
    ...
    if (rules.some((r) => r.country_code === country)) {
      toast.error("Rule already exists for this country");
      return;
    }
    setRules((rs) => [
      ...rs,
      ...
      <Card>
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">
                <Globe className="h-5 w-5" /> Geo-Fencing Login Restrictions
              </CardTitle>
              <CardDescription>
                Allow or block login attempts by country based on IP geolocation
              </CardDescription>
            </div>
        ...
        <CardContent className="space-y-5">
          <div className="flex items-center gap-4 p-3 border rounded-lg">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground">
                Applied to any country not in the rules below
              </p>
            </div>
            <Select
              value={defaultAction}
              onValueChange={(v) => setDefaultAction(v as "allow" | "block")}
            >
          ...
          <div className="flex gap-2">
            ...
            <Select
              value={action}
              onValueChange={(v) => setAction(v as "allow" | "block")}
            >
