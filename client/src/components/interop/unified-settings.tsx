...
export function UnifiedInteropSettings() {
  ...
  useEffect(() => {
    identityClient().get<InteropSettings>('/settings/interop')
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 text-sm font-semibold">
        <Settings2 className="w-4 h-4" />Paramètres d'interopérabilité
      </div>

      <div className="space-y-2">
        {([
          ['auto_contact_from_form', 'Créer contacts depuis réponses formulaires'],
        ...
        ] as [keyof InteropSettings, string][]).map(([key, label]) => (
      ...
      <Button onClick={save} disabled={saving} className="h-8 gap-2 text-sm w-full">
...
export function CrossModuleAnalytics() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await identityClient().get<ModuleMetric[]>('/analytics/cross-module', {
        params: { period },
      });
      setMetrics(data);
    } catch {
      ...
      setMetrics([
        { module: 'docs', label: 'Documents', icon: '📄', created: 12, updated: 34, deleted: 2, active_users: 5 },
        { module: 'tasks', label: 'Tâches', icon: '✅', created: 28, updated: 45, deleted: 8, active_users: 7 },
        { module: 'mail', label: 'Emails', icon: '✉️', created: 67, updated: 12, deleted: 5, active_users: 4 },
        { module: 'contacts', label: 'Contacts', icon: '👤', created: 8, updated: 15, deleted: 1, active_users: 3 },
        { module: 'calendar', label: 'Calendrier', icon: '📅', created: 14, updated: 22, deleted: 3, active_users: 6 },
      ]);
    } finally { setLoading(false); }
  }, [period]);
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm font-semibold">
          <BarChart3 className="w-4 h-4" />Analytics multi-modules
        </div>
        <div className="flex gap-1">
          {(['day', 'week', 'month'] as const).map(p => (
            <Button key={p} size="sm" variant={period === p ? 'default' : 'ghost'} onClick={() => setPeriod(p)} className="h-6 px-2 text-xs">
      ...
      <div className="space-y-1.5">
        {metrics.sort((a, b) => (b.created + b.updated) - (a.created + a.updated)).map(m => {
          const activity = m.created + m.updated;
          const maxActivity = Math.max(...metrics.map(x => x.created + x.updated));
