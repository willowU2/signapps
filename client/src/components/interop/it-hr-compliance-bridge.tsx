...
import { Monitor, FileText, AlertTriangle, Loader2, CheckCircle } from 'lucide-react';
...
export function EmployeeAssetsPanel({ employeeId }: { employeeId: string }) {
  ...
  useEffect(() => {
    itAssetsClient().get<ItAsset[]>('/assets', { params: { assigned_to: employeeId } })
  ...
  return (
    <div className="space-y-1">
      <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Matériel IT assigné</p>
      {assets.length === 0 ? (
...
export function ComplianceToLegalForm({
  ...
  if (linked) return (
    <Badge variant="outline" className="text-xs gap-1">
      <FileText className="w-3 h-3" />Formulaire légal lié
    </Badge>
  );

  return (
    <Button size="sm" variant="ghost" onClick={link} disabled={loading} className="h-7 gap-1 text-xs">
...
export function MonitoringAlertToTicket({
  ...
  const createTicket = async () => {
    ...
    try {
      ...
      const queue = JSON.parse(localStorage.getItem('interop-it-tickets') || '[]');
      queue.push({ alertId, alertTitle, severity, serviceAffected, queued_at: new Date().toISOString() });
  ...
  if (done) return (
    <Badge variant="outline" className="text-xs gap-1">
      <CheckCircle className="w-3 h-3 text-green-500" />Ticket IT créé
    </Badge>
  );

  return (
    <Button size="sm" variant="outline" onClick={createTicket} disabled={loading} className="h-7 gap-1 text-xs">
      {loading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <AlertTriangle className={`w-3.5 h-3.5 ${severityColor}`} />}
      Créer ticket IT
    </Button>
  );
}
