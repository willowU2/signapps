...
export function DesignToSocialPost({
  ...
  if (done) return (
    <Badge variant="outline" className="text-xs gap-1">
      <CheckCircle className="w-3 h-3 text-green-500" />Post social créé
    </Badge>
  );

  return (
    <Button size="sm" variant="ghost" onClick={export_} disabled={loading} className="h-7 gap-1 text-xs">
...
export function DesignToEmailTemplate({
  ...
  if (done) return (
    <Badge variant="outline" className="text-xs gap-1">
      <Mail className="w-3 h-3" />Template mail créé
    </Badge>
  );

  return (
    <Button size="sm" variant="ghost" onClick={useAsHeader} disabled={loading} className="h-7 gap-1 text-xs">
...
export function SheetsToCrmReport({
  ...
  if (done) return (
    <Badge variant="outline" className="text-xs gap-1">
      <BarChart3 className="w-3 h-3" />Données CRM mises à jour
    </Badge>
  );

  return (
    <Button size="sm" variant="ghost" onClick={pushToCrm} disabled={loading} className="h-7 gap-1 text-xs">
