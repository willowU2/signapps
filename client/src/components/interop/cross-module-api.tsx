...
import { Code2, Copy, ExternalLink, Key, Plus, Trash2, RefreshCw, Shield, Check } from 'lucide-react';
...
export function CrossModuleApiKeys() {
  ...
  const [scopes, setScopes] = useState<string[]>(['read:docs', 'read:contacts']);
  ...
  const create = async () => {
    ...
    setCreating(true);
    try {
      const { data } = await identityClient().post<ApiKey & { key: string }>('/api-keys', {
        name: name.trim(),
        scopes,
      });
  ...
  const revoke = async (id: string) => {
    ...
    try { await identityClient().delete(`/api-keys/${id}`); }
    catch { /* optimistic */ }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm font-semibold">
          <Code2 className="w-4 h-4" />API unifiée — Clés d'accès
        </div>
        <Button size="sm" onClick={() => { setOpen(true); setName(''); setScopes(['read:docs', 'read:contacts']); setNewKey(null); }} className="h-7 gap-1 text-xs">
          <Plus className="w-3.5 h-3.5" />Créer une clé
        </Button>
      </div>
        ...
        <CardContent className="p-3 flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            ...
            <code className="text-[10px] text-muted-foreground break-all">
              {typeof window !== 'undefined' ? window.location.origin : 'https://your-instance'}/api/v1
            </code>
          </div>
          <Button size="sm" variant="ghost" className="h-7 w-7 p-0" asChild>
            <a href="/api" target="_blank"><ExternalLink className="w-3.5 h-3.5" /></a>
          </Button>
        </CardContent>
      </Card>
      ...
      <div className="space-y-2">
        {keys.length === 0 && (
          <p className="text-xs text-muted-foreground py-2">Aucune clé API configurée</p>
        )}
        {keys.map(k => (
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {k.scopes.slice(0, 3).map(s => (
                  <Badge key={s} variant="secondary" className="text-[9px] h-3.5 px-1">{s}</Badge>
                ))}
                {k.scopes.length > 3 && <span className="text-[10px] text-muted-foreground">+{k.scopes.length - 3}</span>}
              </div>
              <p className="text-[10px] text-muted-foreground mt-0.5">
                {k.key_preview} · Créée {new Date(k.created_at).toLocaleDateString('fr-FR')}
                {k.last_used && ` · Utilisée ${new Date(k.last_used).toLocaleDateString('fr-FR')}`}
              </p>
            </div>
            <Button size="sm" variant="ghost" onClick={() => revoke(k.id)} className="h-7 w-7 p-0 text-destructive hover:text-destructive">
        ...
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Key className="w-4 h-4" />Nouvelle clé API
            </DialogTitle>
          </DialogHeader>
            ...
            <div className="space-y-3 py-2">
              <div className="p-3 bg-green-50 dark:bg-green-950/30 rounded-lg border border-green-200">
                <p className="text-xs font-medium text-green-700 dark:text-green-400 mb-1.5 flex items-center gap-1">
                  <Check className="w-3.5 h-3.5" />Copiez cette clé maintenant
                </p>
                <code className="text-xs break-all">{newKey}</code>
                <Button size="sm" variant="outline" onClick={() => copyKey(newKey)} className="mt-2 h-7 gap-1 text-xs w-full">
                  <Copy className="w-3 h-3" />Copier
                </Button>
              </div>
              <p className="text-xs text-muted-foreground">Cette clé ne sera plus affichée après fermeture.</p>
            </div>
          ) : (
                  ...
                  <div className="space-y-1.5">
                    {AVAILABLE_SCOPES.map(s => (
                      <div key={s.id} className="flex items-center gap-2">
                        ...
                        />
                        <Label htmlFor={`scope-${s.id}`} className="text-xs cursor-pointer">{s.label}</Label>
                      </div>
                    ))}
                  </div>
                </ScrollArea>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setOpen(false)} className="h-7 text-xs">
              {newKey ? 'Fermer' : 'Annuler'}
            </Button>
            {!newKey && (
              <Button onClick={create} disabled={creating || !name.trim() || !scopes.length} className="h-7 gap-1 text-xs">
                <Key className="w-3 h-3" />Créer
              </Button>
            )}
          </DialogFooter>
        </DialogContent>
