...
export function CrossModuleSearchBar() {
  ...
  const search = useCallback(
    async (q: string) => {
      ...
      setLoading(true);
      try {
        const { data } = await identityClient().get<SearchResult[]>(
          "/search/cross-module",
          {
            ...
            {results.map((r) => {
              ...
              return (
                <a
                  key={r.id}
                  href={r.url}
                  className="flex items-start gap-3 p-2.5 hover:bg-muted/50 border-b last:border-0"
                >
                  <span
                    className={`text-xs px-1.5 py-0.5 rounded shrink-0 ${mod?.color || "bg-muted text-muted-foreground"}`}
                  >
...
export function UnifiedContactCard({ email }: { email: string }) {
  ...
  useState(() => {
    contactsClient()
      .get<UnifiedContact>("/contacts/unified", { params: { email } })
  ...
  if (loading)
    return <div className="animate-pulse h-12 w-48 rounded bg-muted" />;
