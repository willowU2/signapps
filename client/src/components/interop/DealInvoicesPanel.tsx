...
export function DealInvoicesPanel({ dealId, onRefresh }: Props) {
  ...
  useEffect(() => {
    dealsApi.get(dealId).then(setDeal);
  }, [dealId]);
    ...
    <div className="space-y-3">
      ...
      {invoices.length > 0 && (
        <div className="flex gap-4 text-xs">
          <span>
            Facturé: <strong>{fmt(totalInvoiced)}</strong>
          </span>
          <span className="text-emerald-600">
            Payé: <strong>{fmt(totalPaid)}</strong>
          </span>
        </div>
      )}
          ...
          {invoices.map((inv: LocalInvoice) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <span
                  className={`text-xs rounded-full px-2 py-0.5 font-medium ${STATUS_STYLES[inv.status]}`}
                >
