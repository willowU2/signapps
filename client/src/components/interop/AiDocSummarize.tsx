...
import { Sparkles, Hash, CheckCheck, BookOpen, Loader2, Copy, X } from 'lucide-react';
    ...
    <div className="rounded-lg border bg-background shadow-sm p-3 space-y-3 min-w-[320px]">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="w-full grid grid-cols-4 h-8">
          ...
          <TabsTrigger value="proofread" className="text-xs">Correct.</TabsTrigger>
          <TabsTrigger value="related" className="text-xs">Liés</TabsTrigger>
        </TabsList>

        <TabsContent value="summary" className="mt-2 space-y-2">
          <Button size="sm" className="w-full gap-1.5" onClick={() => run('summary')} disabled={loading}>
            {loading && activeTab === 'summary' ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Sparkles className="h-3.5 w-3.5" />}
            Résumer
          </Button>
          {summary && (
            <div className="text-xs rounded border bg-primary/5 p-2 whitespace-pre-wrap relative">
              {summary}
              <button onClick={() => { navigator.clipboard.writeText(summary); toast.success('Copié'); }}
        ...
        <TabsContent value="keywords" className="mt-2 space-y-2">
          <Button size="sm" className="w-full gap-1.5" onClick={() => run('keywords')} disabled={loading}>
            {loading && activeTab === 'keywords' ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Hash className="h-3.5 w-3.5" />}
        ...
        <TabsContent value="proofread" className="mt-2 space-y-2">
          <Button size="sm" className="w-full gap-1.5" onClick={() => run('proofread')} disabled={loading}>
            {loading && activeTab === 'proofread' ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <CheckCheck className="h-3.5 w-3.5" />}
            Corriger
          </Button>
          {proofread && (
            <div className="text-xs rounded border bg-primary/5 p-2 whitespace-pre-wrap relative">
              {proofread}
              <button onClick={() => { navigator.clipboard.writeText(proofread); toast.success('Copié'); }}
                className="absolute top-1 right-1 p-1 rounded hover:bg-accent">
                <Copy className="h-3 w-3" />
              </button>
            </div>
        ...
        <TabsContent value="related" className="mt-2 space-y-2">
          <Button size="sm" className="w-full gap-1.5" onClick={() => run('related')} disabled={loading}>
            {loading && activeTab === 'related' ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <BookOpen className="h-3.5 w-3.5" />}
            Suggestions
          </Button>
          {related.length > 0 && (
