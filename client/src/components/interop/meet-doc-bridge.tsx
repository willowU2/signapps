...
import { FileText, Presentation, UserCircle, Loader2, ExternalLink } from 'lucide-react';
...
export function MeetRecordingToDoc({
  ...
  const createDoc = async () => {
    ...
    try {
      ...
      const local = { meetingId, meetingTitle, transcript, queued_at: new Date().toISOString() };
  ...
  if (docId) return (
    <Button size="sm" variant="outline" asChild className="h-7 gap-1 text-xs">
      <Link href={`/docs/${docId}`}>
        <FileText className="w-3.5 h-3.5" />Ouvrir le document
  ...
  return (
    <Button size="sm" variant="outline" onClick={createDoc} disabled={loading} className="h-7 gap-1 text-xs">
...
export function SlidesToMeet({
  ...
  return (
    <Button size="sm" variant="outline" onClick={presentInMeet} disabled={loading} className="h-7 gap-1 text-xs">
...
export function MeetAttendeeContacts({ attendeeEmails }: { attendeeEmails: string[] }) {
  const [contacts, setContacts] = useState<ContactCard[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!attendeeEmails.length) { setLoading(false); return; }
    const promises = attendeeEmails.slice(0, 8).map(email =>
      contactsClient().get<ContactCard[]>('/contacts', { params: { email } })
