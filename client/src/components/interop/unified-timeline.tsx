    ...
    try {
      ...
      try {
        const local = JSON.parse(
          localStorage.getItem("activity-tracker") || "[]",
        );
        setActivities(
          local.slice(0, limit).map((l: Record<string, unknown>) => ({
            id: l.id as string,
            actor_id: "vous",
      ...
      <ScrollArea className={compact ? "h-48" : "h-80"}>
        <div className="relative pl-4 space-y-0 pr-2">
          ...
          {filtered.map((a, i) => (
            <div
              key={a.id}
              className={`relative flex gap-3 ${i < filtered.length - 1 ? "pb-3" : ""}`}
            >
              {/* Dot */}
              <div
                className={`absolute left-[-7px] w-3 h-3 rounded-full border-2 border-background mt-1 ${MODULE_COLOR[a.entity_type] || "bg-muted-foreground"}`}
              />
              ...
              <div className="flex-1 min-w-0 ml-2">
                <div className="flex items-start gap-2">
                  <Badge
                    variant="outline"
                    className={`text-[10px] h-4 px-1 shrink-0 ${compact ? "hidden" : ""}`}
                  >
