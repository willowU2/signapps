...
function loadContactCustomFields(): CustomFieldDef[] {
  ...
  try {
    return JSON.parse(
      localStorage.getItem("contacts:custom_fields") ?? "[]",
    ) as CustomFieldDef[];
  } catch {
    return [];
  }
...
function loadContactFieldValues(contactId: string): CustomFieldValue[] {
  ...
  try {
    const all = JSON.parse(
      localStorage.getItem("contacts:custom_field_values") ?? "[]",
    ) as (CustomFieldValue & { contactId: string })[];
...
export function ContactCustomFieldsCrm({
  contactId,
  dealId,
  onCopyToDeal,
}: Props) {
    ...
    if (dealId) {
      try {
        const stored = JSON.parse(
          localStorage.getItem(`crm:deal_custom_fields:${dealId}`) ?? "{}",
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide flex items-center gap-1">
          <Settings2 className="h-3 w-3" /> Champs personnalisés (
          {fields.length})
        </p>
          ...
          return (
            <div key={field.id} className="space-y-1">
              ...
              <Input
                ...
                placeholder={
                  contactVal || `Valeur ${field.label.toLowerCase()}`
                }
                className="h-7 text-xs"
                type={
                  field.type === "number"
