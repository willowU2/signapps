...
export function SocialAttachDrive({ onAttach, maxFiles = 4 }: SocialAttachDriveProps) {
  ...
  const toggle = (node: DriveNode) => {
    setSelected((prev) => {
      ...
      if (prev.length >= maxFiles) { toast.error(`Max ${maxFiles} fichiers`); return prev; }
      return [...prev, node];
    });
  };

  const handleConfirm = () => {
    const attachments = selected.map((n) => ({ node: n, url: driveNodeUrl(n.id) }));
            ...
            <div className="flex flex-wrap gap-1 pb-2 border-b">
              {selected.map((n) => (
                <Badge key={n.id} variant="secondary" className="gap-1 text-xs">
                  {n.name.slice(0, 20)}
                  <button onClick={() => toggle(n)} className="ml-0.5 hover:text-destructive">
                    <X className="h-3 w-3" />
                  </button>
                </Badge>
              ))}
          ...
          <div className="max-h-64 overflow-y-auto space-y-1">
            ...
            {!isLoading && nodes.map((node) => {
              ...
              return (
                ...
                >
                  <button className="flex-1 flex items-center gap-2 text-sm" onClick={() => toggle(node)}>
                    {node.node_type === 'folder'
                      ? <Folder className="h-4 w-4 text-yellow-500 shrink-0" />
                      : <File className="h-4 w-4 text-blue-500 shrink-0" />}
                    <span className="truncate">{node.name}</span>
                    {isSelected && <Plus className="h-3.5 w-3.5 text-primary ml-auto rotate-45" />}
                  </button>
                  {node.node_type === 'folder' && (
                    <button
            ...
            {!isLoading && nodes.length === 0 && (
              <p className="text-sm text-muted-foreground text-center py-4">Aucun fichier dans Drive</p>
            )}
          </div>
