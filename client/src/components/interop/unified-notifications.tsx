...
export function CrossModuleNotificationCenter() {
  ...
  const load = useCallback(async () => {
    try {
      const { data } =
        await notifClient().get<CrossNotification[]>("/cross-module");
      setNotifs(data);
    } catch {
      // Try notificationsApi as secondary source
      try {
        const { data } = await notificationsApi.list();
        const mapped: CrossNotification[] = (
  ...
  const markRead = async (id: string) => {
    ...
    try {
      await notifClient().patch(`/cross-module/${id}`, { read: true });
    } catch {
      /* optimistic */
    }
  };
  ...
  const markAllRead = async () => {
    ...
    try {
      await notifClient().post("/cross-module/mark-all-read", {});
    } catch {
      /* optimistic */
    }
  };
  ...
  const toggleDnd = async () => {
    ...
    setDnd(next);
    try {
      await notifClient().put("/preferences", { do_not_disturb: next });
    } catch {
      localStorage.setItem("notif-dnd", String(next));
    }
      ...
      <PopoverContent className="w-80 p-0" align="end">
        <div className="flex items-center justify-between p-3 border-b">
          <div className="flex items-center gap-2">
            ...
            {unread > 0 && (
              <Badge className="text-xs h-4 px-1.5">{unread}</Badge>
            )}
          </div>
        ...
        <ScrollArea className="h-72">
          {loading && (
            <div className="animate-pulse m-3 h-12 rounded bg-muted" />
          )}
...
export function ModuleHealthDashboard() {
  ...
  return (
    <div className="space-y-3">
      ...
      <div
        className={`p-2.5 rounded-lg border flex items-center gap-2 ${healthy === total ? "border-green-200 bg-green-50/50 dark:bg-green-950/20" : "border-orange-200 bg-orange-50/50 dark:bg-orange-950/20"}`}
      >
        {healthy === total ? (
