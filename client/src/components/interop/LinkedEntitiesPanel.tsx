...
import { Mail, CheckSquare, CalendarDays, ExternalLink, Plus, Link2 } from "lucide-react";
...
function LinkRow({ link, entityType, entityId }: { link: CrossLink; entityType: string; entityId: string }) {
  ...
  return (
    <a href={href} className="flex items-center gap-2 py-1.5 px-2 rounded-md hover:bg-muted/60 text-sm group transition-colors">
      <span className="text-muted-foreground">{TYPE_ICON[otherType]}</span>
      <span className="flex-1 truncate">{otherTitle || otherId.slice(0, 12) + "…"}</span>
      <Badge variant="secondary" className="text-[10px] h-4 px-1">{link.relation}</Badge>
      <ExternalLink className="h-3 w-3 opacity-0 group-hover:opacity-60 transition-opacity" />
    </a>
  );
}

export function LinkedEntitiesPanel({ entityType, entityId, className }: Props) {
  const { links } = useInteropLinks(entityType, entityId);
  const [collapsed, setCollapsed] = useState(false);

  if (links.length === 0) {
    return (
      <div className={cn("rounded-lg border border-dashed border-border/60 p-3 text-center text-xs text-muted-foreground", className)}>
