...
export function EmailToEventDialog({ open, onOpenChange, mail }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      interopStore.addLink({ sourceType: "mail", sourceId: mail.id, sourceTitle: mail.subject, targetType: "event", targetId: eventId, targetTitle: title, relation: "created_from" });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[460px]">
        <DialogHeader>
          ...
          <DialogDescription>Créer un événement depuis cet email.</DialogDescription>
        </DialogHeader>
