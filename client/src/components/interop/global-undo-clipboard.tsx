...
export function pushUndo(entry: Omit<UndoEntry, "id" | "timestamp">) {
  const stack: UndoEntry[] = JSON.parse(
    localStorage.getItem(UNDO_STORAGE_KEY) || "[]",
  );
  stack.unshift({
    ...entry,
    id: `u-${Date.now()}`,
        ...
        <ScrollArea className="max-h-48">
          <div className="space-y-1 pr-1">
            ...
            {stack.map((entry) => (
              <div
                key={entry.id}
                className="flex items-center gap-2 p-1.5 rounded border hover:bg-muted/40"
              >
...
export function CrossModuleBatchOps({ items }: { items: BatchItem[] }) {
  ...
  const execute = async (op: BatchOperation) => {
    if (!selected.size) {
      toast.error("Sélectionnez des éléments");
      return;
    }
    setOperation(op);
    setLoading(true);
    const batch = items.filter((i) =>
      selected.has(`${i.entity_type}:${i.entity_id}`),
    );
    ...
    <div className="space-y-2">
        ...
        <div className="space-y-1 pr-1">
          {items.map((item) => {
            ...
            return (
              <div
                key={key}
                className="flex items-center gap-2 p-1.5 rounded border hover:bg-muted/30"
              >
                <Checkbox
      ...
      {selected.size > 0 && (
        <div className="flex gap-1.5 flex-wrap">
          {(
            ["archive", "delete", "tag", "share", "export"] as BatchOperation[]
          ).map((op) => (
            <Button
              key={op}
              size="sm"
