...
export function CalendarContactSuggestions({
  query,
  onSelect,
  className,
}: Props) {
  ...
  return (
    <div
      className={cn(
        "rounded-lg border border-border shadow-sm bg-background overflow-hidden",
        className,
      )}
    >
...
export function useCalendarAvailability(email: string | null) {
  const [slots, setSlots] = useState<
    { start: string; end: string; free: boolean }[]
  >([]);
    ...
    (async () => {
      try {
        ...
        const res = await fetch(
          `${CALENDAR_URL}/freebusy?email=${encodeURIComponent(email)}&start=${start}&end=${end}`,
          { credentials: "include" },
        );
        if (!res.ok) return;
