...
async function fetchTodayItems(): Promise<TodayItem[]> {
  ...
  await Promise.allSettled([
    ...
    mailApi.list({ folder_type: "inbox", limit: 5 }).then((emails) => {
      (Array.isArray(emails) ? emails : [])
        ...
        .forEach((e) =>
          items.push({
            ...
            subtitle: `De : ${e.sender_name || e.sender}`,
            time: e.received_at,
            href: "/mail",
          }),
        );
    ...
    (async () => {
      ...
      const { data: evList } = await calendarApi.listEvents(
        calendars[0].id,
        start,
        end,
      );
      (Array.isArray(evList) ? evList : []).forEach((e) =>
        items.push({
          id: e.id,
          type: "event",
          title: e.title,
...
export function TodayView({ className, maxItems = 20 }: Props) {
  ...
  const grouped = items
    .slice(0, maxItems)
    .reduce<Record<string, TodayItem[]>>((acc, item) => {
  ...
  if (items.length === 0) {
    return (
      <div
        className={cn(
          "rounded-xl border border-dashed border-border p-8 text-center text-muted-foreground",
          className,
        )}
      >
            ...
            <div className="space-y-1">
              {typeItems.map((item) => (
                <a
                  ...
                  href={item.href}
                  className={cn(
                    "flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm hover:opacity-90 transition-opacity group",
                    cfg.bg,
                  )}
                >
