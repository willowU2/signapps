...
export function ContactSocialProfiles({ contactId }: Props) {
  ...
  const [form, setForm] = useState<{ platform: SocialProfile["platform"]; url: string }>({
    platform: "linkedin",
    url: "",
  })
    ...
    <div className="space-y-3">
      ...
      {adding && (
        <div className="flex gap-2 items-end">
          <Select value={form.platform} onValueChange={v => setForm(f => ({ ...f, platform: v as SocialProfile["platform"] }))}>
      ...
      {profiles.length === 0 ? (
        <p className="text-xs text-muted-foreground italic">Aucun profil social lié.</p>
      ) : (
        <div className="flex flex-wrap gap-2">
          {profiles.map(p => (
            <div key={p.id} className={`flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium ${PLATFORM_COLORS[p.platform] ?? ""}`}>
              <span className="font-bold">{PLATFORM_ICONS[p.platform]}</span>
              <a href={p.url} target="_blank" rel="noreferrer" className="hover:underline max-w-32 truncate">
                {p.url.replace(/^https?:\/\/(www\.)?/, "")}
              </a>
              <button onClick={() => handleDelete(p.id)} className="ml-0.5 hover:opacity-60">
