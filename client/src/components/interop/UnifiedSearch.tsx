...
import { Search, CheckSquare, CalendarDays, Mail, Loader2, Sparkles } from "lucide-react";
...
async function searchAll(q: string): Promise<SearchResult[]> {
  ...
  await Promise.allSettled([
    // Mail
    searchApi.search({ q, limit: 5 }).then((emails: any[]) => {
      (Array.isArray(emails) ? emails : []).forEach((e: any) => results.push({ id: e.id, module: "mail", title: e.subject || "(Sans objet)", excerpt: e.snippet, date: e.received_at, href: "/mail" }));
    }),
    // Tasks — best-effort
    (async () => {
      ...
      if (!calId) return;
      const res = await fetch(`${API}/calendars/${calId}/tasks?q=${encodeURIComponent(q)}&limit=5`, { credentials: "include" });
      if (!res.ok) return;
      const data = await res.json();
      (data.data ?? data ?? []).forEach((t: any) => results.push({ id: t.id, module: "task", title: t.title, excerpt: t.description, date: t.due_date, href: "/tasks" }));
    })(),
    // Events — best-effort
    (async () => {
      const API = process.env.NEXT_PUBLIC_CALENDAR_API || "http://localhost:3011/api/v1";
      ...
      const calId = (cals.data ?? cals)?.[0]?.id;
      if (!calId) return;
      const res = await fetch(`${API}/calendars/${calId}/events?q=${encodeURIComponent(q)}&limit=5`, { credentials: "include" });
      if (!res.ok) return;
      const data = await res.json();
      (data.data ?? data ?? []).forEach((e: any) => results.push({ id: e.id, module: "event", title: e.title, excerpt: e.notes, date: e.start_time, href: "/cal" }));
    })(),
  ]);
...
export function UnifiedSearchBar({ className, placeholder = "Rechercher dans tous les modules…" }: Props) {
    ...
    <div className={cn("relative", className)}>
      ...
      {open && results.length > 0 && (
        <div className="absolute top-full mt-1 w-full z-50 rounded-lg border border-border shadow-lg bg-background overflow-hidden max-h-80 overflow-y-auto">
          {results.map(r => (
            <a key={`${r.module}-${r.id}`} href={r.href}
            ...
            >
              ...
              {r.date && <span className="text-[11px] text-muted-foreground shrink-0">{new Date(r.date).toLocaleDateString("fr-FR")}</span>}
            </a>
          ))}
        </div>
      )}
...
export async function extractActionItems(emailBody: string, emailId: string, calendarId: string): Promise<number> {
  // Simple heuristic: lines starting with action verbs or "TODO:" / "Action:" / "•"
  const lines = emailBody.split(/\n/).map(l => l.trim()).filter(l => l.length > 5);
  const actionPatterns = [/^(TODO|Action|À faire|Veuillez|Please|Merci de|Rappel)[:\s]/i, /^[-•*]\s+\w{3,}/];
  const actionItems = lines.filter(l => actionPatterns.some(p => p.test(l))).slice(0, 5);
  ...
  for (const item of actionItems) {
    try {
      const res = await fetch(`${API}/calendars/${calendarId}/tasks`, {
        ...
        body: JSON.stringify({ title: item.replace(/^[-•*]\s+/, ""), priority: 1 }),
      });
      if (res.ok) {
        const d = await res.json();
        interopStore.addLink({ sourceType: "mail", sourceId: emailId, sourceTitle: "Email", targetType: "task", targetId: d.id, targetTitle: item, relation: "extracted_action" });
        created++;
      }
    } catch { /* best-effort */ }
  }
