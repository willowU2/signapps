...
export function LmsCourseToHrSkill({
  ...
  const sync = async () => {
    ...
    try {
      ...
      const queue = JSON.parse(localStorage.getItem('interop-skill-queue') || '[]');
      queue.push({ userId, courseId, courseTitle, queued_at: new Date().toISOString() });
  ...
  if (synced) return (
    <Badge variant="outline" className="text-xs gap-1">
      <GraduationCap className="w-3 h-3" />Compétence RH mise à jour
    </Badge>
  );
...
export async function triggerHrOnboarding(userId: string, userEmail: string, userName: string) {
  try {
    ...
    const pending = JSON.parse(localStorage.getItem('interop-onboarding-queue') || '[]');
    pending.push({ userId, userEmail, userName, queued_at: new Date().toISOString() });
...
export function HrOnboardingTrigger({
  ...
  if (done) return (
    <Badge variant="outline" className="text-xs gap-1">
      <UserCheck className="w-3 h-3" />Onboarding déclenché
    </Badge>
  );

  return (
    <Button size="sm" variant="outline" onClick={trigger} disabled={loading} className="h-7 gap-1 text-xs">
