...
export function EventFollowUpSuggestion({ event, mode, onDismiss }: Props) {
  ...
  const labels: Record<typeof mode, { title: string; buttonLabel: string; color: string }> = {
    follow_up: { title: `Créer un suivi pour « ${event.title} »`, buttonLabel: "Créer la tâche de suivi", color: "text-blue-500" },
    prepare: { title: `Préparer « ${event.title} »`, buttonLabel: "Créer la tâche de préparation", color: "text-emerald-500" },
    invite: { title: `Préparer l'invitation : « ${event.title} »`, buttonLabel: "Créer la tâche de préparation", color: "text-purple-500" },
  };
  ...
  const handleCreate = async () => {
    setCreating(true);
    try {
      const taskTitle = mode === "follow_up"
        ? `Suivi : ${event.title}`
        : `Préparer : ${event.title}`;
      const dueDate = mode === "follow_up"
      ...
      if (Array.isArray(calendars) && calendars.length > 0) {
        ...
        stored.push({ id: taskId, title: taskTitle, due_date: dueDate, status: "open", created_at: new Date().toISOString() });
        localStorage.setItem("email-tasks", JSON.stringify(stored));
      }

      interopStore.addLink({ sourceType: "event", sourceId: event.id, sourceTitle: event.title, targetType: "task", targetId: taskId, targetTitle: taskTitle, relation: mode });
      interopStore.addNotification({ sourceModule: "calendar", type: "task_suggested", title: "Tâche créée", body: taskTitle });
...
export function useEventTaskSuggestions(event: ScheduleBlock | null) {
  ...
  const isDismissed = (mode: string) => dismissed.includes(`${event?.id}:${mode}`);
  const dismiss = (mode: string) => setDismissed(prev => [...prev, `${event?.id}:${mode}`]);

  const isEndingSoon = event ? (new Date(event.end ?? event.start).getTime() - Date.now() < 15 * 60000) : false;
  const isUpcoming = event ? (new Date(event.start).getTime() - Date.now() < 48 * 3600000 && new Date(event.start).getTime() > Date.now()) : false;
