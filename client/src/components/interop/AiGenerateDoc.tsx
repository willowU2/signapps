...
export function AiGenerateDoc() {
  ...
  const handleGenerate = async () => {
    if (!prompt.trim()) { toast.error('Entrez un sujet'); return; }
    setLoading(true);
    setPreview('');
    try {
  ...
  const handleCreate = async () => {
    if (!preview || !docName.trim()) { toast.error('Générez d\'abord le document'); return; }
    setCreating(true);
    try {
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col">
          ...
          <div className="flex-1 overflow-auto space-y-4">
            ...
            <Button className="w-full gap-1.5" onClick={handleGenerate} disabled={loading || !prompt.trim()}>
              ...
              <>
                ...
                <div className="rounded-lg border bg-muted/10 p-3">
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className="text-xs font-semibold uppercase tracking-wider text-primary">Aperçu</span>
                  </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreate} disabled={!preview || creating || !docName.trim()} className="gap-1.5">
              {creating ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <ExternalLink className="h-3.5 w-3.5" />}
              Créer et ouvrir
            </Button>
          </DialogFooter>
        </DialogContent>
