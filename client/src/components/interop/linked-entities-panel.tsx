      ...
      <div className="flex items-center justify-between">
        ...
        {allowAdd && (
          <Button
            ...
            onClick={() =>
              toast.info("Glissez une entité ici pour créer un lien")
            }
          >
            ...
            {Object.entries(grouped).map(([type, refs]) => (
              <div key={type}>
                <p className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1.5">
                  {type.replace("_", " ")} ({refs.length})
                </p>
                <div className="space-y-1">
                  {refs.map((r) => {
                    const otherId =
                      r.source_type === entityType ? r.target_id : r.source_id;
                    const href = TYPE_HREF[type]?.(otherId);
                    return (
                      <div
                        key={r.id}
                        className="flex items-center gap-2 p-1.5 rounded border hover:bg-muted/40 group"
                      >
...
export function DuplicateDetector({
  ...
  const check = async () => {
    setLoading(true);
    try {
      const { data } = await identityClient().get<DuplicateEntry[]>(
        "/dedup/check",
        {
          params: {
  ...
  return (
    <div className="space-y-1.5">
      <p className="text-xs text-orange-600 font-medium flex items-center gap-1">
        <AlertCircle className="w-3.5 h-3.5" />
        {dupes.length} doublon(s) potentiel(s)
      </p>
      {dupes.map((d) => (
