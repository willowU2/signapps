...
export function OverdueInvoicesCrmFlag({ compact = false }: Props) {
  const [items, setItems] = useState<
    Awaited<ReturnType<typeof getOverdueInvoicesWithDeals>>
  >([]);
  ...
  const handleMarkPaid = async (invoiceId: string) => {
    ...
    if (updatedDeal) {
      toast.success(
        `Facture payée. Deal "${updatedDeal.title}" mis à jour → Gagné.`,
      );
    } else {
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        {items.map(({ invoice, deal }) => (
          <div
            key={invoice.id}
            className="flex items-center justify-between gap-3 rounded-lg border border-red-200 bg-red-50 dark:bg-red-950/20 px-3 py-2 text-sm"
          >
