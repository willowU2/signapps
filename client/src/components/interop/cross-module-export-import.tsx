...
export function SmartImport() {
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.success(
        `Fichier importé vers ${routed.module} (${Math.round(routed.confidence * 100)}% confiance)`,
      );
    } catch {
    ...
    <div className="space-y-2">
      <Label className="flex items-center gap-2 cursor-pointer p-3 border-2 border-dashed rounded-lg hover:border-primary/50 transition-colors">
        ...
        <span className="text-sm text-muted-foreground">
          {loading
            ? "Analyse en cours…"
            : "Importer un fichier (détection automatique)"}
        </span>
        <input
          type="file"
          className="hidden"
      ...
      {result && (
        <div className="flex items-center gap-2 text-xs">
          ...
          <Badge variant="outline" className="gap-1">
            {result.module} <ArrowRight className="w-2.5 h-2.5" />
          </Badge>
