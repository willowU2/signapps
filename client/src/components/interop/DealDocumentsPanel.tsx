...
function loadLinkedFiles(entityType: "deal" | "contact", entityId: string): LinkedFile[] {
  if (typeof window === "undefined") return []
  try {
    const all = JSON.parse(localStorage.getItem("crm:linked_files") ?? "[]") as LinkedFile[]
    return all.filter(f => f.entityType === entityType && f.entityId === entityId)
  } catch {
    return []
  }
}
...
function removeLinkedFile(fileId: string, entityId: string) {
  ...
  try {
    const all = JSON.parse(localStorage.getItem("crm:linked_files") ?? "[]") as LinkedFile[]
    localStorage.setItem("crm:linked_files", JSON.stringify(
...
export function DealDocumentsPanel({ entityType, entityId, entityName }: Props) {
    ...
    <div className="space-y-3">
      ...
      {files.length === 0 ? (
        <div className="text-xs text-muted-foreground italic">
          Aucun document lié.{" "}
          <Link href={driveUrl} className="text-primary hover:underline inline-flex items-center gap-0.5">
        ...
        <div className="space-y-1.5">
          {files.map(file => (
            <div key={file.fileId} className="flex items-center justify-between gap-2 rounded-lg border bg-card px-3 py-2 text-sm">
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                {file.url && (
                  <a href={file.url} target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-foreground">
                    <ExternalLink className="h-3 w-3" />
                  </a>
                )}
                <Button size="sm" variant="ghost" className="h-6 w-6 p-0 text-muted-foreground" onClick={() => handleUnlink(file.fileId)}>
                  ×
                </Button>
              </div>
            </div>
