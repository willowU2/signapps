...
import { Receipt, BookOpen, FilePlus, Loader2, CheckCircle } from 'lucide-react';
...
export function PoToInvoice({ po }: { po: PurchaseOrder }) {
  ...
  const createInvoice = async () => {
    ...
    try {
      ...
      toast.error('Impossible de créer la facture — service billing indisponible');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (done) return (
    <Badge variant="outline" className="text-xs gap-1">
      <Receipt className="w-3 h-3" />Facture créée
    </Badge>
  );

  return (
    <Button size="sm" variant="outline" onClick={createInvoice} disabled={loading} className="h-7 gap-1 text-xs">
...
export async function syncPaymentToAccounting(paymentId: string, amount: number, currency: string, description: string) {
  try {
    ...
    const queue = JSON.parse(localStorage.getItem('interop-accounting-queue') || '[]');
    queue.push({ paymentId, amount, currency, description, queued_at: new Date().toISOString() });
...
export function AccountingToInvoice({
  ...
  if (done) return (
    <Badge variant="outline" className="text-xs gap-1">
      <CheckCircle className="w-3 h-3 text-green-500" />Facture générée
    </Badge>
  );

  return (
    <Button size="sm" variant="outline" onClick={generate} disabled={loading} className="h-7 gap-1 text-xs">
...
export function PaymentAccountingSync({
  ...
  if (done) return (
    <Badge variant="outline" className="text-xs gap-1">
      <BookOpen className="w-3 h-3" />Comptabilité mise à jour
    </Badge>
  );

  return (
    <Button size="sm" variant="outline" onClick={sync} disabled={loading} className="h-7 gap-1 text-xs">
