    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          ...
          <Button size="sm" variant="outline" className="h-6 text-xs ml-1" onClick={() => setAdding(v => !v)}>
      ...
      {adding && (
        <div className="space-y-2">
          ...
          <div className="flex gap-2 justify-end">
            <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => { setAdding(false); setContent("") }}>
              Annuler
            </Button>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-2 max-h-60 overflow-y-auto">
          {filtered
            ...
            .map(note => (
              <div key={note.id} className="rounded-lg border bg-muted/30 px-3 py-2 text-sm space-y-1">
                <div className="flex items-start justify-between gap-2">
                  <p className="flex-1 whitespace-pre-wrap break-words">{note.content}</p>
                  <Button size="sm" variant="ghost" className="h-5 w-5 p-0 flex-shrink-0 text-muted-foreground hover:text-destructive" onClick={() => handleDelete(note.id)}>
                ...
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  <span>
                    {formatDistanceToNow(new Date(note.createdAt), { locale: fr, addSuffix: true })}
                  </span>
                </div>
              </div>
            ))}
