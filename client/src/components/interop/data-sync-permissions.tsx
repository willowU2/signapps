...
export function DataSyncRules() {
      ...
      setRules([
        {
          ...
          field_mapping: { email: "email", name: "full_name" },
          enabled: true,
        },
        {
          id: "r2",
          ...
          field_mapping: { title: "title", due_date: "start" },
          enabled: false,
        },
      ]);
    } finally {
  ...
  const toggle = async (rule: SyncRule) => {
    ...
    try {
      await identityClient().patch(`/sync/rules/${rule.id}`, {
        ...
        >
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-muted-foreground">
              {Object.keys(r.field_mapping).join(", ")}
            </p>
          </div>
...
export function PermissionInheritance({
  ...
  useEffect(() => {
    identityClient()
      .get<{ inherit: boolean }>(`/permissions/inherit`, {
        params: { entity_type: entityType, entity_id: entityId },
      })
      .then(({ data }) => setInherit(data.inherit))
      .catch(() => {});
  }, [entityType, entityId]);
  ...
  const toggle = async () => {
    ...
    try {
      ...
      toast.success(
        next ? "Héritage des permissions activé" : "Héritage désactivé",
      );
    } catch {
  ...
  return (
    <div className="flex items-center gap-3 p-2.5 rounded-lg border">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs font-medium">
          Hériter les permissions des entités liées
        </p>
        <p className="text-[10px] text-muted-foreground">
          Les accès partagés s'appliquent aux éléments liés
        </p>
      </div>
...
export function CrossModuleTemplates() {
  ...
  const load = useCallback(async () => {
    try {
      const { data } = await identityClient().get<CrossTemplate[]>(
        "/templates/cross-module",
      );
      setTemplates(data);
    } catch {
      setTemplates([
        {
          ...
          modules: ["tasks", "calendar", "contacts", "hr"],
          description: "Crée tâches + événements + fiche RH",
        },
        {
          id: "t2",
          name: "Nouveau client",
          modules: ["crm", "contacts", "mail", "tasks"],
          description:
            "CRM lead + contact + mail de bienvenue + tâches de suivi",
        },
        {
          id: "t3",
          name: "Lancement produit",
          modules: ["tasks", "calendar", "docs", "social"],
          description: "Plan projet + présentation + post social",
        },
      ]);
    } finally {
      setLoading(false);
  ...
  const apply = async (templateId: string) => {
    ...
    try {
      await identityClient().post(
        `/templates/cross-module/${templateId}/apply`,
        {},
      );
  ...
  const create = async () => {
    ...
    try {
      const { data } = await identityClient().post<CrossTemplate>(
        "/templates/cross-module",
        {
          name: newName,
          modules: ["tasks", "docs"],
