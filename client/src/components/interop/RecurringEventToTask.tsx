...
export function RecurringEventToTask({
  eventId,
  eventTitle,
  recurrenceFrequency,
  nextOccurrences = [],
  ...
  return (
    <div
      className={`flex items-center gap-2 rounded-lg border border-border/50 bg-muted/20 px-3 py-2 text-sm ${className}`}
    >
      <Repeat className="h-4 w-4 text-indigo-500 shrink-0" />
      <span className="flex-1 text-muted-foreground">
        Événement récurrent ({recurrenceFrequency}) — créer des tâches de
        préparation ?
      </span>
      {!done ? (
        <Button
