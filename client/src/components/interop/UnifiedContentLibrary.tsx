...
export function UnifiedContentLibrary() {
  ...
  const { data: driveNodes = [], isLoading: loadingDrive } = useQuery<
    DriveNode[]
  >({
  ...
  const mediaUrls: { url: string; name: string; postId: string }[] =
    posts.flatMap((p) =>
      (Array.isArray(p.mediaUrls) ? p.mediaUrls : []).map(
        (url: string, i: number) => ({
          url,
          name: `Media ${i + 1}`,
          postId: p.id,
  ...
  const filteredFiles = files.filter(
    (f) => !lower || f.name.toLowerCase().includes(lower),
  );
  const filteredPosts = posts.filter(
    (p) => !lower || p.content.toLowerCase().includes(lower),
  );
  const filteredMedia = mediaUrls.filter(
    (m) => !lower || m.name.toLowerCase().includes(lower),
  );
  ...
  return (
    <>
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent
          side="right"
          className="w-[420px] sm:w-[500px] flex flex-col"
        >
          <SheetHeader>
          ...
          <Tabs
            defaultValue="docs"
            className="flex-1 flex flex-col mt-3 min-h-0"
          >
            <TabsList className="grid grid-cols-4 h-8">
              <TabsTrigger value="docs" className="text-xs gap-1">
                <FileText className="h-3 w-3" />
                Docs
                <Badge variant="secondary" className="h-4 text-xs ml-0.5">
                  {filteredDocs.length}
                </Badge>
              </TabsTrigger>
            ...
            <TabsContent
              value="docs"
              className="flex-1 overflow-y-auto mt-2 space-y-1"
            >
              {loadingDrive && (
            ...
            <TabsContent
              value="social"
              className="flex-1 overflow-y-auto mt-2 space-y-2"
            >
              {loadingPosts && (
                <p className="text-xs text-muted-foreground text-center py-4">
                  Chargement...
