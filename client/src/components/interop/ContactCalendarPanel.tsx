...
export function ContactCalendarPanel({
  contactId,
  contactEmail,
  contactName,
  dealId,
      ...
      try {
        ...
        if (calId) {
          ...
          const filtered: CalendarEvent[] = eventsArr
            .filter((e) =>
              e.attendees?.some((a) =>
                ((a as { email?: string }).email ?? String(a))
                  .toLowerCase()
            ...
            .map((e) => ({
              ...
              attendees: e.attendees?.map(
                (a) => (a as { email?: string }).email ?? String(a),
              ),
              location: e.location,
              meetLink: (e as { meetLink?: string }).meetLink,
            }));
          setEvents(filtered);
        } else {
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide flex items-center gap-1">
          <Calendar className="h-3 w-3" /> Événements (
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          <p className="text-xs text-muted-foreground italic">
            Aucun événement avec ce contact.
          </p>
          {showScheduleButton && (
            <Button
              size="sm"
