...
export function ContactBirthdayCrmContext({ contact }: Props) {
  const [deals, setDeals] = useState<
    Awaited<ReturnType<typeof getDealsForContact>>
  >([]);
  ...
  return (
    <div
      className={`rounded-lg border p-3 space-y-2 ${isUpcoming ? "border-amber-300 bg-amber-50 dark:bg-amber-950/20" : "border-border bg-card"}`}
    >
      <div className="flex items-center gap-2">
        <Gift
          className={`h-4 w-4 ${isUpcoming ? "text-amber-500" : "text-muted-foreground"}`}
        />
      ...
      {isUpcoming && contact.email && (
        ...
        >
          <a
            href={`mailto:${contact.email}?subject=${encodeURIComponent(`Joyeux anniversaire ${contact.name.split(" ")[0]} !`)}`}
          >
