...
export function SmartSuggestions({
  ...
  useEffect(() => {
    ...
    aiClient()
      .post<Suggestion[]>("/suggest/next-action", {
          ...
          <CardContent className="p-2.5 flex items-center gap-2">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5 mb-0.5">
                ...
                <Badge
                  variant="outline"
                  className="text-[10px] h-3.5 px-1 text-violet-600 border-violet-300"
                >
...
export async function smartRoute(
  text: string,
): Promise<{ module: string; url: string; confidence: number }> {
  try {
    ...
    if (lower.includes("@") || lower.includes("email"))
      return { module: "mail", url: "/mail", confidence: 0.7 };
    if (lower.includes("réunion") || lower.includes("meeting"))
      return { module: "calendar", url: "/calendar", confidence: 0.8 };
    if (lower.includes("facture") || lower.includes("invoice"))
      return { module: "billing", url: "/billing", confidence: 0.75 };
    if (lower.includes("fichier") || lower.includes("document"))
      return { module: "drive", url: "/drive", confidence: 0.7 };
