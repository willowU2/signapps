...
export function NotificationProjectPrefs() {
  ...
  function toggleProject(id: string, enabled: boolean) {
    setPrefs((prev) =>
      prev.map((p) => (p.projectId === id ? { ...p, enabled } : p)),
    );
  }

  function setChannel(id: string, channel: NotifChannel) {
    setPrefs((prev) =>
      prev.map((p) => (p.projectId === id ? { ...p, channel } : p)),
    );
  }

  function toggleEvent(
    id: string,
    event: keyof ProjectNotifPrefs["events"],
    value: boolean,
  ) {
    setPrefs((prev) =>
      prev.map((p) =>
        p.projectId === id
          ? { ...p, events: { ...p.events, [event]: value } }
          : p,
      ),
    );
  }
    ...
    <Card>
      ...
      <CardContent className="space-y-3">
            ...
            <div className="flex items-center gap-2 p-2.5">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <Select
                  value={p.channel}
                  onValueChange={(v) =>
                    setChannel(p.projectId, v as NotifChannel)
                  }
                  disabled={!p.enabled}
                >
                  ...
                  <SelectContent>
                    {(Object.keys(CHANNEL_LABELS) as NotifChannel[]).map(
                      (c) => (
                ...
                <Switch
                  checked={p.enabled}
                  onCheckedChange={(v) => toggleProject(p.projectId, v)}
                />
              </div>
            </div>
            {p.enabled && (
              <div className="border-t bg-muted/20 px-3 py-2">
                <div className="flex flex-wrap gap-x-4 gap-y-1.5">
                  {(
                    Object.keys(p.events) as Array<
                      keyof ProjectNotifPrefs["events"]
                    >
                  ).map((ev) => (
        ...
        <div className="text-xs text-muted-foreground">
          {prefs.filter((p) => p.enabled).length} projet(s) avec notifications
          actives
        </div>
      </CardContent>
    </Card>
