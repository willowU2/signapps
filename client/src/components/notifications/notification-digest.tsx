  ...
  sections: [
    {
      ...
      count: 3,
      items: [
        "Jalon atteint: API JWT",
        "Risque: Sous-effectif QA",
        "Budget dépassé: Analytics",
      ],
    },
    {
      module: "RH",
      count: 2,
      items: [
        "Congé approuvé: Alice Martin (7-11 avr.)",
        "Nouvelle embauche: Marc Dubois",
      ],
    },
    {
      module: "Tâches",
      count: 2,
      items: ["2 tâches en retard", "4 tâches dues aujourd'hui"],
    },
  ],
};
...
export function NotificationDigest() {
  ...
  function updateModule(
    module: keyof DigestPreferences["includeModules"],
    value: boolean,
  ) {
    setPrefs((p) => ({
      ...p,
      includeModules: { ...p.includeModules, [module]: value },
    }));
  }

  function sendTestDigest() {
    toast.success("Digest de test envoyé", {
      description: "Vérifiez votre boîte email dans quelques instants.",
    });
  }
      ...
      <CardContent className="space-y-4">
        {prefs.enabled && (
          <>
              ...
              <div className="flex-1 space-y-1">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setPrefs((p) => ({
                      ...p,
                      frequency: v as DigestPreferences["frequency"],
                    }))
                  }
                >
            ...
            <div className="space-y-2">
              ...
              {(
                Object.keys(prefs.includeModules) as Array<
                  keyof typeof prefs.includeModules
                >
              ).map((mod) => (
                <div key={mod} className="flex items-center justify-between">
                  <Label className="text-xs capitalize">{mod}</Label>
                  <Switch
                    checked={prefs.includeModules[mod]}
                    onCheckedChange={(v) => updateModule(mod, v)}
                  />
                </div>
              ))}
            </div>
              ...
              <div className="rounded-lg border bg-muted/30 p-3 space-y-2 text-xs">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="secondary">
                    {PREVIEW_DATA.totalNotifications} notifications
                  </Badge>
                </div>
        ...
        {!prefs.enabled && (
          <p className="text-xs text-muted-foreground">
            Le digest email est désactivé. Activez-le pour recevoir un résumé
            régulier.
          </p>
        )}
      </CardContent>
