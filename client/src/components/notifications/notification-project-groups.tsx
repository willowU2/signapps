...
const DEMO_NOTIFS: ProjectNotification[] = [
  ...
  },
  {
    ...
    message: "Tests de charge assignés à Alice.",
    type: "task",
    read: false,
  ...
  },
  {
    ...
    message: "Retard prévu sur le composant graphique.",
    type: "risk",
    read: true,
  ...
  },
  {
    ...
    projectName: "Dashboard Analytics",
    title: "Membre ajouté",
    message: "Emma Leroy rejoint le projet.",
    type: "member",
    read: false,
...
export function NotificationProjectGroups({
  notifications = DEMO_NOTIFS,
}: NotificationProjectGroupsProps) {
  ...
  const groups = useMemo(() => {
    const map = new Map<
      string,
      { projectName: string; items: ProjectNotification[] }
    >();
    for (const n of notifications) {
      const g = map.get(n.projectId) ?? {
  ...
  function markAllRead(projectId: string) {
    ...
    if (group)
      setRead((prev) => new Set([...prev, ...group.items.map((n) => n.id)]));
  }
                  ...
                  {group.items.map((n) => (
                    <div
                      ...
                      className={cn(
                        ...
                        read.has(n.id) ? "opacity-60" : "bg-blue-50/30",
                      )}
                    >
                      <Bell
                        className={cn(
