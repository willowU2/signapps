...
function mapApiToNotification(record: NotificationRecord): Notification {
  ...
  return {
    ...
    message: record.recipient_address
      ? `Envoye a ${record.recipient_address}`
      : `Via ${record.channel}`,
...
function getLocalActivityNotifications(): Notification[] {
  ...
  try {
    const history = JSON.parse(
      localStorage.getItem("signapps-recent-history") || "[]",
    );
    if (history.length > 0) {
  ...
  localNotifs.push({
    ...
    message:
      "Votre espace de travail est pret. Explorez les modules depuis le menu lateral.",
    status: "info",
...
export function NotificationPopover() {
  ...
  const clearAll = () => {
    setNotifications([]);
    try {
      localStorage.removeItem("signapps-notifications-cache");
    } catch {}
  };
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        className="w-[380px] p-0 shadow-lg border-border/60"
        align="end"
      >
            ...
            <div className="divide-y">
              {notifications.map((notification) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${statusColorClass} shadow-sm border border-background/20`}
                    >
                    ...
                    <div className="flex-1 space-y-1.5 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-center gap-2 min-w-0">
                          <p
                            className={`text-sm font-medium leading-none truncate ${!notification.read ? "text-foreground" : "text-foreground/80"}`}
                          >
                            {notification.title}
                          </p>
                        </div>
