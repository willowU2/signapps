        ...
        <div className="flex flex-wrap gap-1">
          <span
            className={cn(
              ...
              priorityColors[
                (task.priority as keyof typeof priorityColors) || "medium"
              ],
            )}
          >
          ...
          {task.tags &&
            task.tags.slice(0, 2).map((tag) => (
              <span
                key={tag}
                className="text-[10px] uppercase font-semibold px-2 py-0.5 rounded-sm bg-secondary text-secondary-foreground"
              >
                {tag}
              </span>
            ))}
