...
const INIT_REVIEWERS: Reviewer[] = [
  {
    id: "1",
    name: "Alice Martin (vous)",
    role: "self",
    submitted: true,
    scores: { comm: 4, collab: 5, leader: 3, tech: 5, deliv: 4, innov: 4 },
    comment: "Bonne année, quelques axes d'amélioration en leadership.",
  },
  {
    id: "2",
    name: "Jean-Paul Directeur",
    role: "manager",
    submitted: true,
    scores: { comm: 4, collab: 4, leader: 4, tech: 5, deliv: 5, innov: 3 },
    comment:
      "Excellent contributeur technique, montée en leadership nécessaire.",
  },
...
function avgScore(reviewers: Reviewer[], criteriaId: string): number {
  ...
  return (
    submitted.reduce((s, r) => s + (r.scores![criteriaId] || 0), 0) /
    submitted.length
  );
}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Revue multi-évaluateurs avec scoring par critère
          </p>
        </div>
        <div className="flex items-center gap-2 text-sm">
          <CheckCircle className="w-4 h-4 text-green-600" />
          <span className="font-medium">
            {submittedCount}/{reviewers.length} évaluations soumises
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {categories.map((cat) => {
          ...
          const avg =
            catCriteria.reduce((s, c) => s + avgScore(reviewers, c.id), 0) /
            catCriteria.length;
          return (
        ...
        <div className="rounded-lg border bg-card overflow-hidden">
          <div className="bg-muted border-b px-4 py-3 flex items-center justify-between">
            <h3 className="font-semibold text-foreground flex items-center gap-2">
              <Users className="w-4 h-4" /> Évaluateurs
            </h3>
            ...
            {reviewers.map((r) => (
              ...
              >
                <div>
                  ...
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full font-medium ${ROLE_COLORS[r.role] || "bg-muted text-muted-foreground"}`}
                  >
          ...
          <div className="rounded-lg border bg-card overflow-hidden">
            <div className="bg-blue-50 border-b px-4 py-3">
              ...
              <p className="text-xs text-muted-foreground">
                {reviewers.find((r) => r.id === activeReviewer)?.name}
              </p>
            </div>
