...
const TASK_TEMPLATES: Omit<OnboardingTask, "id" | "completed">[] = [
  ...
  {
    title: "Créer le compte email",
    category: "it",
    dueDay: -1,
    assignee: "IT",
  },
  {
    title: "Configurer VPN & accès",
    category: "access",
    dueDay: 1,
    assignee: "IT",
  },
  {
    title: "Badge et accès physique",
    category: "access",
    dueDay: 1,
    assignee: "RH",
  },
  {
    title: "Remise du matériel (pc, téléphone)",
    category: "equipment",
    dueDay: 1,
    assignee: "IT",
  },
  ...
  {
    title: "Formation politique de sécurité",
    category: "training",
    dueDay: 3,
    assignee: "RH",
  },
  ...
  {
    title: "Entretien RH d'intégration",
    category: "meeting",
    dueDay: 7,
    assignee: "RH",
  },
  ...
  {
    title: "Signature contrat et documents légaux",
    category: "admin",
    dueDay: 1,
    assignee: "RH",
  },
  {
    title: "Mutuelle & avantages",
    category: "admin",
    dueDay: 7,
    assignee: "RH",
  },
...
export function OnboardingChecklist() {
  ...
  const filtered =
    filterCategory === "all"
      ? hire.tasks
      : hire.tasks.filter((t) => t.category === filterCategory);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Checklist de tâches par embauche
          </p>
        </div>
        <button
      ...
      <div className="flex gap-2 flex-wrap">
        {hires.map((h) => (
          <button
            key={h.id}
            onClick={() => setActiveHire(h.id)}
            className={`px-4 py-2 rounded-lg border text-sm font-medium transition-colors ${activeHire === h.id ? "bg-blue-600 text-white border-blue-600" : "bg-card hover:bg-muted"}`}
          >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewHireForm((f) => ({ ...f, name: e.target.value }))
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewHireForm((f) => ({ ...f, position: e.target.value }))
                }
                className="w-full border rounded px-3 py-2 text-sm"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewHireForm((f) => ({ ...f, startDate: e.target.value }))
                }
                className="w-full border rounded px-3 py-2 text-sm"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewHireForm((f) => ({ ...f, department: e.target.value }))
                }
                className="w-full border rounded px-3 py-2 text-sm"
              />
            </div>
        ...
        <div className="flex items-center justify-between mb-2">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {hire.position} · {hire.department} · Arrivée : {hire.startDate}
            </p>
          </div>
          <div className="text-right">
            <p className="text-2xl font-bold text-blue-600">{progress}%</p>
            <p className="text-xs text-muted-foreground">
              {completedCount}/{hire.tasks.length} tâches
            </p>
          </div>
        </div>
      ...
      <div className="flex gap-2 flex-wrap">
        {(["all", ...Object.keys(CATEGORY_LABELS)] as const).map((cat) => (
          <button
            ...
            className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${filterCategory === cat ? "bg-gray-800 text-white" : cat === "all" ? "bg-muted hover:bg-gray-200 text-muted-foreground" : `${CATEGORY_COLORS[cat]} hover:opacity-80`}`}
          >
        ...
        {filtered.map((task) => (
          ...
          >
            ...
            <div className="flex-1">
              <p
                className={`text-sm font-medium ${task.completed ? "line-through text-gray-400" : "text-foreground"}`}
              >
                {task.title}
              </p>
              <div className="flex items-center gap-3 mt-0.5">
                <span
                  className={`text-xs px-1.5 py-0.5 rounded font-medium ${CATEGORY_COLORS[task.category]}`}
                >
                  {CATEGORY_LABELS[task.category]}
                </span>
                <span className="text-xs text-muted-foreground flex items-center gap-1">
                  <Calendar className="w-3 h-3" /> J{task.dueDay > 0 ? "+" : ""}
                  {task.dueDay}
                </span>
