...
const PROJECTS = ["SignApps Platform", "Support client", "Administration", "Formation", "R&D"];
...
export function WeeklyTimesheet() {
  ...
  const handleChange = (project: string, day: string, val: string) => {
    ...
    setEntries(prev => ({ ...prev, [project]: { ...prev[project], [day]: Math.min(24, Math.max(0, num)) } }));
    setSaved(false);
  };

  const getDayTotal = (day: string) => PROJECTS.reduce((s, p) => s + (entries[p]?.[day] || 0), 0);
  const getProjectTotal = (project: string) => DAYS.reduce((s, d) => s + (entries[project]?.[d] || 0), 0);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleSave} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${saved ? "bg-green-100 text-green-700" : "bg-blue-600 hover:bg-blue-700 text-white"}`}>
      ...
      <div className="rounded-lg border bg-blue-50 p-4 flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-medium text-blue-900">Total semaine : <span className="font-bold">{weekTotal.toFixed(1)}h</span></p>
        ...
        <div className="ml-auto flex-1 max-w-xs">
          <div className="h-2 rounded-full bg-blue-200">
            <div className="h-full rounded-full bg-blue-600 transition-all" style={{ width: `${Math.min(100, (weekTotal / 37.5) * 100)}%` }} />
          </div>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
                ...
                {DAYS.map(day => (
                  <td key={day} className="px-2 py-2 text-center">
                    <input
                      ...
                      onChange={e => handleChange(project, day, e.target.value)}
          ...
          <tfoot>
            <tr className="bg-muted font-bold border-t-2">
              ...
              <td className="px-4 py-3 text-center text-blue-700">{weekTotal.toFixed(1)}h</td>
            </tr>
          </tfoot>
        </table>
      </div>
