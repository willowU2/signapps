...
import { GraduationCap, AlertTriangle, CheckCircle, Clock, Plus, X } from "lucide-react";
...
const CERTS: Certification[] = [
  { id: "1", employeeName: "Alice Martin", certName: "AWS Solutions Architect", provider: "Amazon", issueDate: "2024-03-01", expiryDate: "2026-03-01", status: "expiring", category: "Cloud", mandatory: false },
  { id: "2", employeeName: "Bob Dupont", certName: "CKA - Kubernetes Admin", provider: "CNCF", issueDate: "2023-06-15", expiryDate: "2025-06-15", status: "expired", category: "DevOps", mandatory: true },
  { id: "3", employeeName: "Claire Bernard", certName: "Sécurité incendie", provider: "INRS", issueDate: "2025-01-10", expiryDate: "2027-01-10", status: "valid", category: "Sécurité", mandatory: true },
  { id: "4", employeeName: "David Petit", certName: "Expert-Comptable", provider: "OEC", issueDate: "2018-09-01", status: "valid", category: "Finance", mandatory: true },
  { id: "5", employeeName: "Emma Leroy", certName: "UX Design Certificate", provider: "NN/g", issueDate: "2025-11-01", expiryDate: "2028-11-01", status: "valid", category: "Design", mandatory: false },
  { id: "6", employeeName: "Marc Dubois", certName: "Rust Systems Programming", provider: "Linux Foundation", issueDate: "2025-02-15", status: "in_progress", category: "Dev", mandatory: false },
];

const TRAININGS: Training[] = [
  { id: "t1", title: "RGPD & Protection des données", provider: "CNIL", duration: "1 jour", category: "Conformité", dueDate: "2026-06-30", enrolledCount: 8, maxCapacity: 12, status: "open" },
  { id: "t2", title: "Sécurité au bureau", provider: "INRS", duration: "Demi-journée", category: "Sécurité", dueDate: "2026-04-30", enrolledCount: 12, maxCapacity: 12, status: "full" },
  { id: "t3", title: "Management d'équipe distante", provider: "RH Consultant", duration: "2 jours", category: "Management", dueDate: "2026-07-15", enrolledCount: 5, maxCapacity: 10, status: "open" },
  { id: "t4", title: "Next.js 16 Advanced", provider: "Vercel", duration: "3 jours", category: "Dev", dueDate: "2026-05-15", enrolledCount: 4, maxCapacity: 8, status: "open" },
];

const STATUS_CONFIG: Record<CertStatus, { label: string; icon: React.ReactNode; color: string }> = {
  valid: { label: "Valide", icon: <CheckCircle className="w-4 h-4 text-green-600" />, color: "bg-green-100 text-green-700" },
  expiring: { label: "Expire bientôt", icon: <Clock className="w-4 h-4 text-amber-600" />, color: "bg-amber-100 text-amber-700" },
  expired: { label: "Expiré", icon: <AlertTriangle className="w-4 h-4 text-red-600" />, color: "bg-red-100 text-red-700" },
  in_progress: { label: "En cours", icon: <Clock className="w-4 h-4 text-blue-600" />, color: "bg-blue-100 text-blue-700" },
};
...
export function TrainingCertificationManagement() {
  ...
  const filtered = certFilter === "all" ? certs : certs.filter(c => c.status === certFilter);
              ...
              <tbody className="divide-y">
                {filtered.map(cert => (
                  <tr key={cert.id} className={`hover:bg-gray-50 ${cert.status === "expired" ? "bg-red-50/50" : cert.status === "expiring" ? "bg-amber-50/50" : ""}`}>
                    ...
                    <td className="px-4 py-3 text-center">{cert.mandatory ? <CheckCircle className="w-4 h-4 text-blue-600 mx-auto" /> : <span className="text-gray-300">—</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
