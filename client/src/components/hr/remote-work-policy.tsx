...
const INIT_POLICIES: EmployeePolicy[] = [
  {
    ...
    daysRemote: 3,
    daysOffice: 2,
    remoteWeekdays: ["Lun", "Mar", "Mer"],
    officeWeekdays: ["Jeu", "Ven"],
    notes: "",
  },
  {
    id: "2",
    ...
    daysRemote: 4,
    daysOffice: 1,
    remoteWeekdays: ["Lun", "Mar", "Jeu", "Ven"],
    officeWeekdays: ["Mer"],
    notes: "Infrastructure critique, présence mer. obligatoire",
  },
  {
    id: "3",
    name: "Claire Bernard",
    ...
    daysOffice: 4,
    remoteWeekdays: ["Mer"],
    officeWeekdays: ["Lun", "Mar", "Jeu", "Ven"],
    notes: "",
  },
  {
    id: "4",
    ...
    daysRemote: 2,
    daysOffice: 3,
    remoteWeekdays: ["Lun", "Ven"],
    officeWeekdays: ["Mar", "Mer", "Jeu"],
    notes: "",
  },
  {
    id: "5",
    ...
    daysRemote: 3,
    daysOffice: 2,
    remoteWeekdays: ["Lun", "Mar", "Ven"],
    officeWeekdays: ["Mer", "Jeu"],
    notes: "",
  },
];
...
export function RemoteWorkPolicy() {
    ...
    setPolicies((prev) =>
      prev.map((p) => {
        ...
        if (type === "remote") {
          ...
          const newRemote = has
            ? p.remoteWeekdays.filter((d) => d !== day)
            : [...p.remoteWeekdays, day];
          const newOffice = p.officeWeekdays.filter((d) => d !== day);
          if (!has) newOffice.splice(newOffice.indexOf(day), 1);
          return {
            ...p,
            remoteWeekdays: newRemote,
            officeWeekdays: WEEKDAYS.filter((d) => !newRemote.includes(d)),
            daysRemote: newRemote.length,
            daysOffice: WEEKDAYS.length - newRemote.length,
          };
        }
        return p;
      }),
  ...
  const avgRemote =
    Math.round(
      (policies.reduce((s, p) => s + p.daysRemote, 0) / policies.length) * 10,
    ) / 10;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Configuration bureau/télétravail par employé
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="rounded-lg border bg-orange-50 p-4">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              min={0}
              max={5}
              className="w-16 border rounded px-2 py-1 text-sm font-bold text-orange-900 bg-transparent"
            />
        ...
        <div className="rounded-lg border bg-green-50 p-4">
          ...
          <p className="text-2xl font-bold text-green-900">
            {policies.filter((p) => p.daysRemote <= globalMax).length}/
            {policies.length}
          </p>
        </div>
      </div>
      ...
      <div className="rounded-lg border bg-amber-50 p-4 flex items-start gap-3">
        ...
        <p className="text-sm text-amber-800">
          Sélectionnez les jours de télétravail pour chaque employé. Les jours
          non sélectionnés sont automatiquement comptés comme jours au bureau.
          Cliquez sur un employé pour modifier sa configuration.
        </p>
      </div>
      ...
      <div className="space-y-3">
        {policies.map((emp) => {
          ...
          return (
            <div
              key={emp.id}
              className={`rounded-lg border bg-card p-4 transition-all ${overLimit ? "border-red-300 bg-red-50/30" : ""}`}
            >
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {overLimit && (
                      <span className="text-xs bg-red-100 text-red-700 px-2 py-0.5 rounded-full font-medium">
                        Dépasse la politique ({emp.daysRemote}j)
                      </span>
                    )}
                    {isSaved && (
                    ...
                    {WEEKDAYS.map((day) => {
                      ...
                      return (
                        <button
                          key={day}
                          onClick={() =>
                            isEditing && handleToggleDay(emp.id, day, "remote")
                          }
                          disabled={!isEditing}
                ...
                <div className="flex gap-2 flex-shrink-0">
                  {isEditing ? (
                    ...
                    <button
                      ...
                      className="border hover:bg-muted px-3 py-1.5 rounded-lg text-sm font-medium text-muted-foreground"
                    >
                      Modifier
                    </button>
                  )}
