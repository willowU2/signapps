...
export function AvailableResources({
  ...
  const resources = useMemo(() => {
    return DEMO_RESOURCES.filter((e) => {
      ...
      .map((e) => {
        ...
        const skillMatch =
          requiredSkills.length > 0
            ? (requiredSkills.filter((s) => e.skills.includes(s)).length /
  ...
  function handleAssign(emp: (typeof resources)[0]) {
    ...
    toast.success(`${emp.name} assigné(e) au projet`, {
      description: `${emp.capacity}% de capacité disponible.`,
    });
  }
        ...
        {requiredSkills.length > 0 && (
          <div className="flex items-center gap-1.5 flex-wrap text-[10px] text-muted-foreground">
            ...
            {requiredSkills.map((s) => (
              <Badge key={s} variant="outline" className="text-[10px] py-0 h-4">
                {s}
              </Badge>
            ))}
          </div>
            ...
            >
              ...
              <div className="min-w-0 flex-1 space-y-1">
                <div className="flex items-center gap-1.5">
                  ...
                  {requiredSkills.length > 0 && (
                    <Badge
                      variant={emp.skillMatch >= 70 ? "default" : "secondary"}
              ...
              >
                {assigned.has(emp.id) ? (
                  ...
                  <>
                    <UserPlus className="size-3" /> Assigner
                  </>
                )}
              </Button>
            </div>
