      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Demandes, approbations et soldes
          </p>
        </div>
        <button
        ...
        {LEAVE_BALANCES.map((b) => (
          <div key={b.type} className="rounded-lg border bg-card p-4">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              {b.used} utilisés · {b.pending} en attente
            </p>
          </div>
        ))}
      </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                value={form.type}
                onChange={(e) =>
                  setForm((f) => ({ ...f, type: e.target.value as LeaveType }))
                }
          ...
          <div className="flex gap-1">
            {(["all", "pending", "approved", "rejected"] as const).map((s) => (
              <button
                ...
                className={`px-3 py-1 rounded-md text-xs font-medium transition-colors ${filterStatus === s ? "bg-blue-600 text-white" : "text-muted-foreground hover:bg-muted"}`}
              >
                {s === "all"
                  ? "Tous"
                  : s === "pending"
        ...
        <div className="divide-y">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  {r.startDate} → {r.endDate} · {r.days} jour
                  {r.days > 1 ? "s" : ""}
                </p>
                {r.reason && (
          ...
          {filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <h3 className="text-lg font-semibold">Aucune demande de conge</h3>
              <p className="text-sm text-muted-foreground mt-1 max-w-sm">
                Les demandes de conges apparaitront ici.
              </p>
            </div>
          )}
        </div>
