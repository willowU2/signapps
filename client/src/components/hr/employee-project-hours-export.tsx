...
export function EmployeeProjectHoursExport() {
  ...
  const totalHours = filtered.reduce(
    (acc, r) => acc + r.regularHours + r.overtimeHours,
    0,
  );
  ...
  function handleExport() {
    ...
    toast.success(`Export réussi`, {
      description: `${filtered.length} lignes exportées en CSV.`,
    });
  }
            ...
            <tbody className="divide-y">
              {filtered.map((row, i) => (
                <tr key={i} className="hover:bg-muted/30">
                  ...
                  <td className="px-2 py-1.5 truncate max-w-[80px] text-muted-foreground">
                    {row.projectName.split(" ")[0]}
                  </td>
            ...
            <tfoot className="bg-muted/30 font-medium">
              <tr>
                <td colSpan={2} className="px-2 py-1.5 text-xs">
                  Total ({filtered.length} lignes)
                </td>
                <td colSpan={2} className="px-2 py-1.5 text-right text-xs">
                  {totalHours}h
                </td>
                <td className="px-2 py-1.5 text-right text-xs text-blue-600">
                  {totalBillable}h
                </td>
              </tr>
            </tfoot>
