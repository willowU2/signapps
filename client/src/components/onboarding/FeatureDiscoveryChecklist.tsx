...
const FEATURES: Feature[] = [
  { id: 'send_mail', label: 'Envoyer un email', description: 'Composez et envoyez votre premier email', href: '/mail', xp: 10 },
  { id: 'create_doc', label: 'Créer un document', description: 'Créez un document collaboratif', href: '/docs', xp: 15 },
  { id: 'schedule_event', label: 'Planifier un événement', description: 'Ajoutez un événement au calendrier', href: '/calendar', xp: 10 },
  { id: 'upload_file', label: 'Uploader un fichier', description: 'Déposez un fichier dans Drive', href: '/drive', xp: 10 },
  { id: 'start_chat', label: 'Démarrer une conversation', description: 'Envoyez un message à un collègue', href: '/chat', xp: 5 },
  { id: 'create_task', label: 'Créer une tâche', description: 'Ajoutez votre première tâche', href: '/tasks', xp: 5 },
  { id: 'invite_user', label: 'Inviter un utilisateur', description: 'Ajoutez un membre à votre espace', href: '/admin/users', xp: 20 },
  { id: 'customize_theme', label: 'Personnaliser le thème', description: 'Changez les couleurs et l\'apparence', href: '/settings/preferences', xp: 5 },
];
      ...
      <CollapsibleContent>
        <div className="border-t divide-y">
          {FEATURES.map(f => {
            ...
            return (
              <div key={f.id} className={`flex items-center gap-3 px-3 py-2 transition-colors ${isDone ? 'bg-muted/20' : 'hover:bg-muted/30'}`}>
                <button
                  onClick={() => markDone(f.id)}
                  disabled={isDone}
                  className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${
                    isDone ? 'border-primary bg-primary text-primary-foreground' : 'border-muted-foreground/30 hover:border-primary'
                  }`}
                >
        ...
        {completed.length === FEATURES.length && (
          <div className="p-3 bg-primary/5 text-center">
            <p className="text-sm font-medium text-primary">Bravo ! Vous avez tout exploré 🎉</p>
          </div>
        )}
      </CollapsibleContent>
    </Collapsible>
