...
import { HelpCircle, X, ExternalLink, BookOpen, ChevronRight } from 'lucide-react';
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className="w-80 p-0" align="end">
          ...
          <ScrollArea className="max-h-64 p-3">
            ...
            {selected.href && (
              <a href={selected.href} target="_blank" rel="noopener noreferrer" className="mt-3 flex items-center gap-1 text-xs text-primary hover:underline">
      ...
      <PopoverContent className="w-72 p-0" align="end">
        <div className="p-3 border-b flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <p className="font-medium text-sm">Aide{context ? ` — ${context}` : ''}</p>
          </div>
          <Button size="sm" variant="ghost" className="h-6 w-6 p-0" onClick={() => setOpen(false)}>
            <X className="w-3.5 h-3.5" />
          </Button>
          ...
          {articles.map(article => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground line-clamp-1 mt-0.5">{article.content}</p>
              </div>
...
const GLOBAL_ARTICLES: HelpArticle[] = [
  { id: '1', title: 'Premiers pas', content: 'Découvrez comment configurer votre espace de travail SignApps en quelques minutes.', tags: ['démarrage'] },
  { id: '2', title: 'Gérer les utilisateurs', content: 'Ajoutez des membres, créez des groupes et gérez les permissions via la section Admin.', tags: ['admin'] },
  { id: '3', title: 'Créer un document', content: 'Accédez à Docs et cliquez sur "Nouveau". Vous pouvez collaborer en temps réel.', tags: ['docs'] },
  { id: '4', title: 'Configurer la messagerie', content: 'Allez dans Paramètres > Mail pour configurer votre serveur SMTP/IMAP.', tags: ['mail'] },
  { id: '5', title: 'Partager des fichiers', content: 'Dans Drive, cliquez droit sur un fichier pour le partager avec des membres ou des groupes.', tags: ['drive'] },
  { id: '6', title: 'Raccourcis clavier', content: 'Ctrl+K pour la recherche globale, Ctrl+/ pour les raccourcis, Ctrl+N pour nouveau.', tags: ['clavier'] },
];
...
export function ResourceCenter() {
  ...
  const filtered = search
    ? GLOBAL_ARTICLES.filter(a => a.title.toLowerCase().includes(search.toLowerCase()) || a.content.toLowerCase().includes(search.toLowerCase()))
    : GLOBAL_ARTICLES;
