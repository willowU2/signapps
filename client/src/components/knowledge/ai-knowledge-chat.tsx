...
export function AiKnowledgeChat({
  ...
  compact = false,
}: AiKnowledgeChatProps) {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "welcome",
      role: "assistant",
      content:
        "Bonjour ! Je suis votre assistant de connaissances. Posez-moi des questions sur vos documents internes.",
    },
  ]);
      ...
      setMessages((prev) =>
        prev
          ...
          .concat({
            ...
            role: "assistant",
            content:
              "Désolé, une erreur s'est produite. Vérifiez que le service IA est disponible.",
          }),
      );
    } finally {
      setLoading(false);
  ...
  return (
    <div
      className={`flex flex-col ${compact ? "h-80" : "h-full min-h-[400px]"}`}
    >
      {/* Header */}
      {!compact && (
        ...
        <div className="space-y-3">
          {messages.map((msg) => (
            ...
            >
              ...
              <div
                className={`max-w-[80%] space-y-1.5 ${msg.role === "user" ? "items-end" : "items-start"} flex flex-col`}
              >
                <div
