...
export function MeetInfoCard({ roomId, onClose }: MeetInfoCardProps) {
  const meetingLink =
    typeof window !== "undefined"
      ? `${window.location.origin}/meet?room=${roomId}`
      : `https://meet.signapps.com/${roomId}`;
  ...
  return (
    <div className="absolute top-4 left-4 z-20 w-[360px] bg-background dark:bg-[#1f1f1f] rounded-xl shadow-lg border border-[#e2e2e2] dark:border-[#ffffff1a] overflow-hidden flex flex-col p-6 animate-in fade-in slide-in-from-left-4 duration-300">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-xl font-medium text-[#1f1f1f] dark:text-[#e8eaed]">
      ...
      <p className="text-sm text-[#444746] dark:text-[#9aa0a6] mb-4">
        Vous pouvez également partager ces informations de connexion avec les
      ...
      <div className="flex items-center justify-between mb-4 bg-muted dark:bg-[#28292a] p-3 rounded-md">
        <span className="text-sm text-[#1f1f1f] dark:text-[#e8eaed] font-medium truncate mr-2">
          {meetingLink}
        </span>
        <Button
          variant="ghost"
      ...
      <Button
        variant="link"
        className="text-[#0b57d0] justify-start p-0 h-auto mb-6 text-sm"
      >
      ...
      <Button
        variant="link"
        className="text-[#0b57d0] justify-start p-0 h-auto mb-4 text-sm flex items-center"
      >
