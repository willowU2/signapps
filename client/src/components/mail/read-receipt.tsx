...
export function injectTrackingPixel(html: string, emailId: string, trackingBaseUrl: string): string {
...
export function ReadReceiptIndicator({ status, size = "sm" }: ReadReceiptIndicatorProps) {
    ...
    if (status.read) {
        return (
            <Tooltip>
                ...
                <TooltipContent>
                    Read {status.readAt ? format(new Date(status.readAt), "MMM d 'at' HH:mm") : ""}
                    {status.viewCount && status.viewCount > 1 ? ` (${status.viewCount} views)` : ""}
                </TooltipContent>
            </Tooltip>
        )
    }
...
export function ReadReceiptToggle({ enabled, onToggle }: ReadReceiptToggleProps) {
    return (
        <div className="flex items-center gap-3 py-2">
            <Switch id="read-receipt" checked={enabled} onCheckedChange={onToggle} />
            <Label htmlFor="read-receipt" className="flex items-center gap-2 cursor-pointer text-sm">
                {enabled ? <Eye className="h-4 w-4 text-blue-500" /> : <EyeOff className="h-4 w-4 text-muted-foreground" />}
                Read receipt
                {enabled && (
...
export function SentEmailRowStatus({ emailId, sentAt, receiptsEnabled }: SentEmailRowStatusProps) {
