...
export function AiCategorizerPanel() {
  ...
  const [activeCategory, setActiveCategory] = useState<EmailCategory | null>(
    null,
  );
  ...
  const runCategorization = useCallback(async () => {
    ...
    try {
      ...
      const result = (await mailApi.categorizeInbox({
        settings,
      })) as unknown as { stats: CategoryStats[]; total: number };
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          >
            <RefreshCw
              className={cn("h-3.5 w-3.5 mr-1", running && "animate-spin")}
            />
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              <Label htmlFor="cat-auto" className="text-sm">
                Auto-apply (no confirmation)
              </Label>
              <Switch
                id="cat-auto"
                checked={settings.auto_apply}
                onCheckedChange={(v) => saveSettings({ auto_apply: v })}
              />
            </div>
          </div>
          ...
          {stats.length > 0 && (
            <div className="space-y-2 pt-2 border-t">
              <p className="text-xs font-medium text-muted-foreground">
                Distribution ({totalEmails} emails)
              </p>
              {CATEGORIES.map((cat) => {
                ...
                const pct =
                  totalEmails > 0
                    ? Math.round((s.count / totalEmails) * 100)
                    : 0;
                return (
                  <button
                    key={cat.id}
