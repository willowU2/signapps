...
function actionLabel(action: RuleAction): string {
  switch (action.type) {
    case "move_to": return `Déplacer → ${action.folder}`;
    case "label": return `Étiquette: ${action.tag}`;
    case "forward": return `Transférer → ${action.email}`;
    case "mark_read": return "Marquer lu";
...
export function RulesEditor() {
  ...
  const [newConditions, setNewConditions] = useState<RuleCondition[]>([emptyCondition()]);
  ...
  const saveRule = async () => {
    ...
    try {
      const res = await fetch("/api/mail/rules", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: newName, conditions: newConditions, actions: newActions }),
      });
      ...
      {rules.map((rule) => (
        <Card key={rule.id}>
          ...
          <CardContent className="py-2 px-4 text-xs text-muted-foreground space-y-1">
            <p>Conditions : {rule.conditions.map((c) => `${c.field} ${c.operator} "${c.value}"`).join(", ")}</p>
        ...
        <Card>
          <CardContent className="pt-4 space-y-4">
            <div className="space-y-1.5">
              ...
              <Input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Ma règle" />
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Button variant="ghost" size="sm" onClick={() => setNewConditions((p) => [...p, emptyCondition()])}>
              ...
              {newConditions.map((cond, i) => (
                <div key={i} className="flex gap-2 flex-wrap">
                  <Select value={cond.field} onValueChange={(v) => {
                    setNewConditions((p) => p.map((c, idx) => idx === i ? { ...c, field: v as RuleCondition["field"] } : c));
                  }}>
                  ...
                  <Select value={cond.operator} onValueChange={(v) => {
                    setNewConditions((p) => p.map((c, idx) => idx === i ? { ...c, operator: v as RuleCondition["operator"] } : c));
                  }}>
                    <SelectTrigger className="w-[130px]"><SelectValue /></SelectTrigger>
                    <SelectContent>{CONDITION_OPERATORS.map((o) => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}</SelectContent>
                  </Select>
                  <Input className="w-[160px]" value={cond.value} onChange={(e) => {
                    setNewConditions((p) => p.map((c, idx) => idx === i ? { ...c, value: e.target.value } : c));
                  }} placeholder="valeur" />
                  {newConditions.length > 1 && (
                    <Button variant="ghost" size="icon" className="h-9 w-9" onClick={() => setNewConditions((p) => p.filter((_, idx) => idx !== i))}>
                      <Trash2 className="h-4 w-4" />
                    </Button>
                  )}
                </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label>Actions</Label>
                <Button variant="ghost" size="sm" onClick={() => setNewActions((p) => [...p, emptyAction()])}>
                  <Plus className="h-3 w-3 mr-1" /> Ajouter
                </Button>
              </div>
              {newActions.map((action, i) => (
                <div key={i} className="flex items-center gap-2">
                  <ActionEditor action={action} onChange={(a) => setNewActions((p) => p.map((x, idx) => idx === i ? a : x))} />
                  {newActions.length > 1 && (
                    <Button variant="ghost" size="icon" className="h-9 w-9" onClick={() => setNewActions((p) => p.filter((_, idx) => idx !== i))}>
                      <Trash2 className="h-4 w-4" />
                    </Button>
                  )}
                </div>
            ...
            <div className="flex gap-2 justify-end">
              <Button variant="outline" onClick={() => { setCreating(false); setNewName(""); setNewConditions([emptyCondition()]); setNewActions([emptyAction()]); }}>
                Annuler
              </Button>
