...
> = {
  ...
  text: {
    ...
    label: "Text",
    defaultContent: {
      body: "Write your content here. Tell your story, share updates, or provide information.",
    },
  },
  image: {
    icon: <Image className="h-4 w-4" />,
    label: "Image",
    defaultContent: { src: "", alt: "Image description", width: "100%" },
  },
  button: {
  ...
  divider: {
    ...
    label: "Divider",
    defaultContent: { color: "#e0e0e0", margin: "16px" },
  },
  footer: {
    icon: <AlignLeft className="h-4 w-4" />,
    label: "Footer",
...
function renderBlockHtml(block: Block): string {
  ...
  switch (type) {
    ...
    case "image":
      return content.src
        ? `<table width="100%" style="padding:16px 32px;"><tr><td style="text-align:center;"><img src="${content.src}" alt="${content.alt}" style="max-width:${content.width};height:auto;" /></td></tr></table>`
        : "";
    case "button":
    ...
    <div className="border border-border rounded-xl p-3 space-y-2 bg-background group">
      ...
      {type === "header" && (
        <div className="grid grid-cols-2 gap-2">
          ...
          <div>
            ...
            <Input
              type="color"
              className="h-7 mt-1 p-0.5 w-full"
              value={content.bg}
      ...
      {type === "footer" && (
        <div className="space-y-2">
          ...
          <div>
            <Label className="text-xs">Unsubscribe label</Label>
            <Input
              className="h-7 text-xs mt-1"
              value={content.unsubscribe}
              onChange={(e) => updateContent("unsubscribe", e.target.value)}
            />
          </div>
        </div>
      )}
  ...
  const [blocks, setBlocks] = useState<Block[]>([
    {
      id: generateId(),
      type: "header",
      content: { ...BLOCK_TEMPLATES.header.defaultContent },
    },
    {
      id: generateId(),
      type: "text",
      content: { ...BLOCK_TEMPLATES.text.defaultContent },
    },
    {
      id: generateId(),
      type: "button",
      content: { ...BLOCK_TEMPLATES.button.defaultContent },
    },
    {
      id: generateId(),
      type: "footer",
      content: { ...BLOCK_TEMPLATES.footer.defaultContent },
    },
  ]);
          ...
          <div className="space-y-2">
            ...
            {blocks.length === 0 && (
              <div className="text-center py-12 text-muted-foreground border-2 border-dashed rounded-xl">
                <p className="text-sm">
                  Add blocks from the palette to build your newsletter
                </p>
              </div>
            )}
          </div>
