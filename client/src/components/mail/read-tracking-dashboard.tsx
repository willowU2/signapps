...
export function ReadTrackingDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRecords(recs as unknown as TrackingRecord[]);
      setStats(agg as unknown as AggregateStats);
    } catch {
      ...
      {stats && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            ...
            {
              label: "Avg Opens",
              value: stats.avg_opens_per_email.toFixed(1),
              icon: <Eye className="h-4 w-4" />,
            },
        ...
        <CardContent className="p-0">
          <ScrollArea className="h-72">
            ...
            {records.map((r) => (
              <div
                key={r.email_id}
                className="flex items-start gap-3 px-4 py-3 border-b last:border-0 hover:bg-muted/30"
              >
                <div
                  className={cn(
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-xs text-muted-foreground">
                      Sent{" "}
                      {formatDistanceToNow(new Date(r.sent_at), {
                        addSuffix: true,
                      })}
                    </span>
                    {r.opened && r.first_open_at && (
                      <span className="text-xs text-green-600">
                        Opened{" "}
                        {formatDistanceToNow(new Date(r.first_open_at), {
                          addSuffix: true,
                        })}
                      </span>
                    )}
                  ...
                  {r.open_count > 0 && (
                    <div className="flex items-center gap-2 mt-1.5">
                      <Progress
                        value={Math.min(r.open_count * 20, 100)}
                ...
                <Badge
                  variant={r.opened ? "default" : "secondary"}
