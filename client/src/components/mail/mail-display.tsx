...
function detectActions(text: string): string[] {
  ...
  // Date patterns
  const dateRe =
    /\b(le \d{1,2}\s+\w+|vendredi prochain|lundi prochain|mardi prochain|mercredi prochain|jeudi prochain|samedi prochain|avant le|deadline\s*:\s*[\w\s]+)/gi;
...
const FRENCH_WORDS =
  /\b(bonjour|merci|veuillez|cordialement|vous|nous|les|des|et|est|avec|pour|dans|sur|par|que|qui|mais|donc|car)\b/gi;
const GERMAN_WORDS =
  /\b(bitte|danke|sehr|hallo|guten|mit|von|und|die|der|das|sie|ich|für|auf|nicht|auch|ist)\b/gi;
const SPANISH_WORDS =
  /\b(hola|gracias|favor|buenos|estimado|para|con|los|las|una|que|por|como|este|tiene|puede)\b/gi;
...
function detectUnsubscribe(html: string): string | null {
  const re =
    /https?:\/\/[^\s"'<>]+(?:unsubscribe|se-d%C3%A9sabonner|desinscrire|désabonner|opt.?out)[^\s"'<>]*/i;
...
export function MailDisplay({
  ...
  const unsubscribeUrl = mail
    ? detectUnsubscribe(mail.body_html || mail.text || "")
    : null;
  ...
  useEffect(() => {
    ...
    contactsApi
      ...
      .catch(() => {
        /* silent — CRM context is optional */
      });
  }, [mail?.email]);
  ...
  const handleSummarize = useCallback(async () => {
    ...
    await stream(
      ...
      {
        systemPrompt:
          "You are a professional email assistant. Provide a clear, concise summary. Use bullet points for key takeaways.",
        language: "en",
      },
    );
    ...
    contactHoverTimeout.current = setTimeout(async () => {
      ...
      try {
        ...
        const match = contacts.data?.find(
          (c: any) => c.email?.toLowerCase() === mail.email.toLowerCase(),
        );
        if (match) {
          setContactInfo({
            name: `${match.first_name} ${match.last_name}`.trim(),
            email: match.email || mail.email,
            phone: match.phone,
          });
        } else {
  ...
  const handleSenderMouseLeave = () => {
    ...
    contactHoverTimeout.current = setTimeout(
      () => setShowContactPopover(false),
      300,
    );
  };
    ...
    <div className="flex h-full flex-col bg-background relative">
        ...
        <Toolbar className="border-b-0 bg-background/50 backdrop-blur-md">
          <ToolbarGroup>
            <ToolbarButton
              disabled={!mail}
              title="Archiver"
              onClick={() => mail && onArchive?.(mail.id)}
            >
          ...
          <ToolbarGroup>
            ...
            <SnoozeDatePicker
              onSnooze={(isoStr, label) => mail && onSnooze?.(mail.id, label)}
            >
          ...
          <ToolbarGroup className="ml-auto">
              ...
              >
                {isStreaming ? (
                  <SpinnerInfinity
                    ...
                    color="currentColor"
                    speed={120}
                    className="h-3.5 w-3.5 mr-2  text-purple-500"
                  />
                ) : (
            ...
            <ToolbarButton
              disabled={!mail}
              title="Reply"
              onClick={() => scrollToReplyBox("reply")}
            >
            ...
            <ToolbarButton
              disabled={!mail}
              title="Reply all"
              onClick={() => scrollToReplyBox("replyAll")}
            >
            ...
            <ToolbarButton
              disabled={!mail}
              title="Forward"
              onClick={() => scrollToReplyBox("forward")}
            >
        ...
        <div className="flex flex-1 flex-col overflow-y-auto">
          ...
          <div className="flex flex-col px-8 pt-6 pb-2 bg-background">
            <div className="flex items-center gap-3 mb-6 w-full flex-wrap">
              ...
              <span className="bg-muted text-muted-foreground text-xs px-2 rounded flex items-center gap-1 cursor-pointer hover:bg-muted/80 transition-colors leading-none h-5">
                Boîte de réception <X className="h-3 w-3" />
              </span>
            </div>
            ...
            <div className="flex items-start gap-4 text-sm w-full">
              <Avatar className="h-10 w-10 mt-0.5">
                <AvatarImage
                  alt={mail.name}
                  src={`https://avatar.vercel.sh/${mail.email}.png`}
                />
              ...
              <div className="grid gap-0 flex-1">
                <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                  ...
                  <div className="relative inline-block">
                    ...
                    {showContactPopover && (
                      <div
                        ...
                        onMouseEnter={() => {
                          if (contactHoverTimeout.current)
                            clearTimeout(contactHoverTimeout.current);
                          setShowContactPopover(true);
                        }}
                        onMouseLeave={handleSenderMouseLeave}
                      >
                  ...
                  <span className="text-xs text-muted-foreground font-normal cursor-pointer hover:underline">
                    &lt;{mail.email}&gt;
                  </span>
                ...
                {showDealDialog && (
                  <div className="mt-2 flex items-center gap-2 p-2 rounded-lg border border-blue-200 dark:border-blue-800/50 bg-blue-50 dark:bg-blue-900/20">
                    ...
                    <Button
                      ...
                      disabled={dealLinking || !dealSearchQ.trim()}
                    >
                      {dealLinking ? "…" : "Lier"}
                    </Button>
                    <button
                ...
                <div className="shrink-0 flex items-center gap-4 text-xs text-muted-foreground mt-2">
                  <span>
                    ...
                    {Math.max(
                      1,
                      Math.floor(
                        (new Date().getTime() - new Date(mail.date).getTime()) /
                          (1000 * 3600 * 24),
                      ),
                    )}{" "}
                    jours)
            ...
            <div className="mx-4 mt-4 rounded-2xl border border-purple-200/60 dark:border-purple-800/60 bg-gradient-to-br from-purple-50/90 to-indigo-50/90 dark:from-purple-950/40 dark:to-indigo-950/40 p-5 shadow-premium backdrop-blur-xl transition-all duration-300 overflow-hidden relative">
              ...
              <div className="flex items-center justify-between mb-3 relative z-10">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-purple-800 dark:text-purple-300 tracking-tight">
                    AI Summary
                  </span>
                  {isStreaming && (
                    <span className="text-[10px] uppercase tracking-wider font-bold text-purple-500 animate-pulse ml-2 bg-purple-100 dark:bg-purple-900/50 px-2 py-0.5 rounded-full">
                      generating
                    </span>
                  )}
                </div>
              ...
              {summaryLoading ? (
                ...
                <p className="text-sm text-foreground whitespace-pre-wrap leading-relaxed relative z-10">
                  {summaryText}
                  {isStreaming && (
                    <span className="inline-block w-2 h-4 bg-purple-500 animate-pulse ml-1 align-middle rounded-sm" />
                  )}
                </p>
              )}
            </div>
          ...
          {mail.attachments && mail.attachments.length > 0 && (
            <>
              <AttachmentPreviewBar
                ...
                onDownloadAll={() =>
                  mail.attachments?.forEach((a) => window.open(a.url, "_blank"))
                }
              />
          ...
          <div className="flex items-center gap-3 px-8 py-6 mb-4">
            <Button
              variant="outline"
              className="rounded-full px-5 h-9 text-foreground/80 border-border font-medium hover:bg-accent transition-colors"
              onClick={() => scrollToReplyBox("reply")}
            >
                ...
                {crmDeals.map((deal) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="font-bold text-blue-500 dark:text-blue-400 mr-0.5">
                        CRM
                      </span>
                      {deal.title} —{" "}
                      {isOverdue ? (
            ...
            <form className="mt-2">
              <div className="grid gap-3 relative">
                <Textarea
                  ...
                  placeholder={
                    replyMode === "forward"
                      ? `Forward to… (add recipient above, then type message)`
                      : replyMode === "replyAll"
                        ? `Reply all to ${mail.name}...`
                        : `Write your reply to ${mail.name}...`
                  }
                  value={
                    replyText +
                    (interimReplyText
                  ...
                  {sending ? (
                    ...
                    <div className="flex items-center gap-2 pointer-events-auto">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent
                          align="end"
                          className="w-[200px] rounded-xl shadow-xl border-border"
                        >
                          <DropdownMenuItem
                            className="rounded-lg cursor-pointer font-medium py-2"
                            onClick={() =>
                              setReplyText(
                          ...
                          <DropdownMenuItem
                            className="rounded-lg cursor-pointer font-medium py-2"
                            onClick={() =>
                              setReplyText("Sounds good to me, please proceed.")
                            }
                          >
                            "Sounds good to me"
                          </DropdownMenuItem>
                      ...
                      <ScheduleSendPopup
                        onSchedule={(sendAt) => {
                          ...
                          toast.success(
                            `Reply scheduled for ${sendAt.toLocaleString()}`,
                          );
                        }}
                        disabled={
                          !(replyText.trim() || interimReplyText.trim())
                        }
                      />
                      <Button
                        onClick={handleSend}
      ...
      {mail && (
        <EmailToTaskDialog
          ...
          onOpenChange={(v) => {
            setShowTaskDialog(v);
            if (!v) setTaskChipText(null);
          }}
          emailSubject={
            taskChipText ? `[Action] ${taskChipText}` : mail.subject
          }
          emailBody={mail.text}
          emailFrom={mail.name}
          emailId={mail.id}
