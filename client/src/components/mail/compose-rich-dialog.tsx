...
import { X, Minimize2, Maximize2, Paperclip, CalendarDays, ChevronDown, ShieldCheck, Loader2 } from "lucide-react";
...
function coachingBadgeVariant(value: string): "default" | "destructive" | "secondary" | "outline" {
...
export function ComposeRichDialog({
    ...
    const [originalBodyBeforeRewrite, setOriginalBodyBeforeRewrite] = useState<string | null>(null)
            ...
            try {
                ...
                for (const cal of cals) {
                    ...
                    for (const ev of events) {
                        ...
                        setMeetingReminder(`PS\u00a0: On se voit ${dayLabel} à ${timeLabel} pour "${ev.title}".`)
                        return
                    }
                }
            } catch {
    ...
    const loadAvailableSlots = useCallback(async () => {
        ...
        try {
            ...
            setAvailableSlots(slots.length > 0 ? slots : ["Aucun créneau trouvé cette semaine."])
        } catch {
    ...
    const insertAvailability = (slots: string[]) => {
        ...
        toast.info("Disponibilités copiées dans le presse-papier.", { description: text })
    ...
    const handleSend = useCallback(
        async (html: string, _design: object) => {
            ...
            try {
                ...
                let attachmentData: { name: string; type: string; size: number; data: string }[] = []
                if (attachments.length > 0) {
                    attachmentData = await Promise.all(
                        attachments.map(file => new Promise<{ name: string; type: string; size: number; data: string }>((resolve, reject) => {
                            const reader = new FileReader()
                            reader.onload = () => resolve({ name: file.name, type: file.type, size: file.size, data: (reader.result as string).split(',')[1] || '' })
                ...
                await mailApi.send({
                    ...
                    subject: subject.trim() || "(Sans objet)",
                    body_html: bodyHtml,
                    metadata: attachmentData.length > 0 ? JSON.stringify({ attachments: attachmentData }) : undefined,
                });
                    ...
                    <div className="px-4 py-2 space-y-2 border-b bg-muted/30">
                                ...
                                {showAvailability && (
                                    <div className="absolute bottom-8 left-0 z-50 bg-popover border border-border rounded-xl shadow-xl p-3 min-w-[280px]">
                                        <div className="flex items-center justify-between mb-2">
                                            <span className="text-xs font-semibold text-foreground">Créneaux libres</span>
                                            <button type="button" onClick={() => setShowAvailability(false)} className="text-muted-foreground hover:text-foreground">
                                        ...
                                        {loadingSlots ? (
                                            ...
                                            <div className="space-y-1">
                                                {availableSlots.map((slot, i) => (
                                                    <div key={i} className="text-xs py-0.5 text-foreground">• {slot}</div>
                                                ))}
                                                {availableSlots.length > 0 && !availableSlots[0].startsWith('Aucun') && !availableSlots[0].startsWith('Impossible') && (
                                                    <Button
                                                        type="button"
                                                        size="sm"
                        ...
                        {coachingResult && (
                            <div className="w-full mt-1.5 p-2.5 rounded-lg border border-teal-200/60 dark:border-teal-800/60 bg-teal-50/60 dark:bg-teal-900/20 flex flex-wrap gap-2 items-center">
                                <span className="text-[11px] font-semibold text-teal-700 dark:text-teal-400 mr-1">Analyse :</span>
                                {(Object.entries(coachingResult) as [string, string][]).map(([key, value]) => (
                                    <Badge
                                        key={key}
                        ...
                        {rewrittenBody && (
                            <div className="w-full mt-1.5 p-2.5 rounded-lg border border-violet-200/60 dark:border-violet-800/60 bg-violet-50/60 dark:bg-violet-900/20">
                                <div className="flex items-center justify-between mb-1.5">
                                    <span className="text-[11px] font-semibold text-violet-700 dark:text-violet-400">Version réécrite :</span>
                                    <div className="flex items-center gap-2">
                                        <button
                                            type="button"
                                            className="text-[11px] text-violet-600 dark:text-violet-400 hover:underline font-medium"
                                            onClick={() => {
                                                navigator.clipboard.writeText(rewrittenBody).catch(() => {})
