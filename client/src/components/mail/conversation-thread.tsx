...
function groupByThread(mails: Mail[]): ConversationThread[] {
  ...
  for (const mail of mails) {
    ...
    const key =
      mail.thread_id ||
      mail.subject.split(" ").slice(0, 3).join(" ").toLowerCase();
      ...
      >
        ...
        <span
          className={cn(
            ...
            !latest.read ? "font-bold" : "font-medium",
          )}
        >
          {latest.name}
        </span>
  ...
  return (
    <div
      className={cn(
        ...
        hasSelected && "bg-primary/5 dark:bg-primary/10",
      )}
    >
      {/* Thread header */}
      <button
          ...
          {thread.messages.map((msg) => (
            ...
            >
              <Avatar className="h-6 w-6 shrink-0">
                <AvatarImage
                  src={`https://avatar.vercel.sh/${msg.email}.png`}
                  alt={msg.name}
                />
              ...
              <span
                className={cn(
                  ...
                  !msg.read ? "font-semibold" : "font-normal",
                )}
              >
