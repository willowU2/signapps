...
export function EmailAliases({ accountId, accountEmail }: EmailAliasesProps) {
  ...
  function openEdit(alias: EmailAlias) {
    setEditAlias(alias)
    setForm({ alias_email: alias.alias_email, display_name: alias.display_name })
    setDialogOpen(true)
  }
    ...
    <Card>
      ...
      <CardContent className="space-y-2">
          ...
          <div key={alias.id} className="flex items-center justify-between rounded-md border px-3 py-2">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0">
                ...
                {alias.display_name && (
                  <p className="text-xs text-muted-foreground truncate">{alias.display_name}</p>
                )}
              </div>
              {alias.is_default && <Badge variant="outline" className="text-xs flex-shrink-0">Default</Badge>}
              {!alias.verified && <Badge variant="destructive" className="text-xs flex-shrink-0">Unverified</Badge>}
            </div>
            <div className="flex items-center gap-1 ml-2 flex-shrink-0">
              {!alias.is_default && alias.verified && (
                <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => setDefault(alias.id)}>
                  <Check className="h-3.5 w-3.5" />
                </Button>
              )}
              <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => openEdit(alias)}>
                <Pencil className="h-3.5 w-3.5" />
              </Button>
              <Button size="icon" variant="ghost" className="h-7 w-7 text-destructive" onClick={() => handleDelete(alias.id)}>
        ...
        {!loading && aliases.length === 0 && (
          <p className="text-xs text-muted-foreground py-2 text-center">No aliases configured</p>
        )}
      </CardContent>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editAlias ? "Edit Alias" : "Add Email Alias"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={e => setForm(p => ({ ...p, alias_email: e.target.value }))}
