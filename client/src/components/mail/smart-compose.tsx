  ...
  const fetchSuggestion = useCallback(async (text: string) => {
    ...
    try {
      const res = await aiApi.chat(
        ...
        {
          systemPrompt:
            "You complete email drafts professionally and concisely. Output ONLY the continuation text.",
          language: "fr",
        },
      );
    ...
    <div className="relative">
      ...
      {suggestion && (
        <div className="absolute bottom-2 left-3 right-3 flex items-center gap-2 text-xs text-muted-foreground bg-background/90 backdrop-blur-sm rounded-md px-2 py-1.5 border">
          ...
          <kbd className="ml-auto shrink-0 text-[10px] px-1 py-0.5 rounded border bg-muted font-mono">
            Tab
          </kbd>
        </div>
      )}
