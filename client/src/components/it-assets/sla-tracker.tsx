...
function computeSla(ticket: SlaTicket, policy: SlaPolicy, now: Date): TicketSla {
    ...
    const resolutionPct = Math.min((elapsedMin / policy.resolutionMinutes) * 100, 100)

    let resolutionStatus: SlaStatus
    if (resolved) {
        const resolvedElapsed = (resolved - created) / 60000
        resolutionStatus = resolvedElapsed <= policy.resolutionMinutes ? "met" : "breached"
...
const STATUS_META: Record<SlaStatus, { badge: string; icon: React.ReactNode }> = {
    met:        { badge: "bg-emerald-500/10 text-emerald-600", icon: <CheckCircle className="h-3.5 w-3.5 text-emerald-500" /> },
    breached:   { badge: "bg-red-500/10 text-red-600",         icon: <AlertTriangle className="h-3.5 w-3.5 text-red-500" /> },
    approaching:{ badge: "bg-orange-500/10 text-orange-600",   icon: <Clock className="h-3.5 w-3.5 text-orange-500" /> },
    open:       { badge: "bg-muted text-muted-foreground",     icon: <Clock className="h-3.5 w-3.5 text-muted-foreground" /> },
}

const STATUS_LABEL: Record<SlaStatus, string> = {
    met: "Respecte", breached: "Depasse", approaching: "Proche limite", open: "En cours",
}
...
export function SlaTracker({ tickets, policies = DEFAULT_SLA_POLICIES }: SlaTrackerProps) {
    const now = new Date()

    const slaData = useMemo(() =>
        tickets.map(ticket => {
            const policy = policies.find(p => p.priority === ticket.priority) ?? policies[2]
    ...
    const breached  = slaData.filter(s => s.resolutionStatus === "breached").length
    const approaching = slaData.filter(s => s.resolutionStatus === "approaching").length
    const open      = slaData.filter(s => s.resolutionStatus === "open").length
    const total     = slaData.length
    const compliance = total > 0
        ? Math.round(((met + open + approaching) / total) * 100)
        : 100
    ...
    const chartData = ["critical", "high", "medium", "low"].map(prio => {
        ...
        return {
            ...
            "En cours": group.filter(s => ["open", "approaching"].includes(s.resolutionStatus)).length,
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-4 gap-3">
                {[
                    { label: "Conformite", value: `${compliance}%`, cls: "text-emerald-600" },
            ...
            <Card>
                ...
                <CardContent className="p-0">
                    {slaData.length === 0 ? (
                        <p className="p-4 text-sm text-muted-foreground text-center">Aucun ticket</p>
                    ) : (
                        <div className="divide-y">
                            {slaData
                                .sort((a, b) => {
                                    const order: Record<SlaStatus, number> = { breached: 0, approaching: 1, open: 2, met: 3 }
                                    return order[a.resolutionStatus] - order[b.resolutionStatus]
                                })
                                .map(({ ticket, resolutionStatus, resolutionRemainingMin, resolutionPct }) => {
                                        ...
                                        <div key={ticket.id} className="flex items-center gap-3 px-4 py-3">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center gap-2 mb-1">
                                                    ...
                                                    <Badge className={`text-xs shrink-0 ${PRIORITY_COLORS[ticket.priority]}`}>
                                                        {ticket.priority}
                                                    </Badge>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    <Progress value={resolutionPct} className="h-1.5 flex-1" />
                                                    <span className="text-xs text-muted-foreground shrink-0 w-32 text-right">
                                                        {fmtRemaining(resolutionRemainingMin)}
                                                    </span>
                                                </div>
                                            </div>
                                            <Badge className={`text-xs shrink-0 ${statusMeta.badge}`}>
