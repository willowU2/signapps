...
export function AssetDepreciation({ assetName, purchaseDate }: Props) {
  const defaultYear = purchaseDate
  ...
  const totalDepreciated =
    data.length > 0
      ? (parseFloat(cost) || 0) -
        (currentEntry?.bookValue ?? (parseFloat(cost) || 0))
      : 0;
              ...
              {[
                {
                  ...
                  value: `€${currentEntry?.bookValue.toFixed(0) ?? "—"}`,
                  color: "text-blue-600",
                },
                {
                  label: "Total Depreciated",
                  value: `€${totalDepreciated.toFixed(0)}`,
                  color: "text-orange-600",
                },
                {
                  label: "Annual Depreciation",
                  value: `€${data[1]?.depreciation.toFixed(0) ?? "—"}`,
