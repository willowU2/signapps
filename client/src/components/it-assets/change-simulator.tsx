...
function getDownstreamIds(
  ...
  while (queue.length > 0) {
    ...
    for (const r of relationships) {
      ...
      if (
        ["hosted_on", "part_of"].includes(r.relationship_type) &&
        r.target_ci_id === cur &&
        !visited.has(r.source_ci_id)
      ) {
        queue.push(r.source_ci_id);
      }
    }
...
export function ChangeSimulator({
  ...
  function simulate() {
    if (!targetId) {
      toast.error("Selectionnez un equipement cible");
      return;
    }
    ...
    if (bizHours)
      warnings.push(
        "Changement pendant les heures de bureau — risque eleve d'impact utilisateur",
      );
    if (!inMaint)
      warnings.push(
        "Hors fenetre de maintenance — planifiez de preference le week-end ou entre 00h et 06h",
      );
    if (downstreamIds.length >= 5)
      warnings.push(
        `Impact important : ${downstreamIds.length} services/equipements dependants`,
      );
    if (changeType === "decommission")
      warnings.push(
        "DECOMMISSIONNEMENT IRREVERSIBLE — verifier que tous les services sont migres",
      );
        ...
        {simResult && colors && (
          <div className="space-y-3 rounded-lg border bg-muted/30 p-4">
            ...
            {simResult.warnings.map((w, i) => (
              <div
                key={i}
                className="flex items-start gap-2 rounded bg-amber-500/10 px-2 py-1.5"
              >
