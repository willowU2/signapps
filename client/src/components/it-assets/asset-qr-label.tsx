...
export function AssetQrLabel({ asset, open, onOpenChange }: AssetQrLabelProps) {
  ...
  const qrValue = JSON.stringify({ id: asset.id, name: asset.name, serial: asset.serial_number ?? "" })
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xs">
        ...
        <div className="flex flex-col items-center gap-4 py-2">
          <div ref={printRef} className="border-2 border-border rounded-lg p-4 text-center">
            ...
            <p className="text-xs text-muted-foreground">{asset.type} · {asset.location ?? "No location"}</p>
            <p className="text-xs text-muted-foreground font-mono mt-1">{asset.id.slice(0, 12)}…</p>
          </div>
