...
export function AssetAssignment({ asset }: Props) {
  ...
  const handleAssign = async () => {
    ...
    setSaving(true);
    try {
      await itAssetsApi.updateHardware(asset.id, {
        assigned_user_id: form.user,
      });
      setHistory((h) => [
        {
  ...
  const handleUnassign = async () => {
    ...
    try {
      ...
      setHistory((h) =>
        h.map((r, i) =>
          i === 0 ? { ...r, unassigned_at: new Date().toISOString() } : r,
        ),
      );
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center gap-3 rounded-lg bg-muted/50 p-3">
          ...
          <div>
            ...
            {asset.assigned_user_id ? (
              <Badge className="mt-1 bg-teal-500/10 text-teal-700 border-teal-500/20">
                {asset.assigned_user_id}
              </Badge>
            ) : (
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {history.map((r) => (
                <div
                  key={r.id}
                  className="flex items-start justify-between text-xs border rounded-md px-3 py-2"
                >
                  <div>
                    ...
                    <div className="text-muted-foreground mt-0.5">
                      ...
                      {r.unassigned_at &&
                        ` → ${format(new Date(r.unassigned_at), "MMM d, yyyy HH:mm")}`}
                    </div>
                  </div>
                  <Badge
                    variant={r.unassigned_at ? "secondary" : "outline"}
        ...
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="space-y-3 py-1">
            <div className="space-y-1.5">
              ...
              <Input
                placeholder="e.g. john.doe or employee ID"
                value={form.user}
                onChange={(e) =>
