...
function buildGraph(discoveries: NetworkDiscovery[]): { nodes: Node[]; edges: Edge[] } {
...
export function NetworkTopology({ discoveries, onNodeClick }: NetworkTopologyProps) {
        ...
        return (
            <Card>
                <CardContent className="py-12 flex flex-col items-center text-center gap-3">
                    ...
                    <p className="text-muted-foreground">Lancez un scan pour voir la topologie reseau</p>
                </CardContent>
            </Card>
        )
    }
                ...
                <CardHeader className="pb-2">
                    <div className="flex items-center justify-between">
                        <CardTitle className="text-base flex items-center gap-2">
                            ...
                            Topologie reseau
                            <Badge variant="outline" className="ml-1 text-xs">{nodes.length} hotes</Badge>
                        </CardTitle>
                        <div className="flex gap-1">
                            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setZoom(z => Math.min(z + 0.2, 3))}>
                                <ZoomIn className="h-3.5 w-3.5" />
                            </Button>
                            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setZoom(z => Math.max(z - 0.2, 0.3))}>
                    ...
                    <CardContent className="pt-4 pb-3">
                        <div className="flex items-start justify-between">
                            <div className="space-y-1">
                                ...
                                {selected.hostname && (
                                    <p className="text-sm text-muted-foreground">{selected.hostname}</p>
                                )}
                                {selected.os_guess && (
                                    <Badge variant="outline" className="text-xs">{selected.os_guess}</Badge>
                                )}
                            </div>
                            <div className="text-xs text-muted-foreground space-y-1 text-right">
                                ...
                                <div className="flex flex-wrap gap-1 justify-end">
                                    {selected.open_ports.map(p => (
                                        <Badge key={p} variant="outline" className="text-xs py-0 h-4 font-mono">{p}</Badge>
                                    ))}
                                </div>
                            </div>
                        </div>
