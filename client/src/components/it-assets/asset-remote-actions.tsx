...
function commandStatusBadge(status: AgentCommand["status"]) {
  ...
  return (
    <Badge className={`text-xs border ${meta.className}`}>{meta.label}</Badge>
  );
}
...
export function AssetRemoteActions({ asset }: AssetRemoteActionsProps) {
  ...
  const handleWakeOnLan = async () => {
    ...
    try {
      ...
      toast.error(
        msg.includes("MAC")
          ? "Aucune adresse MAC enregistree pour cette machine"
          : msg,
      );
    } finally {
      setWaking(false);
  ...
  const handlePushFile = async () => {
    ...
    try {
      ...
      toast.success(
        `Fichier "${pushFile.filename}" mis en file pour transfert`,
      );
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex flex-wrap gap-2">
            ...
            >
              <Zap
                className={`h-3.5 w-3.5 text-amber-500 ${waking ? "animate-pulse" : ""}`}
              />
          ...
          <div className="mt-3">
            ...
            >
              <RefreshCw
                className={`h-3 w-3 ${loadingCmds ? "animate-spin" : ""}`}
              />
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div
            ...
            onClick={() =>
              document.getElementById(`file-input-${asset.id}`)?.click()
            }
          >
          ...
          <div>
            ...
            >
              <Download
                className={`h-3 w-3 ${loadingFiles ? "animate-spin" : ""}`}
              />
      ...
      <AlertDialog open={!!pendingCmd} onOpenChange={() => setPendingCmd(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {pendingCmd === "reboot" &&
                "La machine va redemarrer. Les sessions en cours seront fermees."}
              {pendingCmd === "shutdown" &&
                "La machine va s'eteindre. L'action est irreversible depuis le logiciel."}
              {pendingCmd === "lock" && "La session en cours sera verrouilee."}{" "}
              Cette commande sera executee par l'agent lors de sa prochaine
              connexion.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
          ...
          <div className="space-y-3 py-2">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) =>
                  setPushFile((f) => ({ ...f, filename: e.target.value }))
                }
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) =>
                  setPushFile((f) => ({ ...f, target_path: e.target.value }))
                }
            ...
            {pushFile.content_base64 && (
              <p className="text-xs text-emerald-600">
                Fichier charge (
                {Math.round((pushFile.content_base64.length * 0.75) / 1024)} KB)
              </p>
            )}
          </div>
          <DialogFooter>
