...
const escalationApi = {
  ...
  create: (data: CreatePolicyRequest) =>
    client.post<EscalationPolicy>("/it-assets/escalation-policies", data),
  delete: (id: string) => client.delete(`/it-assets/escalation-policies/${id}`),
  toggle: (id: string, enabled: boolean) =>
    client.patch<EscalationPolicy>(`/it-assets/escalation-policies/${id}`, {
      enabled,
    }),
};
...
function LevelCard({
  ...
  return (
    <div
      className={`rounded-lg border-l-4 ${color} pl-4 py-3 pr-3 space-y-2 bg-card`}
    >
      <div className="flex items-center gap-2">
        <span
          className={`text-xs font-bold px-2 py-0.5 rounded-full ${color.replace("border-", "bg-")} bg-opacity-10`}
        >
          Level {level}
        </span>
...
const BLANK_FORM: CreatePolicyRequest = {
  ...
  l1_delay_minutes: 0,
  l1_notify_email: "",
  l1_notify_label: "Assigned tech",
  l2_delay_minutes: 15,
  l2_notify_email: "",
  l2_notify_label: "Team lead",
  l3_delay_minutes: 30,
  l3_notify_email: "",
  l3_notify_label: "Manager",
  l3_sms_number: "",
};
...
function PolicyForm({
  ...
  const set = <K extends keyof CreatePolicyRequest>(
    k: K,
    v: CreatePolicyRequest[K],
  ) => setForm((f) => ({ ...f, [k]: v }));
        ...
        <div className="space-y-4">
          <div>
            ...
            <Input
              ...
              placeholder="Standard IT escalation"
            />
          </div>
          <div>
          ...
          <div className="space-y-3">
            <LevelCard
              ...
              delayLabel="0-15 min"
              form={{
                email: form.l1_notify_email,
                label: form.l1_notify_label,
              }}
              onChange={(f, v) =>
                set(f === "email" ? "l1_notify_email" : "l1_notify_label", v)
              }
            />
            ...
            <LevelCard
              level={2}
              label="Team lead escalation"
              color="border-yellow-500"
              delayLabel="15-30 min"
              form={{
                email: form.l2_notify_email,
                label: form.l2_notify_label,
              }}
              onChange={(f, v) =>
                set(f === "email" ? "l2_notify_email" : "l2_notify_label", v)
              }
            />
            <div className="flex items-center justify-center">
              <ChevronRight className="h-4 w-4 text-muted-foreground rotate-90" />
            ...
            <LevelCard
              level={3}
              label="Management escalation + SMS"
              color="border-red-500"
              delayLabel="30 min+"
              form={{
                email: form.l3_notify_email,
                label: form.l3_notify_label,
                sms: form.l3_sms_number,
              }}
              onChange={(f, v) => {
                if (f === "sms") set("l3_sms_number", v);
                else
                  set(f === "email" ? "l3_notify_email" : "l3_notify_label", v);
              }}
              showSms
            />
          </div>
...
function PolicyLevelBadge({
  ...
  return (
    <span
      className={`text-xs px-1.5 py-0.5 rounded font-medium ${colors[level]}`}
    >
      L{level}: {label || email}
    </span>
  );
...
export function AlertEscalation() {
  ...
  const toggleMutation = useMutation({
    mutationFn: ({ id, enabled }: { id: string; enabled: boolean }) =>
      escalationApi.toggle(id, enabled),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["escalation-policies"] }),
  });
                ...
                <TableRow key={p.id}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1 flex-wrap">
                      <PolicyLevelBadge
                        level={1}
                        label={p.l1.notify_label}
                        email={p.l1.notify_email}
                      />
                      <ChevronRight className="h-3 w-3 text-muted-foreground" />
                      <PolicyLevelBadge
                        level={2}
                        label={p.l2.notify_label}
                        email={p.l2.notify_email}
                      />
                      <ChevronRight className="h-3 w-3 text-muted-foreground" />
                      <PolicyLevelBadge
                        level={3}
                        label={p.l3.notify_label}
                        email={p.l3.notify_email}
                      />
                      {p.l3.sms_number && (
