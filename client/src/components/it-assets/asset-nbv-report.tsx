...
export function AssetNbvReport({ assets }: Props) {
  const byType = useMemo(() => {
    ...
    return Object.entries(map).map(([type, total]) => ({ type, total: Math.round(total) })).sort((a, b) => b.total - a.total)
  }, [assets])
  ...
  const depreciationRate = totalCost > 0 ? ((totalCost - totalNbv) / totalCost * 100).toFixed(1) : "0"
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Estimated asset values using straight-line depreciation (5yr, 10% salvage)</CardDescription>
      </CardHeader>
        ...
        {assets.length === 0 ? (
          ...
          <>
            <div className="grid grid-cols-3 gap-4">
              {[
                { label: "Total NBV", value: `€${totalNbv.toLocaleString()}`, color: "text-blue-600" },
                { label: "Original Cost", value: `€${totalCost.toLocaleString()}`, color: "text-foreground" },
                { label: "Depreciated", value: `${depreciationRate}%`, color: "text-orange-600" },
              ...
              <div className="h-52">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={byType}>
                    ...
                    <Tooltip formatter={(v) => [`€${Number(v).toLocaleString()}`, "NBV"]} />
              ...
              <div className="h-44">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={yearlyTrend}>
                    ...
                    <YAxis tick={{ fontSize: 11 }} tickFormatter={v => `€${v}`} />
                    <Tooltip formatter={(v) => [`€${Number(v).toLocaleString()}`, "Fleet NBV"]} />
                    <Line type="monotone" dataKey="nbv" stroke="#8b5cf6" strokeWidth={2} dot={{ r: 3 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
