...
function UptimePill({ pct }: { pct: number }) {
  const color =
    pct >= 99 ? "bg-emerald-500" : pct >= 95 ? "bg-yellow-500" : "bg-red-500";
  return (
    <span
      className={`text-xs font-semibold text-white px-2 py-0.5 rounded-full ${color}`}
    >
      {pct.toFixed(1)}%
    </span>
  );
...
export function UptimeChecker() {
  ...
  const addMonitor = () => {
    ...
    setMonitors((prev) => [
      ...prev,
      { id, url: url.trim(), expected_status: expected, checks: [] },
    ]);
    setUrl("");
    setExpected(200);
  };
        ...
        <div className="space-y-3">
          {monitors.map((m) => {
            ...
            return (
              <Card
                key={m.id}
                className={`border ${isUp === false ? "border-destructive/30" : ""}`}
              >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {lastCheck && (
                        <p className="text-xs text-muted-foreground">
                          Last: {new Date(lastCheck.ts).toLocaleTimeString()} —{" "}
                          {lastCheck.latency_ms}ms
                          {lastCheck.status
                            ? ` — HTTP ${lastCheck.status}`
                            : ""}
                        </p>
                      )}
                    </div>
