...
import { Bot, Send, X, Minimize2, Maximize2, Cpu, HardDrive, AlertTriangle } from "lucide-react"
import { aiApi } from "@/lib/api/ai"
import { HardwareAsset, AgentMetric, ITAlert, Patch } from "@/lib/api/it-assets"
...
function buildSystemPrompt(
    ...
    if (asset) {
        ...
        if (asset.manufacturer) lines.push(`Fabricant: ${asset.manufacturer} ${asset.model ?? ""}`)
    }

    if (metrics && metrics.length > 0) {
        const latest = metrics[0]
        lines.push(`\nDernieres metriques:`)
        if (latest.cpu_usage != null) lines.push(`  CPU: ${latest.cpu_usage.toFixed(1)}%`)
        if (latest.memory_usage != null) lines.push(`  RAM: ${latest.memory_usage.toFixed(1)}%`)
        if (latest.disk_usage != null) lines.push(`  Disque: ${latest.disk_usage.toFixed(1)}%`)
        ...
        if (metrics.length > 1) {
            ...
            if (latest.disk_usage != null && oldest.disk_usage != null) {
                ...
                lines.push(`  Tendance disque: ${diff >= 0 ? "+" : ""}${diff.toFixed(1)}% sur la periode`)
            }
        }
    }
    ...
    if (alerts && alerts.length > 0) {
        ...
        if (active.length > 0) {
            ...
            active.slice(0, 5).forEach(a => lines.push(`  - Alerte sur equipement ${a.hardware_id} (valeur: ${a.value ?? "N/A"})`))
        }
    }

    if (patches && patches.length > 0) {
        const critical = patches.filter(p => p.severity === "critical" && p.status === "pending")
        const pending = patches.filter(p => p.status === "pending")
        lines.push(`\nPatches: ${pending.length} en attente dont ${critical.length} critiques`)
        critical.slice(0, 3).forEach(p => lines.push(`  - [CRITIQUE] ${p.title}`))
    }
...
export function AiItAssistant({ selectedAsset, metrics, alerts, patches }: AiItAssistantProps) {
    ...
    const sendMessage = useCallback(async (text: string) => {
        ...
        try {
            const systemPrompt = buildSystemPrompt(selectedAsset, metrics, alerts, patches)
            const res = await aiApi.chat(q, { systemPrompt, language: "fr" })
            const answer = (res.data as { answer?: string; response?: string })?.answer
        ...
        >
            ...
            <div className="flex items-center justify-between rounded-t-xl border-b bg-primary px-4 py-2.5">
                ...
                <div className="flex items-center gap-1">
                    <button onClick={() => setMinimized(m => !m)} className="rounded p-1 text-primary-foreground/80 hover:bg-white/20">
                        {minimized ? <Maximize2 className="h-3.5 w-3.5" /> : <Minimize2 className="h-3.5 w-3.5" />}
                    </button>
                    <button onClick={() => setOpen(false)} className="rounded p-1 text-primary-foreground/80 hover:bg-white/20">
                ...
                <>
                    ...
                    {(metrics?.length || alerts?.length || patches?.length) ? (
                        <div className="flex flex-wrap gap-1.5 border-b px-3 py-2">
                            ...
                            {patches && patches.filter(p => p.severity === "critical" && p.status === "pending").length > 0 && (
                    ...
                    <div className="flex-1 overflow-y-auto p-3 space-y-3">
                        {messages.length === 0 && (
                            <div className="space-y-2">
                                <p className="text-xs text-muted-foreground text-center">Questions suggeries :</p>
                                {SUGGESTIONS.map(s => (
                                    <button
                                        key={s}
