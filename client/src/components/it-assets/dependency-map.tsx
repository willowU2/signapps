...
function nodeColor(status: string, highlight: boolean, dimmed: boolean): string {
...
function typeLabel(type: string): string {
    const map: Record<string, string> = {
        server: "SRV", workstation: "WS", network: "NET",
        storage: "STG", application: "APP", service: "SVC",
        database: "DB", other: "OTH",
    }
...
function getImpacted(nodeId: string, edges: GraphEdge[]): Set<string> {
    ...
    while (queue.length > 0) {
        ...
        for (const e of edges) {
            const next = e.source === cur ? e.target
                : e.target === cur ? e.source
                : null
...
export function DependencyMap({ cis, relationships }: DependencyMapProps) {
    ...
    const [tooltip, setTooltip] = useState<{ ci: ConfigurationItem; x: number; y: number } | null>(null)
    ...
    const handleNodeHover = useCallback((nodeId: string | null, x: number, y: number) => {
        if (nodeId == null) { setTooltip(null); return }
    ...
    return (
        <Card>
            ...
            <CardContent className="p-0">
                ...
                {selectedCi && (
                    <div className="flex items-center gap-2 border-b px-4 py-2 bg-amber-500/5">
                        ...
                        <span className="text-xs text-amber-700">
                            Impact de <strong>{selectedCi.name}</strong> : {impacted.size - 1} element(s) dependant(s)
                        </span>
                ...
                {/* SVG Canvas */}
                <div className="relative overflow-hidden rounded-b-xl" style={{ height: H }}>
                    <svg
                        ref={svgRef}
                        width="100%"
                        height={H}
                    ...
                    >
                        ...
                        {edges.map((e, i) => {
                            ...
                            if (!s || !t) return null
                            const isHighlighted = impacted.has(e.source) && impacted.has(e.target)
                            return (
                                <line
                                    key={i}
                                    ...
                                    strokeDasharray={e.relType === "connected_to" ? "4 2" : undefined}
                                    markerEnd="url(#arrow)"
                            ...
                            return (
                                <g
                                    ...
                                    onMouseEnter={e => {
                                        ...
                                        if (rect) handleNodeHover(n.id, e.clientX - rect.left, e.clientY - rect.top)
                                    }}
                    ...
                    {tooltip && (
                        ...
                        >
                            ...
                            <p className="text-muted-foreground">{tooltip.ci.ci_type} — {tooltip.ci.status}</p>
                        </div>
                    )}
                </div>

                <p className="px-4 py-2 text-xs text-muted-foreground border-t">
                    Cliquez sur un noeud pour voir l&apos;impact sur les elements dependants
                </p>
            </CardContent>
        </Card>
    )
