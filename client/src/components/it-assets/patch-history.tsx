...
function SeverityBadge({ severity }: { severity?: Patch["severity"] }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-xs px-1.5 py-0.5 rounded border font-medium ${cfg.className}`}
    >
      {cfg.icon}
      {cfg.label}
    </span>
...
function StatusIcon({ status }: { status: Patch["status"] }) {
  if (status === "installed" || status === "deployed")
        ...
        <div className="flex items-start justify-between gap-2 flex-wrap">
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
              ...
              <Badge
                variant={
                  patch.status === "installed" || patch.status === "deployed"
                    ? "default"
...
export function PatchHistory({ hardwareId }: PatchHistoryProps) {
  ...
  const sorted = [...filtered].sort((a, b) => {
    const da = new Date(
      a.installed_at ?? a.deployed_at ?? a.detected_at,
    ).getTime();
    const db = new Date(
      b.installed_at ?? b.deployed_at ?? b.detected_at,
    ).getTime();
    return db - da;
  });
  ...
  const stats = {
    ...
    installed: devicePatches.filter(
      (p) => p.status === "installed" || p.status === "deployed",
    ).length,
    failed: devicePatches.filter((p) => p.status === "rejected").length,
    pending: devicePatches.filter(
        ...
        {isLoading ? (
          ...
          <div className="py-8 text-center text-muted-foreground text-sm">
            ...
            {devicePatches.length === 0
              ? "No patch history for this device."
