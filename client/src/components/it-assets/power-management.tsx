...
function saveSchedules(s: PowerSchedule[]) {
  localStorage.setItem(LS_SCHEDULES, JSON.stringify(s));
}
...
function saveProfiles(p: PowerProfile[]) {
...
export function PowerManagement({ assets }: PowerManagementProps) {
  ...
  function addSchedule() {
    if (!sAssetId) {
      toast.error("Selectionnez un equipement");
      return;
    }
  ...
  function saveProfile() {
    ...
    if (existing >= 0) {
      updated = profiles.map((p, i) =>
        i === existing
          ? { ...p, watts: pWatts, hoursPerDay: pHours, costPerKwh: pCost }
          : p,
      );
    } else {
      updated = [
  ...
  const totalMonthlyCost = profiles.reduce(
    (sum, p) => sum + calcMonthlyCost(p.watts, p.hoursPerDay, p.costPerKwh),
    0,
  );
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-medium">
                Cout energetique estimé du parc
              </p>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select
                  value={sAction}
                  onValueChange={(v) => setSAction(v as "wake" | "sleep")}
                >
                  <SelectTrigger className="mt-1 h-8 text-sm">
                    <SelectValue />
                  </SelectTrigger>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium truncate">
                        {getAssetName(s.assetId)}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {s.action === "wake" ? "Allumer" : "Eteindre"} à{" "}
                        {s.time} — {s.days.map((d) => DAYS[d]).join(", ")}
                      </p>
                    </div>
                    <button
                    ...
                    >
                      <span
                        className={`block h-3.5 w-3.5 rounded-full bg-white shadow transition-transform mx-0.5 ${s.enabled ? "translate-x-4" : "translate-x-0"}`}
                      />
                    </button>
                    <button
