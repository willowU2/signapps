  ...
  return (
    ...
    >
      ...
      <Avatar className="h-10 w-10 shrink-0">
        {person.avatar_url && <AvatarImage src={person.avatar_url} alt={`${person.first_name} ${person.last_name}`} />}
      ...
      <div className="flex-1 min-w-0">
        ...
        {primaryPosition && (
          <p className="text-xs text-foreground/80 truncate mt-0.5">{primaryPosition}</p>
        )}
        ...
        {activeRoles.length > 0 && (
          <div className="flex items-center gap-1 flex-wrap mt-1.5">
            {activeRoles.map((role) => {
              const cfg = ROLE_CONFIG[role.role_type] ?? { label: role.role_type, color: 'bg-muted text-muted-foreground' };
              return (
                <span
                  key={role.id}
                  className={cn('inline-flex items-center text-[10px] px-1.5 py-0.5 rounded-full font-medium', cfg.color)}
                >
                  {cfg.label}
                </span>
              );
