...
const ENDPOINTS: Array<{
  ...
  {
    service: "identity",
    svc: ServiceName.IDENTITY,
    endpoint: "/health",
    method: "GET",
  },
  {
    service: "storage",
    svc: ServiceName.STORAGE,
    endpoint: "/health",
    method: "GET",
  },
  {
    service: "mail",
    svc: ServiceName.MAIL,
    endpoint: "/health",
    method: "GET",
  },
  {
    service: "calendar",
    svc: ServiceName.CALENDAR,
    endpoint: "/health",
    method: "GET",
  },
  ...
  const probe = async () => {
    ...
    for (const ep of ENDPOINTS) {
      ...
      const status: EndpointHealth["status"] =
        errors === 3 ? "down" : avg > 500 ? "degraded" : "healthy";
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={ep.history}>
                    <Line
                      type="monotone"
                      dataKey="ms"
                      stroke={ep.status === "healthy" ? "#22c55e" : "#ef4444"}
                      dot={false}
                      strokeWidth={1.5}
                    />
                  </LineChart>
