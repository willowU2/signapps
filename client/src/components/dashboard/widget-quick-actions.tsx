    ...
    <Card className="h-full flex flex-col overflow-hidden border-border/50 bg-gradient-to-br from-background to-muted/20 relative shadow-sm">
      ...
      <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-3 relative z-10 overflow-y-auto min-h-0 pb-6 [scrollbar-width:thin]">
        {actions.map((action, i) => (
          ...
          >
            <div
              className={`relative h-full flex flex-col justify-between space-y-3 rounded-xl border border-border/40 bg-background/50 p-4 transition-all duration-500 hover:shadow-md ${action.borderColor} overflow-hidden`}
            >
              {/* Background gradient fade on hover */}
              <div
                className={`absolute inset-0 bg-gradient-to-br ${action.gradient} opacity-0 transition-opacity duration-500 group-hover:opacity-100 pointer-events-none z-0`}
              />

              <div className="relative z-10 flex items-center justify-between">
                <div
                  className={`p-2.5 rounded-xl ${action.bgColor} ${action.color} group-hover:scale-110 group-hover:-rotate-3 transition-all duration-300 ease-out shadow-sm`}
                >
              ...
              <div className="relative z-10 space-y-0.5">
                <h3 className="font-semibold text-sm tracking-tight text-foreground transition-colors">
                  {action.title}
                </h3>
