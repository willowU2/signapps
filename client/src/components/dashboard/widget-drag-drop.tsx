...
import { useDashboardWidgets, type DashboardWidget, type WidgetSize } from '@/hooks/use-dashboard-widgets';
...
export function DraggableWidget({ widget, onDragStart, onDrop, onRemove, onResize, children }: DraggableWidgetProps) {
  ...
  return (
    ...
    >
      ...
      <div className={cn(
        ...
        <Button
          variant="ghost" size="sm" className="h-5 w-5 p-0 bg-background/80 shadow-sm"
...
export function DashboardWidgetCustomizer() {
  const { layout, visibleWidgets, moveWidget, resizeWidget, toggleWidget, addWidget, removeWidget } = useDashboardWidgets();
  ...
  return (
    ...
    >
      {visibleWidgets
        .sort((a, b) => a.position.row !== b.position.row ? a.position.row - b.position.row : a.position.col - b.position.col)
        .map(w => (
          <DraggableWidget
            key={w.id}
            widget={w}
