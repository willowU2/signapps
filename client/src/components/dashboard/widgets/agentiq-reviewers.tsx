...
export function AgentiqReviewersWidget({ widget }: Partial<WidgetRenderProps> = {}) {
  ...
  const reviewers: Reviewer[] = Array.isArray(data) ? data : data?.reviewers ?? [];
            ...
            <div key={r.id} className="flex items-center gap-2 py-1.5 border-b last:border-0">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  <span className={`text-xs font-semibold ${DOMAIN_COLORS[r.domain] ?? ''}`}>
                    {r.domain}
                  </span>
                  <Badge
                    variant="secondary"
