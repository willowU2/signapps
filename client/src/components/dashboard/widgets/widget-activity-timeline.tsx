...
import {
  useCrossModuleActivity,
  type ActivityModule,
} from "@/hooks/use-cross-module-activity";
...
export function WidgetActivityTimeline({
  widget,
}: Partial<WidgetRenderProps> = {}) {
  const [view, setView] = useState<ViewMode>("mine");
  const [moduleFilter, setModuleFilter] = useState<ActivityModule | "all">(
    "all",
  );
  ...
  const modules = [
    ...new Set(activities.map((a) => a.module)),
  ] as ActivityModule[];
          ...
          <div className="flex gap-1 overflow-x-auto">
            ...
            {modules.slice(0, 5).map((m) => (
              <Button
                key={m}
                size="sm"
                variant={moduleFilter === m ? "secondary" : "ghost"}
                className="h-5 text-xs px-1.5 shrink-0 capitalize"
                ...
                <div key={a.id} className="flex items-start gap-2 py-1.5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <Badge
                        className={`text-xs h-4 px-1 ${MODULE_COLORS[a.module] || ""}`}
                      >
                        {a.module}
                      </Badge>
