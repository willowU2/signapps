            ...
            filteredBySearch.length === 0 ? (
              ...
              <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                {filteredBySearch.map((app) => (
                  <AppTile key={app.id} app={app} onDragStart={handleDragStart} pinApp={pinApp} pinnedApps={pinnedApps} router={router} />
                ))}
              </div>
            )
          ) : (
            APP_CATEGORIES.map((cat) => {
              ...
              return (
                <div key={cat} className="mb-2">
                  <button onClick={() => toggleCategory(cat)} className="mb-1.5 flex w-full items-center gap-2 text-left">
...
function AppTile({
  ...
  pinApp: (p: { href: string; icon: string; label: string; color: string }) => void;
  ...
  return (
    ...
    >
      <div className="flex w-full items-start justify-between gap-1">
        <div className={cn('flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-muted', app.color)}>
        ...
        <button
          ...
          className={cn(
            ...
            isPinned ? 'text-primary opacity-100' : 'opacity-0 hover:bg-muted group-hover:opacity-100'
          )}
        >
          <Pin className="h-2.5 w-2.5" />
        </button>
      </div>
      <p className="text-xs font-medium leading-tight truncate w-full">{app.label}</p>
    </div>
  );
}
