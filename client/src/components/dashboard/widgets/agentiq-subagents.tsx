...
export function AgentiqSubagentsWidget({ widget }: Partial<WidgetRenderProps> = {}) {
  ...
  const subagents: SubAgent[] = Array.isArray(data) ? data : data?.subagents ?? [];
    ...
    <Card className="h-full flex flex-col">
      <CardHeader className="pb-2 shrink-0">
        <CardTitle className="text-sm flex items-center gap-1.5">
          ...
          Sous-Agents
          {subagents.length > 0 && (
            <Badge variant="secondary" className="text-xs ml-auto">{subagents.length}</Badge>
          )}
        </CardTitle>
      </CardHeader>
                ...
                return (
                  <div key={agent.id} className="rounded-lg border bg-card p-2">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${MODEL_BADGE[mk]}`}>
                        {mk}
                      </span>
                    </div>
                    <p className="text-[10px] text-muted-foreground truncate mb-1">{agent.task}</p>
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] text-muted-foreground">{agent.parent}</span>
                      {agent.duration !== undefined && (
                        <span className="text-[10px] text-muted-foreground">{formatDuration(agent.duration)}</span>
                      )}
                    </div>
                  </div>
                );
