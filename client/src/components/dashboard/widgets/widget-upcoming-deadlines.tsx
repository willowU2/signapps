...
export function WidgetUpcomingDeadlines({
  widget,
    ...
    <Card className="h-full flex flex-col">
      <CardHeader className="pb-2 shrink-0">
        <CardTitle className="text-sm flex items-center justify-between">
          ...
          {!isLoading && total > 0 && (
            <Badge
              variant={grouped.overdue?.length ? "destructive" : "secondary"}
              className="text-xs"
            >
              {total}
            </Badge>
            ...
            <div className="space-y-3 pt-2">
              {sections.map((s) => {
                ...
                return (
                  <div key={s.key}>
                    <p
                      className={`text-xs font-semibold uppercase tracking-wider mb-1.5 flex items-center gap-1 ${s.color}`}
                    >
                          ...
                          <div className="flex items-center gap-2 py-1 px-2 rounded hover:bg-muted/50 group">
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-1.5">
                                <Badge
                                  className={`text-xs h-3.5 px-1 ${MODULE_COLORS[d.module] || "bg-muted text-muted-foreground"}`}
                                >
                                  {d.module}
                                </Badge>
                                <span className="text-xs text-muted-foreground">
                                  {d.daysLeft < 0
                                    ? `${Math.abs(d.daysLeft)}j de retard`
                                    : d.daysLeft === 0
                                      ? "Aujourd'hui"
                                      : `J-${d.daysLeft}`}
                                </span>
