...
import { Upload, Layers, CheckCircle, Loader2, Download, X } from "lucide-react"
  ...
  const processFile = (item: BatchFile): Promise<BatchFile> => {
    return new Promise(resolve => {
      ...
      img.onload = () => {
        ...
        if (!ctx) { resolve({ ...item, status: "error" }); return }
        ...
        resolve({ ...item, status: "done", outputUrl, outputName: `${base}_${canvas.width}x${canvas.height}.${ext}` })
      }
      img.onerror = () => { URL.revokeObjectURL(url); resolve({ ...item, status: "error" }) }
      img.src = url
    })
  }
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="space-y-1.5">
            ...
            <Input type="number" value={maxWidth} onChange={e => setMaxWidth(e.target.value)} className="h-8 text-sm" />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">Quality (%)</Label>
            <Input type="number" min="1" max="100" value={quality} onChange={e => setQuality(e.target.value)} className="h-8 text-sm" disabled={format === "png"} />
          </div>
          <div className="space-y-1.5">
            ...
            {files.map(item => (
              <div key={item.id} className="flex items-center justify-between rounded-md border px-3 py-1.5">
                <div className="flex items-center gap-2 min-w-0">
                  {item.status === "processing" && <Loader2 className="h-3.5 w-3.5 animate-spin text-blue-500 shrink-0" />}
                  {item.status === "done" && <CheckCircle className="h-3.5 w-3.5 text-emerald-500 shrink-0" />}
                  {item.status === "pending" && <div className="h-3.5 w-3.5 rounded-full bg-muted shrink-0" />}
                ...
                <div className="flex items-center gap-1 ml-2">
                  {item.status === "done" && (
                    <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => downloadFile(item)}>
                      <Download className="h-3 w-3" />
                    </Button>
                  )}
                  <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => remove(item.id)}>
                    <X className="h-3 w-3" />
                  </Button>
                </div>
              </div>
        ...
        {files.length > 0 && (
          <div className="flex items-center gap-3">
            <Button onClick={runBatch} disabled={processing || pendingCount === 0} className="flex-1">
              {processing ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Processing…</> : <><Layers className="h-4 w-4 mr-2" />Process {pendingCount} files</>}
            </Button>
