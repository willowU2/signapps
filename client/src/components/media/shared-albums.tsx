...
import { FolderOpen, Plus, Image, Trash2, Upload, X, FolderPlus, ChevronRight } from "lucide-react"
...
export function SharedAlbums() {
  const [albums, setAlbums] = useState<Album[]>([
    { id: "1", name: "Team Photos", description: "Company events", files: [], created_at: new Date().toISOString() },
  ])
  ...
  const createAlbum = () => {
    ...
    const album: Album = { id: Date.now().toString(), name: form.name, description: form.description || undefined, files: [], created_at: new Date().toISOString() }
    setAlbums(a => [...a, album])
  ...
  const addFilesToAlbum = (albumId: string, files: FileList | null) => {
    ...
    setAlbums(a => a.map(album => album.id === albumId ? { ...album, files: [...album.files, ...newFiles] } : album))
    setSelectedAlbum(album => album?.id === albumId ? { ...album!, files: [...album!.files, ...newFiles] } : album)
  }

  const removeFile = (albumId: string, fileId: string) => {
    setAlbums(a => a.map(album => album.id === albumId ? { ...album, files: album.files.filter(f => f.id !== fileId) } : album))
    setSelectedAlbum(album => album?.id === albumId ? { ...album!, files: album!.files.filter(f => f.id !== fileId) } : album)
  }
    ...
    return (
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-3">
          <div>
            <button onClick={() => setSelectedAlbum(null)} className="text-xs text-muted-foreground hover:text-foreground flex items-center gap-1 mb-1">
          ...
          <label className="cursor-pointer">
            ...
            <Button size="sm" variant="outline" asChild>
              <span><Upload className="h-3.5 w-3.5 mr-1" />Add Files</span>
            </Button>
          </label>
        </CardHeader>
        <CardContent>
          {album.files.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {album.files.map(file => (
                <div key={file.id} className="relative group aspect-square rounded-lg border overflow-hidden bg-muted">
                  {file.type === "image" ? (
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center">
                      ...
                      <p className="text-xs text-muted-foreground mt-1 px-2 text-center truncate">{file.name}</p>
                    </div>
                  )}
        ...
        <CardContent>
          {albums.length === 0 ? (
            ...
            <div className="space-y-2">
              {albums.map(album => (
                <div key={album.id} className="flex items-center justify-between rounded-lg border p-3 hover:bg-muted/30 cursor-pointer group"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{album.files.length} file{album.files.length !== 1 ? "s" : ""}</p>
                    </div>
                  </div>
        ...
        <DialogContent className="sm:max-w-xs">
          ...
          <div className="space-y-3 py-1">
            <div className="space-y-1.5">
              ...
              <Input placeholder="e.g. Team Photos" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
            </div>
            <div className="space-y-1.5">
