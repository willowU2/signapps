...
export function ImageEditor() {
  ...
  const applyEdits = useCallback(() => {
    ...
    const w = img.naturalWidth,
      h = img.naturalHeight;
    const newW =
      Math.abs(w * Math.cos(absRad)) + Math.abs(h * Math.sin(absRad));
    const newH =
      Math.abs(w * Math.sin(absRad)) + Math.abs(h * Math.cos(absRad));
    canvas.width = newW;
    canvas.height = newH;
  ...
  const rotate = (dir: 1 | -1) =>
    setState((s) => ({ ...s, rotation: (s.rotation + dir * 90 + 360) % 360 }));
        ...
        {!imageUrl ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 gap-3">
              {[
                ...
                <div key={key} className="flex items-center gap-3">
                  <Label className="w-24 text-xs shrink-0">
                    {label} {state[key]}%
                  </Label>
                  <Slider
                    min={min}
            ...
            <div className="flex flex-wrap gap-2">
              <Button size="sm" variant="outline" onClick={() => rotate(-1)}>
              ...
              <Button
                size="sm"
                variant={state.grayscale ? "default" : "outline"}
                onClick={() =>
            ...
            <div className="flex items-center gap-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setState((s) => ({ ...s, overlayText: e.target.value }))
                }
