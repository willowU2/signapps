...
export function VideoPlayerWithChapters() {
  ...
  const addChapter = () => {
    ...
    setChapters((c) =>
      [
        ...c,
        { id: Date.now().toString(), title: chapterForm.title, time },
        ...
        {chapters.length > 0 && (
          <div className="space-y-1">
            ...
            {chapters.map((c) => (
              <div
                key={c.id}
                className="flex items-center justify-between rounded-md hover:bg-muted/50 px-2 py-1.5 cursor-pointer group"
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`text-sm ${activeChapter?.id === c.id ? "text-primary font-medium" : ""}`}
                  >
                    {c.title}
                  </span>
                </div>
          ...
          <div className="space-y-3 py-1">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) =>
                  setChapterForm((f) => ({ ...f, time: e.target.value }))
                }
              />
              <p className="text-xs text-muted-foreground">
                Leave empty to use current position ({formatTime(currentTime)})
              </p>
            </div>
          </div>
          <DialogFooter>
            ...
            <Button onClick={addChapter} disabled={!chapterForm.title.trim()}>
              Add
            </Button>
          </DialogFooter>
        </DialogContent>
