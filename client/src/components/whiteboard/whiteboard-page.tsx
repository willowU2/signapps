...
function useWhiteboardCollab(roomId: string, svgRef: React.RefObject<SVGSVGElement | null>) {
    ...
    useEffect(() => {
        ...
        const doc = new Y.Doc();
        docRef.current = doc;
        const baseWs = process.env.NEXT_PUBLIC_COLLAB_WS_URL || "ws://localhost:3013";
        ...
        provider.awareness.setLocalStateField("user", { name: "Utilisateur", color: myColor });
        ...
        yShapes.observe(() => {
            ...
            svgRef.current.querySelectorAll("[data-remote]").forEach((el) => el.remove());
            // Re-add from Yjs
...
export function WhiteboardPage() {
    ...
    const { isConnected, remoteCursors, broadcastShape, broadcastCursor } = useWhiteboardCollab(roomId, svgRef);

    // DW4: Insert a shape preset
    const insertShape = useCallback((preset: (typeof SHAPE_PRESETS)[number]) => {
        ...
        if (child) {
            ...
            // Broadcast to peers
            const shapeId = (child as SVGElement).getAttribute("data-id") || `shape-${Date.now()}`;
    ...
    return (
        <div className="flex flex-col h-full bg-background">
            ...
            <div className="flex items-center justify-between px-4 py-2 border-b shrink-0 bg-background/95">
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1 text-xs text-muted-foreground">
                        {isConnected ? (
                            <><Wifi className="h-3.5 w-3.5 text-green-500" /><span className="text-green-600">Connecté</span></>
                        ) : (
                            <><WifiOff className="h-3.5 w-3.5 text-muted-foreground" /><span>Local</span></>
                        )}
                    </div>
                    {/* Collaborator count */}
                    {collabCount > 0 && (
                        <div className="flex items-center gap-1 text-xs text-muted-foreground">
                            ...
                            <span>{collabCount} autre{collabCount > 1 ? "s" : ""}</span>
                        </div>
                    )}
                </div>
            ...
            {/* Canvas area */}
            <div className="flex-1 overflow-auto relative" onMouseMove={(e: React.MouseEvent) => {
                ...
                {Object.entries(remoteCursors).map(([id, cursor]) => (
                    <div
                        ...
                        style={{ left: cursor.x + 16, top: cursor.y + 16, transform: "translate(0, 0)" }}
                    >
                        <div
