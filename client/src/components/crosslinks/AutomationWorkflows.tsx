...
export function AutomationWorkflows() {
  ...
  const [form, setForm] = useState({
    name: '', trigger_module: 'docs', trigger_event: '',
    action_module: 'mail', action_type: '',
  });
  ...
  const toggle = async (w: Workflow) => {
    try {
      await client().patch(`/automation/workflows/${w.id}`, { enabled: !w.enabled });
  ...
  const remove = async (id: string) => {
    ...
    try { await client().delete(`/automation/workflows/${id}`); }
    catch { toast.error('Erreur'); }
  };
  ...
  const save = async () => {
    try {
      ...
      const local: Workflow = {
        ...
        action_config: {}, enabled: true, runs: 0, created_at: new Date().toISOString(),
      };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">{workflows.length} automation(s) configurée(s)</p>
      ...
      <div className="space-y-2">
        {workflows.map(w => (
          <div key={w.id} className="flex items-center gap-3 p-3 rounded-lg border">
            <Zap className={`w-4 h-4 shrink-0 ${w.enabled ? 'text-yellow-500' : 'text-muted-foreground'}`} />
            <div className="flex-1 min-w-0">
              <p className="font-medium text-sm truncate">{w.name || 'Sans nom'}</p>
              <div className="flex items-center gap-1 text-xs text-muted-foreground">
                ...
                {w.runs > 0 && <Badge variant="secondary" className="ml-2 text-xs h-4">{w.runs} runs</Badge>}
              </div>
            </div>
            <div className="flex items-center gap-2">
              <Switch checked={w.enabled} onCheckedChange={() => toggle(w)} className="scale-75" />
              <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-destructive hover:text-destructive" onClick={() => remove(w.id)}>
        ...
        {!loading && workflows.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 text-muted-foreground border-2 border-dashed rounded-lg">
            ...
            <Button size="sm" variant="outline" onClick={() => setOpen(true)} className="mt-2">Créer</Button>
          </div>
        )}
      </div>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1">
              <Label>Nom</Label>
              <Input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Mon automatisation" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <Label>Module déclencheur</Label>
                <Select value={form.trigger_module} onValueChange={v => setForm(f => ({ ...f, trigger_module: v, trigger_event: '' }))}>
              ...
              <div className="space-y-1">
                ...
                <Select value={form.action_module} onValueChange={v => setForm(f => ({ ...f, action_module: v, action_type: '' }))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{MODULES.map(m => <SelectItem key={m.value} value={m.value}>{m.label}</SelectItem>)}</SelectContent>
                </Select>
              </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={save} disabled={!form.name || !form.trigger_event || !form.action_type}>Créer</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
