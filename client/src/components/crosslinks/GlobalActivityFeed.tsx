...
const MODULE_COLORS: Record<string, string> = {
  ...
  navigated:
    "bg-muted text-muted-foreground dark:bg-gray-950 dark:text-gray-300",
...
export function GlobalActivityFeed({ compact = false, limit = 50 }: Props) {
  ...
  const { activities: localActivities, clear: clearLocal } =
    useActivityTracker();
  ...
  const mergedActivities = useMemo(() => {
    const localConverted = (localActivities || [])
      .slice(0, limit)
      .map(localToActivity);
  ...
  const filtered = useMemo(() => {
    ...
    if (moduleFilter !== "all")
      result = result.filter(
        (a) => a.entity_type === moduleFilter || a.action === moduleFilter,
      );
    if (search) {
      ...
      <ScrollArea className={compact ? "h-64" : "h-96"}>
        <div className="space-y-1 pr-2">
          {filtered.map((a) => (
            ...
            >
              <span
                className={`text-xs px-1.5 py-0.5 rounded font-medium shrink-0 ${MODULE_COLORS[a.entity_type] || MODULE_COLORS[a.action] || "bg-muted text-muted-foreground"}`}
              >
                {MODULE_LABELS[a.entity_type] ||
                  MODULE_LABELS[a.action] ||
                  a.entity_type}
              </span>
              <div className="flex-1 min-w-0">
                <p className="text-sm leading-tight">
                  <span className="font-medium">
                    {a.actor_id === "vous" ? "Vous" : a.actor_id.slice(0, 8)}
                  </span>{" "}
                ...
                {a.metadata?.details != null && (
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {String(a.metadata.details as unknown)}
                  </p>
                )}
              </div>
          ...
          {filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center py-8 text-muted-foreground">
              ...
              <p className="text-xs mt-1">
                Vos actions apparaitront ici au fil de votre utilisation.
              </p>
            </div>
          )}
        </div>
