...
export function EntityLinks({ entityType, entityId }: Props) {
  ...
  const grouped = links.reduce<Record<string, EntityReference[]>>(
    (acc, link) => {
      const otherType =
    ...
    <div className="space-y-3">
      ...
      {Object.entries(grouped).map(([type, refs]) => (
        <div key={type}>
          ...
          {refs.map((ref) => {
            const otherId =
              ref.source_type === entityType ? ref.target_id : ref.source_id;
            return (
              <div
                key={ref.id}
                className="text-sm pl-2 py-0.5 border-l-2 border-muted"
              >
                {otherId.slice(0, 8)}...{" "}
