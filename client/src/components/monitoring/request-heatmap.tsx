...
function genHeatmapData(): number[][] {
  return DAYS.map((_, dayIdx) => HOURS.map((_, hour) => {
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        ...
        <CardDescription className="text-xs">Activity by hour of day × day of week (req/min)</CardDescription>
      </CardHeader>
      <CardContent>
          ...
          <div className="min-w-[600px]">
            ...
            <div className="flex mb-1">
              ...
              {HOURS.filter((_, i) => i % 3 === 0).map(h => (
                <div key={h} className="flex-1 text-center text-xs text-muted-foreground" style={{ flexBasis: `${(3/24)*100}%` }}>{h}</div>
              ))}
            </div>
            {/* Grid */}
            {DAYS.map((day, dIdx) => (
            ...
            <div className="flex items-center gap-2 mt-3 justify-end">
              ...
              {["bg-blue-200 dark:bg-blue-900/30", "bg-blue-400 dark:bg-blue-700/50", "bg-orange-400 dark:bg-orange-700/60", "bg-orange-500", "bg-red-500"].map((c, i) => (
                <div key={i} className={`h-3 w-6 rounded-sm ${c}`} />
              ))}
              <span className="text-xs text-muted-foreground">High ({maxVal})</span>
            </div>
          </div>
        </div>
      </CardContent>
