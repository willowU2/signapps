  ...
  const chartData = useMemo(
    () =>
      data.map((d, i) => ({
        ...d,
        trend: regression
          ? +(regression.intercept + regression.slope * i).toFixed(2)
          : undefined,
      })),
    [data, regression],
  );
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        <Select
          value={period}
          onValueChange={(v) => setPeriod(v as "30d" | "90d")}
        >
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={chartData}>
              ...
              <Tooltip
                ...
                formatter={(v, name) => [
                  `${Number(v).toFixed(1)}${name === "value" ? "%" : ""}`,
                  name === "value" ? metric : "Trend",
                ]}
              />
              <Area
                type="monotone"
