...
export function WeeklyMetricsReport() {
  ...
  const handleSendNow = async () => {
    ...
    toast.success(
      `Weekly report sent to ${config.recipients.split(",").length} recipient(s)`,
    );
  };
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <select
            ...
            onChange={(e) =>
              setConfig((c) => ({ ...c, sendDay: parseInt(e.target.value) }))
            }
          >
            {DAY_LABELS.map((d, i) => (
        ...
        {config.enabled && (
          <div className="flex items-center gap-2 text-xs text-muted-foreground bg-muted/50 rounded-lg p-2">
            ...
            <span>
              Next send: <span className="font-medium">{nextSendDate()}</span>
            </span>
          </div>
        )}
