...
function calcErrorBudget(slo: SLO) {
  ...
  const consumedMinutes =
    totalMinutes *
    Math.max(0, (slo.target_percent - slo.current_percent) / 100);
  const remaining = Math.max(0, budgetMinutes - consumedMinutes);
  const remainingPct =
    budgetMinutes > 0 ? (remaining / budgetMinutes) * 100 : 0;
  ...
  const [slos, setSlos] = useState<SLO[]>([
    {
      ...
      sli_description: "HTTP 5xx rate < 0.1%",
      target_percent: 99.9,
    ...
    },
    {
      ...
      sli_description: "p99 request latency",
      target_percent: 99.0,
    ...
    <Card>
              ...
              return (
                <div key={slo.id} className="rounded-lg border p-3 space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-3 text-center">
                    ...
                    <div
                      className={`rounded p-2 ${critical ? "bg-red-500/10" : "bg-muted/50"}`}
                    >
                      <div className="flex items-center justify-center gap-1">
                        {critical && <Flame className="h-3 w-3 text-red-500" />}
                        <p
                          className={`text-sm font-bold ${critical ? "text-red-600" : ""}`}
                        >
                          {eb.remainingPct.toFixed(0)}%
                        </p>
                      </div>
                      <p className="text-xs text-muted-foreground">
                        Budget Left
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-xs text-muted-foreground">
                      ...
                      <span>
                        {eb.remaining.toFixed(0)} /{" "}
                        {eb.budgetMinutes.toFixed(0)} min remaining
                      </span>
                    </div>
                    <Progress
                      value={eb.remainingPct}
                      className={`h-2 ${critical ? "[&>div]:bg-red-500" : "[&>div]:bg-indigo-500"}`}
                    />
                    {eb.burnRate > 1 && (
                      <p className="text-xs text-red-600 flex items-center gap-1">
                        <Flame className="h-3 w-3" />
                        Burn rate: {eb.burnRate.toFixed(1)}x — budget will be
                        exhausted early
                      </p>
                    )}
                  </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>Add SLO</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 py-1">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                placeholder="e.g. HTTP 2xx rate"
                value={form.sli_description}
                onChange={(e) =>
                  setForm((f) => ({ ...f, sli_description: e.target.value }))
                }
              />
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, target_percent: e.target.value }))
                  }
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  max="100"
                  step="0.01"
                  value={form.current_percent}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, current_percent: e.target.value }))
                  }
                />
              </div>
              <div className="space-y-1.5">
