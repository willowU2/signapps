...
export function TranslationMemory() {
  ...
  const approve = (id: string) => {
    setEntries((es) =>
      es.map((e) => (e.id === id ? { ...e, approved: true } : e)),
    );
...
${entries.map((e) => `    <tu><tuv xml:lang="en"><seg>${e.source}</seg></tuv><tuv xml:lang="${e.locale}"><seg>${e.translation}</seg></tuv></tu>`).join("\n")}
    ...
    a.href = url;
    a.download = "translation-memory.tmx";
    a.click();
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">
                <BookOpen className="h-5 w-5" /> Translation Memory
              </CardTitle>
              <CardDescription>
                Stored translations are reused automatically to ensure
                consistency
              </CardDescription>
            </div>
            <div className="flex gap-2">
              <Button size="sm" variant="outline" onClick={exportTMX}>
                <Download className="mr-2 h-4 w-4" /> Export TMX
              </Button>
            ...
            {filtered.map((e) => (
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  <Badge
                    variant="outline"
                    className="text-xs w-8 justify-center shrink-0"
                  >
                  ...
                  <div className="min-w-0">
                    ...
                    <p
                      className={`text-xs text-muted-foreground truncate ${e.locale === "ar" || e.locale === "he" ? "text-right" : ""}`}
                      dir={
                        e.locale === "ar" || e.locale === "he" ? "rtl" : "ltr"
                      }
                    >
                      → {e.translation}
                    </p>
