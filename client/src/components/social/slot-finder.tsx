...
const BEST_HOURS_BY_PLATFORM: Record<
  string,
...
export function SlotFinder({ onSchedule }: SlotFinderProps) {
  ...
  useEffect(() => {
    const loadTimeSlots = async () => {
      try {
        ...
        setTimeSlots(
          res.data.map((s: { dayOfWeek: number; hour: number }) => ({
  ...
  const recommendations = useMemo((): SlotRecommendation[] => {
    ...
    for (let dayOffset = 0; dayOffset <= 14 && slots.length < 15; dayOffset++) {
      ...
      for (const platform of activePlatforms.length > 0
        ? activePlatforms
        : ["twitter", "linkedin"]) {
      ...
      platformHours.forEach((hours) =>
        hours.forEach((h) => hoursToCheck.add(h)),
      );
      ...
      for (const hour of Array.from(hoursToCheck).sort((a, b) => a - b)) {
        ...
        const occupied = scheduledDatetimes.some(
          (t) => Math.abs(t - slotMs) < 30 * 60 * 1000,
        );
        if (occupied) continue;

        // Find which platforms recommend this hour
        const matchingPlatforms = (
          activePlatforms.length > 0 ? activePlatforms : ["twitter", "linkedin"]
        ).filter((p) => {
        ...
        if (configuredHours.includes(hour))
          reasons.unshift("Configured time slot");

        const score =
          matchingPlatforms.length + (configuredHours.includes(hour) ? 2 : 0);

        slots.push({
          date: slotDate,
          platforms:
            matchingPlatforms.length > 0 ? matchingPlatforms : activePlatforms,
