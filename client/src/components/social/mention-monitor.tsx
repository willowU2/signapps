...
import { Bell, Plus, Trash2, Search, RefreshCw, ExternalLink, MessageSquare } from 'lucide-react';
...
const POSITIVE_WORDS = ['merci', 'super', 'bravo', 'love', 'amazing', 'great', 'excellent', 'fantastic', 'awesome', 'perfect', 'wonderful', 'brilliant', 'outstanding'];
...
export function MentionMonitor() {
  ...
  const mentions = useMemo((): MentionItem[] => {
    ...
    return inboxItems
      ...
      .map((item) => {
        const matchedKeyword = keywords.find((kw) => (item.content ?? '').toLowerCase().includes(kw)) ?? '';
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Track brand keywords across platforms and analyze sentiment</p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          {keywords.length === 0 ? (
            <p className="text-xs text-muted-foreground">No keywords yet. Add your brand name, product names or hashtags.</p>
          ) : (
            <div className="flex flex-wrap gap-2">
              {keywords.map((kw) => (
                <div key={kw} className="flex items-center gap-1 bg-muted rounded-full px-3 py-1">
            ...
            return (
              <button
                ...
                className={`p-3 rounded-xl border text-center transition-all hover:shadow-sm ${
                  sentimentFilter === s ? 'ring-2 ring-primary/40 bg-muted/50' : 'hover:bg-muted/30'
                }`}
              >
      ...
      {mentions.length > 0 && (
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            <Switch id="show-read" checked={showReadItems} onCheckedChange={setShowReadItems} />
            <Label htmlFor="show-read" className="text-sm cursor-pointer">Show read</Label>
          </div>
        </div>
      )}
      ...
      {keywords.length === 0 ? (
        <div className="text-center py-10 text-muted-foreground">
          ...
          <p className="text-sm mt-1">We'll scan your inbox for mentions matching your keywords</p>
        </div>
      ) : filtered.length === 0 ? (
              ...
              <CardContent className="pt-3 pb-3">
                <div className="flex items-start gap-3">
                  {mention.authorAvatar ? (
                    ...
                    <img src={mention.authorAvatar} alt="" className="w-9 h-9 rounded-full shrink-0" />
                  ) : (
                    <div
                      className="w-9 h-9 rounded-full shrink-0 flex items-center justify-center text-white text-xs font-bold"
                      style={{ backgroundColor: PLATFORM_COLORS[mention.platform] ?? '#6b7280' }}
                    >
