...
import { getPlatformCharLimit, getCharLimitColor, PLATFORM_LABELS, PLATFORM_COLORS } from './platform-utils';
import {
  Heart,
  MessageCircle,
  Repeat2,
...
function CharCount({ content, platform }: { content: string; platform: SocialAccount['platform'] }) {
...
function MediaGrid({ mediaUrls }: { mediaUrls: string[] }) {
  ...
  return (
    <div className={`grid gap-0.5 rounded-xl overflow-hidden ${mediaUrls.length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>
...
/* ---------- Twitter / X Preview ---------- */
function TwitterPreview({ content, accountName, accountAvatar, mediaUrls }: Omit<PostPreviewProps, 'platform'>) {
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            <span className="font-bold text-sm truncate">{accountName ?? 'User'}</span>
            <svg width="16" height="16" viewBox="0 0 22 22" fill="#1D9BF0" className="shrink-0">
...
/* ---------- Mastodon Preview ---------- */
function MastodonPreview({ content, accountName, accountAvatar, mediaUrls }: Omit<PostPreviewProps, 'platform'>) {
  const handle = accountName?.toLowerCase().replace(/\s+/g, '') ?? 'user';
  return (
    <div className="rounded-2xl border bg-card p-4 space-y-3 font-[system-ui]" style={{ borderColor: '#6364FF30' }}>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <span className="text-muted-foreground text-sm">@{handle}@mastodon.social</span>
        </div>
...
/* ---------- Bluesky Preview ---------- */
function BlueskyPreview({ content, accountName, accountAvatar, mediaUrls }: Omit<PostPreviewProps, 'platform'>) {
  const handle = accountName?.toLowerCase().replace(/\s+/g, '') ?? 'user';
  return (
    <div className="rounded-2xl border bg-card p-4 space-y-3 font-[system-ui]" style={{ borderColor: '#0085FF20' }}>
      <div className="flex items-start gap-3">
        <AvatarCircle
          src={accountAvatar}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            <svg width="14" height="14" viewBox="0 0 24 24" fill="#0085FF" className="shrink-0">
          ...
          <span className="text-muted-foreground text-sm">@{handle}.bsky.social</span>
        </div>
        <MoreHorizontal className="h-4 w-4 text-muted-foreground shrink-0" />
      </div>
    ...
    <div className="rounded-2xl border bg-card dark:bg-zinc-50 text-zinc-900 space-y-0 font-[system-ui] overflow-hidden">
      ...
      <div className="p-4 pb-2">
        <div className="flex items-start gap-3">
          ...
          />
          <div className="flex-1 min-w-0">
            <span className="font-bold text-sm">{accountName ?? 'Professional User'}</span>
    ...
    <div className="rounded-2xl border bg-card dark:bg-zinc-50 text-zinc-900 space-y-0 font-[system-ui] overflow-hidden">
      ...
      <div className="p-3">
        <div className="flex items-start gap-2.5">
          ...
          />
          <div className="flex-1 min-w-0">
            <span className="font-semibold text-sm">{accountName ?? 'User'}</span>
