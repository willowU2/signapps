...
function SentimentBadge({ text }: { text: string | undefined }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-xs font-medium ${cfg.className}`}
      title={cfg.label}
    >
      {cfg.emoji}
    </span>
...
function InboxBadge({ type }: { type: string | undefined }) {
  ...
  return (
    <span
      className={`px-2 py-0.5 rounded-full text-xs font-medium ${type ? (map[type] ?? "") : ""}`}
    >
      {type}
    </span>
  );
...
export function SocialInbox() {
  ...
  const platforms = [
    ...new Set(
      inboxItems.map((i) => i.platform).filter((p): p is string => !!p),
    ),
  ];
    ...
    let items =
      selectedChannelIds.length > 0
        ? inboxItems.filter((item) =>
            selectedChannelIds.includes(item.accountId),
          )
        : inboxItems;
    if (sentimentFilter !== "all") {
      items = items.filter(
        (item) => analyzeSentiment(item.content ?? "") === sentimentFilter,
      );
    }
    return items;
            ...
            {isLoadingInbox ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <h3 className="text-lg font-semibold">
                  Boite de reception vide
                </h3>
                <p className="text-sm text-muted-foreground mt-1 max-w-sm">
                  Les commentaires, mentions et messages apparaitront ici.
                </p>
              </div>
            ) : (
                ...
                >
                  ...
                  {item.authorAvatar ? (
                    ...
                    <div
                      ...
                      style={{
                        backgroundColor:
                          PLATFORM_COLORS[item.platform ?? ""] ?? "#6b7280",
                      }}
                    >
                ...
                <div className="flex items-start gap-3">
                  {selectedItem.authorAvatar ? (
                    ...
                    <div
                      ...
                      style={{
                        backgroundColor:
                          PLATFORM_COLORS[selectedItem.platform ?? ""] ??
                          "#6b7280",
                      }}
                    >
