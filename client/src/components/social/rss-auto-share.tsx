...
import { Rss, RefreshCw, ExternalLink, Clock, CheckCircle, XCircle, Send } from "lucide-react";
...
const DEMO_ITEMS: FeedItem[] = [
  {
    ...
    description: "Explore how Rust and Axum power zero-latency AI inference without cloud dependencies.",
  ...
  },
  {
    ...
    description: "A roundup of the best alternatives to proprietary productivity software.",
  ...
  },
  {
    ...
    description: "WebAssembly is no longer just for browsers — it powers edge compute and AI models.",
  ...
  },
  {
    ...
    description: "Step-by-step guide to replacing cloud services with self-hosted alternatives.",
...
export function RssAutoShare() {
  ...
  const updateConfig = (patch: Partial<AutoShareConfig>) => setConfig((c) => ({ ...c, ...patch }));
  ...
  const pendingCount = items.filter((i) => i.autoShareStatus === "pending" || i.autoShareStatus === "queued").length;
    ...
    <div className="space-y-4">
        ...
        <CardContent className="space-y-3">
          <div className="grid grid-cols-3 gap-3">
            <div className="space-y-1">
              ...
              <Select value={config.delay} onValueChange={(v) => updateConfig({ delay: v })}>
                <SelectTrigger className="h-8 text-xs">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="immediate" className="text-xs">Immediate</SelectItem>
                  <SelectItem value="15min" className="text-xs">15 min</SelectItem>
            ...
            <div className="flex flex-col gap-1.5 justify-end">
              <div className="flex items-center gap-2">
                <Switch
                  ...
                  onCheckedChange={(v) => updateConfig({ includeDescription: v })}
                  className="scale-75"
                />
      ...
      <div className="space-y-2">
        {items.map((item) => {
          ...
          return (
            <Card key={item.id} className={item.autoShareStatus === "skipped" ? "opacity-50" : ""}>
              <CardContent className="py-3 px-3 space-y-2">
                <div className="flex items-start gap-2">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className="text-xs font-medium truncate">{item.title}</span>
                      <Badge variant={statusCfg.color} className="text-[10px] gap-0.5 shrink-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5 text-[10px] text-muted-foreground">
                      ...
                      <a href={item.link} target="_blank" rel="noopener noreferrer" className="hover:underline flex items-center gap-0.5">
                        <ExternalLink className="w-2.5 h-2.5" />
                      </a>
                    </div>
                  </div>
                  {item.autoShareStatus !== "shared" && item.autoShareStatus !== "skipped" && (
                    <div className="flex gap-1 shrink-0">
                      <Button size="sm" className="h-6 text-[10px] px-2" onClick={() => handleShare(item)}>
                        <Send className="w-2.5 h-2.5 mr-1" />
                        Share
                      </Button>
                      <Button size="sm" variant="ghost" className="h-6 text-[10px] px-2" onClick={() => handleSkip(item.id)}>
                        Skip
                      </Button>
                    </div>
                  )}
