...
export function WeeklyPdfReport({
  weekData = DEMO_WEEK,
  weekLabel,
}: WeeklyPdfReportProps) {
  ...
  const captureChartAsDataUrl = useCallback(
    async (
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${getServiceUrl(ServiceName.SOCIAL)}/social/reports/weekly-pdf`,
        {
          method: "POST",
      ...
      window.print();
      toast.info(
        "PDF generation via print dialog (signapps-office service not running)",
      );
    } finally {
      setGenerating(false);
      setGenerated(true);
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-4 gap-2">
            {[
              ...
              <div
                key={s.label}
                className="text-center p-2 bg-muted/30 rounded-md"
              >
          ...
          <div className="space-y-2 border rounded-md p-3">
            ...
            {[
              ...
              },
              {
                ...
                value: includeEngagement,
                set: setIncludeEngagement,
              },
            ].map((opt) => (
              <div
                key={opt.label}
