...
export function SocialAnalytics() {
  ...
  const followerChartData = followerHistory.reduce(
    (
      acc: Record<string, { date: string; [k: string]: number | string }>,
      point,
    ) => {
      const dateKey = point.date.slice(0, 10);
      if (!acc[dateKey]) acc[dateKey] = { date: dateKey };
      acc[dateKey][point.platform] =
        ((acc[dateKey][point.platform] as number) ?? 0) + point.followers;
      return acc;
    },
    {},
  );
  ...
  const engagementChartData = platformEngagement.map((pe) => ({
    ...
    posts: pe.posts,
    fill:
      PLATFORM_COLORS[pe.platform as keyof typeof PLATFORM_COLORS] ?? "#6b7280",
  }));
  ...
  const platformsInHistory = [
    ...new Set(followerHistory.map((d) => d.platform)),
  ];

  // ---------------------------------------------------------------------------
  // Stories & Reels placeholder (waiting for real API)
  // ---------------------------------------------------------------------------
  const storiesTimelineData: Array<{
    date: string;
    views: number;
                    ...
                    <CardContent className="pt-6">
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          {stat.change != null && (
                            <p
                              className={`text-xs mt-1 ${stat.change >= 0 ? "text-green-500" : "text-red-500"}`}
                            >
                              {stat.change >= 0 ? "+" : ""}
                        ...
                        <div
                          className={`p-2 bg-muted rounded-lg ${stat.color}`}
                        >
              ...
              <Card>
                ...
                <CardContent>
                  {engagementChartData.length === 0 ? (
                    <div className="h-48 flex flex-col items-center justify-center text-center">
                      <MousePointer className="h-10 w-10 text-muted-foreground/30 mb-3" />
                      <p className="text-sm text-muted-foreground">
                        Aucune donnee d'engagement
                      </p>
                    </div>
                  ) : (
                    <ResponsiveContainer width="100%" height={220}>
              ...
              <CardContent>
                {topPosts.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <p className="text-sm font-medium">Aucun post analyse</p>
                    <p className="text-xs text-muted-foreground mt-1">
                      Les posts les plus performants apparaitront ici.
                    </p>
                  </div>
                ) : (
