...
export function OnCallPlanner({
  ...
  const [localAssignments, setLocalAssignments] =
    React.useState<OnCallAssignment[]>(assignments);
  const [selectedForSwap, setSelectedForSwap] = React.useState<string | null>(
    null,
  );
  const [swapCandidates, setSwapCandidates] = React.useState<
    OnCallAssignment[]
  >([]);
  ...
  const initiateSwap = (employeeId: string) => {
    ...
    setSwapCandidates(
      localAssignments.filter((a) => a.employeeId === employeeId),
    );
  };
  ...
  const completeSwap = (targetEmployeeId: string) => {
    ...
    const sourceAssignments = localAssignments.filter(
      (a) => a.employeeId === selectedForSwap,
    );
    const targetAssignments = localAssignments.filter(
      (a) => a.employeeId === targetEmployeeId,
    );

    const updated = localAssignments
      .filter(
        (a) =>
          a.employeeId !== selectedForSwap && a.employeeId !== targetEmployeeId,
      )
      .concat(
      ...
      <div className="flex items-center justify-between border-b pb-4">
        <div>
          <h3 className="text-lg font-semibold">
            Planification des gardes/astreintes
          </h3>
          <p className="text-sm text-muted-foreground">
            {format(weekDays[0], "EEEE d MMM")} -{" "}
            {format(weekDays[6], "EEEE d MMM yyyy")}
          </p>
        </div>
