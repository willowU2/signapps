...
import type { OrgNodeWithStats, CoverageGap, GapSeverity, ValidateCoverageResponse } from '@/types/workforce';

// Severity configurations
const SEVERITY_CONFIG: Record<GapSeverity, { icon: React.ComponentType<{ className?: string }>; color: string; label: string }> = {
...
export function ValidationDashboard({
  ...
  const { data, isLoading, refetch, isFetching } = useQuery({
    queryKey: ['workforce', 'validation', orgNodeId, format(weekStart, 'yyyy-MM-dd')],
    queryFn: () => validationApi.validateCoverage({
  ...
  const gapsByDay = React.useMemo(() => {
    ...
    return validation.gaps.reduce((acc, gap) => {
      ...
      <div className={cn('flex h-full items-center justify-center', className)}>
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Choisissez une unité organisationnelle pour voir la validation de couverture
          </p>
        </div>
      </div>
    );
        ...
        <>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {validation.summary.covered_slots}/{validation.summary.total_slots}
                </div>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Sur-effectif</CardTitle>
            ...
            <CardContent>
              {validation.gaps.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-8">
                  ...
                  <p className="mt-4 text-lg font-medium">Couverture complète</p>
                  <p className="text-sm text-muted-foreground">
                    ...
                    {[1, 2, 3, 4, 5, 6, 0].map((dayOfWeek) => {
                      ...
                      return (
                        <div key={dayOfWeek} className="space-y-2">
                          <h4 className="text-sm font-medium">{DAYS[dayOfWeek]}</h4>
                                  ...
                                  <div className="flex-1 space-y-1">
                                    <div className="flex items-center justify-between">
                                      <span className="text-sm font-medium">
                                        {gap.slot_label || `${gap.start_time} - ${gap.end_time}`}
                                      </span>
                                      <Badge
                                        variant="outline"
                                    ...
                                    <div className="flex items-center gap-4 text-xs text-muted-foreground">
                                      ...
                                      <span className="flex items-center gap-1">
                                        ...
                                        {gap.assigned_employees}/{gap.required_employees} assignés
                                      </span>
                                    </div>
