  ...
  useEffect(() => {
    if (hasSearched && searchQuery) {
      setResults([
        ...
        {
          id: "a2",
          name: "ProjectArchive_Jan.zip",
          archivedDate: new Date(Date.now() - 120 * 24 * 3600000),
          size: 15.8,
        },
        {
          id: "a3",
          name: "OldBackup_2023.tar.gz",
    ...
    <div className="space-y-6">
      ...
      {hasSearched && results.length === 0 && searchQuery && (
        <div className="border rounded-lg p-8 text-center bg-muted">
          <p className="text-muted-foreground">
            Aucun résultat trouvé for "{searchQuery}"
          </p>
        </div>
      )}
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center space-x-4 text-sm text-muted-foreground">
                  <div className="flex items-center space-x-1">
                    ...
                    <span>
                      Archived: {item.archivedDate.toLocaleDateString()}
                    </span>
                  </div>
                  <div className="flex items-center space-x-1">
      ...
      {!hasSearched && (
        <div className="border rounded-lg p-8 text-center bg-muted">
          ...
          <p className="text-muted-foreground">
            Enter a search query to find archived items
          </p>
        </div>
      )}
    </div>
