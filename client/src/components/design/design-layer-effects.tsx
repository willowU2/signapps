...
export default function DesignLayerEffects({ fabricCanvasRef }: LayerEffectsProps) {
  ...
  const setProp = useCallback(<K extends keyof EffectState>(key: K, value: EffectState[K]) => {
    setEffects((prev) => {
          ...
          <>
            ...
            <div className="space-y-1">
              <div className="flex justify-between">
                ...
                <span className="text-[10px] tabular-nums">{effects.shadowBlur}</span>
              </div>
              <Slider value={[effects.shadowBlur]} min={0} max={60} step={1} onValueChange={([v]) => setProp("shadowBlur", v)} />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-1">
                <div className="flex justify-between">
                  <Label className="text-[10px]">Offset X</Label>
                  <span className="text-[10px] tabular-nums">{effects.shadowOffsetX}</span>
                </div>
                <Slider value={[effects.shadowOffsetX]} min={-30} max={30} step={1} onValueChange={([v]) => setProp("shadowOffsetX", v)} />
              </div>
              <div className="space-y-1">
                <div className="flex justify-between">
                  <Label className="text-[10px]">Offset Y</Label>
                  <span className="text-[10px] tabular-nums">{effects.shadowOffsetY}</span>
                </div>
                <Slider value={[effects.shadowOffsetY]} min={-30} max={30} step={1} onValueChange={([v]) => setProp("shadowOffsetY", v)} />
              </div>
            </div>
          </>
        )}
        ...
        {effects.blurEnabled && (
          <div className="space-y-1">
            <div className="flex justify-between">
              ...
              <span className="text-[10px] tabular-nums">{effects.blurAmount}px</span>
            </div>
            <Slider value={[effects.blurAmount]} min={1} max={40} step={1} onValueChange={([v]) => setProp("blurAmount", v)} />
          </div>
        )}
      </Section>
          ...
          <>
            ...
            <div className="space-y-1">
              <div className="flex justify-between">
                ...
                <span className="text-[10px] tabular-nums">{effects.glowSize}px</span>
              </div>
              <Slider value={[effects.glowSize]} min={2} max={50} step={1} onValueChange={([v]) => setProp("glowSize", v)} />
            </div>
          </>
        )}
      </Section>
