...
export default function DesignFontsLibrary({
  fabricCanvasRef,
}: DesignFontsLibraryProps) {
  ...
  const handleApply = useCallback(
    async (font: FontItem) => {
      ...
      try {
        ...
        setFonts((prev) =>
          prev.map((f) =>
            f.family === font.family ? { ...f, loaded: true } : f,
          ),
        );
        ...
        if (canvas) {
          ...
          if (
            ...
            toast.info(
              `Font "${font.family}" loaded — select a text object to apply`,
            );
          }
        }
            ...
            >
              ...
              <div className="flex items-center gap-1 shrink-0">
                {font.loaded && (
                  <Badge
                    variant="outline"
                    className="text-[9px] px-1 py-0 h-4 text-green-600 border-green-300"
                  >
                    Loaded
                  </Badge>
                )}
