...
export default function DesignDashboard() {
  ...
  const [selectedFormat, setSelectedFormat] = useState<DesignFormat>(
    DESIGN_FORMATS[0],
  );
  ...
  const handleCreateNew = () => {
    const id = createDesign(
      newName.trim() || "Design sans titre",
      selectedFormat,
    );
    ...
    if (store.currentDesign) {
      ...
      if (typeof window !== "undefined") {
        localStorage.setItem(
          `design-${id}`,
          JSON.stringify(designWithTemplate),
        );
      }
    }
                  ...
                  <div className="flex items-center justify-between gap-2 mb-1">
                    ...
                    <DropdownMenu>
                      ...
                      >
                        <DropdownMenuItem
                          onClick={() =>
                            handleRenameOpen(design.id, design.name)
                          }
                          className="gap-2"
                        >
        ...
        <DialogContent className="sm:max-w-[520px] max-h-[80vh] overflow-y-auto">
          ...
          >
            <DialogHeader>
              ...
              <DialogDescription>
                Choose a name and format for your new design.
              </DialogDescription>
            </DialogHeader>
                ...
                {formatCategories.map((cat) => (
                  <div key={cat} className="space-y-1.5">
                    ...
                    <div className="grid grid-cols-3 gap-1.5">
                      {DESIGN_FORMATS.filter((f) => f.category === cat).map(
                        (fmt) => (
                          <button
                            key={fmt.id}
                            type="button"
