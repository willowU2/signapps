...
export default function DesignHistoryPanel({ fabricCanvasRef }: DesignHistoryPanelProps) {
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        {history.length === 0 ? (
          ...
          <div className="space-y-0.5 px-2">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[9px] opacity-60">{formatTime(entry.timestamp)}</p>
                  </div>
            ...
            <button
              ...
              onClick={() => { while (useDesignStore.getState().undoStack.length > 0) undo(); }}
            >
      ...
      <div className="px-3 py-2 border-t text-[10px] text-muted-foreground">
        {undoStack.length} undo{undoStack.length !== 1 ? "s" : ""} · {redoStack.length} redo{redoStack.length !== 1 ? "s" : ""} available
      </div>
    </div>
  );
}
