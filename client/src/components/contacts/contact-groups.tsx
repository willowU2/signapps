    ...
    onGroupsChange(
      groups.map((g) =>
        g.id !== groupId
          ...
          : {
              ...g,
              memberIds: g.memberIds.includes(contactId)
                ? g.memberIds
                : [...g.memberIds, contactId],
            },
      ),
    );
              ...
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  ...
                  >
                    ...
                    {g.name}
                    <Badge className={`text-xs ${g.color}`}>
                      {members.length}
                    </Badge>
                  </CardTitle>
                  <Button
                    ...
                    <div className="space-y-1">
                      {nonMembers.length === 0 ? (
                        <p className="text-xs text-muted-foreground text-center py-2">
                          Tous les contacts sont membres.
                        </p>
                      ) : (
                        nonMembers.slice(0, 8).map((c) => (
                          <button
                            key={c.id}
                            onClick={() => handleAddMember(g.id, c.id)}
                      ...
                      <Button
                        ...
                        onClick={() => setAddingMemberId(null)}
                      >
                        Fermer
                      </Button>
                    </div>
