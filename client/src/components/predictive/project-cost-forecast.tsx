...
export const ProjectCostForecast: React.FC = () => {
  ...
  const getStatusBadge = (status: string): string => {
    switch (status) {
      ...
      case 'overrun': return 'bg-red-600 text-white';
    ...
    <div className="p-6 bg-white rounded-lg shadow">
      ...
      {overrunProjects.length > 0 && (
        <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg flex items-start gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-red-700">{overrunProjects.length} project(s) have cost overrun risk</p>
          </div>
        </div>
      )}
      ...
      <div className="mb-6 grid grid-cols-1 gap-3">
        {projects.map((project) => {
          ...
          const variancePercent = ((variance / project.plannedBudget) * 100).toFixed(1);
          const maxBudget = Math.max(project.plannedBudget, project.predictedCost);
                ...
                <div className="flex gap-1 h-6">
                  ...
                  <div className="flex-1 bg-gray-200 rounded-r overflow-hidden relative">
                    <div
                      ...
                      style={{ width: `${((project.predictedCost - project.spent) / maxBudget) * 100}%` }}
                    />
                  </div>
                </div>
              </div>
