...
import { ChevronLeft, ChevronRight, X, Play, Pause, Maximize2, Check } from "lucide-react";
...
export function NotePresentation({ notes, startIndex = 0, onClose }: NotePresentationProps) {
  const [current, setCurrent] = useState(startIndex);
  const [autoPlay, setAutoPlay] = useState(false);
  const [intervalMs, setIntervalMs] = useState(5000);

  const prev = useCallback(() => setCurrent((c) => Math.max(0, c - 1)), []);
  const next = useCallback(() => setCurrent((c) => Math.min(notes.length - 1, c + 1)), [notes.length]);
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      setCurrent((c) => {
        if (c >= notes.length - 1) { setAutoPlay(false); return c; }
        return c + 1;
      });
    }, intervalMs);
  ...
  const checkPct = note.checklistItems.length > 0
    ? Math.round((checked.length / note.checklistItems.length) * 100) : 0;
    ...
    <div className="fixed inset-0 z-50 bg-black/95 flex flex-col" onClick={onClose}>
      ...
      <div className="flex items-center justify-between px-6 py-4" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-3">
          <span className="text-white/60 text-sm">{current + 1} / {notes.length}</span>
          {/* Progress dots */}
      ...
      <div className="flex-1 flex items-center justify-center px-16" onClick={(e) => e.stopPropagation()}>
        <div
          ...
          style={{ backgroundColor: bg ?? "#303134", border: "1px solid rgba(255,255,255,0.1)" }}
        >
          {note.title && (
            <h1 className="text-3xl sm:text-4xl font-bold text-white mb-6 leading-tight">{note.title}</h1>
          )}
          ...
          {note.hasChecklist && note.checklistItems.length > 0 ? (
            <div className="space-y-3">
              ...
              {unchecked.map((item) => (
                <div key={item.id} className="flex items-center gap-3 text-white text-lg">
                  <div className="size-5 rounded-sm border-2 border-white/50 shrink-0" />
                  <span>{item.text}</span>
                </div>
              ))}
              {checked.map((item) => (
                <div key={item.id} className="flex items-center gap-3 text-white/40 text-lg line-through">
            ...
            note.content && (
              <p className="text-xl text-white/90 whitespace-pre-wrap leading-relaxed">{note.content}</p>
            )
          )}
