...
import { Calendar, AlertTriangle, CheckCircle2, RefreshCw, Bell, Plus, Trash2 } from "lucide-react"
...
const STATUS_CONFIG: Record<ExpiryStatus, { label: string; variant: "destructive" | "default" | "secondary" | "outline"; icon: React.ReactNode }> = {
  expired: { label: "Expired", variant: "destructive", icon: <AlertTriangle className="h-3.5 w-3.5" /> },
  critical: { label: "Critical", variant: "destructive", icon: <AlertTriangle className="h-3.5 w-3.5" /> },
  warning: { label: "Expiring soon", variant: "default", icon: <Bell className="h-3.5 w-3.5" /> },
  ok: { label: "Valid", variant: "secondary", icon: <CheckCircle2 className="h-3.5 w-3.5" /> },
}
...
export function DocExpiryAlerts() {
  ...
  const filtered = records.filter(r =>
    filter === "all" || getStatus(r.expiry_date, r.renewal_reminder_days) === filter
  )

  const counts = {
    expired: records.filter(r => getStatus(r.expiry_date, r.renewal_reminder_days) === "expired").length,
    critical: records.filter(r => getStatus(r.expiry_date, r.renewal_reminder_days) === "critical").length,
    warning: records.filter(r => getStatus(r.expiry_date, r.renewal_reminder_days) === "warning").length,
    ok: records.filter(r => getStatus(r.expiry_date, r.renewal_reminder_days) === "ok").length,
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        {(["all", "expired", "critical", "warning", "ok"] as const).map(s => (
          ...
          >
            {s === "all" ? `All (${records.length})` : `${s.charAt(0).toUpperCase() + s.slice(1)} (${counts[s]})`}
          </button>
        ))}
      </div>
      ...
      <ScrollArea className="h-72">
        ...
        {filtered.map(r => {
          ...
          return (
            <div key={r.id} className="flex items-start justify-between border-b py-3 px-1 last:border-0">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 mb-0.5">
                  <p className="text-sm font-medium truncate">{r.doc_title}</p>
                  <Badge variant={conf.variant} className="text-xs gap-1 flex-shrink-0">
              ...
              <Button size="icon" variant="ghost" className="h-7 w-7 flex-shrink-0 ml-2 text-destructive" onClick={() => deleteRecord(r.id)}>
        ...
        {!loading && filtered.length === 0 && (
          <p className="text-center text-sm text-muted-foreground py-8">No records</p>
        )}
      </ScrollArea>
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3 py-2">
            <div className="space-y-1.5">
              ...
              <Input value={form.doc_title} onChange={e => setForm(p => ({ ...p, doc_title: e.target.value }))} placeholder="NDA with ACME Corp" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <Input type="number" min={1} value={form.renewal_reminder_days} onChange={e => setForm(p => ({ ...p, renewal_reminder_days: Number(e.target.value) }))} />
              </div>
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">Category</Label>
              <Input value={form.category} onChange={e => setForm(p => ({ ...p, category: e.target.value }))} placeholder="contract / license / certificate…" />
            </div>
            <div className="flex items-center justify-between">
