...
export function DocMergeFields({ onInsert }: DocMergeFieldsProps) {
  ...
  const grouped = (
    ["contacts", "deals", "company", "user", "custom"] as FieldSource[]
  )
    .map((src) => ({
      source: src,
                ...
                {group.fields.map((field) => (
                  <div
                    ...
                    onClick={() => {
                      ...
                      toast.success(`Inserted ${field.token}`);
                    }}
                  >
          ...
          <Collapsible open={addOpen} onOpenChange={setAddOpen}>
            ...
            <CollapsibleContent className="space-y-2 mt-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((p) => ({ ...p, default_value: e.target.value }))
                }
                className="h-7 text-xs"
              />
              <Button size="sm" className="w-full" onClick={addCustomField}>
                Add
              </Button>
            </CollapsibleContent>
          </Collapsible>
