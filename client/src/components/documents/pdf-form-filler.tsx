...
export function PdfFormFiller() {
  ...
  const handleUpload = useCallback(async (file: File) => {
    ...
    try {
      ...
      const res = await fetch("/api/docs/pdf/detect-fields", {
        method: "POST",
        body: formData,
      });
      ...
      toast.error(
        "Détection des champs échouée — vous pouvez les ajouter manuellement",
      );
    } finally {
      setDetecting(false);
    }
  ...
  async function fillPdf() {
    if (!pdfFile) {
      toast.error("Téléversez un PDF d'abord");
      return;
    }
    ...
    try {
      ...
      const progressInterval = setInterval(() => {
        setJob((j) =>
          j ? { ...j, progress: Math.min(j.progress + 10, 80) } : j,
        );
      }, 300);

      const res = await fetch("/api/docs/pdf/fill", {
        method: "POST",
        body: formData,
      });
      ...
      setJob((j) =>
        j ? { ...j, status: "done", progress: 100, output_url: url } : j,
      );
      toast.success("PDF filled successfully");
    } catch {
      setJob((j) =>
        j ? { ...j, status: "error", progress: 0, error: "Fill failed" } : j,
      );
        ...
        {pdfFile ? (
          ...
          <>
            ...
            <p className="text-sm text-muted-foreground">
              Drop a PDF form or click to upload
            </p>
          </>
        )}
      </div>
