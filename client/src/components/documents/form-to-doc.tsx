  ...
  async function triggerManual(templateId: string) {
    ...
    try {
      const res = await fetch(
        `/api/docs/form-templates/${templateId}/generate`,
        { method: "POST" },
      );
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Form: <span className="font-medium">{t.form_name}</span> →
                    Doc:{" "}
                    <span className="font-medium">{t.doc_template_name}</span>
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {t.field_mappings.length} field mappings ·{" "}
                    {t.generated_count} docs generated
                  </p>
                  {t.last_generated_at && (
                    <p className="text-xs text-muted-foreground">
                      Last:{" "}
                      {format(new Date(t.last_generated_at), "MMM d, HH:mm")}
                    </p>
                  )}
                </div>
