...
function buildPreview(s: Omit<NumberingScheme, "id" | "preview">): string {
  ...
  const year = s.year_in_number
    ? `${new Date().getFullYear()}${s.separator}`
    : "";
...
export function DocNumberingSchemes() {
  ...
  async function loadSchemes() {
    ...
    try {
      ...
      setSchemes(
        (data.data ?? []).map((s: NumberingScheme) => ({
          ...s,
  ...
  async function handleSave() {
    if (!form.name || !form.prefix) {
      toast.error("Le nom et le préfixe sont requis");
      return;
    }
    setSaving(true);
    try {
      ...
      if (editScheme) {
        ...
        setSchemes((prev) =>
          prev.map((s) =>
            s.id === editScheme.id
              ? { ...updated, preview: buildPreview(updated) }
              : s,
          ),
        );
        ...
        setSchemes((prev) => [
          ...prev,
          { ...created, preview: buildPreview(created) },
        ]);
    ...
    <Card>
      ...
      <CardContent className="space-y-2">
        ...
        {!loading && schemes.length === 0 && (
          <p className="text-xs text-muted-foreground text-center py-4">
            No numbering schemes configured
          </p>
        )}
        {schemes.map((s) => (
          <div
          ...
          >
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-muted-foreground">
                Current: {s.current_value} · Next: {s.current_value + 1}
              </p>
            </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>
              {editScheme ? "Edit Scheme" : "New Numbering Scheme"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  value={form.padding}
                  onChange={(e) =>
                    setForm((p) => ({ ...p, padding: Number(e.target.value) }))
                  }
                  className="h-8"
                />
              </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <Switch
                checked={form.year_in_number}
                onCheckedChange={(v) =>
                  setForm((p) => ({ ...p, year_in_number: v }))
                }
              />
            </div>
            <div className="flex items-center justify-between">
