...
export function BulkPdfGenerator() {
  ...
  async function runBulk() {
    ...
    if (!rows.length) {
      toast.error("Téléversez les données CSV");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        const blob = await res.blob();
        setJob((j) =>
          j ? { ...j, running: false, zip_url: URL.createObjectURL(blob) } : j,
        );
      }
    } catch {}
    ...
    toast.success(
      `${updated.filter((r) => r.status === "done").length} PDFs generated`,
    );
  }
        ...
        <Card>
          <CardContent className="pt-4 pb-3 space-y-2">
            <div className="flex items-center justify-between text-sm">
              <span>
                {job.done}/{job.total} generated
              </span>
              {job.errors > 0 && (
                <span className="text-destructive">{job.errors} errors</span>
              )}
            </div>
