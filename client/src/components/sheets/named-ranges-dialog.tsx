...
export function NamedRangesDialog({
  ...
  const handleAdd = () => {
    ...
    if (!validateName(name)) {
      toast.error(
        "Nom invalide — lettres, chiffres et _ uniquement, commence par une lettre",
      );
      return;
    }
    if (!range) {
      toast.error("La plage est requise");
      return;
    }
    if (namedRanges.some((nr) => nr.name === name)) {
  ...
  return (
    ...
    >
      <div
        className="bg-background dark:bg-[#2d2e30] rounded-xl shadow-2xl p-4 w-[480px] max-h-[70vh] overflow-y-auto flex flex-col gap-3"
      ...
      >
        ...
        <div className="flex-1 space-y-1 max-h-[280px] overflow-y-auto">
          ...
          {namedRanges.map((nr) => (
            <div
              key={nr.name}
              className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-[#f1f3f4] dark:hover:bg-[#3c4043]"
            >
              {editingName === nr.name ? (
                <>
                  <input
                    className="flex-1 h-7 bg-[#f1f3f4] dark:bg-[#3c4043] rounded px-2 text-[12px] outline-none font-mono"
                    value={editName}
                    onChange={(e) => setEditName(e.target.value)}
                    placeholder="Nom"
                  />
                  <input
                    className="flex-1 h-7 bg-[#f1f3f4] dark:bg-[#3c4043] rounded px-2 text-[12px] outline-none font-mono uppercase"
                    value={editRange}
                    onChange={(e) => setEditRange(e.target.value.toUpperCase())}
                    placeholder="A1:B10"
                  />
                  <button
                ...
                <>
                  <span className="flex-1 text-[12px] font-semibold text-[#1a73e8]">
                    {nr.name}
                  </span>
                  <span className="flex-1 text-[12px] font-mono text-muted-foreground">
                    {nr.range}
                  </span>
                  <button
                    onClick={() => {
