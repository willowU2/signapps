...
export function AdvancedValidationDialog({
  current,
  onApply,
  onClose,
}: AdvancedValidationDialogProps) {
  const [type, setType] = useState<AdvancedValidation["type"]>(
    current?.type || "list",
  );
  const [listValues, setListValues] = useState(
    current?.type === "list" ? current.values.join(", ") : "",
  );
  const [numMin, setNumMin] = useState(
    current?.type === "number_range" ? String(current.min) : "",
  );
  const [numMax, setNumMax] = useState(
    current?.type === "number_range" ? String(current.max) : "",
  );
  const [dateMin, setDateMin] = useState(
    current?.type === "date_range" ? current.minDate : "",
  );
  const [dateMax, setDateMax] = useState(
    current?.type === "date_range" ? current.maxDate : "",
  );
  const [regexPattern, setRegexPattern] = useState(
    current?.type === "regex" ? current.pattern : "",
  );
  const [formula, setFormula] = useState(
    current?.type === "custom_formula" ? current.formula : "",
  );
  const [message, setMessage] = useState(
    (current as { message?: string } | undefined)?.message || "",
  );
  ...
  const handleApply = () => {
    ...
    switch (type) {
      ...
      case "boolean":
        ...
        break;
      case "number_range":
        if (isNaN(Number(numMin)) || isNaN(Number(numMax))) {
          toast.error("Valeurs numériques requises");
          return;
        }
        v = {
          type: "number_range",
      ...
      case "custom_formula":
        if (!formula.startsWith("=")) {
          toast.error("La formule doit commencer par =");
          return;
        }
  ...
  return (
    ...
    >
      <div
        className="bg-background dark:bg-[#2d2e30] rounded-xl shadow-2xl p-4 w-[400px]"
        ...
        <div className="space-y-3">
          <div>
            ...
            <select
              ...
              value={type}
              onChange={(e) =>
                setType(e.target.value as AdvancedValidation["type"])
              }
            >
          ...
          {type === "list" && (
            <div>
              ...
              <input
                className="w-full h-8 bg-[#f1f3f4] dark:bg-[#3c4043] rounded px-2 text-[12px] outline-none"
                placeholder="Oui, Non, En attente"
                value={listValues}
          ...
          {type === "regex" && (
            <div>
              ...
              <input
                className="w-full h-8 bg-[#f1f3f4] dark:bg-[#3c4043] rounded px-2 text-[12px] outline-none font-mono"
                placeholder="^\d{5}$"
                value={regexPattern}
          ...
          {type === "custom_formula" && (
            <div>
              <label className="text-[11px] text-muted-foreground mb-1 block">
                Formule (doit retourner VRAI/FAUX)
              </label>
              <input
                className="w-full h-8 bg-[#f1f3f4] dark:bg-[#3c4043] rounded px-2 text-[12px] outline-none font-mono"
          ...
          <div className="flex gap-2 pt-1">
            <button
              onClick={() => {
                onApply(undefined);
                onClose();
                toast.info("Validation supprimée");
              }}
              className="flex-1 h-8 border border-[#dadce0] dark:border-[#5f6368] rounded text-[12px] hover:bg-muted"
            >
              Supprimer
            </button>
            <button
              onClick={handleApply}
              className="flex-1 h-8 bg-[#1a73e8] text-white rounded text-[12px] font-medium hover:bg-[#1557b0]"
            >
              Appliquer
            </button>
          </div>
