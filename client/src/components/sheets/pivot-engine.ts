...
export function buildPivot(
    rawData: string[][], config: PivotConfig
): { headers: string[], rows: string[][] } {
    if (config.rows.length === 0 || config.values.length === 0) return { headers: [], rows: [] }
    ...
    if (config.columns.length > 0) {
        ...
        const allColValues = [...new Set(filtered.map(r => r[colField.colIndex] ?? ''))].sort()
        const headers = [
            ...config.rows.map(r => r.name),
            ...allColValues.flatMap(cv => config.values.map(v => `${cv} (${v.agg} ${v.field.name})`))
        ]
        ...
        for (const row of filtered) {
            ...
            colMap.get(colKey)!.push(config.values.map(v => {
                const raw = row[v.field.colIndex]; return raw !== undefined ? Number(raw) : NaN
            }))
        }
        ...
        for (const [rowKey, colMap] of grouped) {
            ...
            for (const cv of allColValues) {
                ...
                for (let vi = 0; vi < config.values.length; vi++) {
                    vals.push(fmtNum(computeAggregate(entries.map(e => e[vi]).filter(n => !isNaN(n)), config.values[vi].agg)))
                }
            }
    ...
    for (const row of filtered) {
        ...
        groups.get(rowKey)!.push(config.values.map(v => {
            const raw = row[v.field.colIndex]; return raw !== undefined ? Number(raw) : NaN
        }))
    }

    const headers = [...config.rows.map(r => r.name), ...config.values.map(v => `${v.agg}(${v.field.name})`)]
    const resultRows: string[][] = []
    for (const [rowKey, entries] of groups) {
        resultRows.push([
            ...rowKey.split(' | '),
            ...config.values.map((v, vi) => fmtNum(computeAggregate(entries.map(e => e[vi]).filter(n => !isNaN(n)), v.agg)))
        ])
    }
    return { headers, rows: resultRows }
}
