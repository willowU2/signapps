...
export function AiSheetsDialog({
  onClose,
  selectionBounds,
  data,
  onApplyResult,
  ...
  return (
    <div className="absolute top-12 right-4 w-96 bg-background dark:bg-[#2d2e30] border border-[#dadce0] dark:border-[#5f6368] rounded-xl shadow-2xl z-50 flex flex-col overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-purple-100 to-indigo-100 dark:from-purple-900/40 dark:to-indigo-900/40 border-b border-[#dadce0] dark:border-[#5f6368]">
        ...
        <button
          onClick={onClose}
          className="text-[#5f6368] hover:text-[#202124] dark:hover:text-[#e8eaed] transition-colors"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
          ...
          <div className="mt-2 space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button
                size="sm"
                onClick={handleApply}
                disabled={!activeCell || isStreaming}
                className="h-8 text-xs bg-purple-600 hover:bg-purple-700 text-white shadow-sm transition-colors"
              >
                Appliquer à{" "}
                {activeCell
                  ? `${indexToCol(activeCell.c)}${activeCell.r + 1}`
