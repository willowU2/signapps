...
const CHART_TYPES: { type: ChartType; label: string; icon: React.ReactNode }[] =
  [
    ...
    { type: "pie", label: "Circulaire", icon: <PieIcon className="w-5 h-5" /> },
    {
      type: "scatter",
      label: "Nuage",
      icon: <ScatterIcon className="w-5 h-5" />,
    },
...
function parseSelectionData(
  ...
  for (let c = bounds.minC + (numCols > 1 ? 1 : 0); c <= bounds.maxC; c++) {
    const name =
      headerRow >= 0 ? getValue(headerRow, c) : `Serie ${c - bounds.minC}`;
    const values: number[] = [];
    for (let r = dataStartRow; r <= bounds.maxR; r++) {
  ...
  if (numCols === 1) {
    ...
    return {
      ...
      series: [
        {
          name: headerRow >= 0 ? getValue(headerRow, bounds.minC) : "Valeurs",
          values,
        },
      ],
    };
...
export function ChartDialog({
  ...
  const exportPNG = useCallback(async () => {
    ...
    try {
      ...
      if (!svg) {
        toast.error("Pas de graphique a exporter");
        return;
      }
      ...
      >
        ...
        >
          <p className="text-sm text-center">
            Selectionnez une plage de donnees avant de creer un graphique.
          </p>
          <button
            onClick={onClose}
  ...
  return (
    ...
    >
      <div
        className="bg-background dark:bg-[#2d2e30] rounded-xl shadow-2xl w-[750px] max-h-[85vh] flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-[220px] border-r border-[#dadce0] dark:border-[#5f6368] p-3 space-y-3 overflow-y-auto">
            {/* Chart type selector */}
            <div>
              <div className="text-[11px] font-medium text-[#5f6368] dark:text-[#9aa0a6] mb-1.5 uppercase tracking-wide">
                Type
              </div>
            ...
            {/* Title */}
            <div>
              <label className="text-[11px] font-medium text-[#5f6368] dark:text-[#9aa0a6] mb-1 block uppercase tracking-wide">
                Titre
              </label>
              <input
                className="w-full h-7 bg-[#f1f3f4] dark:bg-[#3c4043] rounded px-2 text-[12px] outline-none"
            ...
            {/* Color palette */}
            <div>
              <div className="text-[11px] font-medium text-[#5f6368] dark:text-[#9aa0a6] mb-1 uppercase tracking-wide flex items-center gap-1">
              ...
              <ResponsiveContainer width="100%" height="100%">
                {config.type === "bar" ? (
                  ...
                  <PieChart>
                    ...
                    <Pie
                      data={chartData.map((d, i) => ({
                        name: d.label,
                        value: d[parsed.series[0]?.name ?? ""] as number,
                      }))}
                      dataKey="value"
                      nameKey="name"
                      cx="50%"
                  ...
                  <ScatterChart>
                    ...
                    {config.showLegend && <Legend />}
                    <Scatter
                      name={parsed.series[0]?.name || "Data"}
                      data={parsed.labels.map((_, i) => ({
                        x: parsed.series[0]?.values[i] ?? 0,
                        y:
                          parsed.series[1]?.values[i] ??
                          parsed.series[0]?.values[i] ??
                          0,
                      }))}
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-[#dadce0] dark:border-[#5f6368]">
          <button
            onClick={exportPNG}
            className="flex items-center gap-1.5 px-3 h-8 border border-[#dadce0] dark:border-[#5f6368] rounded text-[12px] hover:bg-[#f1f3f4] dark:hover:bg-[#3c4043]"
          >
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() => {
                onInsertChart(config, parsed);
                toast.success("Graphique insere");
                onClose();
              }}
