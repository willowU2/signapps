...
export function MacroEditor({
  ...
  const runMacro = useCallback(() => {
    ...
    const api: MacroApi = {
      getCell: (row: number, col: number): string => {
        ...
        return (
          evaluatedData[`${row},${col}`] || data[`${row},${col}`]?.value || ""
        );
      },
    ...
    try {
      ...
      logs.push(`[ERREUR] ${err instanceof Error ? err.message : String(err)}`);
    }
    ...
    if (logs.length > 0) {
      const first = logs[0];
      if (first.startsWith("[ERREUR]"))
        toast.error(first.replace("[ERREUR] ", ""));
      else if (first.startsWith("[WARN]"))
        toast.warning(first.replace("[WARN] ", ""));
  ...
  const handleSave = useCallback(() => {
    ...
    if (existingIdx >= 0) {
      ...
      docsClient
        .put(`/docs/${sheetId}/macros/${encodeURIComponent(name)}`, { code })
        .catch(() => {});
    } else {
      updated = [...savedMacros, macro];
      docsClient
        .post(`/docs/${sheetId}/macros`, { name, code })
        .catch(() => {});
    }
  ...
  const handleDelete = useCallback(
    (name: string) => {
      ...
      saveMacrosToStorage(sheetId, updated);
      docsClient
        .delete(`/docs/${sheetId}/macros/${encodeURIComponent(name)}`)
        .catch(() => {});
      ...
      >
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-2 px-4 py-2 border-b border-[#e3e3e3] dark:border-[#5f6368] bg-[#f8f9fa] dark:bg-[#252526]">
            <input
              className="h-7 w-[180px] bg-[#f1f3f4] dark:bg-[#3c4043] rounded px-2 text-[12px] outline-none border border-transparent focus:border-[#1a73e8]"
            ...
            <button
              ...
              className={cn(
                "flex items-center gap-1 px-3 h-7 rounded text-[12px] font-medium transition-colors",
                isRunning
              ...
              {showSaved && (
                <div className="absolute top-8 left-0 bg-background dark:bg-[#2d2e30] border border-[#dadce0] dark:border-[#5f6368] rounded-lg shadow-xl z-50 w-[240px] py-1 max-h-[200px] overflow-y-auto">
                  {savedMacros.length === 0 && (
                    <div className="px-3 py-2 text-[12px] text-[#9aa0a6] italic">
                      Aucune macro sauvegardee
                    </div>
                  )}
                  {savedMacros.map((m) => (
                    <div
                      key={m.name}
                      className="flex items-center gap-1 px-2 py-1 hover:bg-[#f1f3f4] dark:hover:bg-[#3c4043]"
                    >
                      <button
          ...
          <div className="flex-1 flex flex-col min-h-0">
            <div className="flex items-center px-4 py-1 bg-[#1e1e1e] text-[#808080] text-[11px]">
              ...
              <span className="ml-auto">
                API: api.getCell(), api.setCell(), api.getRange(),
                api.setRange(), api.getSheetName(), api.alert()
              </span>
            </div>
            <textarea
              ref={textareaRef}
          ...
          {output.length > 0 && (
            <div className="border-t border-[#5f6368] bg-[#1e1e1e] max-h-[120px] overflow-y-auto">
              <div className="flex items-center px-4 py-1 text-[11px] text-[#808080] border-b border-[#333]">
                Console
              </div>
          ...
          <div className="flex items-center gap-2 px-4 py-1.5 bg-[#fff3cd] dark:bg-[#554800] text-[11px] text-[#856404] dark:text-[#ffc107] border-t border-[#ffc107]/30">
            ...
            <span>
              Les macros s'executent dans un contexte restreint. Seules les
              fonctions de l'API sont disponibles.
            </span>
          </div>
        </div>
      </div>
