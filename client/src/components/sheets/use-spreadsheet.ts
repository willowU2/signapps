...
export function useSpreadsheet(
    ...
    wsProvider.awareness.setLocalStateField("user", {
      name:
        (typeof window !== "undefined" &&
          (window as unknown as { __signAppsUser?: { name?: string } })
            .__signAppsUser?.name) ||
        "Utilisateur",
      color: myColor,
    });
  ...
  useEffect(() => {
    ...
    const syncSheets = () => {
      ...
      setActiveSheetIndex((prev) =>
        Math.min(prev, Math.max(0, arr.length - 1)),
      );
    };
  ...
  const getGridMap = useCallback(
    () => doc.getMap<CellData>(activeMapKey),
    [doc, activeMapKey],
  );
  ...
  const setCellStyle = useCallback(
    (r: number, c: number, style: Partial<CellStyle>) => {
      const gridMap = getGridMap();
      const key = `${r},${c}`;
  ...
  const setCellFull = useCallback(
    (r: number, c: number, cellData: CellData) => {
  ...
  const setCellComment = useCallback(
    (r: number, c: number, comment: string | undefined) => {
      const gridMap = getGridMap();
      const key = `${r},${c}`;
      const existing = gridMap.get(key) || { value: "" };
  ...
  const setCellValidation = useCallback(
    (r: number, c: number, validation: CellValidation | undefined) => {
      const gridMap = getGridMap();
      const key = `${r},${c}`;
      const existing = gridMap.get(key) || { value: "" };
  ...
  const deleteCellRange = useCallback(
  ...
  const setCellRange = useCallback(
    (startR: number, startC: number, range: (CellData | undefined)[][]) => {
      const gridMap = getGridMap();
      doc.transact(() => {
  ...
  const sortColumn = useCallback(
    (col: number, ascending: boolean) => {
      ...
      const rowsData = Array.from(rowMap.entries()).map(([r, cells]) => ({
        r,
        cells,
      }));
  ...
  const setSheetColor = useCallback(
  ...
  const getCrossSheetValue = useCallback(
    (sheetName: string, r: number, c: number): string => {
      const sheetsMetaV2 = doc.getArray<any>("sheets-meta-v2");
      const arr = sheetsMetaV2.toArray();
      // Allow unquoted names space resilient search
      const cleanName = sheetName.replace(/^'|'$/g, "");
      const match = arr.find(
        (s: any) => s.name.toUpperCase() === cleanName.toUpperCase(),
      );
      if (!match) return "";
