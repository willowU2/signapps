...
export function RevenueAnalytics() {
  ...
  const churnRate = (MRR_DATA[MRR_DATA.length - 1].churn_mrr / currentMRR * 100).toFixed(2);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'MRR', value: `$${(currentMRR / 1000).toFixed(1)}k`, trend: `${isPositive ? '+' : ''}${mrrGrowth}%`, up: isPositive, icon: DollarSign },
          { label: 'Churn Rate', value: `${churnRate}%`, trend: 'vs last month', up: false, icon: AlertCircle },
          { label: 'Avg LTV', value: `$${avgLTV.toFixed(0)}`, trend: 'all plans', up: true, icon: Users },
          { label: 'ARR', value: `$${(currentMRR * 12 / 1000).toFixed(0)}k`, trend: 'annualized', up: true, icon: TrendingUp },
          ...
          <Card key={label}>
            <CardContent className="pt-4">
              ...
              <div className="flex items-center gap-1 mt-1">
                ...
                <span className={`text-xs ${up ? 'text-green-600' : 'text-red-600'}`}>{trend}</span>
              </div>
            </CardContent>
          </Card>
        ))}
        ...
        <CardContent className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={MRR_DATA}>
              ...
              <Area type="monotone" dataKey="mrr" stroke="#3b82f6" fill="url(#mrrGrad)" strokeWidth={2} name="MRR" />
              <Area type="monotone" dataKey="new_mrr" stroke="#22c55e" fill="none" strokeWidth={2} name="New MRR" strokeDasharray="4 2" />
            </AreaChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">LTV by Segment</CardTitle></CardHeader>
        <CardContent>
          <div className="space-y-3">
            {LTV_DATA.map(d => (
              <div key={d.segment} className="flex items-center gap-4">
                <Badge variant="outline" className="w-24 justify-center">{d.segment}</Badge>
                <div className="flex-1 h-6 bg-muted rounded-full overflow-hidden">
                  <div className="h-full bg-primary rounded-full" style={{ width: `${(d.ltv / 12000) * 100}%` }} />
                </div>
                <div className="text-sm font-medium w-24 text-right">${d.ltv.toLocaleString()} LTV</div>
                <div className="text-xs text-muted-foreground w-20 text-right">${d.arpu}/mo · {d.months}mo avg</div>
              </div>
            ))}
          </div>
        </CardContent>
