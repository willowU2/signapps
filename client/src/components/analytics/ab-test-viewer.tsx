...
import { TrendingUp, TrendingDown, FlaskConical, AlertCircle, CheckCircle2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold flex items-center gap-2"><FlaskConical className="h-5 w-5" /> A/B Test Results</h2>
      ...
      {tests.map(test => {
        ...
        const z = zScore(control.visitors, control.conversions, variant.visitors, variant.conversions);
        const sig = significance(z);
        const controlRate = ((control.conversions / control.visitors) * 100).toFixed(2);
        const variantRate = ((variant.conversions / variant.visitors) * 100).toFixed(2);
        const lift = (((variant.conversions / variant.visitors) - (control.conversions / control.visitors)) / (control.conversions / control.visitors) * 100).toFixed(1);
        ...
        return (
          <Card key={test.id}>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">Goal: {test.goal} · Started {new Date(test.started_at).toLocaleDateString()}</p>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-3 md:grid-cols-2">
                {test.variants.map((v, i) => {
                  ...
                  return (
                    <div key={v.name} className={`border rounded-lg p-3 ${isWinner ? 'border-green-500/50 bg-green-500/5' : ''}`}>
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-medium">{v.name}</span>
                        {isWinner && <Badge className="bg-green-500 text-white text-xs">Winner</Badge>}
                        {i === 0 && <Badge variant="outline" className="text-xs">Control</Badge>}
                      </div>
                      <div className="text-2xl font-bold">{rate}%</div>
                      <p className="text-xs text-muted-foreground">{v.conversions.toLocaleString()} / {v.visitors.toLocaleString()} visitors</p>
                      <Progress value={parseFloat(rate)} max={10} className="mt-2 h-2" />
                    </div>
                  );
                })}
              </div>
