...
const FormatBadge: React.FC<{ format: ReportFormat }> = ({ format }) => {
  ...
  return (
    <span
      className={`inline-block px-2 py-1 rounded text-xs font-semibold ${colors[format]}`}
    >
      {format}
    </span>
  );
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      schedule: e.target.value as ReportConfig["schedule"],
                    })
                  }
                  className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
