...
import {
  DynamicMenuCategory,
  DynamicMenuItem,
} from "../docs/editor/editor-menu-config";
      ...
      return (
        <DropdownMenuItem key={item.id} asChild>
          <a href={item.href}>
            {Icon && (
              <Icon
                className={`mr-2 h-4 w-4 ${item.iconOpacity ? `opacity-${item.iconOpacity}` : ""}`}
              />
            )}
        ...
        <DropdownMenu key={category.id}>
          <DropdownMenuTrigger asChild>
            <Button
              variant="ghost"
              size="sm"
              className="h-[26px] px-2 font-normal rounded-sm hover:bg-[#f1f3f4] dark:hover:bg-[#303134] text-inherit transition-colors"
            >
              {category.label}
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent
            align="start"
            className={category.contentClassName || "w-[300px]"}
          >
