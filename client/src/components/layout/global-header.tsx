...
export function GlobalHeader() {
  ...
  const otherMembers = members.filter(
    (m) => m.user_id !== user?.id && onlineUserIds.includes(m.user_id),
  );
  ...
  const rawBreadcrumbItems = pathSegments.map((segment, index) => {
    ...
    // Shorten long IDs
    const displayLabel =
      label.length > 20 && index > 0 ? `${label.substring(0, 8)}...` : label;
  ...
  const breadcrumbItems =
    sectionGroup && pathSegments.length > 0
      ? [
          {
            label: sectionGroup.section,
            url: sectionGroup.sectionUrl,
            isLast: false,
          },
          ...rawBreadcrumbItems,
        ]
                    ...
                    <TooltipTrigger asChild>
                      <PopoverTrigger asChild>
                        <AvatarMore
                          ...
                          className="cursor-pointer transition-transform hover:scale-110 hover:z-50"
                        />
                      </PopoverTrigger>
                    </TooltipTrigger>
                    <TooltipContent>
                  ...
                  <PopoverContent className="w-56 p-2" align="end">
                    ...
                    <div className="flex flex-col gap-1 max-h-[300px] overflow-y-auto w-full">
                      {hiddenMembers.map((m) => (
                        <div
                          key={m.user_id}
                          className="flex items-center gap-2 p-1.5 hover:bg-muted rounded-md cursor-pointer transition-colors"
                        >
