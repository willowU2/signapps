  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      if (
        ...
        if (file && task) {
          ...
          taskAttachmentsApi
            ...
            .then(() => {
              toast.success(
                `"${file.name}" attaché à la tâche "${task.title}"`,
              );
            })
            .catch((err) => {
      ...
      // Handle Task dropped onto a Calendar Slot (only if feature enabled)
        ...
        activeType === "task" &&
        overType === "calendar-slot" &&
        FEATURES.DND_TASK_TO_CALENDAR
      ) {
          ...
          if (calendarId) {
            calendarApi
              ...
              .then((response) => {
                ...
                toast.success(
                  `Tâche "${task.title || task.label}" planifiée pour le ${formattedDate}`,
                );
              })
              .catch((err) => {
      ...
      // Handle File dropped onto Calendar Day (only if feature enabled)
        ...
        toast.success(
          `Started new event for "${file.name}" on ${formattedDate}`,
        );
      }
    }
  };
