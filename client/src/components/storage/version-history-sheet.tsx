...
function computeLineDiff(a: string, b: string) {
  ...
  while (ai < aLines.length || bi < bLines.length) {
    ...
    if (aLine === bLine) {
      ...
      aLine !== null &&
      (bLine === null || !bLines.slice(bi).includes(aLine))
    ) {
...
function DiffViewer({
  ...
  useEffect(() => {
    if (!open) return;
    const isText =
      versionA.content_type.startsWith("text/") ||
      [
        "application/json",
        "application/javascript",
        "application/xml",
      ].includes(versionA.content_type);
    if (!isText) {
      setDiffLines([]);
      return;
    }
    ...
    (async () => {
      ...
      try {
        ...
        const textA =
          typeof ra.data === "string"
            ? ra.data
            : new TextDecoder().decode(ra.data);
        const textB =
          typeof rb.data === "string"
            ? rb.data
            : new TextDecoder().decode(rb.data);
...
export function VersionHistorySheet({
  ...
  const [diffPair, setDiffPair] = useState<[FileVersion, FileVersion] | null>(
    null,
  );
            ...
            {loading ? (
              ...
              <div className="h-full flex flex-col items-center justify-center p-8 text-center text-muted-foreground">
                ...
                <p className="text-xs mt-2 max-w-[250px]">
                  When this file is overwritten, its older contents will appear
                  here.
                </p>
              </div>
            ) : (
                        ...
                        <div className="flex items-center gap-1.5">
                          {index < versions.length - 1 && (
                            <Button
                              ...
                              onClick={() =>
                                setDiffPair([versions[index + 1], version])
                              }
                            >
