  ...
  for (const raw of lines) {
    ...
    if (line.startsWith("```")) {
      if (!inCode) {
        ...
        inCode = false;
        out.push(
          `<pre class="bg-slate-900 text-slate-100 rounded-lg p-4 overflow-x-auto my-3 text-sm font-mono"><code>${escHtml(codeBuf.join("\n"))}</code></pre>`,
        );
        codeBuf = [];
      }
      continue;
    ...
    if (hm) {
      ...
      out.push(
        `<h${lvl} class="${sizes[lvl - 1]} font-bold mt-4 mb-2">${inline(hm[2])}</h${lvl}>`,
      );
      continue;
    }
    ...
    if (line.startsWith("> ")) {
      closeList();
      out.push(
        `<blockquote class="border-l-4 border-primary/40 pl-4 italic text-muted-foreground my-2">${inline(line.slice(2))}</blockquote>`,
      );
      continue;
    }
