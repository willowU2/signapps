...
export function FolderTree({
  ...
  const loadChildren = useCallback(
    async (prefix: string): Promise<TreeNode[]> => {
      try {
  ...
  const handleOpenChange = (newOpen: string[]) => {
    ...
    const newlyOpened = newOpen.filter(
  ...
  const renderNode = (node: TreeNode) => {
    ...
    return (
      <FolderItem key={node.path} value={node.path}>
        <FolderTrigger onClick={() => onSelectFolder(node.path)}>
          <div className="flex items-center gap-2">
            {node.loading && (
              <SpinnerInfinity size={14} className="opacity-50" />
            )}
            <span>{node.name}</span>
          </div>
        </FolderTrigger>
        <FolderPanel>
          <SubFiles>{node.children.map((child) => renderNode(child))}</SubFiles>
        </FolderPanel>
      </FolderItem>
    );
  };
      ...
      {rootLoading ? (
        ...
        >
          <FolderItem value="root_bucket">
            ...
            <FolderPanel>
              <SubFiles>{roots.map((node) => renderNode(node))}</SubFiles>
            </FolderPanel>
          </FolderItem>
        </Files>
      )}
