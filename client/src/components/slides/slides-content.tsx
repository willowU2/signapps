...
export function SlidesContent({
  ...
  const activeSlide = slideState.slides.find(
    (s) => s.id === slideState.activeSlideId,
  );

  const [localTitle, setLocalTitle] = useState(
    activeSlide?.title || "Présentation sans titre",
  );
  ...
  const saveToDrive = useCallback(async () => {
    ...
    try {
      ...
      const targetKey =
        documentId && documentId !== "new-presentation"
          ? `${documentId}.signslides`
  ...
  const exportToPPTX = useCallback(() => {
    ...
    toast.success(
      `Présentation exportée (${allSlidesData.length} diapositives)`,
    );
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden font-sans bg-[#f8f9fa] dark:bg-[#1f1f1f] min-h-0 relative">
      <div className="flex flex-1 overflow-hidden relative min-h-0">
        {/* Sidebar (Slides List) */}
        <div className="w-48 border-r border-border bg-background/50 z-20">
          <SlideSidebar
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-4 lg:p-8 min-w-0">
          {slideState.activeSlideId ? (
            ...
            <div className="text-gray-400 text-sm">
              Créez une diapositive pour commencer
            </div>
          )}
        </div>
      </div>
