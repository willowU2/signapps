...
// Minimal duck-typed canvas interface (avoids importing fabric at compile-time)
...
export function computeAutoLayout(
  ...
  const otherObjects = objects.filter(
    (o) =>
      !isTextObject(o.type) && !isImageObject(o.type) && !isShapeObject(o.type),
  );
  ...
  const spacing = Math.max(
    0,
    (usableHeight - totalHeight) / (objects.length + 1),
  );
...
export function applyLayoutToCanvas(
  ...
  suggestions.forEach((suggestion) => {
    const obj = objects.find((o) => o.id === suggestion.id);
    if (!obj) return;
...
export function extractCanvasObjectInfo(
  canvas: FabricCanvas,
): CanvasObjectInfo[] {
  return canvas
    .getObjects()
...
- Retourne SEULEMENT le JSON array, rien d'autre.`;
  ...
  try {
    const response = await aiApi.chat(prompt, {
      ...
      systemPrompt:
        "Tu es un moteur de layout JSON strict. Retourne uniquement un JSON array valide sans markdown.",
    });
...
export async function performAutoLayout(
  ...
  if (options?.useAi && options.routeConfig) {
    ...
    suggestions = await computeAiLayout(
      objects,
      canvasWidth,
      canvasHeight,
      options.routeConfig,
    ...
    if (suggestions) {
      ...
      toast.info(
        "IA indisponible, utilisation de la disposition automatique.",
        { id: toastId },
      );
    }
  }
