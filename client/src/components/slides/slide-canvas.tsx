...
interface SlideCanvasProps {
  ...
  updateObject: (
    id: string,
    data: fabric.Object | Record<string, unknown>,
  ) => void;
...
export function SlideCanvas({
  ...
  const actualBg =
    pageConfig.backgroundColor === "#ffffff" && resolvedTheme === "dark"
  ...
  useEffect(() => {
    ...
    import("fabric").then((fabricModule) => {
      ...
      (c as unknown as fabric.IText).on(
        "text:changed" as never,
        (e: { target?: FabricObjectWithId }) => {
          const target = e.target;
          ...
          if (
            currentGhostText &&
            ghostedObject &&
            ghostedObject.text?.endsWith(currentGhostText)
          ) {
          ...
          debounceTimer = setTimeout(async () => {
            try {
              ...
              const response = await aiApi.chat(prompt, {
                ...
                systemPrompt:
                  "Tu es un assistant de frappe prédictive hyper-rapide (Smart Compose). Tu ne réponds QUE par la complétion exacte de la phrase, en minuscules de préférence, max 4 mots.",
              });

              const suggestion = response.data.answer.trim();
              if (suggestion && target.isEditing) {
                // Ensure still editing after await
                currentGhostText = suggestion.startsWith(" ")
                  ? suggestion
                  : " " + suggestion;
  ...
  useEffect(() => {
    ...
    import("fabric").then((fabricModule) => {
      Object.entries(objects).forEach(([id, objData]) => {
        const existing = (canvas.getObjects() as FabricObjectWithId[]).find(
          (o) => o.id === id,
        );
        if (existing) {
      ...
      const allObjs = [...canvas.getObjects()] as (FabricObjectWithId & {
        zIndex?: number;
      })[];
  ...
  return (
    <div
      data-testid="slides-editor-canvas"
      className="w-full h-full bg-[#f8f9fa] dark:bg-[#1f1f1f] flex justify-center overflow-auto p-8 relative"
    >
      <div
        ref={wrapperRef}
