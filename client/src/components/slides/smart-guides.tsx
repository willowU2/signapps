...
export function useSmartGuides(
  ...
  useEffect(() => {
    ...
    let overlay = container.querySelector<HTMLCanvasElement>(
    ...
    if (!overlay) {
      ...
      overlay.style.cssText =
        "position:absolute;top:0;left:0;pointer-events:none;z-index:100";
    ...
    const handleMoving = (e: { target?: fabric.Object }) => {
      ...
      if (snapCenterX) {
        guides.v.push(cw / 2);
        obj.set("left", cw / 2 - oW / 2);
      }
      if (snapCenterY) {
        guides.h.push(ch / 2);
        obj.set("top", ch / 2 - oH / 2);
      }
      ...
      for (const other of canvasObjs) {
        ...
        // Left edges
        if (Math.abs(oLeft - otL) < SNAP_THRESHOLD) {
          guides.v.push(otL);
          obj.set("left", otL);
        }
        // Right edges
        if (Math.abs(oRight - (otL + otW)) < SNAP_THRESHOLD) {
          guides.v.push(otL + otW);
          obj.set("left", otL + otW - oW);
        }
        // Centers horizontal
        if (Math.abs(oCenterX - otCx) < SNAP_THRESHOLD) {
          guides.v.push(otCx);
          obj.set("left", otCx - oW / 2);
        }
        // Top edges
        if (Math.abs(oTop - otT) < SNAP_THRESHOLD) {
          guides.h.push(otT);
          obj.set("top", otT);
        }
        // Bottom edges
        if (Math.abs(oBottom - (otT + otH)) < SNAP_THRESHOLD) {
          guides.h.push(otT + otH);
          obj.set("top", otT + otH - oH);
        }
        // Centers vertical
        if (Math.abs(oCenterY - otCy) < SNAP_THRESHOLD) {
          guides.h.push(otCy);
          obj.set("top", otCy - oH / 2);
        }
      }
    ...
    <button
      ...
      title={
        enabled
          ...
          : "Activer les guides intelligents"
      }
