...
export function OmniboxMenu({
  ...
  const items: OmniboxItem[] = [
    ...
    },
    {
      ...
      category: "Basic",
      action: () =>
        onInsertText("Heading", { fontSize: 48, fontWeight: "bold" }),
    },
    {
      id: "table",
      title: "Table",
      description: "Insert a 3x3 editable grid",
      icon: <Table2 className="w-4 h-4 text-muted-foreground" />,
      category: "Basic",
    ...
    },
    {
      id: "image",
      title: "Image Wrapper",
      description: "Import web image via URL",
    ...
    },
    {
      ...
      category: "Shapes",
      action: () => onInsertShape("circle"),
    },
    {
      id: "triangle",
      title: "Triangle",
      description: "Basic triangular shape",
      icon: <Triangle className="w-4 h-4" />,
      category: "Shapes",
      action: () => onInsertShape("triangle"),
    },
    {
      id: "line",
      title: "Line",
      description: "Straight separator line",
      icon: <Minus className="w-4 h-4" />,
      category: "Shapes",
      action: () => onInsertShape("line"),
    },
    ...
    // Smart Blocks
    {
      id: "magic",
      title: "Magic Layout",
      description: "AI generated dashboard layout",
      icon: <Wand2 className="w-4 h-4 text-indigo-500" />,
      category: "Smart Blocks",
      action: onInsertMagicLayout,
    },
    {
      id: "template",
      title: "Strategy Template",
      description: "Pre-built strategy slide",
      icon: <LayoutTemplate className="w-4 h-4 text-indigo-500" />,
      category: "Smart Blocks",
      action: () =>
        onInsertText("Strategy Review", { fontSize: 48, fontWeight: "bold" }),
    },
    ...
    // Smart Chips
    {
      ...
      action: () => onInsertSmartChip("status", "Done", "#10b981"),
    },
    {
      id: "status-todo",
      title: "Status: To Do",
      description: "Gray pending badge",
      icon: <Tag className="w-4 h-4 text-muted-foreground" />,
      category: "Smart Chips",
      action: () => onInsertSmartChip("status", "To Do", "#6b7280"),
    },
    {
      id: "status-urgent",
      title: "Status: Urgent",
      description: "Red alert badge",
      icon: <Tag className="w-4 h-4 text-rose-500" />,
      category: "Smart Chips",
      action: () => onInsertSmartChip("status", "Urgent", "#f43f5e"),
    },
    {
      id: "user-tag",
      title: "User Mention",
      description: "Tag a collaborator",
      icon: <User className="w-4 h-4 text-blue-500" />,
      category: "Smart Chips",
      action: () => onInsertSmartChip("user", "@Etienne"),
    },
    {
      id: "date-tag",
      title: "Date",
    ...
    },
    {
      ...
      description: "Link another document",
      icon: <ImageIcon className="w-4 h-4 text-purple-500" />,
      category: "Smart Chips",
      action: () => onInsertSmartChip("file", "Annual Report.pdf"),
    },

    // Workflows (SaaS Building Blocks)
    {
      id: "workflow-signature",
      title: "eSignature Block",
      description: "Collect a formal signature & date",
      icon: <PenTool className="w-4 h-4 text-purple-500" />,
      category: "Workflows",
      action: () => onInsertWorkflow("signature"),
    },
    {
      id: "workflow-email",
      title: "Email Draft",
      description: "Collaborate on an email template",
      icon: <Mail className="w-4 h-4 text-blue-500" />,
      category: "Workflows",
      action: () => onInsertWorkflow("email"),
    },
    {
      id: "workflow-meeting",
      title: "Meeting Notes",
      ...
      category: "Workflows",
      action: () => onInsertWorkflow("meeting"),
    },
  ];
  ...
  const safeX =
    typeof window !== "undefined" ? Math.min(x, window.innerWidth - 320) : x;
  const safeY =
    typeof window !== "undefined" ? Math.min(y, window.innerHeight - 400) : y;
        ...
        >
                    ...
                    <div className="space-y-0.5">
                      {catItems.map((item, index) => {
                        ...
                        return (
                          ...
                          >
                            <div
                              className={`flex items-center justify-center p-2 rounded-md ${isSelected ? "bg-indigo-100 text-indigo-600" : "bg-muted text-muted-foreground"}`}
                            >
                              {item.icon}
                            </div>
                            <div className="flex flex-col">
                              <span
                                className={`text-sm font-medium ${isSelected ? "text-indigo-900" : "text-foreground"}`}
                              >
                                {item.title}
                              </span>
                              {item.description && (
                                <span
                                  className={`text-xs ${isSelected ? "text-indigo-500" : "text-muted-foreground"}`}
                                >
                                  {item.description}
                                </span>
                              )}
          ...
          <div className="px-4 py-2 bg-muted border-t border-gray-100 flex items-center justify-between text-[10px] text-gray-400">
            <div className="flex items-center gap-1">
              <kbd className="px-1.5 py-0.5 rounded bg-background border border-border font-sans shadow-sm">
                ↑↓
              </kbd>{" "}
              to navigate
            </div>
