...
import {
    Play, ArrowRight, ArrowLeft, ArrowUp, ZoomIn, ZoomOut, Eye, EyeOff,
    Sparkles, X
} from "lucide-react"
...
export const ENTRANCE_ANIMATIONS: Record<EntranceAnimation, { label: string; icon: React.ReactNode }> = {
    none: { label: 'Aucune', icon: <X className="w-3 h-3" /> },
    fadeIn: { label: 'Fondu', icon: <Eye className="w-3 h-3" /> },
    slideInLeft: { label: 'Glisser depuis la gauche', icon: <ArrowRight className="w-3 h-3" /> },
    slideInRight: { label: 'Glisser depuis la droite', icon: <ArrowLeft className="w-3 h-3" /> },
    slideInUp: { label: 'Glisser depuis le bas', icon: <ArrowUp className="w-3 h-3" /> },
    zoomIn: { label: 'Zoom avant', icon: <ZoomIn className="w-3 h-3" /> },
    bounceIn: { label: 'Rebond', icon: <Sparkles className="w-3 h-3" /> },
}

export const EXIT_ANIMATIONS: Record<ExitAnimation, { label: string; icon: React.ReactNode }> = {
    none: { label: 'Aucune', icon: <X className="w-3 h-3" /> },
    fadeOut: { label: 'Fondu sortant', icon: <EyeOff className="w-3 h-3" /> },
    slideOutLeft: { label: 'Glisser vers la gauche', icon: <ArrowLeft className="w-3 h-3" /> },
    slideOutRight: { label: 'Glisser vers la droite', icon: <ArrowRight className="w-3 h-3" /> },
                ...
                <div className="p-4 space-y-6">
                    {!selectedObjectId ? (
                        <p className="text-sm text-muted-foreground text-center py-8">
                            Sélectionnez un objet sur le canevas pour configurer ses animations.
                        </p>
                    ) : (
                        <>
                            {/* Entrance Animation */}
                            <div className="space-y-2">
                                ...
                                <div className="grid grid-cols-2 gap-1.5">
                                    {(Object.keys(ENTRANCE_ANIMATIONS) as EntranceAnimation[]).map(key => {
                                        const anim = ENTRANCE_ANIMATIONS[key]
                                        return (
                                            <button
                                                key={key}
                                                onClick={() => onAnimationChange({ ...animationConfig, entranceType: key })}
                                                className={cn(
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="grid grid-cols-2 gap-1.5">
                                    {(Object.keys(EXIT_ANIMATIONS) as ExitAnimation[]).map(key => {
                                        const anim = EXIT_ANIMATIONS[key]
                                        return (
                                            <button
                                                key={key}
                                                onClick={() => onAnimationChange({ ...animationConfig, exitType: key })}
                                                className={cn(
                                                    "flex items-center gap-1.5 px-2 py-1.5 rounded text-xs transition-colors",
                                                    "border",
                            ...
                            <div className="space-y-2">
                                ...
                                <Slider
                                    ...
                                    onValueChange={([val]) => onAnimationChange({ ...animationConfig, duration: val })}
                                    min={100}
                                    max={2000}
                                    step={100}
                                    className="w-full"
                            ...
                            <div className="space-y-2">
                                ...
                                <Slider
                                    ...
                                    onValueChange={([val]) => onAnimationChange({ ...animationConfig, delay: val })}
                                    min={0}
                                    max={3000}
                                    step={100}
                                    className="w-full"
