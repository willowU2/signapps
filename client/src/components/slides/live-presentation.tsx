...
import {
    ChevronLeft, ChevronRight, X, Timer, MonitorPlay,
    QrCode, Copy, Check, ThumbsUp, Heart, HelpCircle, Pointer
} from "lucide-react"
import {
...
export const REACTION_ICONS: Record<ReactionType, { emoji: string; label: string }> = {
...
function useTimer() {
    ...
    return { seconds, formatted, isRunning, toggle: () => setIsRunning(r => !r), reset: () => setSeconds(0) }
}
...
export function LivePresenterView({
    ...
    const [reactions, setReactions] = useState<Record<ReactionType, number>>({
        thumbsUp: 0, clap: 0, heart: 0, question: 0
    })
        ...
        if (channel) {
            channel.onmessage = (event: MessageEvent<LiveSyncMessage>) => {
                ...
                if (msg.type === 'reaction' && msg.reaction) {
                    setReactions(prev => ({ ...prev, [msg.reaction!]: prev[msg.reaction!] + 1 }))
                }
            }
        }
    ...
    const transition = transitions[current] || { type: 'none' as SlideTransitionType, duration: 500 }
            ...
            <div className="flex items-center justify-between px-4 py-2 bg-gray-800 text-white shrink-0">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1.5 bg-gray-700 rounded px-2 py-1">
                        ...
                        <span className="text-xs text-white/60 max-w-[180px] truncate">{shareUrl}</span>
                        <button onClick={handleCopyLink} className="text-white/60 hover:text-white">
                    ...
                    <button
                        ...
                        className={cn(
                            ...
                            laserActive ? "bg-red-500/30 text-red-400" : "bg-gray-700 text-white/60 hover:text-white"
                        )}
                    >
                    ...
                    <div className="flex items-center gap-2 text-sm">
                        {(Object.keys(REACTION_ICONS) as ReactionType[]).map(key => (
                            <span key={key} className="flex items-center gap-0.5" title={REACTION_ICONS[key].label}>
                    ...
                    <Button variant="ghost" size="sm" onClick={handleEnd} className="text-white/60 hover:text-white h-7">
                        <X className="w-4 h-4" />
                    </Button>
                </div>
            </div>
            ...
            <div className="flex-1 flex min-h-0">
                ...
                >
                    <AnimatePresence mode="wait">
                        <motion.div
                            ...
                            animate="animate"
                            exit="exit"
                            transition={{ duration: transition.duration / 1000, ease: 'easeInOut' }}
                ...
                <div className="w-80 bg-gray-800 flex flex-col border-l border-gray-700 shrink-0">
                    ...
                    <div className="p-3 border-b border-gray-700">
                        <span className="text-xs text-white/50 font-medium mb-2 block">Diapositive suivante</span>
                        <div className="aspect-[16/10] bg-gray-900 rounded overflow-hidden flex items-center justify-center">
                            {current < slides.length - 1 ? (
                                ...
                                <span className="text-xs text-white/30">Fin de la presentation</span>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="flex-1 p-3 overflow-auto">
                        <span className="text-xs text-white/50 font-medium mb-2 block">Notes</span>
        ...
        return (
            <div className="fixed inset-0 z-[9999] bg-black flex items-center justify-center">
                <div className="text-center space-y-4">
                    <h2 className="text-2xl font-bold text-white">Presentation terminee</h2>
                    <p className="text-white/60">Merci d'avoir participe !</p>
                    <Button onClick={onEnd} variant="secondary">Fermer</Button>
                </div>
            </div>
        )
    }
                ...
                <div className="flex items-center gap-2">
                    {(Object.keys(REACTION_ICONS) as ReactionType[]).map(key => (
                        ...
                        >
                            <span>{REACTION_ICONS[key].emoji}</span>
                            {myReactions[key] > 0 && (
                                <span className="text-xs text-white/60">{myReactions[key]}</span>
                            )}
                        </button>
                    ))}
                </div>
