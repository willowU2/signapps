...
import { LivePresenterView, generatePresentationId } from "./live-presentation";
import { performAutoLayout, extractCanvasObjectInfo } from "./ai-layout";
...
interface SlideEditorProps {
  slideState: {
    ...
    updateObject: (
      id: string,
      obj: fabric.Object | Record<string, unknown>,
    ) => void;
    ...
    getAllSlidesWithObjects?: () => Array<{
      id: string;
      objects: Record<string, any> | fabric.Object[];
    }>;
  };
  isReadOnly?: boolean;
}
...
export function SlideEditor({
  ...
  const [copiedFormat, setCopiedFormat] = useState<CopiedTextFormat | null>(
    null,
  );
  ...
  }>({ orientation: "portrait", backgroundColor: "#ffffff" });

  // --- Feature States: Animations, Master Slides, Live Presentation ---
  const [showAnimationPanel, setShowAnimationPanel] = useState(false);
  const [showMasterEditor, setShowMasterEditor] = useState(false);
  const [masterSlides, setMasterSlides] =
    useState<MasterSlide[]>(DEFAULT_MASTERS);
            ...
            if (canvas) {
              ...
              if (objs.length > 0) {
                import("fabric").then((fabricModule) => {
                  const sel = new fabricModule.ActiveSelection(objs, {
                    canvas,
                  });
  ...
  const slideMenus = [
    ...
    },
    {
      ...
      items: [
          ...
          subItems: [
            {
              ...
              action: "slides_insert_image_local",
            },
            {
              label: "Importer depuis une URL",
              action: "slides_insert_image_url",
            },
            {
              label: "Rechercher sur le Web",
              action: "slides_insert_image_search",
            },
            {
              label: "Depuis Google Drive",
              action: "slides_insert_image_drive",
            },
          ],
        },
        ...
        {
          label: "Numéro de diapositive",
          action: "slides_insert_slide_number",
        },
      ],
    },
    {
        ...
        { sep: true },
        {
          ...
          subItems: [
            ...
            { label: "Vers l'arrière", action: "sendBackward" },
            {
              label: "Arrière plan",
              action: "sendToBack",
              shortcut: "Ctrl+Maj+Down",
            },
          ],
        },
        {
          label: "Aligner les objets",
          subItems: [
  ...
  const { collaborators: simCollabs } = useSimulatedMultiplayer(
    activeSlideId || "doc",
    true,
  );
    ...
    mergedCollaborators[1000 + index] = {
      ...
      cursor: guest.cursor
        ? {
            x: guest.cursor.x,
            y: guest.cursor.y,
            slideId: guest.activeSlideId || "doc",
          }
        : undefined,
    };
  });
  ...
  const toggleFormatPainter = useCallback(() => {
    ...
    if (isFormatPainting) {
      ...
      if (
        ...
        toast.error(
          "Veuillez d'abord sélectionner un objet texte pour copier son format",
        );
        return;
      }
      ...
      toast.success(
        "Format copié. Cliquez sur un autre texte pour l'appliquer.",
      );
    }
  }, [isFormatPainting]);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const activeObject =
        canvas.getActiveObject() as FabricObjectWithId | null;
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === "v" && clipboardRef.current) {
        ...
        clipboardRef.current.clone().then((clonedObj: FabricObjectWithId) => {
          ...
          if (clonedObj.type === "activeSelection") {
            ...
            (
              clonedObj as FabricObjectWithId & { canvas: fabric.Canvas }
            ).canvas = canvas;
      ...
      if (
        ...
        const objs =
          activeObject.type === "activeSelection"
            ? (activeObject.getObjects?.() ?? [activeObject])
            : [activeObject];
        // Move selection box
        activeObject.set({
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === "b") {
        ...
        if (activeObject.type === "i-text" || activeObject.type === "textbox") {
          ...
          isUpdatingRef.current = true;
          if (activeObject.id)
            updateObject(activeObject.id, activeObject.toObject());
          isUpdatingRef.current = false;
        }
        return;
      }
  ...
  const handleUpdateActiveObject = useCallback(
    (id: string, updates: Record<string, unknown>) => {
      ...
      if (fabricCanvasRef.current) {
        ...
        setActiveObject(
          currentSelected
            ? Object.assign(
                Object.create(Object.getPrototypeOf(currentSelected)),
                currentSelected,
              )
            : null,
        );
  ...
  const addMagicLayout = async () => {
    ...
    try {
      ...
      import("fabric").then((fabricModule) => {
        ...
        toast.success("Mise en page magique générée avec succès !", {
          id: toastId,
        });
      });
    } catch (error) {
      ...
      toast.error(
        "Impossible de générer la mise en page. Le modèle IA a peut-être retourné un JSON invalide.",
        { id: toastId },
      );
    }
  };
      ...
      if (canvas) {
        ...
        switch (type) {
          ...
          case "triangle":
            ...
            break;
          case "line":
            shape = new fabricModule.Line([50, 50, 200, 50], {
              ...defaultOpts,
              stroke: "#818cf8",
              strokeWidth: 4,
              fill: undefined,
  ...
  const addConnector = useCallback(() => {
    ...
    import("fabric").then((fabricModule) => {
      ...
      if (line.id)
        updateObject(
          line.id,
          line.toObject(["id", "connectedFrom", "connectedTo"]),
        );
      isUpdatingRef.current = false;
  ...
  const addImage = (url: string) => {
    import("fabric").then((fabricModule) => {
      const canvas = fabricCanvasRef.current;
      if (canvas) {
        fabricModule.Image.fromURL(url)
          .then((img: import("fabric").Image) => {
          ...
          .catch((err: unknown) => {
            ...
            alert(
              "Impossible de charger cette image. Vérifiez que l'URL est publique et que le serveur autorise le CORS.",
            );
          });
      }
    });
    ...
    import("fabric").then((fabricModule) => {
      ...
      if (canvas) {
        ...
        if (type === "signature") {
          ...
          const headerBg = new fabricModule.Rect({
            left: startX,
            top: startY,
            width: 340,
            height: 42,
            ...
            rx: 12,
            ry: 12,
          }); // Clip path not possible easily on group, we accept overlap
          const headerLine = new fabricModule.Line(
            [startX, startY + 42, startX + 340, startY + 42],
            { stroke: "#e2e8f0", strokeWidth: 1 },
          );

          const iconBox = new fabricModule.Rect({
            left: startX + 16,
            top: startY + 11,
            width: 20,
            height: 20,
          ...
          const title = new fabricModule.IText("🖋️ eSignature Request", {
            left: startX + 18,
            top: startY + 13,
            fontSize: 13,
            fontFamily: "Inter, sans-serif",
          ...
          const signLabel = new fabricModule.IText("Sign here:", {
            left: startX + 24,
            top: startY + 64,
            fontSize: 12,
            fontFamily: "Inter, sans-serif",
          ...
          const signZoneBg = new fabricModule.Rect({
            left: startX + 24,
            top: startY + 84,
            width: 180,
            height: 50,
          ...
          const signZoneTxt = new fabricModule.IText("Click to sign", {
            left: startX + 76,
            top: startY + 102,
            fontSize: 12,
            fontFamily: "Inter, sans-serif",
          ...
          const dateLabel = new fabricModule.IText("Date:", {
            left: startX + 220,
            top: startY + 64,
            fontSize: 12,
            fontFamily: "Inter, sans-serif",
          ...
          const dateZoneBg = new fabricModule.Rect({
            left: startX + 220,
            top: startY + 84,
            width: 96,
            height: 50,
          ...
          const title = new fabricModule.IText("✉️ Email Draft", {
            left: startX + 20,
            top: startY + 16,
            fontSize: 14,
            fontFamily: "Inter, sans-serif",
          ...
          // Button Send preview
          const btnBg = new fabricModule.Rect({
            left: startX + 340,
            top: startY + 12,
            width: 80,
            height: 26,
          ...
          const btnTxt = new fabricModule.IText("Send", {
            left: startX + 364,
            top: startY + 18,
            fontSize: 12,
            fontFamily: "Inter, sans-serif",
          ...
          const line1 = new fabricModule.Line(
            [startX, startY + 48, startX + 440, startY + 48],
            { stroke: "#e2e8f0", strokeWidth: 1 },
          );
          const toLabel = new fabricModule.IText("To:", {
            left: startX + 20,
            top: startY + 62,
            fontSize: 13,
            fontFamily: "Inter, sans-serif",
          ...
          const toChipBg = new fabricModule.Rect({
            left: startX + 50,
            top: startY + 58,
            width: 140,
            height: 22,
          ...
          const toChipTxt = new fabricModule.IText("👤 client@company.com", {
            left: startX + 58,
            top: startY + 63,
            fontSize: 11,
            fontFamily: "Inter, sans-serif",
          ...
          const line2 = new fabricModule.Line(
            [startX, startY + 90, startX + 440, startY + 90],
            { stroke: "#f1f5f9", strokeWidth: 1 },
          );
          const subjLabel = new fabricModule.IText("Subject:", {
            left: startX + 20,
            top: startY + 104,
            fontSize: 13,
            fontFamily: "Inter, sans-serif",
          ...
          const subjInput = new fabricModule.IText("Project Proposal Q3", {
            left: startX + 80,
            top: startY + 104,
            fontSize: 13,
            fontFamily: "Inter, sans-serif",
          ...
          const line3 = new fabricModule.Line(
            [startX, startY + 132, startX + 440, startY + 132],
            { stroke: "#f1f5f9", strokeWidth: 1 },
          );
          const body = new fabricModule.IText(
          ...
          const title = new fabricModule.IText("📅 Weekly Sync: Product Team", {
            left: startX + 24,
            top: startY + 24,
            fontSize: 16,
            fontFamily: "Inter, sans-serif",
          ...
          // Date chip
          const dateChipBg = new fabricModule.Rect({
            left: startX + 24,
            top: startY + 54,
            width: 90,
            height: 22,
          ...
          const dateChipTxt = new fabricModule.IText("Oct 24, 2024", {
            left: startX + 32,
            top: startY + 59,
            fontSize: 11,
            fontFamily: "Inter, sans-serif",
          ...
          // Attendees
          const attLabel = new fabricModule.IText("Attendees:", {
            left: startX + 130,
            top: startY + 59,
            fontSize: 12,
            fontFamily: "Inter, sans-serif",
          ...
          const att1Bg = new fabricModule.Rect({
            left: startX + 196,
            top: startY + 54,
            width: 64,
            height: 22,
          ...
          const att1Txt = new fabricModule.IText("👤 Alice", {
            left: startX + 204,
            top: startY + 59,
            fontSize: 11,
            fontFamily: "Inter, sans-serif",
          ...
          const att2Bg = new fabricModule.Rect({
            left: startX + 266,
            top: startY + 54,
            width: 60,
            height: 22,
          ...
          const att2Txt = new fabricModule.IText("👤 Bob", {
            left: startX + 274,
            top: startY + 59,
            fontSize: 11,
            fontFamily: "Inter, sans-serif",
          ...
          const line = new fabricModule.Line(
            [startX + 24, startY + 90, startX + 476, startY + 90],
            { stroke: "#f1f5f9", strokeWidth: 1 },
          );

          const agendaTitle = new fabricModule.IText("Notes & Agenda", {
            left: startX + 24,
            top: startY + 110,
            fontSize: 13,
            fontFamily: "Inter, sans-serif",
          ...
          const agendaItems = new fabricModule.IText(
            "• Review Q3 Metrics\n• Discussion on onboarding flow",
            {
              left: startX + 24,
              top: startY + 134,
              fontSize: 13,
          ...
          const actionTitle = new fabricModule.IText("Action Items", {
            left: startX + 260,
            top: startY + 110,
            fontSize: 13,
            fontFamily: "Inter, sans-serif",
          ...
          const actionItems = new fabricModule.IText(
            "☐ Update design system (Alice)\n☐ Schedule user interviews (Bob)",
            {
              left: startX + 260,
              top: startY + 134,
              fontSize: 13,
  ...
  const exportToPPTX = () => {
    ...
    canvas.getObjects().forEach((obj) => {
      ...
      if (
        ...
        slide.addText(o.text || "", {
          ...
          w,
          h,
          fontSize: (textO.fontSize || 18) * scaleY,
          color: fillStr?.replace("#", "") || "000000",
        });
        ...
        slide.addShape(pres.ShapeType.rect, {
          ...
          w,
          h,
          fill: { color: fillStr?.replace("#", "") || "CCCCCC" },
        });
    ...
    const animObjs = canvas.getObjects() as Array<
      FabricObjectWithId & {
    ...
    >;
    const hasAnims = animObjs.some(
      (o) => o.animationType && o.animationType !== "none",
    );
    if (hasAnims) {
      const animNotes = animObjs
        ...
        .map(
          (o) =>
            `${o.type}: ${o.animationType} (${o.animationDuration}ms, delay ${o.animationDelay}ms)`,
        )
        .join("\n");
    ...
    pres.writeFile({
      fileName: `Slide-Export-${new Date().toISOString().slice(0, 10)}.pptx`,
    });
    toast.success(
      "Diapositive exportée au format PPTX" +
        (hasAnims ? " (animations dans les notes)" : ""),
    );
  };
  ...
  useEffect(() => {
    const handleAiAction = async (e: CustomEvent) => {
      ...
      try {
        ...
        if (action === "simplify-text") {
          ...
          systemPrompt =
            "Tu es un relecteur. Ne retourne QUE le texte corrigé/simplifié, sans guillemets ni intro.";
        } else if (action === "grammar-check") {
          prompt = `Corrige les fautes d'orthographe et de grammaire de ce texte : "${currentText}"`;
          systemPrompt =
            "Tu es un correcteur orthographique professionnel. Retourne uniquement le texte corrigé.";
        } else if (action === "translate-en") {
          prompt = `Traduis précisément ceci en anglais fluide : "${currentText}"`;
          systemPrompt =
            "You are a professional translator. Return ONLY the English translation without any surrounding quotes.";
        } else if (action === "expand-text") {
          prompt = `Développe cette idée en un paragraphe complet et professionnel : "${currentText}"`;
          systemPrompt =
            "Tu es rédacteur. Rédige un bout de texte clair et étoffé. Ne réponds que le texte métier.";
        }
    ...
    window.addEventListener(
      "app:ai-action",
      handleAiAction as unknown as EventListener,
    );
    return () =>
            ...
            importedSlide.objects.forEach((obj) => {
              ...
              if (fabricObj) {
                ...
                if (fabricObj.id)
                  updateObject(
                    fabricObj.id,
                    (fabricObj as fabric.Object).toObject(["id"]),
                  );
                isUpdatingRef.current = false;
              }
            });
  ...
  const handleMasterUpdate = useCallback((master: MasterSlide) => {
    setMasterSlides((prev) =>
      prev.map((m) => (m.id === master.id ? master : m)),
    );
  ...
  if (livePresentation?.active && allSlides && getSlideObjects) {
    const slideNodes = allSlides.map((slide) => {
      ...
      return (
        <div
          key={slide.id}
          className="w-full h-full bg-card rounded-lg shadow-lg flex items-center justify-center p-8"
        >
          <div className="text-center space-y-4">
            <h2 className="text-4xl font-bold text-gray-800">{slide.title}</h2>
            <p className="text-muted-foreground">
              {Object.keys(slideObjects).length} objets
            </p>
          </div>
        </div>
      );
    ...
    const slideTransitions = allSlides.map(
      (s) => s.transition || { type: "none" as const, duration: 500 },
    );
    ...
    <div className="flex flex-col w-full h-full gap-0.5 relative animate-fade-in flex-1 min-h-0">
        ...
        <div className="-ml-1.5 flex flex-col pt-0.5">
          <EditorMenu
            ...
            onAction={(action, label) => {
              ...
              if (action === "open")
                toast.info(
                  "Rendez-vous sur l'accueil Drive pour ouvrir une présentation.",
                );
              ...
              if (action === "paste") {
                if (clipboardRef.current && canvas) {
                  clipboardRef.current
                    .clone()
                    .then((clonedObj: FabricObjectWithId) => {
                      canvas.discardActiveObject();
                      clonedObj.set({
                        left: (clonedObj.left || 0) + 20,
                        top: (clonedObj.top || 0) + 20,
              ...
              if (action === "export_pdf") {
                ...
                if (canvas) {
                  ...
                  toast.info(
                    "Export PDF: Utilisez l'impression du navigateur (Ctrl+P) pour créer un PDF de haute qualité.",
                  );
                }
              }
              ...
              if (action === "toggleGrid") {
                ...
                toast.success(showGrid ? "Grille masquée" : "Grille affichée");
              }
              if (action === "toggleSnap") {
                setSnapToGrid(!snapToGrid);
                toast.success(
                  snapToGrid ? "Alignement désactivé" : "Alignement activé",
                );
              }
              if (action === "toggleGuides")
                toast.info("Guides: Fonctionnalité bientôt disponible");
              if (action === "toggleRuler")
                toast.info("Règle: Fonctionnalité bientôt disponible");
              ...
              if (action === "zoomFit")
                toast.success("Zoom ajusté à la fenêtre");
              ...
              if (action === "addShapeRoundedRect") {
                import("fabric").then((fabricModule) => {
                  if (canvas) {
                    ...
                    (shape as FabricObjectWithId).id = Math.random()
                      .toString(36)
                      .substr(2, 9);
                    canvas.add(shape);
                    canvas.setActiveObject(shape);
              ...
              if (action === "addShapeEllipse") {
                import("fabric").then((fabricModule) => {
                  if (canvas) {
                    const shape = new fabricModule.Ellipse({
                      left: 200,
                      top: 200,
                      rx: 60,
                      ry: 40,
              ...
              if (action === "addShapeStar") {
                import("fabric").then((fabricModule) => {
                  if (canvas) {
                    ...
                    const shape = new fabricModule.Polygon(points, {
                      left: 200,
                      top: 200,
                      fill: "#f59e0b",
                    });
              ...
              if (action === "addShapeBubble") {
                import("fabric").then((fabricModule) => {
                  if (canvas) {
                    ...
                    (rect as FabricObjectWithId).id = Math.random()
                      .toString(36)
                      .substr(2, 9);
                    canvas.add(rect);
              ...
              if (action === "addLineArrow") {
                import("fabric").then((fabricModule) => {
                  if (canvas) {
                    const line = new fabricModule.Line([50, 50, 200, 50], {
                      left: 200,
                      top: 200,
                      stroke: "#818cf8",
                      strokeWidth: 3,
                    });
                    (line as FabricObjectWithId).id = Math.random()
                      .toString(36)
                      .substr(2, 9);
                    canvas.add(line);
                    canvas.setActiveObject(line);
                    toast.info(
                      "Ligne avec flèche: ajoutez une forme triangulaire à l'extrémité",
                    );
                  }
                });
              }
              if (action === "addLineElbow")
                toast.info("Connecteur coudé: bientôt disponible");
              if (action === "addLineCurve")
                toast.info("Connecteur courbe: bientôt disponible");
              if (action === "addMagicLayout") addMagicLayout();

              // Smart Chips
              if (action === "addChipUser") addSmartChip("user", "Utilisateur");
              if (action === "addChipDate")
                addSmartChip("date", new Date().toLocaleDateString("fr-FR"));
              if (action === "addChipFile")
                addSmartChip("file", "Document.pdf");
              if (action === "addChipStatus")
                addSmartChip("status", "En cours", "#10b981");
              ...
              if (action === "hideSlide")
                toast.info("Masquer la diapositive: bientôt disponible");
              if (action === "skipSlide")
                toast.info("Sauter la diapositive: bientôt disponible");
              ...
              if (action === "textSuperscript")
                toast.info(
                  "Exposant: sélectionnez le texte dans la zone de texte",
                );
              if (action === "textSubscript")
                toast.info(
                  "Indice: sélectionnez le texte dans la zone de texte",
                );
              ...
              if (action === "objectAlignLeft" && active && canvas) {
                ...
                canvas.requestRenderAll();
                if (activeWithId.id)
                  updateObject(activeWithId.id, active.toObject());
              }
              if (action === "objectAlignCenterH" && active && canvas) {
                ...
                active.set(
                  "left",
                  canvas.width! / 2 -
                    (active.width! * (active.scaleX || 1)) / 2,
                );
                canvas.requestRenderAll();
                if (activeWithId.id)
                  updateObject(activeWithId.id, active.toObject());
              ...
              if (action === "objectAlignRight" && active && canvas) {
                ...
                active.set(
                  "left",
                  canvas.width! - active.width! * (active.scaleX || 1),
                );
                canvas.requestRenderAll();
                if (activeWithId.id)
                  updateObject(activeWithId.id, active.toObject());
              ...
              if (action === "objectAlignCenterV" && active && canvas) {
                ...
                active.set(
                  "top",
                  canvas.height! / 2 -
                    (active.height! * (active.scaleY || 1)) / 2,
                );
                canvas.requestRenderAll();
                if (activeWithId.id)
                  updateObject(activeWithId.id, active.toObject());
              ...
              if (action === "objectAlignBottom" && active && canvas) {
                ...
                active.set(
                  "top",
                  canvas.height! - active.height! * (active.scaleY || 1),
                );
                canvas.requestRenderAll();
                if (activeWithId.id)
                  updateObject(activeWithId.id, active.toObject());
              ...
              if (action === "distributeH")
                toast.info(
                  "Distribution horizontale: sélectionnez plusieurs objets",
                );
              if (action === "distributeV")
                toast.info(
                  "Distribution verticale: sélectionnez plusieurs objets",
                );
              ...
              if (action === "groupItems") {
                ...
                if (active && active.type === "activeSelection") {
                  isUpdatingRef.current = true;
                  const activeWithId = active as FabricObjectWithId;
                  const groupItems = activeWithId.getObjects
                    ? activeWithId.getObjects()
                    : [];
                  const groupId = Math.random().toString(36).substr(2, 9);
                  // toGroup is available on ActiveSelection in fabric v6
                  const group = (
                    active as FabricObjectWithId & {
                      toGroup?: () => FabricObjectWithId;
                    }
                  ).toGroup?.();
                  if (group) {
                    group.id = groupId;
                    updateObject(
                      group.id!,
                      (group as fabric.Object).toObject(["id", "zIndex"]),
                    );
                  }
                  groupItems.forEach((item) => {
                    if (item.id) removeObject(item.id);
                  ...
                  toast.error(
                    "Sélectionnez plusieurs éléments avec Shift ou la souris pour les grouper.",
                  );
                }
              }
              if (action === "ungroupItems") {
                ...
                if (active && active.type === "group") {
                  ...
                  const items = activeWithId.getObjects
                    ? activeWithId.getObjects()
                    : [];
                  ...
                  items.forEach((item) => {
                    ...
                    updateObject(
                      item.id!,
                      (item as fabric.Object).toObject(["id", "zIndex"]),
                    );
                  });
                  isUpdatingRef.current = false;
                  canvas.requestRenderAll();
                  ...
                  toast.error(
                    "Sélectionnez un groupe existant pour le dégrouper.",
                  );
                }
              }

              // Page setup
              if (action === "pageSetup_portrait")
                setPageConfig({ ...pageConfig, orientation: "portrait" });
              if (action === "pageSetup_landscape")
                setPageConfig({ ...pageConfig, orientation: "landscape" });
              ...
              if (action === "toggleSmartGuides")
                setSmartGuidesEnabled((g) => !g);
              // IDEA-034: PPTX import
      ...
      {!isReadOnly && (
        <SlideToolbar
          ...
          currentLayout={
            activeSlideId && getSlideLayout
              ? getSlideLayout(activeSlideId)
              : undefined
          }
          onLayoutChange={
            activeSlideId && updateSlideLayout
          ...
          collaborators={Object.entries(mergedCollaborators).map(
            ([id, state]: [string, any]) => ({
        ...
        <div className="flex-1 w-full flex items-stretch min-h-0 relative">
          ...
          {showAnimationPanel && !isReadOnly && (
            <AnimationPanel
              selectedObjectId={
                activeObject
                  ? (activeObject as FabricObjectWithId).id || null
                  : null
              }
          ...
          {showMasterEditor && !isReadOnly && (
            <MasterSlideEditor
              ...
              activeMasterId={
                activeSlideId && getSlideMaster
                  ? getSlideMaster(activeSlideId) || null
                  : null
              }
      ...
      {isDrawingMode && (
        <div className="absolute top-14 left-1/2 -translate-x-1/2 z-50 bg-amber-500 text-white px-3 py-1 rounded-full text-[12px] font-medium shadow-lg flex items-center gap-2">
          <svg
            ...
            strokeWidth="2"
          >
            <path d="M2 12L5 9 11 3 12 4 6 10 3 13Z" />
            <path d="M10 2L12 4" />
          </svg>
          Mode dessin — cliquez pour désactiver
          <button onClick={toggleDrawingMode} className="ml-1 hover:opacity-80">
            ✕
          </button>
        </div>
      )}
