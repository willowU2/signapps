...
export function SlideThumbnail({ slideId, presentationId }: SlideThumbnailProps) {
    ...
    useEffect(() => {
        ...
        import("fabric").then((fabricModule) => {
            ...
            const fabricObjectsToEnliven = objectEntries.map(([_, objData]) => objData)

            fabricModule.util.enlivenObjects(fabricObjectsToEnliven).then((enlivenedObjects) => {
