...
export function SlidePropertyPanel({ activeObject, updateObjectRemotely, canvasRef }: SlidePropertyPanelProps) {
    ...
    const handlePropertyChange = (key: string, value: any) => {
        ...
        if (key === 'fontSize' || key === 'fontWeight' || key === 'fontStyle' || key === 'text') {
            activeObject.setCoords()
        }

        canvasRef.current.requestRenderAll()
        updateObjectRemotely(activeObject.get('id') as string, activeObject.toObject())
    }
    ...
    const toggleListStyle = (styleType: 'bullet' | 'number' | 'check') => {
        ...
        const isCurrentlyCheck = lines[0]?.startsWith("☐ ") || lines[0]?.startsWith("✅ ");
    ...
    return (
        <div className="w-72 flex-shrink-0 border-l border-gray-100 bg-white h-full flex flex-col overflow-y-auto custom-scrollbar shadow-[-4px_0_24px_-12px_rgba(0,0,0,0.05)]">
            ...
            <div className="p-5 flex flex-col gap-6">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <select
                                ...
                                onChange={(e) => {
                                    ...
                                    if (style === 'Title') { fontSize = 48; fontWeight = 'bold'; }
                                    else if (style === 'Heading 1') { fontSize = 36; fontWeight = 'bold'; }
                                    else if (style === 'Heading 2') { fontSize = 28; fontWeight = 'bold'; }
                                    else if (style === 'Subtitle') { fontSize = 24; fontWeight = 'normal'; }
                                    else if (style === 'Normal Text') { fontSize = 20; fontWeight = 'normal'; }
                    ...
                    <div className="flex flex-col gap-4 pt-4 border-t border-gray-100 animate-fade-in" style={{ animationDelay: '50ms' }}>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                />
                                <span className="text-xs text-gray-400 font-mono w-4 text-right">{activeObject.strokeWidth || 0}</span>
                            </div>
                        </div>
                ...
                {isSmartChip && (
                    <div className="flex flex-col gap-4 pt-4 border-t border-gray-100 animate-fade-in bg-indigo-50/50 p-4 rounded-xl mt-4" style={{ animationDelay: '100ms' }}>
                        <div className="flex items-center gap-2 mb-2">
                            <Hexagon className="w-4 h-4 rounded text-indigo-500" />
                            <h4 className="text-xs font-semibold text-indigo-900 uppercase tracking-widest">Smart Chip</h4>
                        </div>
                        ...
                        {chipType === 'status' && (
                            <div className="space-y-1.5">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    ...
                                    <button onClick={() => handleSmartChipChange('#f59e0b', '⏳ Review')} className="px-2 py-1.5 text-xs font-medium text-white bg-amber-500 rounded hover:bg-amber-600 transition-colors">⏳ Review</button>
                                    <button onClick={() => handleSmartChipChange('#f43f5e', '⚠️ Urgent')} className="px-2 py-1.5 text-xs font-medium text-white bg-rose-500 rounded hover:bg-rose-600 transition-colors">⚠️ Urgent</button>
                                </div>
                            </div>
                        )}
