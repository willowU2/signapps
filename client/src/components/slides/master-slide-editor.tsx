...
import {
    LayoutTemplate, Plus, Pencil, Trash2, Check, Image, Type,
...
export const DEFAULT_MASTERS: MasterSlide[] = [
    {
        ...
        zones: [
            { id: 'z1', type: 'title', label: 'Titre', x: 10, y: 30, width: 80, height: 15, fontSize: 36, fontFamily: 'Inter, sans-serif', color: '#1e293b', align: 'center' },
            { id: 'z2', type: 'content', label: 'Sous-titre', x: 20, y: 50, width: 60, height: 10, fontSize: 18, fontFamily: 'Inter, sans-serif', color: '#64748b', align: 'center' },
        ]
    },
    {
        id: 'master-content',
        ...
        footerText: '',
        zones: [
            { id: 'z1', type: 'title', label: 'Titre', x: 5, y: 3, width: 90, height: 10, fontSize: 28, fontFamily: 'Inter, sans-serif', color: '#1e293b', align: 'left' },
            { id: 'z2', type: 'content', label: 'Contenu', x: 5, y: 16, width: 90, height: 70, fontSize: 16, fontFamily: 'Inter, sans-serif', color: '#334155', align: 'left' },
            { id: 'z3', type: 'slideNumber', label: '#', x: 90, y: 92, width: 8, height: 5, fontSize: 12, color: '#94a3b8', align: 'right' },
        ]
    },
    {
        id: 'master-two-col',
        ...
        zones: [
            { id: 'z1', type: 'title', label: 'Titre', x: 5, y: 3, width: 90, height: 10, fontSize: 28, fontFamily: 'Inter, sans-serif', color: '#1e293b', align: 'left' },
            { id: 'z2', type: 'content', label: 'Colonne gauche', x: 5, y: 16, width: 42, height: 70, fontSize: 16, fontFamily: 'Inter, sans-serif', color: '#334155', align: 'left' },
            { id: 'z3', type: 'content', label: 'Colonne droite', x: 52, y: 16, width: 42, height: 70, fontSize: 16, fontFamily: 'Inter, sans-serif', color: '#334155', align: 'left' },
            { id: 'z4', type: 'slideNumber', label: '#', x: 90, y: 92, width: 8, height: 5, fontSize: 12, color: '#94a3b8', align: 'right' },
        ]
    },
    {
        ...
        showSlideNumber: true,
        zones: [
            { id: 'z1', type: 'title', label: 'Titre', x: 52, y: 5, width: 44, height: 10, fontSize: 24, fontFamily: 'Inter, sans-serif', color: '#1e293b', align: 'left' },
            { id: 'z2', type: 'image', label: 'Image', x: 3, y: 5, width: 44, height: 85, fontSize: 14, color: '#94a3b8', align: 'center' },
            { id: 'z3', type: 'content', label: 'Contenu', x: 52, y: 18, width: 44, height: 68, fontSize: 16, fontFamily: 'Inter, sans-serif', color: '#334155', align: 'left' },
            { id: 'z4', type: 'slideNumber', label: '#', x: 90, y: 92, width: 8, height: 5, fontSize: 12, color: '#94a3b8', align: 'right' },
        ]
    },
    {
        ...
        showSlideNumber: false,
        zones: [
            { id: 'z1', type: 'title', label: 'Titre de section', x: 10, y: 40, width: 80, height: 15, fontSize: 36, fontFamily: 'Inter, sans-serif', color: '#ffffff', align: 'center' },
            { id: 'z2', type: 'content', label: 'Description', x: 20, y: 58, width: 60, height: 8, fontSize: 16, fontFamily: 'Inter, sans-serif', color: '#94a3b8', align: 'center' },
        ]
    },
    {
        id: 'master-blank',
...
function ZoneTypeIcon({ type }: { type: MasterSlideZone['type'] }) {
    switch (type) {
        ...
        case 'content': return <AlignLeft className="w-3 h-3" />
        case 'image': return <Image className="w-3 h-3" />
        case 'footer': return <AlignLeft className="w-3 h-3" />
        case 'slideNumber': return <Hash className="w-3 h-3" />
    }
}

// --- Master Slide Preview ---
function MasterPreview({ master, isActive, onClick }: { master: MasterSlide; isActive: boolean; onClick: () => void }) {
    return (
        <button
            onClick={onClick}
            className={cn(
...
export function MasterSlideEditor({
    ...
    const handleDuplicate = (master: MasterSlide) => {
        const newMaster: MasterSlide = {
            ...
            zones: master.zones.map(z => ({ ...z, id: `z-${Math.random().toString(36).substr(2, 6)}` }))
        }
    ...
    const handleAddZone = (type: MasterSlideZone['type']) => {
        ...
        const newZone: MasterSlideZone = {
            ...
            type,
            label: type === 'title' ? 'Titre' : type === 'content' ? 'Contenu' : type === 'image' ? 'Image' : type === 'slideNumber' ? '#' : 'Pied de page',
            x: 10,
            y: 10,
            width: 30,
            height: 15,
    ...
    const handleUpdateZone = (zoneId: string, updates: Partial<MasterSlideZone>) => {
        if (!editingMaster) return
        setEditingMaster({
            ...editingMaster,
            zones: editingMaster.zones.map(z => z.id === zoneId ? { ...z, ...updates } : z)
        })
    ...
    return (
        <div className="w-80 border-l bg-background flex flex-col h-full">
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b">
                ...
                <Button variant="ghost" size="sm" onClick={editingMaster ? () => setEditingMaster(null) : onClose} className="h-6 w-6 p-0">
                    <X className="w-4 h-4" />
                </Button>
            </div>
                ...
                {!editingMaster ? (
                    ...
                    <div className="p-4 space-y-4">
                        <p className="text-xs text-muted-foreground">
                            Choisissez un modele pour structurer vos diapositives. Les modifications s'appliquent a toutes les diapositives utilisant ce modele.
                        </p>
                            ...
                            {masters.map(master => (
                                <div key={master.id} className="space-y-1.5">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs font-medium truncate">{master.name}</span>
                                        <div className="flex gap-0.5">
                                            ...
                                            {!master.id.startsWith('master-') || masters.length > 1 ? (
                                                <Button
                                                    variant="ghost"
                                                    size="sm"
                    ...
                    <div className="p-4 space-y-4">
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="flex items-center gap-2">
                                <input
                                    ...
                                    onChange={e => setEditingMaster({ ...editingMaster, backgroundColor: e.target.value })}
                        ...
                        <div className="space-y-1">
                            ...
                            <Input
                                ...
                                onChange={e => setEditingMaster({ ...editingMaster, footerText: e.target.value })}
                                className="h-8 text-sm"
                                ...
                                {editingMaster.zones.map(zone => (
                                    ...
                                    >
                                        <div className="flex items-center gap-1">
                                            ...
                                            <span className="text-[7px] truncate" style={{ color: zone.color || '#94a3b8' }}>
                                                {zone.label}
                                            </span>
                                        </div>
                                    </button>
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="flex gap-1 flex-wrap">
                                {(['title', 'content', 'image', 'footer', 'slideNumber'] as MasterSlideZone['type'][]).map(type => (
                                    <Button
                                        key={type}
                                        variant="outline"
                                        size="sm"
                        ...
                        {editingMaster.zones.length > 0 && (
                            <div className="space-y-1">
                                <Label className="text-xs">Zones ({editingMaster.zones.length})</Label>
                                <div className="space-y-1">
                                    {editingMaster.zones.map(zone => (
                                        <div
                                            key={zone.id}
                                        ...
                                        >
                                            ...
                                            <Button
                                                ...
                                                size="sm"
                                                className="h-5 w-5 p-0 text-destructive"
                                                onClick={(e) => { e.stopPropagation(); handleRemoveZone(zone.id) }}
                                            >
                        ...
                        {editingZone && (
                            <div className="space-y-2 border-t pt-3">
                                <Label className="text-xs font-semibold">Zone: {editingZone.label}</Label>
                                <div className="grid grid-cols-2 gap-2">
                                    <div>
                                        ...
                                        <Input
                                            type="number"
                                            value={editingZone.x}
                                            onChange={e => handleUpdateZone(editingZone.id, { x: Number(e.target.value) })}
                                    ...
                                    <div>
                                        ...
                                        <Input
                                            type="number"
                                            value={editingZone.y}
                                            onChange={e => handleUpdateZone(editingZone.id, { y: Number(e.target.value) })}
                                            className="h-7 text-xs"
                                            min={0} max={100}
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <Input
                                            ...
                                            onChange={e => handleUpdateZone(editingZone.id, { width: Number(e.target.value) })}
                                            className="h-7 text-xs"
                                            min={1} max={100}
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <Input
                                            ...
                                            onChange={e => handleUpdateZone(editingZone.id, { height: Number(e.target.value) })}
                                            className="h-7 text-xs"
                                            min={1} max={100}
                                        />
                                    </div>
                                ...
                                <div>
                                    ...
                                    <Input
                                        ...
                                        onChange={e => handleUpdateZone(editingZone.id, { label: e.target.value })}
                                        className="h-7 text-xs"
                                    />
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="flex-1">
                                        ...
                                        <Input
                                            ...
                                            onChange={e => handleUpdateZone(editingZone.id, { fontSize: Number(e.target.value) })}
                                            className="h-7 text-xs"
                                            min={8} max={72}
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onChange={e => handleUpdateZone(editingZone.id, { color: e.target.value })}
                        ...
                        <div className="flex gap-2 pt-2">
                            <Button variant="outline" size="sm" className="flex-1" onClick={() => setEditingMaster(null)}>
                                Annuler
                            </Button>
...
export function getMasterForLayout(layout: SlideLayout, masters: MasterSlide[]): MasterSlide | undefined {
