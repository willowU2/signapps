...
import { importFromFile, importFromText, checkHealth, ImportResult } from '@/lib/api/office';
...
export function useDocumentImport(editor: Editor | null) {
  ...
  const handleImportResult = useCallback((result: ImportResult) => {
    ...
    if (result.success && result.tiptap_json) {
      ...
      toast.success(`Document importé (${result.detected_format.toUpperCase()})`, {
  ...
  const importFromClipboard = useCallback(async (format: 'markdown' | 'html') => {
    if (!editor) {
      toast.error('Éditeur non disponible');
      return;
    }
