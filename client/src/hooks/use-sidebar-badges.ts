...
export function useSidebarBadges() {
  return useQuery<SidebarBadges>({
    ...
    queryFn: async () => {
      const [containersRes, routesRes, storageRes, mailRes, chatRes, tasksRes] = await Promise.allSettled([
        containersApi.list(),
        routesApi.list(),
        storageApi.listBuckets(),
        // Mail unread count -- try to fetch from API, fallback gracefully
        getClient(ServiceName.IDENTITY).get<{ unread_count?: number }>('/mail/unread-count').then(r => r.data),
        // Chat unread messages
        getClient(ServiceName.IDENTITY).get<{ unread_count?: number }>('/chat/unread-count').then(r => r.data),
        // Tasks overdue count
        getClient(ServiceName.IDENTITY).get<{ overdue_count?: number }>('/tasks/overdue-count').then(r => r.data),
      ]);
      ...
      if (chatRes.status === 'fulfilled' && chatRes.value?.unread_count != null) {
        badges.chat = chatRes.value.unread_count;
      }

      // Tasks overdue count (placeholder: uses API if available, otherwise 0)
      if (tasksRes.status === 'fulfilled' && tasksRes.value?.overdue_count != null) {
