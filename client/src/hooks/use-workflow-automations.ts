...
const TEMPLATES: Omit<WorkflowAutomation, 'id' | 'createdAt' | 'runCount'>[] = [
  {
    ...
    conditions: [],
    actions: [{ id: 'a1', type: 'create_task', params: { title: 'Traiter email', assignee: '' } }],
  },
  {
    ...
    conditions: [],
    actions: [{ id: 'a1', type: 'create_invoice', params: { template: 'standard' } }],
  },
  {
  ...
  },
  {
    ...
    conditions: [],
    actions: [{ id: 'a1', type: 'send_email', params: { to: '', subject: 'Tâche en retard' } }],
  },
  {
    ...
    conditions: [],
    actions: [{ id: 'a1', type: 'create_doc', params: { title: 'Notes réunion' } }],
  },
  {
...
export function useWorkflowAutomations() {
  ...
  const add = useCallback((automation: Omit<WorkflowAutomation, 'id' | 'createdAt' | 'runCount'>) => {
  ...
  const update = useCallback((id: string, patch: Partial<WorkflowAutomation>) => {
  ...
  const toggle = useCallback((id: string) => {
    const next = load().map(a => a.id === id ? { ...a, enabled: !a.enabled } : a);
    persist(next);
    setAutomations(next);
  }, []);

  const addFromTemplate = useCallback((index: number) => {
    if (index < 0 || index >= TEMPLATES.length) return;
    return add(TEMPLATES[index]);
  }, [add]);

  return { automations, templates: TEMPLATES, add, update, remove, toggle, addFromTemplate };
}
