...
export function useTranslateDoc() {
  return useCallback(async (text: string, targetLang: string): Promise<string> => {
    return aiChat(
...
export function useExtractKeywords() {
  return useCallback(async (text: string): Promise<string[]> => {
    ...
    return answer.split(',').map((k) => k.trim()).filter(Boolean).slice(0, 10);
  }, []);
}
...
export function useGenerateSocialPost() {
  return useCallback(async (summary: string, platform: string): Promise<string> => {
    return aiChat(
...
export function useSuggestRelatedDocs() {
  return useCallback(async (title: string, content: string): Promise<string[]> => {
    const answer = await aiChat(
...
export function useAutoTagFile() {
  return useCallback(async (filename: string, mimeType: string): Promise<string[]> => {
    const answer = await aiChat(
    ...
    return answer.split(',').map((t) => t.trim()).filter(Boolean).slice(0, 5);
  }, []);
}
