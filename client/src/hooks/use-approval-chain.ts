...
export function useApprovalChain(entityId?: string) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await client.get<ApprovalRequest[]>("/approvals", {
        params,
      });
      setRequests(data);
    } catch {
  ...
  const submit = useCallback(
    async (
      ...
      const steps: Omit<ApprovalStep, "approverId" | "approverName">[] =
  ...
  const decide = useCallback(
    async (requestId: string, approved: boolean, comment?: string) => {
      const { data } = await client.patch<ApprovalRequest>(
  ...
  const myPending = pending.filter(
    (r) => r.steps[r.currentStep]?.approverId === "me",
  );
