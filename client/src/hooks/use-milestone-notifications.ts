...
const EVENT_CONFIG: Record<
  MilestoneEvent,
  { title: string; description: (p: MilestoneNotificationPayload) => string }
> = {
  reached: {
    title: "Jalon atteint",
    description: (p) =>
      `${p.milestoneName} du projet ${p.projectName} a été atteint.`,
  },
  at_risk: {
    title: "Jalon à risque",
    description: (p) =>
      `${p.milestoneName} est à risque. Échéance: ${new Date(p.targetDate).toLocaleDateString("fr-FR")}.`,
  },
  overdue: {
    title: "Jalon en retard",
    description: (p) =>
      `${p.milestoneName} dépasse son échéance du ${new Date(p.targetDate).toLocaleDateString("fr-FR")}.`,
  },
  approaching: {
    title: "Jalon imminent",
    description: (p) =>
      `${p.milestoneName} arrive à échéance dans moins de 48h.`,
  },
};
...
export function useMilestoneNotifications() {
  const sendMilestoneNotification = useCallback(
    (payload: MilestoneNotificationPayload) => {
      ...
      window.dispatchEvent(
        new CustomEvent("agentiq:notification", { detail: notification }),
      );
  ...
  const notifyMilestoneReached = useCallback(
    (payload: Omit<MilestoneNotificationPayload, "event">) =>
      sendMilestoneNotification({ ...payload, event: "reached" }),
    [sendMilestoneNotification],
  );

  const notifyMilestoneAtRisk = useCallback(
    (payload: Omit<MilestoneNotificationPayload, "event">) =>
      sendMilestoneNotification({ ...payload, event: "at_risk" }),
    [sendMilestoneNotification],
  );

  const notifyMilestoneOverdue = useCallback(
    (payload: Omit<MilestoneNotificationPayload, "event">) =>
      sendMilestoneNotification({ ...payload, event: "overdue" }),
    [sendMilestoneNotification],
  );
  ...
  return {
    ...
    notifyMilestoneAtRisk,
    notifyMilestoneOverdue,
  };
}
