...
export function useTrackChanges({
    ...
    const pendingChanges = useMemo(() => getPendingChanges(documentId), [getPendingChanges, documentId]);
    const allChanges = useMemo(() => getChanges(documentId), [getChanges, documentId]);
    ...
    const recordChange = useCallback(
        (type: ChangeType, originalContent?: string, newContent?: string): string | null => {
            if (!user) return null;
