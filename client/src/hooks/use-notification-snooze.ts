...
const SNOOZE_MS: Record<
  Exclude<SnoozeDuration, "tomorrow" | "custom">,
  number
> = {
  "30min": 30 * 60 * 1000,
  "1h": 60 * 60 * 1000,
...
export function useNotificationSnooze() {
  const [snoozed, setSnoozed] = useState<Map<string, SnoozedNotification>>(
    new Map(),
  );
  ...
  const snooze = useCallback(
    (
      ...
      if (duration === "tomorrow") {
        ...
        resumeAt = new Date(
          Date.now() + SNOOZE_MS[duration as keyof typeof SNOOZE_MS],
        );
      }
      ...
      const timer = setTimeout(() => {
        ...
        toast(`Rappel: ${title}`, {
          description: "Notification snoozée revenue.",
        });
      }, delay);
  ...
  const isSnoozed = useCallback(
    (notificationId: string) => snoozed.has(notificationId),
    [snoozed],
  );
  ...
  const getResumeTime = useCallback(
    (notificationId: string): string | null => {
      ...
      return new Date(entry.resumeAt).toLocaleTimeString("fr-FR", {
        hour: "2-digit",
        minute: "2-digit",
      });
    },
  ...
  return {
    snoozed: Array.from(snoozed.values()),
    snooze,
    cancelSnooze,
    isSnoozed,
    getResumeTime,
