...
import { downloadDocument, ExportFormat, ExportComment, checkHealth } from '@/lib/api/office';
...
export function useDocumentExport(
  ...
  const exportDocument = useCallback(async (format: ExportFormat) => {
    ...
    try {
      ...
      const successMessage = format === 'docx' && comments?.length
  ...
  const exportAsDocx = useCallback(() => exportDocument('docx'), [exportDocument]);
  const exportAsPdf = useCallback(() => exportDocument('pdf'), [exportDocument]);
  const exportAsMarkdown = useCallback(() => exportDocument('markdown'), [exportDocument]);
  const exportAsHtml = useCallback(() => exportDocument('html'), [exportDocument]);
  const exportAsText = useCallback(() => exportDocument('text'), [exportDocument]);
