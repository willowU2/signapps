...
export function useProjectPerformanceReview() {
  const [reviews, setReviews] = useState<ProjectCompletionReview[]>(DEMO_PENDING);
  const [scheduling, setScheduling] = useState(false);

  const triggerReviewOnCompletion = useCallback(async (params: {
    projectId: string;
    projectName: string;
    teamMembers: { employeeId: string; employeeName: string; role: string }[];
  }) => {
  ...
  const scheduleReview = useCallback(async (reviewId: string, date: string) => {
    ...
    setReviews((prev) => prev.map((r) => r.id === reviewId ? { ...r, reviewStatus: "scheduled" as const, scheduledDate: date } : r));
    setScheduling(false);
    toast.success("Revue planifiée", { description: `Entretien prévu le ${new Date(date).toLocaleDateString("fr-FR")}.` });
  }, []);

  const completeReview = useCallback((reviewId: string) => {
    setReviews((prev) => prev.map((r) => r.id === reviewId ? { ...r, reviewStatus: "completed" as const } : r));
    toast.success("Revue de performance complétée.");
  }, []);

  const pendingReviews = reviews.filter((r) => r.reviewStatus === "pending");

  return { reviews, pendingReviews, scheduling, triggerReviewOnCompletion, scheduleReview, completeReview };
}
