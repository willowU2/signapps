...
export function useContainers() {
  return useQuery<Container[]>({
    ...
    queryFn: async () => {
      ...
      return ((response.data || []) as unknown as ContainerApiResponse[]).map(
        (c) => {
          ...
          return {
            ...
            memory: dockerInfo?.memory_usage
              ? formatBytes(dockerInfo.memory_usage)
              : "-",
            ports:
              dockerInfo?.ports?.map(
                (p: PortMapping) => `${p.host_port || p.container_port}`,
              ) || [],
            portMappings:
              dockerInfo?.ports
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (isManaged === false && dockerId) {
        switch (action) {
          case "start":
            return containersApi.startDocker(dockerId);
          case "stop":
            return containersApi.stopDocker(dockerId);
          case "restart":
            return containersApi.restartDocker(dockerId);
          case "remove":
            return containersApi.removeDocker(dockerId, true);
          case "update":
            throw new Error(
              "Cannot automatically update an unmanaged container",
            );
        }
      }
      ...
      switch (action) {
        case "start":
          return containersApi.start(id);
        case "stop":
          return containersApi.stop(id);
        case "restart":
          return containersApi.restart(id);
        case "update":
          return containersApi.update(id);
        case "remove":
          return containersApi.remove(id);
      }
    },
    onSuccess: (_, { action }) => {
