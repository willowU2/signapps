...
const EMPLOYEES: Employee[] = [
  ...
  // Product
  {
    ...
    nodeCode: "ENG-PRODUCT",
    functions: ["PM", "MANAGER"],
    contractType: "full-time",
    fteRatio: 1.0,
  },
  ...
  try {
    ...
    for (const nodeType of NODE_TYPES) {
      try {
        ...
        console.log(
          `  ✗ ${nodeType.name}: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    }
    ...
    for (const func of FUNCTIONS) {
      try {
        ...
        console.log(
          `  ✗ ${func.name}: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    }
    ...
    for (const emp of EMPLOYEES) {
      try {
        ...
        if (!nodeId) {
          console.log(
            `  ✗ ${emp.firstName} ${emp.lastName}: org node ${emp.nodeCode} not found`,
          );
          continue;
        }
        ...
        console.log(
          `  ✗ ${emp.firstName} ${emp.lastName}: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    }
