...
export function registerLogicFunctions() {
  ...
  const ifFn: import("./registry").SheetFunction = ({
    args,
    evalArg,
    evalArgNum,
  }) => {
  ...
  const ifsFn: import("./registry").SheetFunction = ({
    args,
    evalArg,
    evalArgNum,
  }) => {
  ...
  const andFn: import("./registry").SheetFunction = ({
    args,
    evalArg,
    resolveRangeStrings,
  }) => {
  ...
  const orFn: import("./registry").SheetFunction = ({
    args,
    evalArg,
    resolveRangeStrings,
  }) => {
  ...
  const notFn: import("./registry").SheetFunction = ({
    args,
    evalArg,
    evalArgNum,
  }) => {
  ...
  const isErrorFn: import("./registry").SheetFunction = ({ args, evalArg }) => {
    if (args.length !== 1) return SHEET_ERRORS.NA;
    return Object.values(SHEET_ERRORS).includes(evalArg(args[0]))
      ? "TRUE"
      : "FALSE";
  };
  ...
  const isErrFn: import("./registry").SheetFunction = ({ args, evalArg }) => {
    if (args.length !== 1) return SHEET_ERRORS.NA;
    const val = evalArg(args[0]);
    return Object.values(SHEET_ERRORS).includes(val) && val !== SHEET_ERRORS.NA
      ? "TRUE"
      : "FALSE";
  };
  ...
  const isTextFn: import("./registry").SheetFunction = ({ args, evalArg }) => {
    if (args.length !== 1) return SHEET_ERRORS.NA;
    const val = evalArg(args[0]);
    return !Object.values(SHEET_ERRORS).includes(val) && isNaN(Number(val))
      ? "TRUE"
      : "FALSE";
  };
  ...
  const isNumberFn: import("./registry").SheetFunction = ({
    args,
    evalArg,
  }) => {
    if (args.length !== 1) return SHEET_ERRORS.NA;
  ...
  const isLogicalFn: import("./registry").SheetFunction = ({
    args,
    evalArg,
  }) => {
    if (args.length !== 1) return SHEET_ERRORS.NA;
  ...
  const uniqueFn: import("./registry").SheetFunction = ({
    args,
    evalArg,
    resolveRangeStrings,
  }) => {
  ...
  const vlookupFn: import("./registry").SheetFunction = ({
    ...
    const parseColRow = (s: string): { col: number; row?: number } | null => {
      ...
      return {
        col: colToIndex(m[1]),
        row: m[2] ? parseInt(m[2]) - 1 : undefined,
      };
    };
    ...
    for (let r = minRow; r <= maxRow; r++) {
      ...
      if (!rawVal && r > minRow + 500 && lastMatch === -1) {
        ...
        for (let probe = r; probe < r + 10 && probe <= maxRow; probe++) {
          if (getData(probe, lookupCol, targetSheet)) {
            allEmpty = false;
            break;
          }
        }
      ...
      const cellVal = rawVal.startsWith("=")
        ? evaluateFormula(
            rawVal,
            getData,
            { r, c: lookupCol, sheet: targetSheet },
            visited,
          )
        : rawVal;
    ...
    return resultRaw.startsWith("=")
      ? evaluateFormula(
          resultRaw,
          getData,
          { r: lastMatch, c: resultCol, sheet: targetSheet },
          visited,
        )
      : resultRaw;
  };
