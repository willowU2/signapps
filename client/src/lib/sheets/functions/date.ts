...
export function registerDateFunctions() {
    ...
    const weekdayFn: import('./registry').SheetFunction = ({ args, evalArg, evalArgNum }: FunctionContext) => {
        ...
        switch (returnType) {
            ...
            case 11: return day === 0 ? 7 : day;            // same as 2
            case 12: return day < 2 ? day + 6 : day - 2;    // 1=Tuesday..7=Monday
            case 13: return day < 3 ? day + 5 : day - 3;    // 1=Wednesday..7=Tuesday
            case 14: return day < 4 ? day + 4 : day - 4;    // 1=Thursday..7=Wednesday
            case 15: return day < 5 ? day + 3 : day - 5;    // 1=Friday..7=Thursday
            case 16: return day < 6 ? day + 2 : day - 6;    // 1=Saturday..7=Friday
            case 17: return day + 1;                         // same as 1
    ...
    const datedifFn: import('./registry').SheetFunction = ({ args, evalArg }: FunctionContext) => {
        if (args.length !== 3) return SHEET_ERRORS.NA;
        const start = parseDate(evalArg(args[0]));
        const end = parseDate(evalArg(args[1]));
        ...
        switch (unit) {
            ...
            case 'YD': {
                ...
                if (startThisYear > end) {
                    ...
                    return Math.floor((end.getTime() - startPrevYear.getTime()) / (1000 * 60 * 60 * 24));
                }
    ...
    const hourFn: import('./registry').SheetFunction = ({ args, evalArg }: FunctionContext) => {
        if (args.length !== 1) return SHEET_ERRORS.NA;
        const d = parseDate(evalArg(args[0]));
        if (isNaN(d.getTime())) return SHEET_ERRORS.VALUE;
        return d.getHours();
    ...
    const minuteFn: import('./registry').SheetFunction = ({ args, evalArg }: FunctionContext) => {
        if (args.length !== 1) return SHEET_ERRORS.NA;
        const d = parseDate(evalArg(args[0]));
        if (isNaN(d.getTime())) return SHEET_ERRORS.VALUE;
        return d.getMinutes();
    ...
    const secondFn: import('./registry').SheetFunction = ({ args, evalArg }: FunctionContext) => {
        if (args.length !== 1) return SHEET_ERRORS.NA;
        const d = parseDate(evalArg(args[0]));
        if (isNaN(d.getTime())) return SHEET_ERRORS.VALUE;
        return d.getSeconds();
