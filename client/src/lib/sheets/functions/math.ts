...
export function registerMathFunctions() {
    ...
    const roundFn: import('./registry').SheetFunction = ({ args, evalArgNum }) => {
    ...
    const roundUpFn: import('./registry').SheetFunction = ({ args, evalArgNum }) => {
         if (args.length < 1 || args.length > 2) return SHEET_ERRORS.NA;
         const factor = Math.pow(10, args.length === 2 ? evalArgNum(args[1]) : 0);
    ...
    const roundDownFn: import('./registry').SheetFunction = ({ args, evalArgNum }) => {
         if (args.length < 1 || args.length > 2) return SHEET_ERRORS.NA;
         const factor = Math.pow(10, args.length === 2 ? evalArgNum(args[1]) : 0);
    ...
    const absFn: import('./registry').SheetFunction = ({ args, evalArgNum }) => args.length !== 1 ? SHEET_ERRORS.NA : Math.abs(evalArgNum(args[0]));
    registerFunction("ABS", absFn);

    const ceilingFn: import('./registry').SheetFunction = ({ args, evalArgNum }) => {
    ...
    const floorFn: import('./registry').SheetFunction = ({ args, evalArgNum }) => {
         if (args.length !== 2) return SHEET_ERRORS.NA;
         const sig = evalArgNum(args[1]);
         return sig === 0 ? SHEET_ERRORS.DIV_ZERO : Math.floor(evalArgNum(args[0]) / sig) * sig;
    };
    ...
    const powerFn: import('./registry').SheetFunction = ({ args, evalArgNum }) => args.length !== 2 ? SHEET_ERRORS.NA : Math.pow(evalArgNum(args[0]), evalArgNum(args[1]));
    ...
    const logFn: import('./registry').SheetFunction = ({ args, evalArgNum }) => {
        if (args.length < 1 || args.length > 2) return SHEET_ERRORS.NA;
        const num = evalArgNum(args[0]), base = args.length === 2 ? evalArgNum(args[1]) : 10;
        return (num <= 0 || base <= 0 || base === 1) ? SHEET_ERRORS.VALUE : Math.log(num) / Math.log(base);
    };
    registerFunction("LOG", logFn);

    const log10Fn: import('./registry').SheetFunction = ({ args, evalArgNum }) => {
        if (args.length !== 1) return SHEET_ERRORS.NA;
        const num = evalArgNum(args[0]);
    ...
    const expFn: import('./registry').SheetFunction = ({ args, evalArgNum }) => args.length !== 1 ? SHEET_ERRORS.NA : Math.exp(evalArgNum(args[0]));
    registerFunction("EXP", expFn);

    const piFn: import('./registry').SheetFunction = ({ args }) => args.length !== 0 ? SHEET_ERRORS.NA : Math.PI;
    registerFunction("PI", piFn);

    const randFn: import('./registry').SheetFunction = ({ args }) => args.length !== 0 ? SHEET_ERRORS.NA : Math.random();
    registerFunction("RAND", randFn);
    registerFunction("ALEA", randFn);

    const randBetweenFn: import('./registry').SheetFunction = ({ args, evalArgNum }) => {
        if (args.length !== 2) return SHEET_ERRORS.NA;
        const min = Math.ceil(evalArgNum(args[0])), max = Math.floor(evalArgNum(args[1]));
    ...
    const intFn: import('./registry').SheetFunction = ({ args, evalArgNum }) => args.length !== 1 ? SHEET_ERRORS.NA : Math.floor(evalArgNum(args[0]));
    registerFunction("INT", intFn);
    registerFunction("ENT", intFn);

    const signFn: import('./registry').SheetFunction = ({ args, evalArgNum }) => args.length !== 1 ? SHEET_ERRORS.NA : Math.sign(evalArgNum(args[0]));
    registerFunction("SIGN", signFn);
    registerFunction("SIGNE", signFn);

    const countFn: import('./registry').SheetFunction = ({ args, evalArg, resolveRangeStrings }) => {
        let count = 0;
        const expanded = [];
        for (const a of args) {
    ...
    const countaFn: import('./registry').SheetFunction = ({ args, evalArg, resolveRangeStrings }) => {
        let count = 0;
        const expanded = [];
        for (const a of args) {
            if (a.includes(':')) expanded.push(...resolveRangeStrings(a));
        ...
        for (const val of expanded) { if (val.trim() !== '') count++; }
        return count;
    };
    registerFunction("COUNTA", countaFn);
    registerFunction("NBVAL", countaFn);

    const countifFn: import('./registry').SheetFunction = ({ args, evalArg, resolveRangeStrings }) => {
        if (args.length < 2) return SHEET_ERRORS.NA;
    ...
    const largeFn: import('./registry').SheetFunction = (ctx) => {
        ...
        const vals = extractArgsNums({ ...ctx, args: args.slice(0, args.length - 1) }).sort((a, b) => b - a);
    ...
    const smallFn: import('./registry').SheetFunction = (ctx) => {
        ...
        if (args.length < 2) return SHEET_ERRORS.NA;
        const k = evalArgNum(args[args.length - 1]);
        const vals = extractArgsNums({ ...ctx, args: args.slice(0, args.length - 1) }).sort((a, b) => a - b);
        return k < 1 || k > vals.length ? SHEET_ERRORS.NA : vals[Math.floor(k) - 1];
    };
    ...
    const rankFn: import('./registry').SheetFunction = (ctx) => {
        ...
        let order = 0, rangeVals: number[] = [];
        if (args.length >= 3 && !isNaN(evalArgNum(args[args.length - 1]))) {
            order = evalArgNum(args[args.length - 1]);
            rangeVals = extractArgsNums({ ...ctx, args: args.slice(1, args.length - 1) });
        } else {
            rangeVals = extractArgsNums({ ...ctx, args: args.slice(1) });
        }
        const sorted = order === 0 ? rangeVals.sort((a, b) => b - a) : rangeVals.sort((a, b) => a - b);
