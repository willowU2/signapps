...
export function registerTextFunctions() {
  ...
  const leftFn: import("./registry").SheetFunction = ({
    args,
    evalArg,
    evalArgNum,
  }) => {
  ...
  const rightFn: import("./registry").SheetFunction = ({
    args,
    evalArg,
    evalArgNum,
  }) => {
  ...
  const midFn: import("./registry").SheetFunction = ({
    args,
    evalArg,
    evalArgNum,
  }) => {
  ...
  const concatenateFn: import("./registry").SheetFunction = ({
    args,
    evalArg,
    resolveRangeStrings,
  }) => {
  ...
  const textJoinFn: import("./registry").SheetFunction = ({
    args,
    evalArg,
    resolveRangeStrings,
  }) => {
  ...
  const substituteFn: import("./registry").SheetFunction = ({
    args,
    evalArg,
    evalArgNum,
  }) => {
    ...
    if (args.length === 4) {
      ...
      let count = 0;
      return text.replace(
        new RegExp(searchFor.replace(/[-/\\^$*+?.()|[\]{}]/g, "\\$&"), "g"),
        (match) => {
          count++;
  ...
  const findFn: import("./registry").SheetFunction = ({
    args,
    evalArg,
    evalArgNum,
  }) => {
  ...
  const searchFn: import("./registry").SheetFunction = ({
    args,
    evalArg,
    evalArgNum,
  }) => {
  ...
  const reptFn: import("./registry").SheetFunction = ({
    args,
    evalArg,
    evalArgNum,
  }) => {
  ...
  const regexExtractFn: import("./registry").SheetFunction = ({
    args,
    evalArg,
  }) => {
    if (args.length !== 2) return SHEET_ERRORS.NA;
  ...
  const regexMatchFn: import("./registry").SheetFunction = ({
    args,
    evalArg,
  }) => {
    if (args.length !== 2) return SHEET_ERRORS.NA;
  ...
  const regexReplaceFn: import("./registry").SheetFunction = ({
    args,
    evalArg,
  }) => {
    if (args.length !== 3) return SHEET_ERRORS.NA;
  ...
  const textFn: import("./registry").SheetFunction = ({ args, evalArg }) => {
    ...
    if (
      formatLower.includes("d") ||
      formatLower.includes("m") ||
      formatLower.includes("y") ||
      formatLower.includes("j") ||
      formatLower.includes("a")
    ) {
    ...
    if (!isNaN(num)) {
      ...
      if (format.includes("%")) {
        ...
        return (
          (num * 100).toFixed(Math.max(0, decimals > 0 ? decimals - 1 : 0)) +
          "%"
        );
      }
