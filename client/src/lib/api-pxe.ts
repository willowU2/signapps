...
export const profileApi = {
  ...
  create: async (data: any) => (await pxeClient.post('/pxe/profiles', data)).data,
  update: async (id: string, data: any) => (await pxeClient.put(`/pxe/profiles/${id}`, data)).data,
  delete: async (id: string) => { await pxeClient.delete(`/pxe/profiles/${id}`) },
};

export const assetApi = {
  list: async () => (await pxeClient.get('/pxe/assets')).data,
  get: async (id: string) => (await pxeClient.get(`/pxe/assets/${id}`)).data,
  register: async (data: any) => (await pxeClient.post('/pxe/assets', data)).data,
  update: async (id: string, data: any) => (await pxeClient.put(`/pxe/assets/${id}`, data)).data,
  delete: async (id: string) => { await pxeClient.delete(`/pxe/assets/${id}`) },
};
