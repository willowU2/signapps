 ...
 */
export async function generateSmartReplies(emailBody: string, senderName: string): Promise<SmartReply[]> {
  try {
    const response = await fetch(`${OLLAMA_BASE}/api/generate`, {
      ...
      body: JSON.stringify({
        ...
        system: "Tu es un assistant email professionnel. Reponds en JSON uniquement.",
        stream: false,
...
function getDefaultReplies(): SmartReply[] {
  return [
    { text: "Merci, bien recu. Je reviens vers vous rapidement.", tone: "positive" },
    { text: "Bien note, je vais y reflechir.", tone: "neutral" },
    { text: "Merci pour votre message. Ce n'est pas possible pour le moment.", tone: "decline" },
  ];
}
