...
export const notificationsApi = {
  ...
  updatePreferences: (data: Partial<NotificationPreferences>) =>
    calendarClient.put<NotificationPreferences>(
      "/notifications/preferences",
      data,
    ),

  getHistory: (params?: { limit?: number; offset?: number }) =>
    calendarClient.get<NotificationHistoryResponse>("/notifications/history", {
      params,
    }),
  ...
  subscribePush: (subscription: object, browserName?: string) =>
    calendarClient.post("/notifications/subscriptions/push", {
      subscription,
  ...
  listPushSubscriptions: () =>
    calendarClient.get<
      Array<{ id: string; browser_name?: string; created_at: string }>
...
export const calendarApi = {
  ...
  listCalendars: () => calendarClient.get<Calendar[]>("/calendars"),

  getCalendar: (id: string) => calendarClient.get<Calendar>(`/calendars/${id}`),

  updateCalendar: (id: string, data: UpdateCalendar) =>
    calendarClient.put<Calendar>(`/calendars/${id}`, data),

  deleteCalendar: (id: string) => calendarClient.delete(`/calendars/${id}`),
  ...
  addMember: (calendarId: string, userId: string, role: string) =>
    calendarClient.post(`/calendars/${calendarId}/members`, {
      user_id: userId,
      role,
    }),
  ...
  getEvent: (id: string) => calendarClient.get<Event>(`/events/${id}`),

  updateEvent: (id: string, data: UpdateEvent) =>
    calendarClient.put<Event>(`/events/${id}`, data),

  deleteEvent: (id: string) => calendarClient.delete(`/events/${id}`),
  ...
  updateRsvp: (attendeeId: string, rsvpStatus: string) =>
    calendarClient.put(`/attendees/${attendeeId}/rsvp`, {
...
export const tasksApi = {
  ...
  createTask: (calendarId: string, data: Record<string, unknown>) =>
    calendarClient.post(`/calendars/${calendarId}/tasks`, data),
  getTask: (taskId: string) => calendarClient.get(`/tasks/${taskId}`),
  updateTask: (taskId: string, data: Record<string, unknown>) =>
    calendarClient.put(`/tasks/${taskId}`, data),
  deleteTask: (taskId: string) => calendarClient.delete(`/tasks/${taskId}`),
};
...
export const pollsApi = {
  ...
  create: (data: CreatePollRequest) =>
    calendarClient.post<Poll>("/polls", data),
...
export const meetingSuggestionsApi = {
  suggest: (data: MeetingSuggestionsRequest) =>
    calendarClient.post<MeetingSuggestionsResponse>(
      "/calendar/meeting-suggestions",
      data,
    ),
};
...
export const importApi = {
  ...
  exportIcs: (calendarId: string) =>
    calendarClient.get(`/calendars/${calendarId}/export`, {
      responseType: "blob",
    }),
};
...
export const caldavFeedApi = {
  ...
  getEventIcs: (calendarId: string, eventId: string) =>
    calendarClient.get(
      `/caldav/calendars/${calendarId}/events/${eventId}.ics`,
      {
        responseType: "text",
      },
    ),
...
export const externalSyncApi = {
  listConnections: () => calendarClient.get("/external-sync/connections"),
...
export const calendarUserSettingsApi = {
  getTimezone: () => calendarClient.get<{ timezone: string }>("/timezones/me"),
...
export const leaveApi = {
  ...
  predict: (days: number, leaveType: string) =>
    calendarClient.get("/leave/balances/predict", {
      params: { days, leave_type: leaveType },
    }),
  teamConflicts: (start: string, end: string) =>
    calendarClient.get("/leave/team-conflicts", { params: { start, end } }),
  delegate: (
    eventId: string,
    assignments: Array<{ task_id: string; assign_to: string }>,
  ) =>
...
export const presenceApi = {
  ...
  createRule: (rule: Record<string, unknown>) =>
    calendarClient.post("/presence/rules", rule),
  updateRule: (id: string, rule: Record<string, unknown>) =>
    calendarClient.put(`/presence/rules/${id}`, rule),
  deleteRule: (id: string) => calendarClient.delete(`/presence/rules/${id}`),
  validate: (action: Record<string, unknown>) =>
    calendarClient.post("/presence/validate", action),
  teamStatus: (date: string) =>
    calendarClient.get("/presence/team-status", { params: { date } }),
  headcount: (date: string, teamId?: string) =>
    calendarClient.get("/presence/headcount", {
...
export const categoriesApi = {
  ...
  create: (cat: Record<string, unknown>) =>
    calendarClient.post("/categories", cat),
  update: (id: string, cat: Record<string, unknown>) =>
    calendarClient.put(`/categories/${id}`, cat),
...
export const timesheetsApi = {
  ...
  update: (id: string, data: Record<string, unknown>) =>
    calendarClient.put(`/timesheets/${id}`, data),
  validate: (week: string) =>
    calendarClient.post("/timesheets/validate", { week }),
...
export const approvalWorkflowsApi = {
  ...
  create: (wf: Record<string, unknown>) =>
    calendarClient.post("/approval-workflows", wf),
  update: (id: string, wf: Record<string, unknown>) =>
    calendarClient.put(`/approval-workflows/${id}`, wf),
...
export const cronJobsApi = {
  ...
  create: (job: Record<string, unknown>) =>
    calendarClient.post("/cron-jobs", job),
  update: (id: string, job: Record<string, unknown>) =>
    calendarClient.put(`/cron-jobs/${id}`, job),
