...
export const securelinkApi = {
  ...
  dashboard: {
    ...
    traffic: () => client().get<DashboardTraffic[]>("/dashboard/traffic"),
  },

  // ─── Tunnels ──────────────────────────────────────────────────────────────
  tunnels: {
    list: () => client().get<Tunnel[]>("/tunnels"),

    create: (data: Partial<Tunnel>) => client().post<Tunnel>("/tunnels", data),
    ...
    get: (id: string) => client().get<Tunnel>(`/tunnels/${id}`),

    delete: (id: string) => client().delete(`/tunnels/${id}`),

    status: (id: string) => client().get<TunnelStatus>(`/tunnels/${id}/status`),

    reconnect: (id: string) => client().post(`/tunnels/${id}/reconnect`),
  },

  // ─── Relays ───────────────────────────────────────────────────────────────
  relays: {
    list: () => client().get<Relay[]>("/relays"),

    create: (data: Partial<Relay>) => client().post<Relay>("/relays", data),

    get: (id: string) => client().get<Relay>(`/relays/${id}`),

    delete: (id: string) => client().delete(`/relays/${id}`),

    test: (id: string) => client().post(`/relays/${id}/test`),

    connect: (id: string) => client().post(`/relays/${id}/connect`),

    disconnect: (id: string) => client().post(`/relays/${id}/disconnect`),

    stats: (id: string) => client().get<RelayStats>(`/relays/${id}/stats`),
  },

  // ─── DNS ──────────────────────────────────────────────────────────────────
  dns: {
    config: () => client().get<DnsConfig>("/dns/config"),

    updateConfig: (data: Partial<DnsConfig>) =>
      client().put<DnsConfig>("/dns/config", data),

    blocklists: () => client().get<DnsBlocklist[]>("/dns/blocklists"),
    ...
    deleteBlocklist: (id: string) => client().delete(`/dns/blocklists/${id}`),

    refreshBlocklist: (id: string) =>
      client().post(`/dns/blocklists/${id}/refresh`),

    records: () => client().get<DnsRecord[]>("/dns/records"),
    ...
    stats: () => client().get<DnsStats>("/dns/stats"),

    resetStats: () => client().post("/dns/stats/reset"),

    query: (domain: string, type?: string) =>
      client().post<DnsQueryResult>("/dns/query", {
        domain,
        type,
      } as DnsQueryPayload),

    flushCache: () => client().post("/dns/cache/flush"),
  },
};
