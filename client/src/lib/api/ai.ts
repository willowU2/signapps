...
export const aiApi = {
  ...
  search: (query: string, limit?: number, collections?: string[]) =>
    aiClient.get<SearchResult[]>("/ai/search", {
      params: { q: query, limit, collections },
    }),
  semanticSearch: (
    query: string,
  ...
  availableModels: () =>
    aiClient.get<{ models: ModelEntry[] }>("/ai/models/available"),
  searchModels: (query: string) =>
    aiClient.get<{ models: ModelEntry[] }>("/ai/models/search", {
      params: { q: query },
    }),
  downloadModel: (modelId: string) =>
    aiClient.post<{ model_id: string; status: string; path?: string }>(
      "/ai/models/download",
      { model_id: modelId },
    ),
  deleteModel: (modelId: string) => aiClient.delete(`/ai/models/${modelId}`),
  getModelStatus: (modelId: string) =>
    aiClient.get<ModelEntry>(`/ai/models/${encodeURIComponent(modelId)}`),
  ...
  getCollection: (name: string) =>
    aiClient.get<KnowledgeBase>(`/ai/collections/${name}`),
  createCollection: (data: CreateCollectionRequest) =>
    aiClient.post<KnowledgeBase>("/ai/collections", data),
  deleteCollection: (name: string) =>
    aiClient.delete(`/ai/collections/${name}`),
  getCollectionStats: (name: string) =>
    aiClient.get<CollectionStats>(`/ai/collections/${name}/stats`),
};
