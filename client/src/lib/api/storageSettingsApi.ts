...
export const storageSettingsApi = {
  ...
  updateSystemSetting: async (key: string, value: string): Promise<void> => {
    await storageClient.put(`${SETTINGS_API_BASE}/${key}`, {
      setting_value: value,
    });
  },
  ...
  async createStorageRule(rule: UpsertStorageRule): Promise<StorageRule> {
    const { data } = await storageClient.post<StorageRule>(
      STORAGE_API_BASE,
      rule,
    );
    return data;
  ...
  async updateStorageRule(
    id: string,
    rule: UpsertStorageRule,
  ): Promise<StorageRule> {
    const { data } = await storageClient.put<StorageRule>(
      `${STORAGE_API_BASE}/${id}`,
      rule,
    );
    return data;
  ...
  async createIndexingRule(rule: UpsertIndexingRule): Promise<IndexingRule> {
    const { data } = await storageClient.post<IndexingRule>(
      AI_INDEX_API_BASE,
      rule,
    );
    return data;
  ...
  async updateIndexingRule(
    id: string,
    rule: UpsertIndexingRule,
  ): Promise<IndexingRule> {
    const { data } = await storageClient.put<IndexingRule>(
      `${AI_INDEX_API_BASE}/${id}`,
      rule,
    );
    return data;
  },
