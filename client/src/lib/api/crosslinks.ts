...
export const linksApi = {
  find: (entityType: string, entityId: string) =>
    client().get<EntityReference[]>("/links", {
  ...
  remove: (id: string) => client().delete(`/links/${id}`),
};
...
export const activitiesApi = {
  ...
  entityHistory: (entityType: string, entityId: string) =>
    client().get<Activity[]>("/activities", {
      params: { entity_type: entityType, entity_id: entityId },
    }),
  recent: (limit = 20) =>
...
export const signaturesApi = {
  ...
  get: (id: string) => client().get<SignatureEnvelope>(`/signatures/${id}`),
  ...
  addStep: (
    envelopeId: string,
    data: { signer_email: string; signer_name?: string; action?: string },
  ...
  signStep: (envelopeId: string, stepId: string) =>
    client().post<EnvelopeStep>(
      `/signatures/${envelopeId}/steps/${stepId}/sign`,
    ),
  declineStep: (envelopeId: string, stepId: string, reason?: string) =>
    client().post<EnvelopeStep>(
      `/signatures/${envelopeId}/steps/${stepId}/decline`,
      { reason },
    ),
