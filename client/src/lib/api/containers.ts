...
export const storeApi = {
  listApps: (category?: string, search?: string) =>
    containersClient.get<StoreApp[]>("/store/apps", {
  ...
  getAppDetails: (sourceId: string | undefined, appId: string) =>
    containersClient.get<AppDetails>(
      `/store/sources/${sourceId}/apps/${appId}/details`,
    ),
  ...
  validateSource: (data: { name: string; url: string }) =>
    containersClient.post<{
      valid: boolean;
      app_count?: number;
      error?: string;
    }>("/store/sources/validate", data),
  deleteSource: (id: string) => containersClient.delete(`/store/sources/${id}`),
  refreshSource: (id: string) =>
    containersClient.post(`/store/sources/${id}/refresh`),
};
...
export const composeApi = {
  listProjects: () =>
    containersClient.get<ComposeProject[]>("/compose/projects"),
  getProject: (name: string) =>
    containersClient.get<ComposeProject>(`/compose/projects/${name}`),
  createProject: (name: string, content: string) =>
    containersClient.post<ComposeProject>("/compose/projects", {
      name,
      content,
    }),
  updateProject: (name: string, content: string) =>
    containersClient.put<ComposeProject>(`/compose/projects/${name}`, {
      content,
    }),
  deleteProject: (name: string, removeVolumes: boolean = false) =>
    containersClient.delete(`/compose/projects/${name}`, {
      params: { remove_volumes: removeVolumes },
    }),
  startProject: (name: string) =>
    containersClient.post(`/compose/projects/${name}/start`),
  stopProject: (name: string) =>
    containersClient.post(`/compose/projects/${name}/stop`),
  restartProject: (name: string) =>
    containersClient.post(`/compose/projects/${name}/restart`),
  pullProject: (name: string) =>
    containersClient.post(`/compose/projects/${name}/pull`),
  logs: (name: string, service?: string, lines: number = 100) =>
    containersClient.get<string>(`/compose/projects/${name}/logs`, {
      params: { service, lines },
    }),
  validate: (content: string) =>
    containersClient.post("/compose/validate", { content }),
  preview: (content: string) =>
    containersClient.post<ComposePreviewResponse>("/compose/preview", {
      content,
    }),
  import: (content: string, autoStart: boolean = true) =>
    containersClient.post<ComposeProject[]>("/compose/import", {
      content,
...
export const containersApi = {
  ...
  create: (data: CreateContainerRequest) =>
    containersClient.post<ContainerInfo>("/containers", data),
  ...
  logs: (id: string, tail: number = 100) =>
    containersClient.get<string>(`/containers/${id}/logs`, {
      params: { tail },
    }),
  stats: (id: string) =>
    containersClient.get<ContainerStats>(`/containers/${id}/stats`),
  ...
  removeDocker: (dockerId: string, force: boolean = false) =>
    containersClient.delete(`/containers/docker/${dockerId}`, {
      params: { force },
    }),
  inspectDocker: (dockerId: string) =>
    containersClient.get<ContainerInfo>(
      `/containers/docker/${dockerId}/inspect`,
    ),
};
...
export const backupsApi = {
  listProfiles: () =>
    containersClient.get<BackupProfile[]>("/backups/profiles"),
  getProfile: (id: string) =>
    containersClient.get<BackupProfile>(`/backups/profiles/${id}`),
  ...
  deleteProfile: (id: string) =>
    containersClient.delete(`/backups/profiles/${id}`),

  listBackups: (profileId?: string) =>
    containersClient.get<BackupItem[]>("/backups/list", {
  ...
  deleteBackup: (backupId: string) =>
    containersClient.delete(`/backups/${backupId}`),
  downloadBackup: (backupId: string) =>
    containersClient.get(`/backups/${backupId}/download`, {
      responseType: "blob",
    }),

  // Aliases used by the UI
  list: () =>
    containersClient.get<{ profiles: BackupProfile[] }>("/backups/profiles"),
  ...
  snapshots: (profileId: string) =>
    containersClient.get<{ snapshots: ContainerBackupSnapshot[] }>(
      `/backups/profiles/${profileId}/snapshots`,
    ),
  restore: (profileId: string, snapshotId: string, targetPath?: string) =>
    containersClient.post(`/backups/profiles/${profileId}/restore`, {
      snapshot_id: snapshotId,
      target_path: targetPath,
    }),
  runs: (profileId: string) =>
    containersClient.get<{ runs: BackupRun[] }>(
      `/backups/profiles/${profileId}/runs`,
    ),
};
...
export const getInstallProgressUrl = (id: string) =>
  `${CONTAINERS_URL}/store/install/${id}/progress`;
...
export const networksApi = {
  ...
  disconnect: (id: string, containerId: string) =>
    containersClient.post(`/networks/${id}/disconnect`, {
      container: containerId,
    }),
};
