...
export const localInvoicesApi = {
  ...
  create: (data: Omit<LocalInvoice, "id" | "createdAt">): LocalInvoice => {
    ...
    save<LocalInvoice>("billing:invoices", [
      ...load<LocalInvoice>("billing:invoices"),
      inv,
    ]);
    return inv;
  },
  ...
  delete: (id: string) => {
    save<LocalInvoice>(
      "billing:invoices",
      load<LocalInvoice>("billing:invoices").filter((i) => i.id !== id),
    );
  },
...
export const contactNotesApi = {
  ...
  byContact: (contactId: string): ContactNote[] =>
    load<ContactNote>("shared:contact_notes").filter(
  ...
  create: (data: Omit<ContactNote, "id" | "createdAt">): ContactNote => {
    ...
    save<ContactNote>("shared:contact_notes", [
      ...load<ContactNote>("shared:contact_notes"),
      note,
    ]);
    return note;
  },
  ...
  delete: (id: string) => {
    save<ContactNote>(
      "shared:contact_notes",
      load<ContactNote>("shared:contact_notes").filter((n) => n.id !== id),
    );
  },
};
...
export const socialProfilesApi = {
  byContact: (contactId: string): SocialProfile[] =>
    load<SocialProfile>("contacts:social").filter(
      (s) => s.contactId === contactId,
    ),

  upsert: (
    data: Omit<SocialProfile, "id"> & { id?: string },
  ): SocialProfile => {
    const all = load<SocialProfile>("contacts:social");
    const idx = all.findIndex(
      (s) => s.contactId === data.contactId && s.platform === data.platform,
    );
    if (idx >= 0) {
  ...
  delete: (id: string) => {
    save<SocialProfile>(
      "contacts:social",
      load<SocialProfile>("contacts:social").filter((s) => s.id !== id),
    );
  },
};
...
export async function getInvoiceValueByStage(): Promise<
...
export async function getOverdueInvoicesWithDeals(): Promise<
  { invoice: LocalInvoice; deal?: Deal }[]
> {
  const invoices = localInvoicesApi
    .list()
    .filter((i) => i.status === "overdue");
  const results: { invoice: LocalInvoice; deal?: Deal }[] = [];
  for (const invoice of invoices) {
    const deal = invoice.dealId
      ? await dealsApi.get(invoice.dealId)
      : undefined;
...
export async function getBillingEnrichedForecast(
  contactEmail?: string,
): Promise<{
  avgDealValue: number;
  totalPaid: number;
...
export async function mergeContactReferences(
  keepId: string,
  removeId: string,
  keepEmail: string,
): Promise<void> {
  ...
  await Promise.all(
    deals
      .filter((d) => d.contactId === removeId)
      .map((d) => dealsApi.update(d.id, { contactId: keepId })),
  );
  // Update activities
  const acts = activitiesApi.list();
  acts
    .filter((a) => a.contactId === removeId)
    .forEach((a) => {
  ...
  invs
    .filter((i) => i.contactId === removeId)
    .forEach((i) => {
      localInvoicesApi.update(i.id, {
        contactId: keepId,
...
export async function exportDealsWithContactsCsv<
  T extends { id: string; name: string; email: string; company?: string },
>(contacts: T[]): Promise<string> {
  const deals = await dealsApi.list();
  const contactMap = new Map(contacts.map((c) => [c.id, c]));
  const emailMap = new Map(contacts.map((c) => [c.email.toLowerCase(), c]));

  const header =
    "Titre,Société,Contact,Email,Valeur,Étape,Probabilité,Clôture,Assigné à";
