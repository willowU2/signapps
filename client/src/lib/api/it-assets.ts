...
export const itAssetsApi = {
  ...
  listHardwareCommands: (hardwareId: string) =>
    itAssetsClient.get<AgentCommand[]>(
      `/it-assets/hardware/${hardwareId}/commands`,
    ),
  ...
  listHardwareFiles: (hardwareId: string) =>
    itAssetsClient.get<FileTransfer[]>(
      `/it-assets/hardware/${hardwareId}/files`,
    ),

  // ND1: Network scanner
  scanNetwork: (subnet: string, timeoutMs?: number) =>
    itAssetsClient.post<ScanResult>("/it-assets/network/scan", {
      subnet,
  ...
  addDiscoveryToInventory: (
    discoveryId: string,
    name: string,
    assetType?: string,
  ) =>
    itAssetsClient.post(
      `/it-assets/network/discoveries/${discoveryId}/add-to-inventory`,
      {
  ...
  listPatches: () => itAssetsClient.get<Patch[]>("/it-assets/patches"),
  ...
  deployPatch: (id: string, hardwareIds?: string[]) =>
    itAssetsClient.post(`/it-assets/patches/${id}/deploy`, {
  ...
  getMetrics: (hwId: string, range?: string) =>
    itAssetsClient.get<AgentMetric[]>(
      `/it-assets/hardware/${hwId}/metrics${range ? `?range=${range}` : ""}`,
    ),
  ...
  listAlerts: () => itAssetsClient.get<ITAlert[]>("/it-assets/alerts"),
  ...
  getEventLogs: (
    ...
    const qs = q.toString();
    return itAssetsClient.get<EventLog[]>(
      `/it-assets/hardware/${hwId}/logs${qs ? `?${qs}` : ""}`,
    );
  },

  // MD4: Fleet overview
  getFleetOverview: () => itAssetsClient.get<FleetOverview>("/it-assets/fleet"),

  // BK1: Components
  listComponents: (hwId: string) =>
    itAssetsClient.get<HardwareComponent[]>(
      `/it-assets/hardware/${hwId}/components`,
    ),
  createComponent: (hwId: string, data: CreateComponentRequest) =>
    itAssetsClient.post<HardwareComponent>(
      `/it-assets/hardware/${hwId}/components`,
      data,
    ),
  ...
  listNetworkInterfaces: (hwId: string) =>
    itAssetsClient.get<NetworkInterfaceAsset[]>(
      `/it-assets/hardware/${hwId}/interfaces`,
    ),
  createNetworkInterface: (hwId: string, data: CreateNetworkInterfaceRequest) =>
    itAssetsClient.post<NetworkInterfaceAsset>(
      `/it-assets/hardware/${hwId}/interfaces`,
      data,
    ),
  updateNetworkInterface: (
    id: string,
    data: Partial<CreateNetworkInterfaceRequest>,
  ) =>
    itAssetsClient.put<NetworkInterfaceAsset>(
      `/it-assets/interfaces/${id}`,
      data,
    ),
  ...
  createMaintenanceWindow: (data: CreateMaintenanceWindowRequest) =>
    itAssetsClient.post<MaintenanceWindow>(
      "/it-assets/maintenance-windows",
      data,
    ),
  updateMaintenanceWindow: (
    id: string,
    data: Partial<CreateMaintenanceWindowRequest>,
  ) =>
    itAssetsClient.put<MaintenanceWindow>(
      `/it-assets/maintenance-windows/${id}`,
      data,
    ),
  ...
  // CM1: CMDB
  listCIs: () => itAssetsClient.get<ConfigurationItem[]>("/it-assets/cmdb/cis"),
  getCI: (id: string) =>
  ...
  deleteCI: (id: string) => itAssetsClient.delete(`/it-assets/cmdb/cis/${id}`),
  listCIRelationships: (ciId: string) =>
    itAssetsClient.get<CIRelationship[]>(
      `/it-assets/cmdb/cis/${ciId}/relationships`,
    ),
  ...
  updateChangeStatus: (id: string, status: string, actorId?: string) =>
    itAssetsClient.put<ChangeRequest>(`/it-assets/changes/${id}/status`, {
      status,
      actor_id: actorId,
    }),
  ...
  getAntivirusStatus: (hwId: string) =>
    itAssetsClient.get<AntivirusStatus>(
      `/it-assets/hardware/${hwId}/security/antivirus`,
    ),
  getAvFleetSummary: () =>
  ...
  getEncryptionStatus: (hwId: string) =>
    itAssetsClient.get<EncryptionStatus[]>(
      `/it-assets/hardware/${hwId}/security/encryption`,
    ),
  getEncryptionFleetSummary: () =>
    itAssetsClient.get<EncryptionFleetSummary>(
      "/it-assets/fleet/security/encryption",
    ),
  ...
  listTickets: (filters?: {
    ...
    const qs = q.toString();
    return itAssetsClient.get<Ticket[]>(
      `/it-assets/tickets${qs ? `?${qs}` : ""}`,
    );
  },
  getTicketStats: () =>
  ...
  updateTicket: (
    id: string,
    data: Partial<CreateTicketRequest> & { status?: string },
  ) => itAssetsClient.patch<Ticket>(`/it-assets/tickets/${id}`, data),
  addTicketComment: (
    id: string,
    data: { content: string; author_name?: string; is_internal?: boolean },
  ) =>
    itAssetsClient.post<TicketComment>(
      `/it-assets/tickets/${id}/comments`,
      data,
    ),
  logTicketTime: (
    id: string,
  ...
  createDeviceDoc: (
    hwId: string,
    data: { title: string; content?: string; doc_type?: string },
  ...
  checkSoftwareCompliance: (hwId: string) =>
    itAssetsClient.get<SoftwarePolicyCheckResult>(
      `/it-assets/hardware/${hwId}/software-check`,
    ),

  // #21: Remediation playbooks
  listPlaybooks: () => itAssetsClient.get<Playbook[]>("/it-assets/playbooks"),
  createPlaybook: (data: {
    name: string;
    description?: string;
    steps: PlaybookStep[];
  ...
  listRecordings: (hwId: string) =>
    itAssetsClient.get<SessionRecording[]>(
      `/it-assets/hardware/${hwId}/recordings`,
    ),

  // #27: Remote file browser
  listRemoteFiles: (agentId: string, path: string) =>
    itAssetsClient.get<RemoteFileEntry[]>(
      `/it-assets/agent/${agentId}/files/list?path=${encodeURIComponent(path)}`,
    ),

  agentCommand: (agentId: string, data: { action: string; path?: string }) =>
    itAssetsClient.post(`/it-assets/agent/commands/queue`, {
      agent_id: agentId,
      ...data,
    }),
  ...
  updatePsaIntegration: (
    id: string,
    data: Partial<CreatePsaIntegrationRequest & { enabled: boolean }>,
  ) =>
    itAssetsClient.put<PsaIntegration>(
      `/it-assets/psa-integrations/${id}`,
      data,
    ),
  ...
  listCloudResources: (provider?: string) => {
    const params = provider ? { provider } : {};
    return itAssetsClient.get<CloudResource[]>("/it-assets/cloud/resources", {
      params,
    });
  },
  syncCloudResources: (provider: string) =>
    itAssetsClient.post<CloudResource[]>(`/it-assets/cloud/resources/sync`, {
      provider,
    }),
  getCloudCredentials: () =>
    itAssetsClient.get<CloudCredentialStatus[]>("/it-assets/cloud/credentials"),
  saveCloudCredentials: (data: SaveCloudCredentialRequest) =>
    itAssetsClient.post("/it-assets/cloud/credentials", data),
  deleteCloudCredentials: (provider: string) =>
    itAssetsClient.delete(`/it-assets/cloud/credentials/${provider}`),
};
...
export interface SaveCloudCredentialRequest {
  provider: "azure" | "aws" | "gcp";
