...
const COMMANDS: VoiceCommand[] = [
  {
    pattern: /cr[ée]+r?\s+(?:une?\s+)?t[aâ]che\s+(.+)/i,
    action: "task.create",
    params: ["title"],
  },
  {
    pattern: /envoyer?\s+(?:un?\s+)?(?:e-?)?mail\s+[àa]\s+(.+)/i,
    action: "email.compose",
    params: ["to"],
  },
  {
    pattern: /planifier?\s+(?:une?\s+)?r[ée]union\s+(.+)/i,
    action: "meeting.schedule",
    params: ["title"],
  },
  { pattern: /rechercher?\s+(.+)/i, action: "search", params: ["query"] },
  {
    pattern: /aller?\s+(?:au?x?\s+)?(.+)/i,
    action: "navigate",
    params: ["destination"],
  },
  { pattern: /ouvrir?\s+(.+)/i, action: "open", params: ["target"] },
  {
    pattern: /dicter?\s+(?:un?\s+)?email/i,
    action: "email.dictate",
    params: [],
  },
  {
    pattern: /wiki|base\s+de\s+connaissance/i,
    action: "navigate",
    params: ["destination"],
  },
];
