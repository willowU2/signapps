...
export const accountApi = {
  ...
  exchangeGoogleOAuthCode: async (
    ...
    const res = await mailClient.post("/mail/oauth/google/callback", {
      code,
      user_id,
    });
    return res.data;
  ...
  // OAuth: get the Microsoft authorization URL
  getMicrosoftOAuthUrl: async (): Promise<{ url: string }> => {
    const res = await mailClient.get("/mail/oauth/microsoft/login");
    return res.data;
  },

  // OAuth: exchange Microsoft code for token and link to account
  exchangeMicrosoftOAuthCode: async (
    code: string,
    user_id: string,
  ): Promise<MailAccount> => {
    const res = await mailClient.post("/mail/oauth/microsoft/callback", {
      code,
      user_id,
    });
    return res.data;
  ...
  getOAuthConfig: async (
    platform: string,
  ): Promise<{ platform: string; client_id: string; configured: boolean }> => {
  ...
  saveOAuthConfig: async (
    ...
    await mailClient.post(`/mail/oauth/config/${platform}`, {
      platform,
      client_id,
      client_secret,
    });
  ...
  createAlias: async (
    accountId: string,
    data: { alias_email: string; display_name: string },
  ): Promise<MailAlias> => {
    const res = await mailClient.post(
      `/mail/accounts/${accountId}/aliases`,
      data,
    );
    return res.data;
  },
  updateAlias: async (
    accountId: string,
    aliasId: string,
    data: Partial<{ alias_email: string; display_name: string }>,
  ): Promise<MailAlias> => {
    const res = await mailClient.patch(
      `/mail/accounts/${accountId}/aliases/${aliasId}`,
      data,
    );
    return res.data;
  },
  ...
  setDefaultAlias: async (
    accountId: string,
    aliasId: string,
  ): Promise<MailAlias> => {
    const res = await mailClient.post(
      `/mail/accounts/${accountId}/aliases/${aliasId}/set-default`,
    );
    return res.data;
  },
  ...
  createDelegation: async (
    accountId: string,
    data: object,
  ): Promise<MailDelegation> => {
    const res = await mailClient.post(
      `/mail/accounts/${accountId}/delegations`,
      data,
    );
    return res.data;
  },
  revokeDelegation: async (
    ...
    await mailClient.delete(
      `/mail/accounts/${accountId}/delegations/${delegationId}`,
    );
  },
};
...
export const mailingListApi = {
  list: async (): Promise<MailingListEntry[]> => {
    const res = await mailClient.get("/mail/mailing-lists");
    return res.data;
  },
  ...
  unsubscribe: async (
    ...
    const res = await mailClient.post("/mail/mailing-lists/unsubscribe", {
      items,
    });
    return res.data;
  },
...
export const spamApi = {
  ...
  updateSettings: async (
    accountId: string,
    data: UpdateSpamSettingsRequest,
  ): Promise<SpamSettings> => {
    const res = await mailClient.patch(
      `/mail/spam/settings/${accountId}`,
      data,
    );
    return res.data;
  },
