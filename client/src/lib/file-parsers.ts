...
function safeResultToString(result: unknown): string {
  ...
  if (typeof result === "object" && result !== null) {
    ...
    if (typeof r.toISOString === "function")
      return r.toISOString().split("T")[0];
    if ("error" in r) return String(r.error || "");
    if ("richText" in r && Array.isArray(r.richText))
      return (r.richText as UnknownRecord[])
        .map((t) => String((t as UnknownRecord)?.text || ""))
        .join("");
...
function extractCellValue(cellValue: ExcelJS.CellValue): string {
  ...
  if (
    ...
    cellValue !== null &&
    typeof (cellValue as MaybeDateLike).toISOString === "function"
  ) {
  ...
  if (typeof cellValue === "object") {
    ...
    if (
      ...
      Array.isArray((cellValue as ExcelJS.CellRichTextValue).richText)
    ) {
    ...
    if (
      ...
      typeof (cellValue as ExcelJS.CellFormulaValue).formula === "string"
    ) {
...
function resolveColor(
  ...
  if (
    ...
    color.theme >= 0 &&
    color.theme < EXCEL_THEME_COLORS.length
  ) {
...
function extractCellStyle(cell: ExcelJS.Cell): CellStyle | undefined {
  ...
  if (protection?.locked === true) {
    style.locked = true;
    hasStyle = true;
  } else if (protection?.locked === false) {
    style.locked = false;
    hasStyle = true;
  }
...
function extractValidation(
  dv: ExcelJS.DataValidation | undefined | null,
...
async function parseSpreadsheet(
  ...
  if (ext === "csv") {
    ...
    await workbook.csv.read(
      new Blob([buffer]).stream() as unknown as Parameters<
        typeof workbook.csv.read
      >[0],
    );
  } else {
    ...
    worksheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
      row.eachCell({ includeEmpty: false }, (cell, colNumber) => {
        ...
        const comment = extractComment(
          cell.note as string | ExcelJS.Comment | undefined,
        );

        // Data validation
        const validation = extractValidation(
          (cell as ExcelJS.Cell & { dataValidation?: ExcelJS.DataValidation })
            .dataValidation,
        );
  ...
  return {
    ...
    rowHeights:
      Object.keys(allRowHeights).length > 0 ? allRowHeights : undefined,
  };
}
...
async function parseSlides(
  buffer: ArrayBuffer,
  fileName: string,
): Promise<{ type: "slides"; data: SlidesFileFormat }> {
  try {
    const text = new TextDecoder().decode(buffer);
    ...
    throw new Error(
      `Failed to parse slides file: ${e instanceof Error ? e.message : String(e)}`,
    );
  }
}
