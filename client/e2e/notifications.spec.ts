...
test.describe('Notification System (Phase 8)', () => {
  ...
  test.describe('Notification Settings Page', () => {
    test('should load notification settings', async ({ page }) => {
      ...
      await expect(page.locator('h1:has-text("Notification Settings")')).toBeVisible();
      await expect(page.locator('[role="tablist"]')).toBeVisible();
    });

    test('should display three tabs: Email, Push, Quiet Hours', async ({ page }) => {
    ...
    test('should change email frequency', async ({ page }) => {
      ...
      const frequencySelect = page.locator('button:has-text("Digest")').first();
      await frequencySelect.click();

      const instantOption = page.locator('text=Instant');
      await instantOption.click();

      await expect(frequencySelect).toContainText('Instant');
    });
    ...
    test('should save notification preferences', async ({ page }) => {
      ...
      const successAlert = page.locator('text=Settings saved successfully');
    ...
    test('should handle API errors gracefully', async ({ page }) => {
      ...
      const errorAlert = page.locator('text=Failed to save');
      await expect(errorAlert).toBeVisible({ timeout: 5000 });
    });
  });

  test.describe('Push Notifications Registration', () => {
    test('should show push registration button if not registered', async ({ page }) => {
      await page.click('[role="tab"]:has-text("Push")');

      const registerButton = page.locator('button:has-text("Enable Push Notifications")');
    ...
    test('should show registered badge if already registered', async ({ page, context }) => {
  ...
  test.describe('Notification History', () => {
    ...
    test('should filter notifications by status', async ({ page }) => {
      ...
      if (await historyLink.isVisible().catch(() => false)) {
        ...
        const statusFilter = page.locator('button[aria-label="Status"]').first();
    ...
    test('should allow resending failed notifications', async ({ page }) => {
      ...
      if (await historyLink.isVisible().catch(() => false)) {
        ...
        if (await resendButtons.first().isVisible({ timeout: 2000 }).catch(() => false)) {
  ...
  test.describe('Notification Workflow', () => {
    test('should complete end-to-end: Create event → Check notifications', async ({ page }) => {
    ...
    test('should respect quiet hours setting', async ({ page }) => {
      // Enable quiet hours for right now
      await page.click('[role="tab"]:has-text("Quiet")');

      const quietToggle = page.locator('id=quiet-enabled');
      await quietToggle.click();

      // Set quiet hours to include now (simplified)
      const startInput = page.locator('id=quiet-start');
      const endInput = page.locator('id=quiet-end');
