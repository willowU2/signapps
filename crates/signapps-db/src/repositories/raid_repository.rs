...
use crate::models::{Disk, RaidArray, RaidEvent, RaidHealth};
...
impl<'a> RaidRepository<'a> {
    ...
    pub async fn find_array(&self, id: Uuid) -> Result<Option<RaidArray>> {
        let array =
            sqlx::query_as::<_, RaidArray>("SELECT * FROM storage.raid_arrays WHERE id = $1")
                .bind(id)
    ...
    pub async fn find_array_by_name(&self, name: &str) -> Result<Option<RaidArray>> {
        let array =
            sqlx::query_as::<_, RaidArray>("SELECT * FROM storage.raid_arrays WHERE name = $1")
                .bind(name)
                .fetch_optional(self.pool.inner())
                .await?;
    ...
    pub async fn list_arrays(&self) -> Result<Vec<RaidArray>> {
        let arrays =
            sqlx::query_as::<_, RaidArray>("SELECT * FROM storage.raid_arrays ORDER BY name")
    ...
    pub async fn update_array_status(&self, id: Uuid, status: &str) -> Result<()> {
        sqlx::query("UPDATE storage.raid_arrays SET status = $2, updated_at = NOW() WHERE id = $1")
            .bind(id)
            .bind(status)
    ...
    pub async fn find_disk(&self, id: Uuid) -> Result<Option<Disk>> {
        let disk = sqlx::query_as::<_, Disk>("SELECT * FROM storage.disks WHERE id = $1")
            .bind(id)
            .fetch_optional(self.pool.inner())
            .await?;
    ...
    pub async fn find_disk_by_path(&self, path: &str) -> Result<Option<Disk>> {
        let disk = sqlx::query_as::<_, Disk>("SELECT * FROM storage.disks WHERE device_path = $1")
            .bind(path)
            .fetch_optional(self.pool.inner())
            .await?;
    ...
    pub async fn list_disks(&self) -> Result<Vec<Disk>> {
        let disks = sqlx::query_as::<_, Disk>("SELECT * FROM storage.disks ORDER BY device_path")
            .fetch_all(self.pool.inner())
            .await?;
    ...
    pub async fn update_disk_status(&self, id: Uuid, status: &str) -> Result<()> {
        sqlx::query("UPDATE storage.disks SET status = $2, updated_at = NOW() WHERE id = $1")
            .bind(id)
            .bind(status)
            .execute(self.pool.inner())
            .await?;
