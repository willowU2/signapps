...
pub enum MarketplaceError {
    /// The requested app was not found in the store.
...
impl AppStore {
    ...
    pub fn register(&mut self, listing: AppListing) -> Result<(), MarketplaceError> {
        self.listings.insert(listing.id.clone(), listing);
        Ok(())
    }

    /// Install an app for a user
    pub fn install(&mut self, user_id: &str, app_id: &str) -> Result<(), MarketplaceError> {
    ...
    pub fn uninstall(&mut self, user_id: &str, app_id: &str) -> Result<(), MarketplaceError> {
