...
/// Require superadmin role (role >= 3).
pub async fn require_superadmin(request: Request, next: Next) -> Result<Response, Error> {
    let claims = request
        .extensions()
        .get::<Claims>()
        .ok_or(Error::Unauthorized)?;
    ...
    if claims.role < 3 {
        return Err(Error::Forbidden("Superadmin access required".to_string()));
    }
